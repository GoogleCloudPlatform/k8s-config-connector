...
type ClusterExistingLustre struct {
	...
	// +optional
	LustreRef *v1alpha1.ResourceRef `json:"lustreRef,omitempty"`
}
...
type ClusterLoginNodes struct {
	...
	/* Optional. How [storage resources][google.cloud.hypercomputecluster.v1.StorageResource] should be mounted on each login node. */
	// +optional
...
type ClusterNewReservedInstances struct {
	...
	// +optional
	ReservationRef *v1alpha1.ResourceRef `json:"reservationRef,omitempty"`
}
...
type ClusterNodeSets struct {
	/* Optional. ID of the compute resource on which this nodeset will run. Must match a key in the cluster's [compute_resources][google.cloud.hypercomputecluster.v1.Cluster.compute_resources]. */
	// +optional
	...
	/* Optional. How [storage resources][google.cloud.hypercomputecluster.v1.StorageResource] should be mounted on each compute node. */
	// +optional
	StorageConfigs []ClusterStorageConfigs `json:"storageConfigs,omitempty"`
}
...
type ClusterPartitions struct {
	...
	/* Required. IDs of the nodesets that make up this partition. Values must match [SlurmNodeSet.id][google.cloud.hypercomputecluster.v1.SlurmNodeSet.id]. */
	// +optional
...
type ClusterStorageConfigs struct {
	/* Required. ID of the storage resource to mount, which must match a key in the cluster's [storage_resources][google.cloud.hypercomputecluster.v1.Cluster.storage_resources]. */
	// +optional
	Id *string `json:"id,omitempty"`
