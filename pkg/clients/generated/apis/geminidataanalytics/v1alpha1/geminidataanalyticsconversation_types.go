...
type ConversationAgentRefs struct {
	/* A reference to an externally managed DataAgent resource. Should be in the format "projects/{{projectID}}/locations/{{location}}/dataAgents/{{dataAgentID}}". */
	// +optional
	External *string `json:"external,omitempty"`
}

type GeminiDataAnalyticsConversationSpec struct {
	/* Required. Agent(s) in the conversation. Currently, only one agent is supported. This field is repeated to allow for future support of multiple agents in a conversation. */
	AgentRefs []ConversationAgentRefs `json:"agentRefs"`

	/* Optional. Open-ended and user-defined labels that can be set by the client to tag a conversation. */
	// +optional
...
type GeminiDataAnalyticsConversationStatus struct {
	/* Conditions represent the latest available observations of the
	   GeminiDataAnalyticsConversation's current state. */
...
// GeminiDataAnalyticsConversation is the Schema for the geminidataanalytics API
