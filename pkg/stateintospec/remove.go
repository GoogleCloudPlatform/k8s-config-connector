...
func removeFieldIfLeaf(ctx context.Context, u *unstructured.Unstructured, fieldPath fieldpath.Path, warnings *Warnings) error {
	...
	if last.FieldName != nil {
		...
		switch v := v.(type) {
		case map[string]any:
			warnings.AddWarningf("skipping field removal of map field (may be an indication of undetermined ownership)", "path", fieldPath)
			return nil
		case []any:
			warnings.AddWarningf("skipping field removal of array field (may be an indication of undetermined ownership)", "path", fieldPath)
			return nil
