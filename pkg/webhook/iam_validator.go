...
func (a *iamValidatorHandler) Handle(_ context.Context, req admission.Request) admission.Response {
	...
	case isIAMAuditConfig(obj):
		...
		if registry.IsIAMDirect(refResourceGVK.GroupKind()) {
			return admission.Errored(http.StatusForbidden, fmt.Errorf("GroupVersionKind %v does not have IAM Audit Config support", obj.GroupVersionKind()))
		}
		if isDCLResource {
...
func (a *iamValidatorHandler) validateIAMPolicy(policy *v1beta1.IAMPolicy, isDCLResource bool) admission.Response {
	...
	if registry.IsIAMDirect(resourceRef.GroupVersionKind().GroupKind()) {
		if doesIAMPolicyHaveConditions(policy) {
			return admission.Errored(http.StatusForbidden, fmt.Errorf("GroupVersionKind %v does not support IAM Conditions in IAM Policy", resourceRef.GroupVersionKind()))
		}
...
func (a *iamValidatorHandler) validateIAMPartialPolicy(partialPolicy *v1beta1.IAMPartialPolicy, isDCLResource bool) admission.Response {
	...
	if registry.IsIAMDirect(resourceRef.GroupVersionKind().GroupKind()) {
		if doesIAMPartialPolicyHaveConditions(partialPolicy) {
			return admission.Errored(http.StatusForbidden, fmt.Errorf("GroupVersionKind %v does not support IAM Conditions in IAM Partial Policy", resourceRef.GroupVersionKind()))
		}
		return allowedResponse
	}
	if isDCLResource {
