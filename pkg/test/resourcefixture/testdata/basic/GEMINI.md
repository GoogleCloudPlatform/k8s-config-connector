...
**For autonomous agents or UI bots navigating this test suite, assume the following expectations:**

*   **Expected "Failure" Exit Codes in `hack/record-gcp`**: The `hack/record-gcp` script explicitly enforces `WRITE_GOLDEN_OUTPUT=1` under the hood. Because GCP dynamic variables (like IPs, certificates, or default generation metadata) will repeatedly differ from previous `_http.log` files, the script will naturally exit with `FAIL` and report an `unexpected diff`. So long as GCP does not return a `400 Bad Request` schema validation during the setup phase, this constitutes a successful live recording. Do not repeatedly modify the test harness attempting to force a clean `PASS` exit code.
*   **Proof of Testing inside Pull Requests**: When auditing or reviewing PRs, the explicit proof that an author successfully ran testing against real GCP is the physical presence of the `_http.log` VCR traces and the fully populated `_generated_object_<testname>.golden.yaml` in their commit tree. If these exist and the status object transitions to `Ready: True`, GCP validation has formally occurred.
*   **Long Running Operations (LROs) Time Constraints**: Execution against live GCP infrastructure heavily mirrors typical spin-up delays. Provisioning actual resources (e.g., GKE Clusters, Node Pools, SQL instances) takes upwards of `10-20 minutes` to traverse the `Create -> Update -> Delete` integration lifecycle. AI agents should spawn these tasks asynchronously and yield execution context rather than strictly polling.
