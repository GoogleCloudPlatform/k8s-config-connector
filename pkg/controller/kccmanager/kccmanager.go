...
func (m *leaderElectionManager) Start(ctx context.Context) error {
	m.leConfig.Callbacks.OnStartedLeading = func(ctx context.Context) {
		...
		if m.syncerInteg != nil {
			if err := m.syncerInteg.EnsureSuspended(ctx); err != nil {
				m.fatal("error ensuring syncer is suspended during startup: %v", err)
			}
		}
	...
	m.leConfig.Callbacks.OnStoppedLeading = func() {
		...
		if m.syncerInteg != nil {
			...
			retries := 5
			for i := 0; i < retries; i++ {
				if err := m.syncerInteg.EnsurePullingFromLeader(stopCtx, myIdentity); err != nil {
					klog.Errorf("error ensuring syncer is pulling from leader (attempt %d/%d): %v", i+1, retries, err)
	...
	m.leConfig.Callbacks.OnNewLeader = func(identity string) {
		...
		if m.syncerInteg != nil {
			...
			retries := 5
			for i := 0; i < retries; i++ {
				if err := m.syncerInteg.EnsurePullingFromLeader(updateCtx, myIdentity); err != nil {
