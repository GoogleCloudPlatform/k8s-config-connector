...
func (a *folderAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	diffs, updateMask, err := compareFolder(ctx, a.actual, a.desired)
	if err != nil {
		return err
	}
	...
	updated, err := a.gcpClient.UpdateFolder(ctx, req)
	if err != nil {
		return fmt.Errorf("updating DataformFolder %s: %w", a.id.String(), err)
	}
...
func (a *folderAdapter) updateStatus(ctx context.Context, op directbase.Operation, latest *dataformpb.Folder) error {
	mapCtx := &direct.MapContext{}
	observedState := DataformFolderObservedState_v1alpha1_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
