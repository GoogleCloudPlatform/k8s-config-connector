...
func init() {
	registry.RegisterModel(krm.AlloydbDatabaseGVK, NewDatabaseModel)
}
...
type DatabaseAdapter struct {
	...
	desired   *krm.AlloydbDatabase
	actual    *alloydbpb.Database
}
...
func (a *DatabaseAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &alloydbpb.GetDatabaseRequest{Name: a.id}
...
func (a *DatabaseAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AlloydbDatabaseSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.AlloydbDatabaseStatus{}
	status.ObservedState = AlloydbDatabaseObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatabaseAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AlloydbDatabaseSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AlloydbDatabaseStatus{}
		status.ObservedState = AlloydbDatabaseObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &alloydbpb.UpdateDatabaseRequest{
		Name:       a.id.External,
	...
	status := &krm.AlloydbDatabaseStatus{}
	status.ObservedState = AlloydbDatabaseObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatabaseAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AlloydbDatabase{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AlloydbDatabaseSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AlloydbDatabaseGVK)
...
func (a *DatabaseAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &alloydbpb.DeleteDatabaseRequest{Name: a.id.String()}
