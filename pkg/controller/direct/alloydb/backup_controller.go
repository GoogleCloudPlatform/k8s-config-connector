...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &alloydbpb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	status := &krm.AlloydbBackupStatus{}
	status.ObservedState = AlloydbBackupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AlloydbBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AlloydbBackupStatus{}
		status.ObservedState = AlloydbBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AlloydbBackupStatus{}
	status.ObservedState = AlloydbBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
