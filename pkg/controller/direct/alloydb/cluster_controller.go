...
func (a *ClusterAdapter) resolveDefaults() {
	obj := a.desired
	...
	if obj.Spec.ContinuousBackupConfig != nil && obj.Spec.ContinuousBackupConfig.Enabled == nil {
		obj.Spec.ContinuousBackupConfig.Enabled = direct.PtrTo(false)
	}
...
func (a *ClusterAdapter) resolveSecretField(ctx context.Context) (*krm.UserPasswordObservedState, error) {
	obj := a.desired
	if obj.Spec.InitialUser != nil && obj.Spec.InitialUser.Password != nil {
		...
		if value != nil && valueFrom != nil {
			return nil, fmt.Errorf("only one of 'spec.initialUser.password.value' " +
				"and 'spec.initialUser.password.valueFrom' should be configured: " +
				"both are configured")
		}
		if value != nil {
			return &krm.UserPasswordObservedState{
				Password: &secret.LegacyObservedState{
					Value: value,
				},
			}, nil
		}
		if valueFrom != nil {
			if valueFrom.SecretKeyRef == nil {
				return nil, fmt.Errorf("'spec.initialUser.password.valueFrom.secretRef' " +
					"should be configured")
			}
			secretValue, secretVersion, err := direct.GetSecret(ctx, valueFrom.SecretKeyRef, obj.Namespace, a.reader)
			if err != nil {
				return nil, err
			}
			...
			return &krm.UserPasswordObservedState{
				Password: &secret.LegacyObservedState{
					ValueFrom: &secret.LegacyValueFromObservedState{
						SecretKeyRef: &v1alpha1.SecretKeyRefObservedState{
...
// TODO: full: displayName
func (a *ClusterAdapter) setObservedMutableButUnreadableFields(ctx context.Context, observedState *krm.AlloyDBClusterObservedState) error {
	obj := a.desired
	observedState.DisplayName = obj.Spec.DisplayName
	observedState.DeletionPolicy = obj.Spec.DeletionPolicy
	userPassword, err := a.resolveSecretField(ctx)
	if err != nil {
		return fmt.Errorf("resolving sensitive field 'spec.initialUser.password': %w", err)
	}
	if userPassword != nil {
		observedState.InitialUser = userPassword
	}
	return nil
}
...
func (a *ClusterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	// MutableButUnreadableFields should in general be resolved first because
	// we want to persist the raw values provided by users.
	observedMBURFields := &krm.AlloyDBClusterObservedState{}
	if err := a.setObservedMutableButUnreadableFields(ctx, observedMBURFields); err != nil {
		return err
	}
	...
	if desired.Spec.DisplayName != nil {
		fmt.Printf("maqiuyu... displayName sent: %s\n", *desired.Spec.DisplayName)
	}
	...
	if desired.Spec.RestoreBackupSource != nil || desired.Spec.RestoreContinuousBackupSource != nil {
		...
		return a.updateStatus(ctx, mapCtx, createOp, created, observedMBURFields)
	}
...
func (a *ClusterAdapter) updateStatus(ctx context.Context, mapCtx *direct.MapContext, createOp *directbase.CreateOperation, reconciledCluster *alloydbpb.Cluster, observedState *krm.AlloyDBClusterObservedState) error {
	status := AlloyDBClusterStatus_FromProto(mapCtx, reconciledCluster)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	observedState.ClusterType = status.ObservedState.ClusterType
