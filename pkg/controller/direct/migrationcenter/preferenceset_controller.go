...
func (a *PreferenceSetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &migrationcenterpb.GetPreferenceSetRequest{Name: a.id}
...
func (a *PreferenceSetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating PreferenceSet %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("PreferenceSet %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated PreferenceSet", "name", a.id.External)
