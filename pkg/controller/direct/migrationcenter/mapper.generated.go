...
func AssetObservedState_FromProto(mapCtx *direct.MapContext, in *pb.Asset) *krm.AssetObservedState {
	...
	out.InsightList = InsightList_FromProto(mapCtx, in.GetInsightList())
	out.PerformanceData = AssetPerformanceData_FromProto(mapCtx, in.GetPerformanceData())
	out.Sources = in.Sources
	out.AssignedGroups = in.AssignedGroups
	return out
}
func AssetObservedState_ToProto(mapCtx *direct.MapContext, in *krm.AssetObservedState) *pb.Asset {
	...
	if oneof := MachineDetails_ToProto(mapCtx, in.MachineDetails); oneof != nil {
		out.AssetDetails = &pb.Asset_MachineDetails{MachineDetails: oneof}
	}
	out.InsightList = InsightList_ToProto(mapCtx, in.InsightList)
	out.PerformanceData = AssetPerformanceData_ToProto(mapCtx, in.PerformanceData)
	out.Sources = in.Sources
	out.AssignedGroups = in.AssignedGroups
	return out
}
