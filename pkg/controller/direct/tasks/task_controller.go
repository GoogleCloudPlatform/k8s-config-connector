...
func init() {
	registry.RegisterModel(krm.TasksTaskGVK, NewTaskModel)
}
...
func (a *TaskAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &taskspb.GetTaskRequest{Name: a.id}
...
func (a *TaskAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := TasksTaskSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.TasksTaskStatus{}
	status.ObservedState = TasksTaskObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TaskAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := TasksTaskSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.TasksTaskStatus{}
		status.ObservedState = TasksTaskObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.TasksTaskStatus{}
	status.ObservedState = TasksTaskObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TaskAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.TasksTask{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(TasksTaskSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.TasksTaskGVK)
...
func (a *TaskAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &taskspb.DeleteTaskRequest{Name: a.id.String()}
