...
func init() {
	registry.RegisterModel(krm.BaremetalsolutionVolumeGVK, NewVolumeModel)
}
...
func (m *modelVolume) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.BaremetalsolutionVolume{}
...
type VolumeAdapter struct {
	...
	desired   *krm.BaremetalsolutionVolume
	actual    *baremetalsolutionpb.Volume
}
...
func (a *VolumeAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &baremetalsolutionpb.GetVolumeRequest{Name: a.id}
...
func (a *VolumeAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BaremetalsolutionVolumeSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &baremetalsolutionpb.CreateVolumeRequest{
	...
	status := &krm.BaremetalsolutionVolumeStatus{}
	status.ObservedState = BaremetalsolutionVolumeObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *VolumeAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BaremetalsolutionVolumeSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BaremetalsolutionVolumeStatus{}
		status.ObservedState = BaremetalsolutionVolumeObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BaremetalsolutionVolumeStatus{}
	status.ObservedState = BaremetalsolutionVolumeObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *VolumeAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BaremetalsolutionVolume{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BaremetalsolutionVolumeSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BaremetalsolutionVolumeGVK)
...
func (a *VolumeAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &baremetalsolutionpb.DeleteVolumeRequest{Name: a.id.String()}
