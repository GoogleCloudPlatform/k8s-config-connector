...
func (a *SSHKeyAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &baremetalsolutionpb.GetSSHKeyRequest{Name: a.id}
...
func (a *SSHKeyAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating SSHKey %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("SSHKey %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated SSHKey", "name", a.id.External)
