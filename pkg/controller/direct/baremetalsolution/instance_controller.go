...
func init() {
	registry.RegisterModel(krm.BaremetalsolutionInstanceGVK, NewInstanceModel)
}
...
func (m *modelInstance) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.BaremetalsolutionInstance{}
...
type InstanceAdapter struct {
	...
	desired   *krm.BaremetalsolutionInstance
	actual    *baremetalsolutionpb.Instance
}
...
func (a *InstanceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &baremetalsolutionpb.GetInstanceRequest{Name: a.id}
...
func (a *InstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BaremetalsolutionInstanceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &baremetalsolutionpb.CreateInstanceRequest{
	...
	status := &krm.BaremetalsolutionInstanceStatus{}
	status.ObservedState = BaremetalsolutionInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BaremetalsolutionInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BaremetalsolutionInstanceStatus{}
		status.ObservedState = BaremetalsolutionInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BaremetalsolutionInstanceStatus{}
	status.ObservedState = BaremetalsolutionInstanceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BaremetalsolutionInstance{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BaremetalsolutionInstanceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BaremetalsolutionInstanceGVK)
...
func (a *InstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &baremetalsolutionpb.DeleteInstanceRequest{Name: a.id.String()}
