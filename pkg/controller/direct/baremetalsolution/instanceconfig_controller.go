...
func init() {
	registry.RegisterModel(krm.BaremetalsolutionInstanceConfigGVK, NewInstanceConfigModel)
}
...
func (m *modelInstanceConfig) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.BaremetalsolutionInstanceConfig{}
...
type InstanceConfigAdapter struct {
	...
	desired   *krm.BaremetalsolutionInstanceConfig
	actual    *baremetalsolutionpb.InstanceConfig
}
...
func (a *InstanceConfigAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &baremetalsolutionpb.GetInstanceConfigRequest{Name: a.id}
...
func (a *InstanceConfigAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BaremetalsolutionInstanceConfigSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &baremetalsolutionpb.CreateInstanceConfigRequest{
	...
	status := &krm.BaremetalsolutionInstanceConfigStatus{}
	status.ObservedState = BaremetalsolutionInstanceConfigObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceConfigAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BaremetalsolutionInstanceConfigSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BaremetalsolutionInstanceConfigStatus{}
		status.ObservedState = BaremetalsolutionInstanceConfigObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &baremetalsolutionpb.UpdateInstanceConfigRequest{
		Name:           a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating InstanceConfig %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("InstanceConfig %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated InstanceConfig", "name", a.id.External)

	status := &krm.BaremetalsolutionInstanceConfigStatus{}
	status.ObservedState = BaremetalsolutionInstanceConfigObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceConfigAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BaremetalsolutionInstanceConfig{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BaremetalsolutionInstanceConfigSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BaremetalsolutionInstanceConfigGVK)
...
func (a *InstanceConfigAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &baremetalsolutionpb.DeleteInstanceConfigRequest{Name: a.id.String()}
