...
func init() {
	registry.RegisterModel(krm.BaremetalsolutionNetworkGVK, NewNetworkModel)
}
...
func (m *modelNetwork) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.BaremetalsolutionNetwork{}
...
type NetworkAdapter struct {
	...
	desired   *krm.BaremetalsolutionNetwork
	actual    *baremetalsolutionpb.Network
}
...
func (a *NetworkAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &baremetalsolutionpb.GetNetworkRequest{Name: a.id}
...
func (a *NetworkAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BaremetalsolutionNetworkSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &baremetalsolutionpb.CreateNetworkRequest{
	...
	status := &krm.BaremetalsolutionNetworkStatus{}
	status.ObservedState = BaremetalsolutionNetworkObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *NetworkAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BaremetalsolutionNetworkSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BaremetalsolutionNetworkStatus{}
		status.ObservedState = BaremetalsolutionNetworkObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BaremetalsolutionNetworkStatus{}
	status.ObservedState = BaremetalsolutionNetworkObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *NetworkAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BaremetalsolutionNetwork{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BaremetalsolutionNetworkSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BaremetalsolutionNetworkGVK)
...
func (a *NetworkAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &baremetalsolutionpb.DeleteNetworkRequest{Name: a.id.String()}
