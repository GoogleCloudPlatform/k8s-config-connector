...
func (a *ReleaseAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &deploypb.GetReleaseRequest{Name: a.id}
...
func (a *ReleaseAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Release %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Release %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Release", "name", a.id.External)
