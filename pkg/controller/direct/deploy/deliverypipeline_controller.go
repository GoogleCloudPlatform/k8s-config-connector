...
func (a *DeliveryPipelineAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &deploypb.GetDeliveryPipelineRequest{Name: a.id}
...
func (a *DeliveryPipelineAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating DeliveryPipeline %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("DeliveryPipeline %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated DeliveryPipeline", "name", a.id.External)
