...
func (m *gcpClient) newUptimeCheckClient(ctx context.Context) (*monitoring.UptimeCheckClient, error) {
	opts, err := m.config.GRPCClientOptions()
	if err != nil {
		return nil, err
	}
	client, err := monitoring.NewUptimeCheckClient(ctx, opts...)
	if err != nil {
		return nil, fmt.Errorf("building uptime check client: %w", err)
	}
	return client, err
}
