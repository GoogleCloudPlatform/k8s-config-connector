...
func init() {
	registry.RegisterModel(krm.MonitoringServiceGVK, NewServiceModel)
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &monitoringpb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := MonitoringServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := MonitoringServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.MonitoringServiceStatus{}
		status.ObservedState = MonitoringServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *ServiceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.MonitoringService{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(MonitoringServiceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
