...
func (a *NotificationChannelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &monitoringpb.GetNotificationChannelRequest{Name: a.id}
...
func (a *NotificationChannelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating NotificationChannel %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("NotificationChannel %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated NotificationChannel", "name", a.id.External)
