...
func Service_FromProto(mapCtx *direct.MapContext, in *pb.Service) *krm.Service {
	...
	out.Custom = Service_Custom_FromProto(mapCtx, in.GetCustom())
	out.AppEngine = Service_AppEngine_FromProto(mapCtx, in.GetAppEngine())
	out.CloudEndpoints = Service_CloudEndpoints_FromProto(mapCtx, in.GetCloudEndpoints())
	out.ClusterIstio = Service_ClusterIstio_FromProto(mapCtx, in.GetClusterIstio())
	out.MeshIstio = Service_MeshIstio_FromProto(mapCtx, in.GetMeshIstio())
	out.IstioCanonicalService = Service_IstioCanonicalService_FromProto(mapCtx, in.GetIstioCanonicalService())
	out.CloudRun = Service_CloudRun_FromProto(mapCtx, in.GetCloudRun())
	out.GkeNamespace = Service_GkeNamespace_FromProto(mapCtx, in.GetGkeNamespace())
	out.GkeWorkload = Service_GkeWorkload_FromProto(mapCtx, in.GetGkeWorkload())
	out.GkeService = Service_GkeService_FromProto(mapCtx, in.GetGkeService())
	out.BasicService = Service_BasicService_FromProto(mapCtx, in.GetBasicService())
	out.Telemetry = Service_Telemetry_FromProto(mapCtx, in.GetTelemetry())
...
func Service_ToProto(mapCtx *direct.MapContext, in *krm.Service) *pb.Service {
	...
	if oneof := Service_Custom_ToProto(mapCtx, in.Custom); oneof != nil {
		out.Identifier = &pb.Service_Custom_{Custom: oneof}
	}
	if oneof := Service_AppEngine_ToProto(mapCtx, in.AppEngine); oneof != nil {
		out.Identifier = &pb.Service_AppEngine_{AppEngine: oneof}
	}
	if oneof := Service_CloudEndpoints_ToProto(mapCtx, in.CloudEndpoints); oneof != nil {
		out.Identifier = &pb.Service_CloudEndpoints_{CloudEndpoints: oneof}
	}
	if oneof := Service_ClusterIstio_ToProto(mapCtx, in.ClusterIstio); oneof != nil {
		out.Identifier = &pb.Service_ClusterIstio_{ClusterIstio: oneof}
	}
	if oneof := Service_MeshIstio_ToProto(mapCtx, in.MeshIstio); oneof != nil {
		out.Identifier = &pb.Service_MeshIstio_{MeshIstio: oneof}
	}
	if oneof := Service_IstioCanonicalService_ToProto(mapCtx, in.IstioCanonicalService); oneof != nil {
		out.Identifier = &pb.Service_IstioCanonicalService_{IstioCanonicalService: oneof}
	}
	if oneof := Service_CloudRun_ToProto(mapCtx, in.CloudRun); oneof != nil {
		out.Identifier = &pb.Service_CloudRun_{CloudRun: oneof}
	}
	if oneof := Service_GkeNamespace_ToProto(mapCtx, in.GkeNamespace); oneof != nil {
		out.Identifier = &pb.Service_GkeNamespace_{GkeNamespace: oneof}
	}
	if oneof := Service_GkeWorkload_ToProto(mapCtx, in.GkeWorkload); oneof != nil {
		out.Identifier = &pb.Service_GkeWorkload_{GkeWorkload: oneof}
	}
	if oneof := Service_GkeService_ToProto(mapCtx, in.GkeService); oneof != nil {
		out.Identifier = &pb.Service_GkeService_{GkeService: oneof}
	}
	out.BasicService = Service_BasicService_ToProto(mapCtx, in.BasicService)
	out.Telemetry = Service_Telemetry_ToProto(mapCtx, in.Telemetry)
	out.UserLabels = in.UserLabels
	return out
}
func ServiceObservedState_FromProto(mapCtx *direct.MapContext, in *pb.Service) *krm.ServiceObservedState {
	...
	// MISSING: IstioCanonicalService
	// MISSING: CloudRun
	out.GkeNamespace = Service_GkeNamespaceObservedState_FromProto(mapCtx, in.GetGkeNamespace())
	out.GkeWorkload = Service_GkeWorkloadObservedState_FromProto(mapCtx, in.GetGkeWorkload())
	out.GkeService = Service_GkeServiceObservedState_FromProto(mapCtx, in.GetGkeService())
	// MISSING: BasicService
	// MISSING: Telemetry
	// MISSING: UserLabels
	return out
...
func ServiceObservedState_ToProto(mapCtx *direct.MapContext, in *krm.ServiceObservedState) *pb.Service {
	...
	// MISSING: IstioCanonicalService
	// MISSING: CloudRun
	if oneof := Service_GkeNamespaceObservedState_ToProto(mapCtx, in.GkeNamespace); oneof != nil {
		out.Identifier = &pb.Service_GkeNamespace_{GkeNamespace: oneof}
	}
	if oneof := Service_GkeWorkloadObservedState_ToProto(mapCtx, in.GkeWorkload); oneof != nil {
		out.Identifier = &pb.Service_GkeWorkload_{GkeWorkload: oneof}
	}
	if oneof := Service_GkeServiceObservedState_ToProto(mapCtx, in.GkeService); oneof != nil {
		out.Identifier = &pb.Service_GkeService_{GkeService: oneof}
	}
	// MISSING: BasicService
	// MISSING: Telemetry
