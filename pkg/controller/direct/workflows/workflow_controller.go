...
import (
	...
	workflowspb "cloud.google.com/go/workflows/v1/workflowspb"
...
func init() {
	registry.RegisterModel(krm.WorkflowsWorkflowGVK, NewWorkflowModel)
}

func NewWorkflowModel(ctx context.Context, config *config.ControllerConfig) (directbase.Model, error) {
	return &modelWorkflow{config: *config}, nil
}

var _ directbase.Model = &modelWorkflow{}

type modelWorkflow struct {
	config config.ControllerConfig
}

func (m *modelWorkflow) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelWorkflow) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.WorkflowsWorkflow{}
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &obj); err != nil {
		return nil, fmt.Errorf("error converting to %T: %w", obj, err)
	}

	id, err := krm.NewWorkflowIdentity(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
...
func (m *modelWorkflow) AdapterForURL(ctx context.Context, url string) (directbase.Adapter, error) {
	// TODO: Support URLs
	return nil, nil
}

type WorkflowAdapter struct {
	id        *krm.WorkflowIdentity
	gcpClient *gcp.Client
	desired   *krm.WorkflowsWorkflow
	actual    *workflowspb.Workflow
}

var _ directbase.Adapter = &WorkflowAdapter{}

// Find retrieves the GCP resource.
// Return true means the object is found. This triggers Adapter `Update` call.
// Return false means the object is not found. This triggers Adapter `Create` call.
// Return a non-nil error requeues the requests.
func (a *WorkflowAdapter) Find(ctx context.Context) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("getting Workflow", "name", a.id)

	req := &workflowspb.GetWorkflowRequest{Name: a.id}
...
func (a *WorkflowAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
...
func (a *WorkflowAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	...
	if len(paths) == 0 {
		...
		status := &krm.WorkflowsWorkflowStatus{}
		status.ObservedState = WorkflowsWorkflowObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	if err != nil {
		return fmt.Errorf("updating Workflow %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Workflow %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Workflow", "name", a.id.External)
...
func (a *WorkflowAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	if a.actual == nil {
...
func (a *WorkflowAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
