...
func (a *ExecutionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &workflowspb.GetExecutionRequest{Name: a.id}
...
func (a *ExecutionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	req := &workflowspb.CreateExecutionRequest{
...
func (a *ExecutionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := WorkflowsExecutionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.WorkflowsExecutionStatus{}
		status.ObservedState = WorkflowsExecutionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &workflowspb.UpdateExecutionRequest{
		Name:       a.id.External,
	...
	status := &krm.WorkflowsExecutionStatus{}
	status.ObservedState = WorkflowsExecutionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ExecutionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &workflowspb.DeleteExecutionRequest{Name: a.id.String()}
