...
func (a *LogBucketAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &loggingpb.GetLogBucketRequest{Name: a.id}
...
func (a *LogBucketAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating LogBucket %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("LogBucket %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated LogBucket", "name", a.id.External)
