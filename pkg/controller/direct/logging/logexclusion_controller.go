...
func (a *LogExclusionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &loggingpb.GetLogExclusionRequest{Name: a.id}
...
func (a *LogExclusionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating LogExclusion %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("LogExclusion %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated LogExclusion", "name", a.id.External)
