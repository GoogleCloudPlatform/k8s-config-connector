...
func (a *LogEntryAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &loggingpb.GetLogEntryRequest{Name: a.id}
...
func (a *LogEntryAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating LogEntry %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("LogEntry %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated LogEntry", "name", a.id.External)
