...
func init() {
	registry.RegisterModel(krm.BigtableSnapshotGVK, NewSnapshotModel)
}
...
type SnapshotAdapter struct {
	...
	desired   *krm.BigtableSnapshot
	actual    *bigtablepb.Snapshot
}
...
func (a *SnapshotAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigtablepb.GetSnapshotRequest{Name: a.id}
...
func (a *SnapshotAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BigtableSnapshotSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &bigtablepb.CreateSnapshotRequest{
	...
	status := &krm.BigtableSnapshotStatus{}
	status.ObservedState = BigtableSnapshotObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SnapshotAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigtableSnapshotSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigtableSnapshotStatus{}
		status.ObservedState = BigtableSnapshotObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigtablepb.UpdateSnapshotRequest{
		Name:       a.id.External,
	...
	status := &krm.BigtableSnapshotStatus{}
	status.ObservedState = BigtableSnapshotObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SnapshotAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BigtableSnapshot{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BigtableSnapshotSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BigtableSnapshotGVK)
...
func (a *SnapshotAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigtablepb.DeleteSnapshotRequest{Name: a.id.String()}
