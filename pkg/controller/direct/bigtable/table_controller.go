...
func (a *TableAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigtablepb.GetTableRequest{Name: a.id}
...
func (a *TableAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Table %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Table %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Table", "name", a.id.External)
