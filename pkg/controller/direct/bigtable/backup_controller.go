...
func init() {
	registry.RegisterModel(krm.BigtableBackupGVK, NewBackupModel)
}
...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigtablepb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BigtableBackupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &bigtablepb.CreateBackupRequest{
	...
	status.ObservedState = BigtableBackupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigtableBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigtableBackupStatus{}
		status.ObservedState = BigtableBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigtablepb.UpdateBackupRequest{
		Name:       a.id.External,
	...
	status := &krm.BigtableBackupStatus{}
	status.ObservedState = BigtableBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BigtableBackup{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BigtableBackupSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BigtableBackupGVK)
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigtablepb.DeleteBackupRequest{Name: a.id.String()}
