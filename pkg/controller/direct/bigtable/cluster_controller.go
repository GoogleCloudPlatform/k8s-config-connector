...
func init() {
	registry.RegisterModel(krm.BigtableClusterGVK, NewClusterModel)
}
...
func (a *ClusterAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigtablepb.GetClusterRequest{Name: a.id}
...
func (a *ClusterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	req := &bigtablepb.CreateClusterRequest{
	...
	if err != nil {
		return fmt.Errorf("Cluster %s waiting creation: %w", a.id, err)
	}
...
func (a *ClusterAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigtableClusterSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigtableClusterStatus{}
		status.ObservedState = BigtableClusterObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigtablepb.UpdateClusterRequest{
		Name:       a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating Cluster %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Cluster %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Cluster", "name", a.id.External)
...
func (a *ClusterAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigtablepb.DeleteClusterRequest{Name: a.id.String()}
