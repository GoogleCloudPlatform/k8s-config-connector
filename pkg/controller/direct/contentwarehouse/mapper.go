...
func Policy_FromProto(mapCtx *direct.MapContext, in *iampb.Policy) *krm.Policy {
	if in == nil {
		return nil
	}
	out := &krm.Policy{}
	...
	if in.Bindings != nil {
		out.Bindings = make([]krm.Binding, len(in.Bindings))
		for i, b := range in.Bindings {
			out.Bindings[i] = krm.Binding{
				Role:    &b.Role,
				Members: b.Members,
			}
		}
...
func Policy_ToProto(mapCtx *direct.MapContext, in *krm.Policy) *iampb.Policy {
	if in == nil {
		return nil
	}
	out := &iampb.Policy{}
