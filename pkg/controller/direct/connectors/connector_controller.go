...
func init() {
	registry.RegisterModel(krm.ConnectorsConnectorGVK, NewConnectorModel)
}
...
type ConnectorAdapter struct {
	...
	desired   *krm.ConnectorsConnector
	actual    *connectorspb.Connector
}
...
func (a *ConnectorAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &connectorspb.GetConnectorRequest{Name: a.id}
...
func (a *ConnectorAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ConnectorsConnectorSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &connectorspb.CreateConnectorRequest{
	...
	status := &krm.ConnectorsConnectorStatus{}
	status.ObservedState = ConnectorsConnectorObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ConnectorAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ConnectorsConnectorSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ConnectorsConnectorStatus{}
		status.ObservedState = ConnectorsConnectorObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &connectorspb.UpdateConnectorRequest{
		Name:       a.id.External,
	...
	status := &krm.ConnectorsConnectorStatus{}
	status.ObservedState = ConnectorsConnectorObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ConnectorAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ConnectorsConnector{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ConnectorsConnectorSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ConnectorsConnectorGVK)
...
func (a *ConnectorAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &connectorspb.DeleteConnectorRequest{Name: a.id.String()}
