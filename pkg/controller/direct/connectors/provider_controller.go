...
func init() {
	registry.RegisterModel(krm.ConnectorsProviderGVK, NewProviderModel)
}
...
type ProviderAdapter struct {
	...
	desired   *krm.ConnectorsProvider
	actual    *connectorspb.Provider
}
...
func (a *ProviderAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &connectorspb.GetProviderRequest{Name: a.id}
...
func (a *ProviderAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ConnectorsProviderSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &connectorspb.CreateProviderRequest{
	...
	status := &krm.ConnectorsProviderStatus{}
	status.ObservedState = ConnectorsProviderObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ProviderAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ConnectorsProviderSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ConnectorsProviderStatus{}
		status.ObservedState = ConnectorsProviderObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &connectorspb.UpdateProviderRequest{
		Name:       a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating Provider %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Provider %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Provider", "name", a.id.External)

	status := &krm.ConnectorsProviderStatus{}
	status.ObservedState = ConnectorsProviderObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ProviderAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ConnectorsProvider{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ConnectorsProviderSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ConnectorsProviderGVK)
...
func (a *ProviderAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &connectorspb.DeleteProviderRequest{Name: a.id.String()}
