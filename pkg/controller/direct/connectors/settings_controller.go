...
func init() {
	registry.RegisterModel(krm.ConnectorsSettingsGVK, NewSettingsModel)
}
...
type SettingsAdapter struct {
	...
	desired   *krm.ConnectorsSettings
	actual    *connectorspb.Settings
}
...
func (a *SettingsAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &connectorspb.GetSettingsRequest{Name: a.id}
...
func (a *SettingsAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ConnectorsSettingsSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &connectorspb.CreateSettingsRequest{
	...
	status := &krm.ConnectorsSettingsStatus{}
	status.ObservedState = ConnectorsSettingsObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SettingsAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ConnectorsSettingsSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ConnectorsSettingsStatus{}
		status.ObservedState = ConnectorsSettingsObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &connectorspb.UpdateSettingsRequest{
		Name:       a.id.External,
	...
	status := &krm.ConnectorsSettingsStatus{}
	status.ObservedState = ConnectorsSettingsObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SettingsAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ConnectorsSettings{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ConnectorsSettingsSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ConnectorsSettingsGVK)
...
func (a *SettingsAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &connectorspb.DeleteSettingsRequest{Name: a.id.String()}
