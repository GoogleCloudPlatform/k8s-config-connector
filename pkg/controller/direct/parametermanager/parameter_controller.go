...
func (m *modelParameter) client(ctx context.Context) (*gcp.Client, error) {
...
func (a *ParameterAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &parametermanagerpb.GetParameterRequest{Name: a.id}
...
func (a *ParameterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	op, err := a.gcpClient.CreateParameter(ctx, req)
	if err != nil {
		return fmt.Errorf("creating Parameter %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Parameter %s waiting creation: %w", a.id, err)
	}
...
func (a *ParameterAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	op, err := a.gcpClient.UpdateParameter(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Parameter %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Parameter %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Parameter", "name", a.id.External)
...
func (a *ParameterAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	op, err := a.gcpClient.DeleteParameter(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete Parameter %s: %w", a.id, err)
	}
	return true, nil
}
