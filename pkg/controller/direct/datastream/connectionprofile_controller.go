...
type ConnectionProfileAdapter struct {
	...
	actual    *datastreampb.ConnectionProfile
}
...
func (a *ConnectionProfileAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datastreampb.GetConnectionProfileRequest{Name: a.id}
...
func (a *ConnectionProfileAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatastreamConnectionProfileSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatastreamConnectionProfileStatus{}
		status.ObservedState = DatastreamConnectionProfileObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *ConnectionProfileAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datastreampb.DeleteConnectionProfileRequest{Name: a.id.String()}
