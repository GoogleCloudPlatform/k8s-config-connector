...
func (a *FederationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &metastorepb.GetFederationRequest{Name: a.id}
...
func (a *FederationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Federation %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Federation %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Federation", "name", a.id.External)
