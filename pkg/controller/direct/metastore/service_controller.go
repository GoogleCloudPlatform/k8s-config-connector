...
import (
	...
	metastorepb "cloud.google.com/go/metastore/v1/metastorepb"
...
func init() {
	registry.RegisterModel(krm.MetastoreServiceGVK, NewServiceModel)
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &metastorepb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := MetastoreServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.MetastoreServiceStatus{}
		status.ObservedState = MetastoreServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *ServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &metastorepb.DeleteServiceRequest{Name: a.id.String()}
