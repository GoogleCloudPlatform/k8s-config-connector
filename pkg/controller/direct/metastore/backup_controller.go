...
func init() {
	registry.RegisterModel(krm.MetastoreBackupGVK, NewBackupModel)
}
...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &metastorepb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := MetastoreBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &metastorepb.UpdateBackupRequest{
		Name:       a.id.External,
	...
	status := &krm.MetastoreBackupStatus{}
	status.ObservedState = MetastoreBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &metastorepb.DeleteBackupRequest{Name: a.id.String()}
