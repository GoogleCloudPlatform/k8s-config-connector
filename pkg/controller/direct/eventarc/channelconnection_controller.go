...
func (a *ChannelConnectionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &eventarcpb.GetChannelConnectionRequest{Name: a.id}
...
func (a *ChannelConnectionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating ChannelConnection %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ChannelConnection %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ChannelConnection", "name", a.id.External)
