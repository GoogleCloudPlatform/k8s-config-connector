...
func (a *TagsTagKeyAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	log := klog.FromContext(ctx)
	log.V(0).Info("creating TagsTagKey", "name", fqn)
	...
	if err != nil {
		return fmt.Errorf("creating TagsTagKey %s: %w", fqn, err)
	}

	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("TagsTagKey %s waiting creation: %w", fqn, err)
	}
	log.V(0).Info("successfully created TagsTagKey", "name", fqn)
...
func (a *TagsTagKeyAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if len(req.UpdateMask.Paths) != 0 {
		...
		log.V(0).Info("successfully updated TagsTagKey", "name", fqn)
		latest = updated
	}
...
func (a *TagsTagKeyAdapter) changedFields(ctx context.Context) (*fieldmaskpb.FieldMask, error) {
	log := klog.FromContext(ctx)
