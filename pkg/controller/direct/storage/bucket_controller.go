...
func (m *modelBucket) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelBucket) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
type BucketAdapter struct {
	id        *krm.BucketIdentity
	gcpClient *gcp.Client
	desired   *krm.StorageBucket
	actual    *storagepb.Bucket
}
...
func (a *BucketAdapter) Find(ctx context.Context) (bool, error) {
	...
	log.V(2).Info("getting Bucket", "name", a.id)

	req := &storagepb.GetBucketRequest{Name: a.id}
	bucketpb, err := a.gcpClient.GetBucket(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("getting Bucket %q: %w", a.id, err)
	}
...
func (a *BucketAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating Bucket", "name", a.id)
	...
	op, err := a.gcpClient.CreateBucket(ctx, req)
	if err != nil {
		return fmt.Errorf("creating Bucket %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Bucket %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created Bucket", "name", a.id)
...
func (a *BucketAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating Bucket", "name", a.id)
	mapCtx := &direct.MapContext{}

	desiredPb := StorageBucketSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.StorageBucketStatus{}
		status.ObservedState = StorageBucketObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	op, err := a.gcpClient.UpdateBucket(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Bucket %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Bucket %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Bucket", "name", a.id.External)
...
func (a *BucketAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting Bucket", "name", a.id)

	req := &storagepb.DeleteBucketRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteBucket(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting Bucket %s: %w", a.id, err)
	}
	log.V(2).Info("successfully deleted Bucket", "name", a.id)

	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete Bucket %s: %w", a.id, err)
	}
	return true, nil
}
