...
func init() {
	registry.RegisterModel(krm.RunRevisionGVK, NewRevisionModel)
}
...
type RevisionAdapter struct {
	...
	desired   *krm.RunRevision
	actual    *runpb.Revision
}
...
func (a *RevisionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &runpb.GetRevisionRequest{Name: a.id}
...
func (a *RevisionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RunRevisionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &runpb.CreateRevisionRequest{
	...
	status := &krm.RunRevisionStatus{}
	status.ObservedState = RunRevisionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *RevisionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RunRevisionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RunRevisionStatus{}
		status.ObservedState = RunRevisionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &runpb.UpdateRevisionRequest{
		Name:       a.id.External,
	...
	status := &krm.RunRevisionStatus{}
	status.ObservedState = RunRevisionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *RevisionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.RunRevision{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(RunRevisionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.RunRevisionGVK)
...
func (a *RevisionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &runpb.DeleteRevisionRequest{Name: a.id.String()}
