...
func init() {
	registry.RegisterModel(krm.RunExecutionGVK, NewExecutionModel)
}
...
type ExecutionAdapter struct {
	...
	desired   *krm.RunExecution
	actual    *runpb.Execution
}
...
func (a *ExecutionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &runpb.GetExecutionRequest{Name: a.id}
...
func (a *ExecutionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RunExecutionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &runpb.CreateExecutionRequest{
	...
	status := &krm.RunExecutionStatus{}
	status.ObservedState = RunExecutionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ExecutionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RunExecutionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RunExecutionStatus{}
		status.ObservedState = RunExecutionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &runpb.UpdateExecutionRequest{
		Name:       a.id.External,
	...
	status := &krm.RunExecutionStatus{}
	status.ObservedState = RunExecutionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ExecutionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.RunExecution{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(RunExecutionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.RunExecutionGVK)
...
func (a *ExecutionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &runpb.DeleteExecutionRequest{Name: a.id.String()}
