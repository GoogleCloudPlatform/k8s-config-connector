...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &runpb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RunServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RunServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RunServiceStatus{}
		status.ObservedState = RunServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *ServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &runpb.DeleteServiceRequest{Name: a.id.String()}
