...
func init() {
	registry.RegisterModel(krm.FilestoreBackupGVK, NewBackupModel)
}
...
type BackupAdapter struct {
	...
	desired   *krm.FilestoreBackup
	actual    *filestorepb.Backup
}
...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &filestorepb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := FilestoreBackupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.FilestoreBackupStatus{}
	status.ObservedState = FilestoreBackupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := FilestoreBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.FilestoreBackupStatus{}
		status.ObservedState = FilestoreBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.FilestoreBackupStatus{}
	status.ObservedState = FilestoreBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.FilestoreBackup{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(FilestoreBackupSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.FilestoreBackupGVK)
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &filestorepb.DeleteBackupRequest{Name: a.id.String()}
