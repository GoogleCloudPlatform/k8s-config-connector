...
func (a *ServiceBindingAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &networkservicespb.GetServiceBindingRequest{Name: a.id}
...
func (a *ServiceBindingAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetworkservicesServiceBindingSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetworkservicesServiceBindingStatus{}
		status.ObservedState = NetworkservicesServiceBindingObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &networkservicespb.UpdateServiceBindingRequest{
		...
		ServiceBinding: desiredPb,
	}
	op, err := a.gcpClient.UpdateServiceBinding(ctx, req)
	if err != nil {
		return fmt.Errorf("updating ServiceBinding %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ServiceBinding %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ServiceBinding", "name", a.id.External)

	status := &krm.NetworkservicesServiceBindingStatus{}
	status.ObservedState = NetworkservicesServiceBindingObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
