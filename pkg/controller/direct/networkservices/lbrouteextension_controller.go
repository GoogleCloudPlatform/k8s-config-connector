...
func (m *modelLbRouteExtension) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelLbRouteExtension) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
func (a *LbRouteExtensionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &networkservicespb.GetLbRouteExtensionRequest{Name: a.id}
...
func (a *LbRouteExtensionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetworkservicesLbRouteExtensionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *LbRouteExtensionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetworkservicesLbRouteExtensionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetworkservicesLbRouteExtensionStatus{}
		status.ObservedState = NetworkservicesLbRouteExtensionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &networkservicespb.UpdateLbRouteExtensionRequest{
		...
		LbRouteExtension: desiredPb,
	}
	op, err := a.gcpClient.UpdateLbRouteExtension(ctx, req)
	if err != nil {
		return fmt.Errorf("updating LbRouteExtension %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("LbRouteExtension %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated LbRouteExtension", "name", a.id.External)
