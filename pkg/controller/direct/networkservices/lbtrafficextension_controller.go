...
func (m *modelLbTrafficExtension) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelLbTrafficExtension) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
type LbTrafficExtensionAdapter struct {
	...
	actual    *networkservicespb.LbTrafficExtension
}
...
func (a *LbTrafficExtensionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &networkservicespb.GetLbTrafficExtensionRequest{Name: a.id}
...
func (a *LbTrafficExtensionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetworkservicesLbTrafficExtensionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status.ObservedState = NetworkservicesLbTrafficExtensionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *LbTrafficExtensionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetworkservicesLbTrafficExtensionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetworkservicesLbTrafficExtensionStatus{}
		status.ObservedState = NetworkservicesLbTrafficExtensionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &networkservicespb.UpdateLbTrafficExtensionRequest{
		...
		LbTrafficExtension: desiredPb,
	}
	op, err := a.gcpClient.UpdateLbTrafficExtension(ctx, req)
	if err != nil {
		return fmt.Errorf("updating LbTrafficExtension %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("LbTrafficExtension %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated LbTrafficExtension", "name", a.id.External)

	status := &krm.NetworkservicesLbTrafficExtensionStatus{}
	status.ObservedState = NetworkservicesLbTrafficExtensionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *LbTrafficExtensionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetworkservicesLbTrafficExtension{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetworkservicesLbTrafficExtensionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
...
func (a *LbTrafficExtensionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &networkservicespb.DeleteLbTrafficExtensionRequest{Name: a.id.String()}
