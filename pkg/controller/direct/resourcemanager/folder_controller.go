...
import (
	...
	resourcemanagerpb "cloud.google.com/go/resourcemanager/v2/resourcemanagerpb"
...
func init() {
	registry.RegisterModel(krm.ResourcemanagerFolderGVK, NewFolderModel)
}
...
type FolderAdapter struct {
	...
	desired   *krm.ResourcemanagerFolder
	actual    *resourcemanagerpb.Folder
}
...
func (a *FolderAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &resourcemanagerpb.GetFolderRequest{Name: a.id}
...
func (a *FolderAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ResourcemanagerFolderSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ResourcemanagerFolderStatus{}
	status.ObservedState = ResourcemanagerFolderObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *FolderAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ResourcemanagerFolderSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ResourcemanagerFolderStatus{}
		status.ObservedState = ResourcemanagerFolderObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	if err != nil {
		return fmt.Errorf("updating Folder %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Folder %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Folder", "name", a.id.External)

	status := &krm.ResourcemanagerFolderStatus{}
	status.ObservedState = ResourcemanagerFolderObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *FolderAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ResourcemanagerFolder{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ResourcemanagerFolderSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ResourcemanagerFolderGVK)
...
func (a *FolderAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &resourcemanagerpb.DeleteFolderRequest{Name: a.id.String()}
