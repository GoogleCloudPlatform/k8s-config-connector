...
package mirroringdeploymentgroup
...
import (
	...
	v1alpha1 "github.com/GoogleCloudPlatform/k8s-config-connector/apis/networksecurity/v1alpha1"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/config"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/controller/direct"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/controller/direct/common"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/controller/direct/directbase"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/controller/direct/networksecurity"
...
func init() {
	registry.RegisterModel(krm.NetworkSecurityMirroringDeploymentGroupGVK, NewModel)
}
...
func (m *model) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	desired := networksecurity.NetworkSecurityMirroringDeploymentGroupSpec_v1alpha1_ToProto(mapCtx, &obj.Spec)
	...
	return &adapter{
		...
		id:               id.(*v1alpha1.NetworkSecurityMirroringDeploymentGroupIdentity),
		desired:          desired,
	}, nil
}
...
type adapter struct {
	...
	id               *v1alpha1.NetworkSecurityMirroringDeploymentGroupIdentity
...
func (a *adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("getting networksecurity mirroring deployment group after creation: %w", err)
	}
	...
	status.ObservedState = networksecurity.NetworkSecurityMirroringDeploymentGroupObservedState_v1alpha1_FromProto(mapCtx, actual)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	req := &pb.UpdateMirroringDeploymentGroupRequest{
		MirroringDeploymentGroup: &pb.MirroringDeploymentGroup{
			Name:        a.id.String(),
	...
	actual, err := a.gcpClient.GetMirroringDeploymentGroup(ctx, &pb.GetMirroringDeploymentGroupRequest{Name: a.id.String()})
	if err != nil {
		return fmt.Errorf("getting networksecurity mirroring deployment group after update: %w", err)
	}
...
func (a *adapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	log.V(2).Info("successfully deleted networksecurity mirroring deployment group", "name", a.id.String())
