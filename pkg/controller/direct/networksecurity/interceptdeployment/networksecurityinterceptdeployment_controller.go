...
func (m *model) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	desired := NetworkSecurityInterceptDeploymentSpec_ToProto(mapCtx, &obj.Spec)
	if mapCtx.Err() != nil {
...
func (a *Adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("InterceptDeployment %s waiting creation: %w", a.id, err)
	}

	// Fetch fully-populated resource after creation
	latest, err := a.gcpClient.GetInterceptDeployment(ctx, &pb.GetInterceptDeploymentRequest{Name: a.id.String()})
	if err != nil {
		return fmt.Errorf("getting InterceptDeployment after creation: %w", err)
	}
...
func (a *Adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	req.InterceptDeployment.Name = a.id.String()
	...
	latest, err := a.gcpClient.GetInterceptDeployment(ctx, &pb.GetInterceptDeploymentRequest{Name: a.id.String()})
	if err != nil {
		return fmt.Errorf("getting InterceptDeployment after update: %w", err)
	}

	log.V(2).Info("successfully updated InterceptDeployment", "name", a.id)
...
func compareInterceptDeployment(ctx context.Context, actual, desired *pb.InterceptDeployment) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	mapCtx := &direct.MapContext{}
	spec := NetworkSecurityInterceptDeploymentSpec_FromProto(mapCtx, actual)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
	maskedActual := NetworkSecurityInterceptDeploymentSpec_ToProto(mapCtx, spec)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
...
func (a *Adapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetworkSecurityInterceptDeployment{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetworkSecurityInterceptDeploymentSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
		return nil, mapCtx.Err()
	}
	...
	u.SetName(a.id.InterceptDeployment)
...
func (a *Adapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	if err != nil {
		if direct.IsNotFound(err) {
			log.V(2).Info("skipping delete for non-existent InterceptDeployment, assuming it was already deleted", "name", a.id)
			return true, nil
		}
	...
	err = a.waitForOperation(ctx, op)
	if err != nil {
		return false, fmt.Errorf("waiting delete InterceptDeployment %s: %w", a.id, err)
	}
...
func (a *Adapter) updateStatus(ctx context.Context, op directbase.Operation, latest *pb.InterceptDeployment) error {
	mapCtx := &direct.MapContext{}
	status := &krm.NetworkSecurityInterceptDeploymentStatus{}
	status.ObservedState = NetworkSecurityInterceptDeploymentObservedState_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
