...
func init() {
	registry.RegisterModel(krm.ChannelProductGVK, NewProductModel)
}
...
type ProductAdapter struct {
	...
	desired   *krm.ChannelProduct
	actual    *channelpb.Product
}
...
func (a *ProductAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &channelpb.GetProductRequest{Name: a.id}
...
func (a *ProductAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ChannelProductSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &channelpb.CreateProductRequest{
	...
	status := &krm.ChannelProductStatus{}
	status.ObservedState = ChannelProductObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ProductAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ChannelProductSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ChannelProductStatus{}
		status.ObservedState = ChannelProductObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &channelpb.UpdateProductRequest{
		Name:       a.id.External,
	...
	status := &krm.ChannelProductStatus{}
	status.ObservedState = ChannelProductObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ProductAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ChannelProduct{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ChannelProductSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ChannelProductGVK)
...
func (a *ProductAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &channelpb.DeleteProductRequest{Name: a.id.String()}
