...
func init() {
	registry.RegisterModel(krm.ChannelReportGVK, NewReportModel)
}
...
type ReportAdapter struct {
	...
	desired   *krm.ChannelReport
	actual    *channelpb.Report
}
...
func (a *ReportAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &channelpb.GetReportRequest{Name: a.id}
...
func (a *ReportAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ChannelReportSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &channelpb.CreateReportRequest{
	...
	status := &krm.ChannelReportStatus{}
	status.ObservedState = ChannelReportObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ReportAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ChannelReportSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ChannelReportStatus{}
		status.ObservedState = ChannelReportObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &channelpb.UpdateReportRequest{
		Name:       a.id.External,
	...
	status := &krm.ChannelReportStatus{}
	status.ObservedState = ChannelReportObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ReportAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ChannelReport{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ChannelReportSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ChannelReportGVK)
...
func (a *ReportAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &channelpb.DeleteReportRequest{Name: a.id.String()}
