...
func init() {
	registry.RegisterModel(krm.FirestoreBackupScheduleGVK, NewBackupScheduleModel)
}
...
type BackupScheduleAdapter struct {
	...
	desired   *krm.FirestoreBackupSchedule
	actual    *firestorepb.BackupSchedule
}
...
func (a *BackupScheduleAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &firestorepb.GetBackupScheduleRequest{Name: a.id}
...
func (a *BackupScheduleAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := FirestoreBackupScheduleSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &firestorepb.CreateBackupScheduleRequest{
	...
	status.ObservedState = FirestoreBackupScheduleObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupScheduleAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := FirestoreBackupScheduleSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.FirestoreBackupScheduleStatus{}
		status.ObservedState = FirestoreBackupScheduleObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &firestorepb.UpdateBackupScheduleRequest{
		Name:           a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating BackupSchedule %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("BackupSchedule %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated BackupSchedule", "name", a.id.External)

	status := &krm.FirestoreBackupScheduleStatus{}
	status.ObservedState = FirestoreBackupScheduleObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupScheduleAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.FirestoreBackupSchedule{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(FirestoreBackupScheduleSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.FirestoreBackupScheduleGVK)
...
func (a *BackupScheduleAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &firestorepb.DeleteBackupScheduleRequest{Name: a.id.String()}
