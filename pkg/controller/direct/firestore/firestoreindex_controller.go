...
func (a *firestoreIndexAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := proto.Clone(a.desired).(*pb.Index)
	...
	opClient, err := newOperationsClient(ctx, a.config)
	if err != nil {
		return fmt.Errorf("building operations client: %w", err)
	}

	created := &pb.Index{}
	if err := direct.WaitOperation(ctx, opClient, op, created); err != nil {
