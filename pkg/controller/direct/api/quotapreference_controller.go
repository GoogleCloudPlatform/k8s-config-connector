...
import (
	...
	gcp "cloud.google.com/go/cloudquotas/apiv1"
...
func (m *modelQuotaPreference) client(ctx context.Context) (*gcp.CloudQuotasClient, error) {
	var opts []option.ClientOption
	opts, err := m.config.RESTClientOptions()
	if err != nil {
		return nil, fmt.Errorf("building QuotaPreference REST client options: %w", err)
	}
	gcpClient, err := gcp.NewCloudQuotasClient(ctx, opts...)
	if err != nil {
...
type QuotaPreferenceAdapter struct {
	...
	actual    *cloudquotaspb.QuotaPreference
}
...
func (a *QuotaPreferenceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &cloudquotaspb.GetQuotaPreferenceRequest{Name: fmt.Sprintf("projects/%s/locations/%s/quotaPreferences/%s", a.desired.Spec.ProjectRef.External, a.desired.Spec.Location, *a.id)}
...
func (a *QuotaPreferenceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	req := &cloudquotaspb.CreateQuotaPreferenceRequest{
		Parent:          fmt.Sprintf("projects/%s/locations/%s", a.desired.Spec.ProjectRef.External, a.desired.Spec.Location),
		QuotaPreference: resource,
		QuotaPreferenceId: *a.id,
	}
	op, err := a.gcpClient.CreateQuotaPreference(ctx, req)
	if err != nil {
		return fmt.Errorf("creating QuotaPreference %s: %w", a.id, err)
	}
	created, err := a.gcpClient.GetQuotaPreference(ctx, &cloudquotaspb.GetQuotaPreferenceRequest{Name: *a.id})
	if err != nil {
	...
	status.ExternalRef = direct.LazyPtr(safeString(a.id))
...
func (a *QuotaPreferenceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating QuotaPreference %s: %w", a.id, err)
	}
	updated, err := a.gcpClient.GetQuotaPreference(ctx, &cloudquotaspb.GetQuotaPreferenceRequest{Name: *a.id})
	if err != nil {
		return fmt.Errorf("QuotaPreference %s waiting update: %w", a.id, err)
	}
	log.V(2).Info("successfully updated QuotaPreference", "name", a.id)
...
func (a *QuotaPreferenceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj.Spec.ProjectRef = &refs.ProjectRef{External: a.desired.Spec.ProjectRef.External}
	obj.Spec.Location = a.desired.Spec.Location
...
func (a *QuotaPreferenceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &cloudquotaspb.DeleteQuotaPreferenceRequest{Name: fmt.Sprintf("projects/%s/locations/%s/quotaPreferences/%s", a.desired.Spec.ProjectRef.External, a.desired.Spec.Location, *a.id)}
	_, err := a.gcpClient.DeleteQuotaPreference(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			// Return success if not found (assume it was already deleted).
			log.V(2).Info("skipping delete for non-existent QuotaPreference, assuming it was already deleted", "name", safeString(a.id))
			return true, nil
		}
