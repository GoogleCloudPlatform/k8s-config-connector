...
func init() {
	registry.RegisterModel(krm.ApiServiceGVK, NewServiceModel)
}
...
type ServiceAdapter struct {
	...
	desired   *krm.ApiService
	actual    *apipb.Service
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apipb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ApiServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &apipb.CreateServiceRequest{
	...
	status := &krm.ApiServiceStatus{}
	status.ObservedState = ApiServiceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApiServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApiServiceStatus{}
		status.ObservedState = ApiServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &apipb.UpdateServiceRequest{
		Name:       a.id.External,
	...
	status := &krm.ApiServiceStatus{}
	status.ObservedState = ApiServiceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ServiceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ApiService{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ApiServiceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ApiServiceGVK)
...
func (a *ServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &apipb.DeleteServiceRequest{Name: a.id.String()}
