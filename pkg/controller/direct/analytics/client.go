...
func (m *gcpClient) newAnalyticsAdminClient(ctx context.Context) (*gcp.AnalyticsAdminClient, error) {
	opts, err := m.config.RESTClientOptions()
	if err != nil {
		return nil, err
	}
	client, err := gcp.NewAnalyticsAdminRESTClient(ctx, opts...)
	if err != nil {
		return nil, fmt.Errorf("building analyticsadmin client: %w", err)
	}
	return client, err
}
