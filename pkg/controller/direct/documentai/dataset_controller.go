...
func init() {
	registry.RegisterModel(krm.DocumentaiDatasetGVK, NewDatasetModel)
}
...
type DatasetAdapter struct {
	...
	desired   *krm.DocumentaiDataset
	actual    *documentaipb.Dataset
}
...
func (a *DatasetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &documentaipb.GetDatasetRequest{Name: a.id}
...
func (a *DatasetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DocumentaiDatasetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &documentaipb.CreateDatasetRequest{
	...
	status := &krm.DocumentaiDatasetStatus{}
	status.ObservedState = DocumentaiDatasetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatasetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DocumentaiDatasetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DocumentaiDatasetStatus{}
		status.ObservedState = DocumentaiDatasetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.DocumentaiDatasetStatus{}
	status.ObservedState = DocumentaiDatasetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatasetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DocumentaiDataset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DocumentaiDatasetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DocumentaiDatasetGVK)
...
func (a *DatasetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &documentaipb.DeleteDatasetRequest{Name: a.id.String()}
