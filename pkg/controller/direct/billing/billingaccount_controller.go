...
func init() {
	registry.RegisterModel(krm.BillingBillingAccountGVK, NewBillingAccountModel)
}
...
type BillingAccountAdapter struct {
	...
	desired   *krm.BillingBillingAccount
	actual    *billingpb.BillingAccount
}
...
func (a *BillingAccountAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &billingpb.GetBillingAccountRequest{Name: a.id}
...
func (a *BillingAccountAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BillingBillingAccountSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.BillingBillingAccountStatus{}
	status.ObservedState = BillingBillingAccountObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BillingAccountAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BillingBillingAccountSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BillingBillingAccountStatus{}
		status.ObservedState = BillingBillingAccountObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	if err != nil {
		return fmt.Errorf("updating BillingAccount %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("BillingAccount %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated BillingAccount", "name", a.id.External)

	status := &krm.BillingBillingAccountStatus{}
	status.ObservedState = BillingBillingAccountObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BillingAccountAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BillingBillingAccount{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BillingBillingAccountSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BillingBillingAccountGVK)
...
func (a *BillingAccountAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &billingpb.DeleteBillingAccountRequest{Name: a.id.String()}
