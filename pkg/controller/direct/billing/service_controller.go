...
func init() {
	registry.RegisterModel(krm.BillingServiceGVK, NewServiceModel)
}
...
type ServiceAdapter struct {
	...
	desired   *krm.BillingService
	actual    *billingpb.Service
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &billingpb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BillingServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &billingpb.CreateServiceRequest{
	...
	status := &krm.BillingServiceStatus{}
	status.ObservedState = BillingServiceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BillingServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BillingServiceStatus{}
		status.ObservedState = BillingServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &billingpb.UpdateServiceRequest{
		Name:       a.id.External,
	...
	status := &krm.BillingServiceStatus{}
	status.ObservedState = BillingServiceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ServiceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BillingService{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BillingServiceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BillingServiceGVK)
...
func (a *ServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &billingpb.DeleteServiceRequest{Name: a.id.String()}
