...
func init() {
	registry.RegisterModel(krm.TranslationExampleGVK, NewExampleModel)
}
...
type ExampleAdapter struct {
	...
	desired   *krm.TranslationExample
	actual    *translationpb.Example
}
...
func (a *ExampleAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &translationpb.GetExampleRequest{Name: a.id}
...
func (a *ExampleAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := TranslationExampleSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &translationpb.CreateExampleRequest{
	...
	status := &krm.TranslationExampleStatus{}
	status.ObservedState = TranslationExampleObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ExampleAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := TranslationExampleSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.TranslationExampleStatus{}
		status.ObservedState = TranslationExampleObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &translationpb.UpdateExampleRequest{
		Name:       a.id.External,
	...
	status := &krm.TranslationExampleStatus{}
	status.ObservedState = TranslationExampleObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ExampleAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.TranslationExample{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(TranslationExampleSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.TranslationExampleGVK)
...
func (a *ExampleAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &translationpb.DeleteExampleRequest{Name: a.id.String()}
