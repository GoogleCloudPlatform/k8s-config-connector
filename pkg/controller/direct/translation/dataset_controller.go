...
func init() {
	registry.RegisterModel(krm.TranslationDatasetGVK, NewDatasetModel)
}
...
type DatasetAdapter struct {
	...
	desired   *krm.TranslationDataset
	actual    *translationpb.Dataset
}
...
func (a *DatasetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &translationpb.GetDatasetRequest{Name: a.id}
...
func (a *DatasetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := TranslationDatasetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.TranslationDatasetStatus{}
	status.ObservedState = TranslationDatasetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatasetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := TranslationDatasetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.TranslationDatasetStatus{}
		status.ObservedState = TranslationDatasetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &translationpb.UpdateDatasetRequest{
		Name:       a.id.External,
	...
	status := &krm.TranslationDatasetStatus{}
	status.ObservedState = TranslationDatasetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatasetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.TranslationDataset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(TranslationDatasetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.TranslationDatasetGVK)
...
func (a *DatasetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &translationpb.DeleteDatasetRequest{Name: a.id.String()}
