...
func init() {
	registry.RegisterModel(krm.TranslationModelGVK, NewModelModel)
}
...
type ModelAdapter struct {
	...
	desired   *krm.TranslationModel
	actual    *translationpb.Model
}
...
func (a *ModelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &translationpb.GetModelRequest{Name: a.id}
...
func (a *ModelAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := TranslationModelSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.TranslationModelStatus{}
	status.ObservedState = TranslationModelObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ModelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := TranslationModelSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.TranslationModelStatus{}
		status.ObservedState = TranslationModelObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &translationpb.UpdateModelRequest{
		Name:       a.id.External,
	...
	status := &krm.TranslationModelStatus{}
	status.ObservedState = TranslationModelObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ModelAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.TranslationModel{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(TranslationModelSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.TranslationModelGVK)
...
func (a *ModelAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &translationpb.DeleteModelRequest{Name: a.id.String()}
