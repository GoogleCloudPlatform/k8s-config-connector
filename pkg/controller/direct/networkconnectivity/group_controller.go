...
func init() {
	registry.RegisterModel(krm.NetworkconnectivityGroupGVK, NewGroupModel)
}
...
func (m *modelGroup) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.NetworkconnectivityGroup{}
...
type GroupAdapter struct {
	...
	desired   *krm.NetworkconnectivityGroup
	actual    *networkconnectivitypb.Group
}
...
func (a *GroupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &networkconnectivitypb.GetGroupRequest{Name: a.id}
...
func (a *GroupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetworkconnectivityGroupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &networkconnectivitypb.CreateGroupRequest{
	...
	status := &krm.NetworkconnectivityGroupStatus{}
	status.ObservedState = NetworkconnectivityGroupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *GroupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetworkconnectivityGroupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetworkconnectivityGroupStatus{}
		status.ObservedState = NetworkconnectivityGroupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.NetworkconnectivityGroupStatus{}
	status.ObservedState = NetworkconnectivityGroupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *GroupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetworkconnectivityGroup{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetworkconnectivityGroupSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.NetworkconnectivityGroupGVK)
...
func (a *GroupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &networkconnectivitypb.DeleteGroupRequest{Name: a.id.String()}
