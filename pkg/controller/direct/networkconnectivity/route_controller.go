...
func init() {
	registry.RegisterModel(krm.NetworkconnectivityRouteGVK, NewRouteModel)
}
...
func (m *modelRoute) client(ctx context.Context) (*gcp.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building Route client: %w", err)
	}
...
func (m *modelRoute) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.NetworkconnectivityRoute{}
...
type RouteAdapter struct {
	...
	desired   *krm.NetworkconnectivityRoute
	actual    *networkconnectivitypb.Route
}
...
func (a *RouteAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &networkconnectivitypb.GetRouteRequest{Name: a.id}
...
func (a *RouteAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetworkconnectivityRouteSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &networkconnectivitypb.CreateRouteRequest{
	...
	status := &krm.NetworkconnectivityRouteStatus{}
	status.ObservedState = NetworkconnectivityRouteObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *RouteAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetworkconnectivityRouteSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetworkconnectivityRouteStatus{}
		status.ObservedState = NetworkconnectivityRouteObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &networkconnectivitypb.UpdateRouteRequest{
		Name:       a.id.External,
		UpdateMask: updateMask,
		Route:      desiredPb,
	}
	op, err := a.gcpClient.UpdateRoute(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Route %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Route %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Route", "name", a.id.External)

	status := &krm.NetworkconnectivityRouteStatus{}
	status.ObservedState = NetworkconnectivityRouteObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *RouteAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetworkconnectivityRoute{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetworkconnectivityRouteSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.NetworkconnectivityRouteGVK)
...
func (a *RouteAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &networkconnectivitypb.DeleteRouteRequest{Name: a.id.String()}
