...
func (m *regionalEndpointModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
...
func (a *regionalEndpointAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	op, err := a.gcpClient.Projects.Locations.RegionalEndpoints.Create(parent, req).RegionalEndpointId(a.id.RegionalEndpoint).Context(ctx).Do()
	if err != nil {
...
func (a *regionalEndpointAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating networkconnectivity regionalendpoint", "name", a.id)
	mapCtx := &direct.MapContext{}

	// RegionalEndpoint does not have any mutable fields in the API.
...
func (a *regionalEndpointAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	fqn := a.id.String()
	op, err := a.gcpClient.Projects.Locations.RegionalEndpoints.Delete(fqn).Context(ctx).Do()
	if err != nil {
		if direct.IsNotFound(err) {
