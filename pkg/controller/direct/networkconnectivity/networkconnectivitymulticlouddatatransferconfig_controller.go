...
func (a *multicloudDataTransferConfigAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	fqn := a.id.String()
	log.V(2).Info("deleting NetworkConnectivityMulticloudDataTransferConfig", "name", fqn)

	op, err := a.gcpClient.Projects.Locations.MulticloudDataTransferConfigs.Delete(fqn).Context(ctx).Do()
	if err != nil {
		if direct.IsNotFound(err) {
			log.V(2).Info("skipping delete for non-existent NetworkConnectivityMulticloudDataTransferConfig, assuming it was already deleted", "name", fqn)
			return true, nil
		}
		return false, fmt.Errorf("deleting MulticloudDataTransferConfig %q: %w", fqn, err)
	}
	log.V(2).Info("successfully deleted NetworkConnectivityMulticloudDataTransferConfig", "name", fqn)
