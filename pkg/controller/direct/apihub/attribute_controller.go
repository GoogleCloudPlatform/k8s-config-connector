...
func (a *AttributeAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apihubpb.GetAttributeRequest{Name: a.id}
...
func (a *AttributeAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Attribute %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Attribute %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Attribute", "name", a.id.External)
