...
func (a *KmsConfigAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &netapppb.GetKmsConfigRequest{Name: a.id}
...
func (a *KmsConfigAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating KmsConfig %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("KmsConfig %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated KmsConfig", "name", a.id.External)
