...
func (a *BackupVaultAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BackupVaultSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupVaultAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BackupVaultSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	desiredPb.Name = a.id.String()

	paths, err := computeDiffBackupVault(mapCtx, a.actual, desiredPb)
	if err != nil {
		return err
	}
...
func (a *BackupVaultAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetAppBackupVault{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BackupVaultSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
...
func computeDiffBackupVault(mapCtx *direct.MapContext, actual, desired *netapppb.BackupVault) ([]string, error) {
