...
func (a *BackupVaultAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &netapppb.GetBackupVaultRequest{Name: a.id}
...
func (a *BackupVaultAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetappBackupVaultSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.NetappBackupVaultStatus{}
	status.ObservedState = NetappBackupVaultObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupVaultAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetappBackupVaultSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetappBackupVaultStatus{}
		status.ObservedState = NetappBackupVaultObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	if err != nil {
		return fmt.Errorf("updating BackupVault %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("BackupVault %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated BackupVault", "name", a.id.External)

	status := &krm.NetappBackupVaultStatus{}
	status.ObservedState = NetappBackupVaultObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupVaultAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetappBackupVault{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetappBackupVaultSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
