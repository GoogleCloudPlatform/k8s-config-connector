...
func init() {
	registry.RegisterModel(krm.NetappSnapshotGVK, NewSnapshotModel)
}
...
type SnapshotAdapter struct {
	...
	desired   *krm.NetappSnapshot
	actual    *netapppb.Snapshot
}
...
func (a *SnapshotAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &netapppb.GetSnapshotRequest{Name: a.id}
...
func (a *SnapshotAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NetappSnapshotSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.NetappSnapshotStatus{}
	status.ObservedState = NetappSnapshotObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SnapshotAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NetappSnapshotSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NetappSnapshotStatus{}
		status.ObservedState = NetappSnapshotObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.NetappSnapshotStatus{}
	status.ObservedState = NetappSnapshotObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SnapshotAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NetappSnapshot{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NetappSnapshotSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.NetappSnapshotGVK)
...
func (a *SnapshotAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &netapppb.DeleteSnapshotRequest{Name: a.id.String()}
