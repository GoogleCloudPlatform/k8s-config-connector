...
func (a *CatalogAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	// MetastoreService doesn't have an UpdateCatalog API. We return an error if there's actually a diff,
	// though there should never be one since there are no mutable fields on BigLakeCatalog other than ID/Parent.
	return fmt.Errorf("updating Catalog is not supported: %s (fields diff: %v)", a.id.String(), updateMask.Paths)
}
