...
func (m *model) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	if externalRef == "" {
		...
		if resourceID != "" && id.TransferConfig != resourceID {
			return nil, fmt.Errorf("BigQueryDataTransferConfig %s/%s spec.resourceID changed or does not match the service generated ID, expect %s, got %s",
				u.GetNamespace(), u.GetName(), id.TransferConfig, resourceID)
		}
	}
...
func (a *Adapter) Find(ctx context.Context) (bool, error) {
	...
	if a.id.TransferConfig == "" { // resource ID is not yet generated by the GCP service
		return false, nil
	}

	log.V(2).Info("getting BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
	req := &bigquerydatatransferpb.GetTransferConfigRequest{Name: FullyQualifiedName(a.id)}
	transferconfigpb, err := a.gcpClient.GetTransferConfig(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("getting BigQueryDataTransferConfig %q: %w", FullyQualifiedName(a.id), err)
	}
...
func (a *Adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
	...
	if err != nil {
		return fmt.Errorf("creating BigQueryDataTransferConfig %s: %w", FullyQualifiedName(a.id), err)
	}
	log.V(2).Info("successfully created BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
	...
	a.id.TransferConfig = serviceGeneratedID
...
func (a *Adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log := klog.FromContext(ctx)
	log.V(2).Info("updating BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
	mapCtx := &direct.MapContext{}
	...
	resource.Name = FullyQualifiedName(a.id) // need to pass service generated ID to GCP API to identify the GCP resource
	...
	if err != nil {
		return fmt.Errorf("updating BigQueryDataTransferConfig %s: %w", FullyQualifiedName(a.id), err)
	}
	log.V(2).Info("successfully updated BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
...
func (a *Adapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj.Spec.ProjectRef = &refv1beta1.ProjectRef{Name: a.id.Project}
...
func (a *Adapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))

	if a.id.TransferConfig == "" {
		return false, nil
	}
	req := &bigquerydatatransferpb.DeleteTransferConfigRequest{Name: FullyQualifiedName(a.id)}
	err := a.gcpClient.DeleteTransferConfig(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting BigQueryDataTransferConfig %s: %w", FullyQualifiedName(a.id), err)
	}
	log.V(2).Info("successfully deleted BigQueryDataTransferConfig", "name", FullyQualifiedName(a.id))
