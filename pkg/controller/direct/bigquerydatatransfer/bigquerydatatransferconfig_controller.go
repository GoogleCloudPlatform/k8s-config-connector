...
func (m *model) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	id := identity.(*krm.BigQueryDataTransferConfigIdentity)

	// Before creation, if spec.resourceID is empty, the transferConfig ID is not yet generated.
	if obj.Status.ExternalRef == nil && obj.Spec.ResourceID == nil {
		id.TransferConfig = ""
	}
...
func (a *Adapter) Find(ctx context.Context) (bool, error) {
	...
	log.V(2).Info("getting BigQueryDataTransferConfig", "name", a.id.String())
	req := &bigquerydatatransferpb.GetTransferConfigRequest{Name: a.id.String()}
	transferconfigpb, err := a.gcpClient.GetTransferConfig(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("getting BigQueryDataTransferConfig %q: %w", a.id.String(), err)
	}
...
func (a *Adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating BigQueryDataTransferConfig", "name", a.id.String())
	...
	if err != nil {
		return fmt.Errorf("creating BigQueryDataTransferConfig %s: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully created BigQueryDataTransferConfig", "name", a.id.String())
...
func (a *Adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log := klog.FromContext(ctx)
	log.V(2).Info("updating BigQueryDataTransferConfig", "name", a.id.String())
	mapCtx := &direct.MapContext{}
	...
	resource.Name = a.id.String() // need to pass service generated ID to GCP API to identify the GCP resource
	...
	if err != nil {
		return fmt.Errorf("updating BigQueryDataTransferConfig %s: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully updated BigQueryDataTransferConfig", "name", a.id.String())
...
func (a *Adapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting BigQueryDataTransferConfig", "name", a.id.String())

	if a.id.TransferConfig == "" {
		return false, nil
	}
	req := &bigquerydatatransferpb.DeleteTransferConfigRequest{Name: a.id.String()}
	err := a.gcpClient.DeleteTransferConfig(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting BigQueryDataTransferConfig %s: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully deleted BigQueryDataTransferConfig", "name", a.id.String())
