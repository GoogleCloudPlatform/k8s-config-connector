...
import (
	...
	managedidentitiespb "cloud.google.com/go/managedidentities/v1/managedidentitiespb"
...
func (a *DomainAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &managedidentitiespb.GetDomainRequest{Name: a.id}
...
func (a *DomainAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Domain %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Domain %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Domain", "name", a.id.External)
