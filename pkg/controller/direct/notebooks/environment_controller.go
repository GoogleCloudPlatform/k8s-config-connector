...
type EnvironmentAdapter struct {
	...
	actual    *notebookspb.Environment
}
...
func (a *EnvironmentAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &notebookspb.GetEnvironmentRequest{Name: a.id}
...
func (a *EnvironmentAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NotebooksEnvironmentSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NotebooksEnvironmentStatus{}
		status.ObservedState = NotebooksEnvironmentObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &notebookspb.UpdateEnvironmentRequest{
		Name:        a.id.External,
	...
	status := &krm.NotebooksEnvironmentStatus{}
	status.ObservedState = NotebooksEnvironmentObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EnvironmentAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &notebookspb.DeleteEnvironmentRequest{Name: a.id.String()}
