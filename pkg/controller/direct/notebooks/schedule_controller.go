...
func (m *modelSchedule) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelSchedule) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
type ScheduleAdapter struct {
	id        *krm.ScheduleIdentity
	gcpClient *gcp.Client
	desired   *krm.NotebooksSchedule
	actual    *notebookspb.Schedule
}
...
func (a *ScheduleAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &notebookspb.GetScheduleRequest{Name: a.id}
...
func (a *ScheduleAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := NotebooksScheduleSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if err != nil {
		return fmt.Errorf("Schedule %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created Schedule", "name", a.id)

	status := &krm.NotebooksScheduleStatus{}
	status.ObservedState = NotebooksScheduleObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ScheduleAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := NotebooksScheduleSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.NotebooksScheduleStatus{}
		status.ObservedState = NotebooksScheduleObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &notebookspb.UpdateScheduleRequest{
		Name:       a.id.External,
		UpdateMask: updateMask,
		Schedule:   desiredPb,
	}
	op, err := a.gcpClient.UpdateSchedule(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Schedule %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Schedule %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Schedule", "name", a.id.External)

	status := &krm.NotebooksScheduleStatus{}
	status.ObservedState = NotebooksScheduleObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ScheduleAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.NotebooksSchedule{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(NotebooksScheduleSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
...
func (a *ScheduleAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &notebookspb.DeleteScheduleRequest{Name: a.id.String()}
