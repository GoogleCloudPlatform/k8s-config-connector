...
func (m *recordSetModel) AdapterForURL(ctx context.Context, url string) (directbase.Adapter, error) {
	...
	// Handle the /{type} suffix at the end of the URL
	if parts := strings.Split(url, "/"); len(parts) >= 3 && parts[len(parts)-3] == "rrsets" {
		recordType = parts[len(parts)-1]
		trimmedURL = strings.Join(parts[:len(parts)-1], "/")
	}

	id := &krm.DNSRecordSetIdentity{}
	if err := id.FromExternal(trimmedURL); err != nil {
		// Not recognized
		return nil, nil
	}
