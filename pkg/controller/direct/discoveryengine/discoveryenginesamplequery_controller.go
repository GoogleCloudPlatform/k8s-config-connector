...
func (m *sampleQueryModel) client(ctx context.Context, projectID string) (*gcp.SampleQueryClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building discoveryengine sample query client: %w", err)
	}
...
func (m *sampleQueryModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	id := identity.(*krm.DiscoveryEngineSampleQueryIdentity)

	mapCtx := &direct.MapContext{}
	desired := DiscoveryEngineSampleQuerySpec_v1alpha1_ToProto(mapCtx, &obj.Spec)
	if mapCtx.Err() != nil {
...
type sampleQueryAdapter struct {
	...
	id        *krm.DiscoveryEngineSampleQueryIdentity
	desired   *discoveryenginepb.SampleQuery
	actual    *discoveryenginepb.SampleQuery
}
...
func (a *sampleQueryAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &discoveryenginepb.GetSampleQueryRequest{Name: a.id.String()}
...
func (a *sampleQueryAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	desired := proto.Clone(a.desired).(*discoveryenginepb.SampleQuery)
...
func (a *sampleQueryAdapter) compare(ctx context.Context, actual, desired *discoveryenginepb.SampleQuery) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	maskedActual, err := mappers.OnlySpecFields(actual, DiscoveryEngineSampleQuerySpec_v1alpha1_FromProto, DiscoveryEngineSampleQuerySpec_v1alpha1_ToProto)
	if err != nil {
		return nil, nil, err
	}
	maskedActual.Name = desired.Name

	clonedDesired := proto.Clone(desired).(*discoveryenginepb.SampleQuery)
...
func (a *sampleQueryAdapter) updateStatus(ctx context.Context, op directbase.Operation, latest *discoveryenginepb.SampleQuery) error {
	status := &krm.DiscoveryEngineSampleQueryStatus{}
	mapCtx := &direct.MapContext{}
	status.ObservedState = DiscoveryEngineSampleQueryObservedState_v1alpha1_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *sampleQueryAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DiscoveryEngineSampleQuery{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DiscoveryEngineSampleQuerySpec_v1alpha1_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
		return nil, mapCtx.Err()
	}
	obj.Spec.ProjectRef = &refs.ProjectRef{External: a.id.Project}
	obj.Spec.Location = &a.id.Location
	obj.Spec.SampleQuerySetRef = &krm.DiscoveryEngineSampleQuerySetRef{
		External: fmt.Sprintf("projects/%s/locations/%s/sampleQuerySets/%s", a.id.Project, a.id.Location, a.id.SampleQuerySet),
	}
	obj.Spec.ResourceID = &a.id.SampleQuery
...
func (a *sampleQueryAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.Info("deleting discoveryengine sample query", "name", a.id.String())

	req := &discoveryenginepb.DeleteSampleQueryRequest{Name: a.id.String()}
	...
	log.V(2).Info("successfully deleted discoveryengine sample query in gcp", "name", a.id)
	return true, nil
}
