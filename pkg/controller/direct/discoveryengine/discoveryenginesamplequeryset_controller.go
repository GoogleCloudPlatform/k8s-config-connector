...
func (m *sampleQuerySetModel) client(ctx context.Context, projectID string) (*gcp.SampleQuerySetClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building discoveryengine sample query set client: %w", err)
	}
...
func (m *sampleQuerySetModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	id := identity.(*krm.DiscoveryEngineSampleQuerySetIdentity)

	mapCtx := &direct.MapContext{}
	desired := DiscoveryEngineSampleQuerySetSpec_v1alpha1_ToProto(mapCtx, &obj.Spec)
	if mapCtx.Err() != nil {
...
type sampleQuerySetAdapter struct {
	...
	id        *krm.DiscoveryEngineSampleQuerySetIdentity
	desired   *discoveryenginepb.SampleQuerySet
...
func (a *sampleQuerySetAdapter) Find(ctx context.Context) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("getting discoveryengine sample query set", "name", a.id.String())

	req := &discoveryenginepb.GetSampleQuerySetRequest{Name: a.id.String()}
...
func (a *sampleQuerySetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating discoveryengine sample query set", "name", a.id.String())

	desired := proto.Clone(a.desired).(*discoveryenginepb.SampleQuerySet)
	...
	log.V(2).Info("successfully created discoveryengine sample query set in gcp", "name", a.id.String())
...
func (a *sampleQuerySetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating discoveryengine sample query set", "name", a.id.String())
	...
	log.V(2).Info("successfully updated discoveryengine sample query set", "name", a.id.String())

	return a.updateStatus(ctx, updateOp, updated)
}

func (a *sampleQuerySetAdapter) compare(ctx context.Context, actual, desired *discoveryenginepb.SampleQuerySet) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	maskedActual, err := mappers.OnlySpecFields(actual, DiscoveryEngineSampleQuerySetSpec_v1alpha1_FromProto, DiscoveryEngineSampleQuerySetSpec_v1alpha1_ToProto)
	if err != nil {
		return nil, nil, err
	}
	maskedActual.Name = desired.Name // Restore identifier field

	clonedDesired := proto.Clone(desired).(*discoveryenginepb.SampleQuerySet)
...
func (a *sampleQuerySetAdapter) updateStatus(ctx context.Context, op directbase.Operation, latest *discoveryenginepb.SampleQuerySet) error {
	status := &krm.DiscoveryEngineSampleQuerySetStatus{}
	mapCtx := &direct.MapContext{}
	status.ObservedState = DiscoveryEngineSampleQuerySetObservedState_v1alpha1_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *sampleQuerySetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DiscoveryEngineSampleQuerySet{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DiscoveryEngineSampleQuerySetSpec_v1alpha1_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
		return nil, mapCtx.Err()
	}
...
func (a *sampleQuerySetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting discoveryengine sample query set", "name", a.id.String())
	...
	log.V(2).Info("successfully deleted discoveryengine sample query set", "name", a.id.String())
	return true, nil
}
