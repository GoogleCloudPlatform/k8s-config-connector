...
func (m *targetSiteModel) AdapterForURL(ctx context.Context, url string) (directbase.Adapter, error) {
	...
	if strings.HasPrefix(url, "//discoveryengine.googleapis.com/") {
		...
		if err != nil {
			...
			return &targetSiteAdapter{
				gcpClient:     gcpClient,
				dataStoreLink: id.DiscoveryEngineDataStoreID,
				id:            id,
			}, nil
		}
	}
...
type targetSiteAdapter struct {
	...
	dataStoreLink *krm.DiscoveryEngineDataStoreID
...
func (a *targetSiteAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating discoveryengine targetsite", "datastore", a.dataStoreLink)

	desired := proto.CloneOf(a.desired)

	parent := a.dataStoreLink.String() + "/siteSearchEngine"
	...
	if err != nil {
		return fmt.Errorf("creating discoveryengine targetsite for %s: %w", parent, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("discoveryengine targetsite for %s waiting creation: %w", parent, err)
	}
	log.V(2).Info("successfully created discoveryengine targetsite in gcp", "name", created.GetName())

	tokens := strings.Split(created.GetName(), "/")
	if len(tokens) != 11 {
		return fmt.Errorf("unexpected format of created target site name: %q", created.GetName())
	}
	targetSiteID := tokens[10]
	a.id = &krm.TargetSiteIdentity{
		DiscoveryEngineDataStoreID: a.dataStoreLink,
...
func (a *targetSiteAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("waiting for update of discoveryengine targetsite %s: %w", a.id.String(), err)
	}
...
func (a *targetSiteAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj.Spec.DataStoreRef = &krm.DiscoveryEngineDataStoreRef{External: a.dataStoreLink.String()}
