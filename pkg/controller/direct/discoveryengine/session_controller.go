...
type SessionAdapter struct {
	...
	desired   *krm.DiscoveryengineSession
	actual    *discoveryenginepb.Session
}
...
func (a *SessionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &discoveryenginepb.GetSessionRequest{Name: a.id}
...
func (a *SessionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DiscoveryengineSessionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status.ObservedState = DiscoveryengineSessionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SessionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DiscoveryengineSessionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DiscoveryengineSessionStatus{}
		status.ObservedState = DiscoveryengineSessionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.DiscoveryengineSessionStatus{}
	status.ObservedState = DiscoveryengineSessionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SessionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &discoveryenginepb.DeleteSessionRequest{Name: a.id.String()}
