...
type servingConfigAdapter struct {
	...
	id        *krm.DiscoveryEngineServingConfigIdentity
	desired   *discoveryenginepb.ServingConfig
...
func (a *servingConfigAdapter) Find(ctx context.Context) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("getting discoveryengine serving config", "name", a.id)

	req := &discoveryenginepb.GetServingConfigRequest{Name: a.id.String()}
	actual, err := a.gcpClient.GetServingConfig(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("getting discoveryengine serving config %q from gcp: %w", a.id.String(), err)
	}
...
func (a *servingConfigAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating discoveryengine serving config", "name", a.id)

	desired := proto.Clone(a.desired).(*discoveryenginepb.ServingConfig)
	desired.Name = a.id.String()

	req := &discoveryenginepb.CreateServingConfigRequest{
		Parent:          a.id.ParentString(),
		ServingConfig:   desired,
		ServingConfigId: a.id.ServingConfig,
	}
	created, err := a.gcpClient.CreateServingConfig(ctx, req)
	if err != nil {
		return fmt.Errorf("creating discoveryengine serving config %s: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully created discoveryengine serving config in gcp", "name", a.id)
...
func (a *servingConfigAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log.V(2).Info("successfully updated discoveryengine serving config", "name", a.id)

	return a.updateStatus(ctx, updateOp, updated)
}

func (a *servingConfigAdapter) compare(ctx context.Context, actual, desired *discoveryenginepb.ServingConfig) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	...
	clonedDesired := proto.Clone(desired).(*discoveryenginepb.ServingConfig)
...
func (a *servingConfigAdapter) updateStatus(ctx context.Context, op directbase.Operation, latest *discoveryenginepb.ServingConfig) error {
...
func (a *servingConfigAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj.Spec.EngineRef = &krm.DiscoveryEngineEngineRef{External: a.id.ParentString()}
...
func (a *servingConfigAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.Info("deleting serving config", "name", a.id.String())

	req := &discoveryenginepb.DeleteServingConfigRequest{Name: a.id.String()}
	err := a.gcpClient.DeleteServingConfig(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return true, nil
		}
		return false, fmt.Errorf("deleting discoveryengine serving config %s: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully deleted discoveryengine serving config", "name", a.id)
