...
func init() {
	registry.RegisterModel(krm.DiscoveryengineChunkGVK, NewChunkModel)
}
...
type ChunkAdapter struct {
	...
	desired   *krm.DiscoveryengineChunk
	actual    *discoveryenginepb.Chunk
}
...
func (a *ChunkAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &discoveryenginepb.GetChunkRequest{Name: a.id}
...
func (a *ChunkAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DiscoveryengineChunkSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &discoveryenginepb.CreateChunkRequest{
	...
	status := &krm.DiscoveryengineChunkStatus{}
	status.ObservedState = DiscoveryengineChunkObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ChunkAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DiscoveryengineChunkSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DiscoveryengineChunkStatus{}
		status.ObservedState = DiscoveryengineChunkObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &discoveryenginepb.UpdateChunkRequest{
		Name:       a.id.External,
	...
	status := &krm.DiscoveryengineChunkStatus{}
	status.ObservedState = DiscoveryengineChunkObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ChunkAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DiscoveryengineChunk{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DiscoveryengineChunkSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DiscoveryengineChunkGVK)
...
func (a *ChunkAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &discoveryenginepb.DeleteChunkRequest{Name: a.id.String()}
