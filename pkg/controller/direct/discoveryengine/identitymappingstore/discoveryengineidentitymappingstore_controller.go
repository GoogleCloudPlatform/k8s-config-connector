...
func (a *IdentityMappingStoreAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := discoveryengine.DiscoveryEngineIdentityMappingStoreSpec_v1alpha1_ToProto(mapCtx, &desired.Spec)
	...
	status.ObservedState = discoveryengine.DiscoveryEngineIdentityMappingStoreObservedState_v1alpha1_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *IdentityMappingStoreAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := discoveryengine.DiscoveryEngineIdentityMappingStoreSpec_v1alpha1_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
	...
	if len(paths) == 0 {
		...
		status := &krm.DiscoveryEngineIdentityMappingStoreStatus{}
		status.ObservedState = discoveryengine.DiscoveryEngineIdentityMappingStoreObservedState_v1alpha1_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mappersErr(mapCtx)
		}
		status.ExternalRef = direct.LazyPtr(a.id.String())
...
func (a *IdentityMappingStoreAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DiscoveryEngineIdentityMappingStore{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(discoveryengine.DiscoveryEngineIdentityMappingStoreSpec_v1alpha1_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
