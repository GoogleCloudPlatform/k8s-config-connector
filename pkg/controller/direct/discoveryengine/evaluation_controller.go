...
func init() {
	registry.RegisterModel(krm.DiscoveryengineEvaluationGVK, NewEvaluationModel)
}
...
func (m *modelEvaluation) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.DiscoveryengineEvaluation{}
...
type EvaluationAdapter struct {
	...
	desired   *krm.DiscoveryengineEvaluation
	actual    *discoveryenginepb.Evaluation
}
...
func (a *EvaluationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &discoveryenginepb.GetEvaluationRequest{Name: a.id}
...
func (a *EvaluationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DiscoveryengineEvaluationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.DiscoveryengineEvaluationStatus{}
	status.ObservedState = DiscoveryengineEvaluationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EvaluationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DiscoveryengineEvaluationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DiscoveryengineEvaluationStatus{}
		status.ObservedState = DiscoveryengineEvaluationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &discoveryenginepb.UpdateEvaluationRequest{
		Name:       a.id.External,
	...
	status := &krm.DiscoveryengineEvaluationStatus{}
	status.ObservedState = DiscoveryengineEvaluationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EvaluationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DiscoveryengineEvaluation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DiscoveryengineEvaluationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DiscoveryengineEvaluationGVK)
...
func (a *EvaluationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &discoveryenginepb.DeleteEvaluationRequest{Name: a.id.String()}
