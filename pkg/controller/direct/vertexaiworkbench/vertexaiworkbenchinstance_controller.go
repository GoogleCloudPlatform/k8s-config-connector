...
func init() {
	registry.RegisterModel(krm.VertexAIWorkbenchInstanceGVK, NewVertexAIWorkbenchInstanceModel)
}

func NewVertexAIWorkbenchInstanceModel(ctx context.Context, config *config.ControllerConfig) (directbase.Model, error) {
...
func (m *modelInstance) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	reader := op.Reader
	obj := &krm.VertexAIWorkbenchInstance{}
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &obj); err != nil {
		return nil, fmt.Errorf("error converting to %T: %w", obj, err)
	}

	id, err := krm.NewVertexAIWorkbenchInstanceIdentity(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
	...
	return &VertexAIWorkbenchInstanceAdapter{
		id:        id,
		gcpClient: gcpClient,
		desired:   obj,
	}, nil
...
type VertexAIWorkbenchInstanceAdapter struct {
	id        *krm.VertexAIWorkbenchInstanceIdentity
	gcpClient *gcp.NotebookClient
	desired   *krm.VertexAIWorkbenchInstance
	actual    *notebookspb.Instance
}

var _ directbase.Adapter = &VertexAIWorkbenchInstanceAdapter{}

// Find retrieves the GCP resource.
// Return true means the object is found. This triggers Adapter `Update` call.
// Return false means the object is not found. This triggers Adapter `Create` call.
// Return a non-nil error requeues the requests.
func (a *VertexAIWorkbenchInstanceAdapter) Find(ctx context.Context) (bool, error) {
...
func (a *VertexAIWorkbenchInstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating Instance", "name", a.id)
	mapCtx := &direct.MapContext{}

	desired := a.desired.DeepCopy()
	resource := VertexAIWorkbenchInstanceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VertexAIWorkbenchInstanceStatus{}
	status.ObservedState = VertexAIWorkbenchInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *VertexAIWorkbenchInstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating Instance", "name", a.id)
	mapCtx := &direct.MapContext{}

	desiredPb := VertexAIWorkbenchInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VertexAIWorkbenchInstanceStatus{}
	if updated != nil {
		status.ObservedState = VertexAIWorkbenchInstanceObservedState_FromProto(mapCtx, updated)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	} else {
		status.ObservedState = VertexAIWorkbenchInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	}
...
func (a *VertexAIWorkbenchInstanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	if a.actual == nil {
	...
	obj := &krm.VertexAIWorkbenchInstance{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VertexAIWorkbenchInstanceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetName(a.id.ID())
	u.SetGroupVersionKind(krm.VertexAIWorkbenchInstanceGVK)
...
func (a *VertexAIWorkbenchInstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
