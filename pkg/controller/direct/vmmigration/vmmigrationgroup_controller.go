...
func init() {
	registry.RegisterModel(krm.VMMigrationGroupGVK, NewModel)
}
...
func (m *model) client(ctx context.Context) (*gcp.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building vmmigration client: %w", err)
	}
...
func (m *model) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	groupIdentity := identity.(*krm.VMMigrationGroupIdentity)
...
func (a *Adapter) Find(ctx context.Context) (bool, error) {
	...
	fqn := a.id.String()
	log.V(2).Info("getting VMMigrationGroup", "name", fqn)
	...
	if err != nil {
		...
		return false, fmt.Errorf("getting VMMigrationGroup %q: %w", fqn, err)
	}
...
func (a *Adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	fqn := a.id.String()
	log := klog.FromContext(ctx)
	log.V(2).Info("creating VMMigrationGroup", "name", fqn)
...
func (a *Adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	fqn := a.id.String()
	log := klog.FromContext(ctx)
	log.V(2).Info("updating VMMigrationGroup", "name", fqn)

	mapCtx := &direct.MapContext{}
	desiredPb := VMMigrationGroupSpec_ToProto(mapCtx, &a.desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	maskedActual, err := mappers.OnlySpecFields(a.actual, VMMigrationGroupSpec_FromProto, VMMigrationGroupSpec_ToProto)
	if err != nil {
		return err
	}
...
func (a *Adapter) updateStatus(ctx context.Context, op directbase.Operation, latest *pb.Group) error {
	...
	status.ObservedState = VMMigrationGroupObservedState_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
