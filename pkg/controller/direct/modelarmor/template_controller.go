...
func (m *modelTemplate) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
func (a *TemplateAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &modelarmorpb.GetTemplateRequest{Name: a.id}
...
func (a *TemplateAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	op, err := a.gcpClient.CreateTemplate(ctx, req)
	if err != nil {
		return fmt.Errorf("creating Template %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Template %s waiting creation: %w", a.id, err)
	}
...
func (a *TemplateAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	op, err := a.gcpClient.UpdateTemplate(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Template %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Template %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Template", "name", a.id.External)
...
func (a *TemplateAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &modelarmorpb.DeleteTemplateRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteTemplate(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete Template %s: %w", a.id, err)
	}
	return true, nil
}
