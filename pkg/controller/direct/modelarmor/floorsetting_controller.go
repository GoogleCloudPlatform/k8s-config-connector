...
import (
	...
	modelarmorpb "cloud.google.com/go/modelarmor/v1/modelarmorpb"
...
func (m *modelFloorSetting) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
...
type FloorSettingAdapter struct {
	...
	actual    *modelarmorpb.FloorSetting
}
...
func (a *FloorSettingAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &modelarmorpb.GetFloorSettingRequest{Name: a.id}
...
func (a *FloorSettingAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating FloorSetting", "name", a.id)
	mapCtx := &direct.MapContext{}

	desired := a.desired.DeepCopy()
	resource := ModelarmorFloorSettingSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &modelarmorpb.CreateFloorSettingRequest{
		Parent:       a.id.Parent().String(),
		FloorSetting: resource,
	}
	op, err := a.gcpClient.CreateFloorSetting(ctx, req)
	if err != nil {
		return fmt.Errorf("creating FloorSetting %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("FloorSetting %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created FloorSetting", "name", a.id)

	status := &krm.ModelarmorFloorSettingStatus{}
	status.ObservedState = ModelarmorFloorSettingObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *FloorSettingAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	op, err := a.gcpClient.UpdateFloorSetting(ctx, req)
	if err != nil {
		return fmt.Errorf("updating FloorSetting %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("FloorSetting %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated FloorSetting", "name", a.id.External)
...
func (a *FloorSettingAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting FloorSetting", "name", a.id)

	req := &modelarmorpb.DeleteFloorSettingRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteFloorSetting(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting FloorSetting %s: %w", a.id, err)
	}
	log.V(2).Info("successfully deleted FloorSetting", "name", a.id)

	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete FloorSetting %s: %w", a.id, err)
	}
	return true, nil
}
