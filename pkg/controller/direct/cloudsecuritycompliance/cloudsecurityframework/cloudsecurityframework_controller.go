...
func (a *Adapter) compare(ctx context.Context, actual, desired *pb.Framework) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	mapCtx := &direct.MapContext{}
	actualSpec := cloudsecuritycompliance.CloudSecurityFrameworkSpec_FromProto(mapCtx, actual)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
	maskedActual := cloudsecuritycompliance.CloudSecurityFrameworkSpec_ToProto(mapCtx, actualSpec)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
