...
func init() {
	registry.RegisterModel(krm.CloudSecurityComplianceFrameworkGVK, NewModel)
}
...
func (m *modelFramework) client(ctx context.Context) (*gcp.ConfigClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building Config client for Framework: %w", err)
	}
...
func (m *modelFramework) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	obj := &krm.CloudSecurityComplianceFramework{}
	...
	id := identity.(*krm.CloudSecurityComplianceFrameworkIdentity)
	...
	desired := cloudsecuritycompliance.CloudSecurityComplianceFrameworkSpec_ToProto(mapCtx, &obj.Spec)
	if mapCtx.Err() != nil {
...
type Adapter struct {
	id        *krm.CloudSecurityComplianceFrameworkIdentity
...
func (a *Adapter) Find(ctx context.Context) (bool, error) {
	...
	log.V(2).Info("getting CloudSecurityComplianceFramework", "name", a.id.String())
	...
	if err != nil {
		...
		return false, fmt.Errorf("error getting CloudSecurityComplianceFramework %q: %w", a.id.String(), err)
	}
...
func (a *Adapter) Create(ctx context.Context, op *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating CloudSecurityComplianceFramework", "name", a.id.String())
	...
	if err != nil {
		return fmt.Errorf("error creating CloudSecurityComplianceFramework %q: %w", a.id.String(), err)
	}
...
func (a *Adapter) Update(ctx context.Context, op *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating CloudSecurityComplianceFramework", "name", a.id.String())
	...
	if !diffs.HasDiff() {
		log.V(2).Info("no diff found for CloudSecurityComplianceFramework", "name", a.id.String())
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("error updating CloudSecurityComplianceFramework %q: %w", a.id.String(), err)
	}

	return a.updateStatus(ctx, op, updated)
}

func (a *Adapter) compare(ctx context.Context, actual, desired *pb.Framework) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	mapCtx := &direct.MapContext{}
	actualSpec := cloudsecuritycompliance.CloudSecurityComplianceFrameworkSpec_FromProto(mapCtx, actual)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
	maskedActual := cloudsecuritycompliance.CloudSecurityComplianceFrameworkSpec_ToProto(mapCtx, actualSpec)
	if mapCtx.Err() != nil {
		return nil, nil, mapCtx.Err()
	}
...
func (a *Adapter) Delete(ctx context.Context, op *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting CloudSecurityComplianceFramework", "name", a.id.String())
	...
	if err != nil {
		...
		return false, fmt.Errorf("error deleting CloudSecurityComplianceFramework %q: %w", a.id.String(), err)
	}
...
func (a *Adapter) updateStatus(ctx context.Context, op directbase.Operation, latest *pb.Framework) error {
	mapCtx := &direct.MapContext{}
	observedState := cloudsecuritycompliance.CloudSecurityComplianceFrameworkObservedState_FromProto(mapCtx, latest)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	status := &krm.CloudSecurityComplianceFrameworkStatus{}
