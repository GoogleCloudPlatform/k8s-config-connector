...
func init() {
	registry.RegisterModel(krm.VmwareengineClusterGVK, NewClusterModel)
}
...
type ClusterAdapter struct {
	...
	desired   *krm.VmwareengineCluster
	actual    *vmwareenginepb.Cluster
}
...
func (a *ClusterAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &vmwareenginepb.GetClusterRequest{Name: a.id}
...
func (a *ClusterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VmwareengineClusterSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VmwareengineClusterStatus{}
	status.ObservedState = VmwareengineClusterObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ClusterAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VmwareengineClusterSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VmwareengineClusterStatus{}
		status.ObservedState = VmwareengineClusterObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.VmwareengineClusterStatus{}
	status.ObservedState = VmwareengineClusterObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ClusterAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VmwareengineCluster{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VmwareengineClusterSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VmwareengineClusterGVK)
...
func (a *ClusterAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &vmwareenginepb.DeleteClusterRequest{Name: a.id.String()}
