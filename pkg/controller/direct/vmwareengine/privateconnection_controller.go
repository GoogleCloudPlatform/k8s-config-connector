...
type PrivateConnectionAdapter struct {
	...
	actual    *vmwareenginepb.PrivateConnection
}
...
func (a *PrivateConnectionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &vmwareenginepb.GetPrivateConnectionRequest{Name: a.id}
...
func (a *PrivateConnectionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VmwareenginePrivateConnectionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VmwareenginePrivateConnectionStatus{}
		status.ObservedState = VmwareenginePrivateConnectionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *PrivateConnectionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &vmwareenginepb.DeletePrivateConnectionRequest{Name: a.id.String()}
