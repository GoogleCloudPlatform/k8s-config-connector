...
func init() {
	registry.RegisterModel(krm.AutomlDatasetGVK, NewDatasetModel)
}
...
func (a *DatasetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &automlpb.GetDatasetRequest{Name: a.id}
...
func (a *DatasetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AutomlDatasetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status.ObservedState = AutomlDatasetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatasetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AutomlDatasetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AutomlDatasetStatus{}
		status.ObservedState = AutomlDatasetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AutomlDatasetStatus{}
	status.ObservedState = AutomlDatasetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatasetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AutomlDataset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AutomlDatasetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AutomlDatasetGVK)
