...
func init() {
	registry.RegisterModel(krm.AutomlModelGVK, NewModelModel)
}
...
type ModelAdapter struct {
	...
	desired   *krm.AutomlModel
	actual    *automlpb.Model
}
...
func (a *ModelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &automlpb.GetModelRequest{Name: a.id}
...
func (a *ModelAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AutomlModelSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.AutomlModelStatus{}
	status.ObservedState = AutomlModelObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ModelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AutomlModelSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AutomlModelStatus{}
		status.ObservedState = AutomlModelObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AutomlModelStatus{}
	status.ObservedState = AutomlModelObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ModelAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AutomlModel{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AutomlModelSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AutomlModelGVK)
...
func (a *ModelAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &automlpb.DeleteModelRequest{Name: a.id.String()}
