...
import (
	...
	gcp "cloud.google.com/go/appengine/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	appenginepb "cloud.google.com/go/appengine/v1/appenginepb"
...
func init() {
	registry.RegisterModel(krm.AppengineInstanceGVK, NewInstanceModel)
}
...
type InstanceAdapter struct {
	...
	desired   *krm.AppengineInstance
	actual    *appenginepb.Instance
}
...
func (a *InstanceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &appenginepb.GetInstanceRequest{Name: a.id}
...
func (a *InstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AppengineInstanceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &appenginepb.CreateInstanceRequest{
	...
	status := &krm.AppengineInstanceStatus{}
	status.ObservedState = AppengineInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AppengineInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AppengineInstanceStatus{}
		status.ObservedState = AppengineInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &appenginepb.UpdateInstanceRequest{
		Name:       a.id.External,
	...
	status := &krm.AppengineInstanceStatus{}
	status.ObservedState = AppengineInstanceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AppengineInstance{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AppengineInstanceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AppengineInstanceGVK)
...
func (a *InstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &appenginepb.DeleteInstanceRequest{Name: a.id.String()}
