...
func FuzzGroupSpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		// Status fields, should not round-trip in spec
		statusFields := sets.New[string]()
		statusFields.Insert(".create_time")
		statusFields.Insert(".update_time")
		statusFields.Insert(".additional_group_keys")
		statusFields.Insert(".name")
		...
		k := CloudIdentityGroupSpec_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := CloudIdentityGroupSpec_ToProto(ctx, k)
		if ctx.Err() != nil {
...
func FuzzGroupObservedState(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		randStream := rand.New(rand.NewSource(seed))
		...
		// Spec fields, should not round-trip in status tests
		specFields := sets.New[string]()
		specFields.Insert(".display_name")
		specFields.Insert(".group_key")
		specFields.Insert(".parent")
		specFields.Insert(".labels")
		specFields.Insert(".description")

		// Legacy fields that are in status and not observed state
		outputButNotObservedState := sets.New[string]()
		outputButNotObservedState.Insert(".create_time")
		outputButNotObservedState.Insert(".update_time")
		outputButNotObservedState.Insert(".name")

		// Remove any spec only or known-unimplemented fields
		clearFields := &fuzz.ClearFields{
			Paths: unimplementedFields.Union(specFields).Union(outputButNotObservedState),
		}
		fuzz.Visit("", p1.ProtoReflect(), nil, clearFields)
		...
		ctx := &direct.MapContext{}
		k := CloudIdentityGroupObservedState_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := CloudIdentityGroupObservedState_ToProto(ctx, k)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from krm to proto: %v", ctx.Err())
		}
