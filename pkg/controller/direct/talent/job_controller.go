...
func init() {
	registry.RegisterModel(krm.TalentJobGVK, NewJobModel)
}
...
func (a *JobAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &talentpb.GetJobRequest{Name: a.id}
...
func (a *JobAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := TalentJobSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.TalentJobStatus{}
	status.ObservedState = TalentJobObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *JobAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := TalentJobSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.TalentJobStatus{}
		status.ObservedState = TalentJobObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.TalentJobStatus{}
	status.ObservedState = TalentJobObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *JobAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.TalentJob{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(TalentJobSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.TalentJobGVK)
...
func (a *JobAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &talentpb.DeleteJobRequest{Name: a.id.String()}
