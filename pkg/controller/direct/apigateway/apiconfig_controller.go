...
func (a *ApiConfigAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigatewaypb.GetApiConfigRequest{Name: a.id}
...
func (a *ApiConfigAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating ApiConfig %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ApiConfig %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ApiConfig", "name", a.id.External)
