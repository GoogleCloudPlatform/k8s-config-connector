...
func (a *ApiAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigatewaypb.GetApiRequest{Name: a.id}
...
func (a *ApiAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Api %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Api %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Api", "name", a.id.External)
