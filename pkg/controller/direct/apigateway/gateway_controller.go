...
func init() {
	registry.RegisterModel(krm.ApigatewayGatewayGVK, NewGatewayModel)
}
...
type GatewayAdapter struct {
	...
	desired   *krm.ApigatewayGateway
	actual    *apigatewaypb.Gateway
}
...
func (a *GatewayAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigatewaypb.GetGatewayRequest{Name: a.id}
...
func (a *GatewayAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ApigatewayGatewaySpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ApigatewayGatewayStatus{}
	status.ObservedState = ApigatewayGatewayObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *GatewayAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApigatewayGatewaySpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApigatewayGatewayStatus{}
		status.ObservedState = ApigatewayGatewayObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.ApigatewayGatewayStatus{}
	status.ObservedState = ApigatewayGatewayObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *GatewayAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ApigatewayGateway{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ApigatewayGatewaySpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ApigatewayGatewayGVK)
...
func (a *GatewayAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &apigatewaypb.DeleteGatewayRequest{Name: a.id.String()}
