...
import (
	...
	gcp "cloud.google.com/go/privatecatalog/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	privatecatalogpb "cloud.google.com/go/privatecatalog/v1beta1/privatecatalogpb"
...
func init() {
	registry.RegisterModel(krm.PrivatecatalogCatalogGVK, NewCatalogModel)
}
...
type CatalogAdapter struct {
	...
	desired   *krm.PrivatecatalogCatalog
	actual    *privatecatalogpb.Catalog
}
...
func (a *CatalogAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &privatecatalogpb.GetCatalogRequest{Name: a.id}
...
func (a *CatalogAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PrivatecatalogCatalogSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &privatecatalogpb.CreateCatalogRequest{
	...
	status := &krm.PrivatecatalogCatalogStatus{}
	status.ObservedState = PrivatecatalogCatalogObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CatalogAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PrivatecatalogCatalogSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PrivatecatalogCatalogStatus{}
		status.ObservedState = PrivatecatalogCatalogObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &privatecatalogpb.UpdateCatalogRequest{
		Name:       a.id.External,
	...
	status := &krm.PrivatecatalogCatalogStatus{}
	status.ObservedState = PrivatecatalogCatalogObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CatalogAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PrivatecatalogCatalog{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PrivatecatalogCatalogSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PrivatecatalogCatalogGVK)
...
func (a *CatalogAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &privatecatalogpb.DeleteCatalogRequest{Name: a.id.String()}
