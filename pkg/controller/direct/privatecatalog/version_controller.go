...
func init() {
	registry.RegisterModel(krm.PrivatecatalogVersionGVK, NewVersionModel)
}
...
type VersionAdapter struct {
	...
	desired   *krm.PrivatecatalogVersion
	actual    *privatecatalogpb.Version
}
...
func (a *VersionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &privatecatalogpb.GetVersionRequest{Name: a.id}
...
func (a *VersionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PrivatecatalogVersionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &privatecatalogpb.CreateVersionRequest{
	...
	status := &krm.PrivatecatalogVersionStatus{}
	status.ObservedState = PrivatecatalogVersionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *VersionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PrivatecatalogVersionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PrivatecatalogVersionStatus{}
		status.ObservedState = PrivatecatalogVersionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &privatecatalogpb.UpdateVersionRequest{
		Name:       a.id.External,
	...
	status := &krm.PrivatecatalogVersionStatus{}
	status.ObservedState = PrivatecatalogVersionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *VersionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PrivatecatalogVersion{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PrivatecatalogVersionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PrivatecatalogVersionGVK)
...
func (a *VersionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &privatecatalogpb.DeleteVersionRequest{Name: a.id.String()}
