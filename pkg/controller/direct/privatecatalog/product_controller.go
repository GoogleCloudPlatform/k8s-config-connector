...
func init() {
	registry.RegisterModel(krm.PrivatecatalogProductGVK, NewProductModel)
}
...
type ProductAdapter struct {
	...
	desired   *krm.PrivatecatalogProduct
	actual    *privatecatalogpb.Product
}
...
func (a *ProductAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &privatecatalogpb.GetProductRequest{Name: a.id}
...
func (a *ProductAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PrivatecatalogProductSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &privatecatalogpb.CreateProductRequest{
	...
	status := &krm.PrivatecatalogProductStatus{}
	status.ObservedState = PrivatecatalogProductObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ProductAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PrivatecatalogProductSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PrivatecatalogProductStatus{}
		status.ObservedState = PrivatecatalogProductObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &privatecatalogpb.UpdateProductRequest{
		Name:       a.id.External,
	...
	status := &krm.PrivatecatalogProductStatus{}
	status.ObservedState = PrivatecatalogProductObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ProductAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PrivatecatalogProduct{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PrivatecatalogProductSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PrivatecatalogProductGVK)
...
func (a *ProductAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &privatecatalogpb.DeleteProductRequest{Name: a.id.String()}
