...
func init() {
	registry.RegisterModel(krm.EdgecontainerClusterGVK, NewClusterModel)
}
...
type ClusterAdapter struct {
	...
	desired   *krm.EdgecontainerCluster
	actual    *edgecontainerpb.Cluster
}
...
func (a *ClusterAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &edgecontainerpb.GetClusterRequest{Name: a.id}
...
func (a *ClusterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := EdgecontainerClusterSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.EdgecontainerClusterStatus{}
	status.ObservedState = EdgecontainerClusterObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ClusterAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := EdgecontainerClusterSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.EdgecontainerClusterStatus{}
		status.ObservedState = EdgecontainerClusterObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.EdgecontainerClusterStatus{}
	status.ObservedState = EdgecontainerClusterObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ClusterAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.EdgecontainerCluster{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(EdgecontainerClusterSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.EdgecontainerClusterGVK)
...
func (a *ClusterAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &edgecontainerpb.DeleteClusterRequest{Name: a.id.String()}
