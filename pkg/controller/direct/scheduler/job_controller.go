...
import (
	...
	gcp "cloud.google.com/go/scheduler/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	schedulerpb "cloud.google.com/go/scheduler/v1/schedulerpb"
...
func init() {
	registry.RegisterModel(krm.SchedulerJobGVK, NewJobModel)
}
...
type JobAdapter struct {
	...
	desired   *krm.SchedulerJob
	actual    *schedulerpb.Job
}
...
func (a *JobAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &schedulerpb.GetJobRequest{Name: a.id}
...
func (a *JobAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SchedulerJobSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.SchedulerJobStatus{}
	status.ObservedState = SchedulerJobObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *JobAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SchedulerJobSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SchedulerJobStatus{}
		status.ObservedState = SchedulerJobObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.SchedulerJobStatus{}
	status.ObservedState = SchedulerJobObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *JobAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SchedulerJob{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SchedulerJobSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SchedulerJobGVK)
...
func (a *JobAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &schedulerpb.DeleteJobRequest{Name: a.id.String()}
