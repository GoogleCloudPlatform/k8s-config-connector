...
func (m *savedQueryModel) client(ctx context.Context, projectID string) (*gcp.Client, error) {
	...
	return gcpClient.newAssetClient(ctx)
}
...
func (m *savedQueryModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	id := idAny.(*krm.SavedQueryIdentity)

	// Since we need to determine the project context to override (e.g. for user project override),
	// if it is folder/org scoped, we fallback to BillingProject from the controller configuration or a default.
	projectID := id.Project
	if projectID == "" {
		projectID = m.config.BillingProject
	}
...
type savedQueryAdapter struct {
	...
	desired   *krm.CloudAssetSavedQuery
...
func (a *savedQueryAdapter) normalizeReferences(ctx context.Context) error {
	if a.desired.Spec.Content == nil {
		return nil
	}
	q := a.desired.Spec.Content.IAMPolicyAnalysisQuery
	if q == nil {
		return nil
	}
	if q.Scope != nil {
		...
		if scope.ProjectRef != nil {
			project, err := refs.ResolveProject(ctx, a.reader, a.desired.GetNamespace(), scope.ProjectRef)
			if err != nil {
				return fmt.Errorf("resolving scope projectRef: %w", err)
			}
			scope.ProjectRef.External = "projects/" + project.ProjectID
		} else if scope.FolderRef != nil {
			folder, err := refs.ResolveFolder(ctx, a.reader, a.desired, scope.FolderRef)
			if err != nil {
				return fmt.Errorf("resolving scope folderRef: %w", err)
			}
			scope.FolderRef.External = "folders/" + folder.FolderID
		} else if scope.OrganizationRef != nil {
			organization, err := refs.ResolveOrganization(ctx, a.reader, a.desired, scope.OrganizationRef)
			if err != nil {
				return fmt.Errorf("resolving scope organizationRef: %w", err)
			}
			scope.OrganizationRef.External = "organizations/" + organization.OrganizationID
		}
	}
	if q.ResourceSelector != nil && q.ResourceSelector.FullResourceName != nil {
		if err := q.ResourceSelector.FullResourceName.Normalize(ctx, a.reader, a.desired.GetNamespace()); err != nil {
			return fmt.Errorf("normalizing resourceSelector fullResourceName: %w", err)
		}
	}
	return nil
}
...
func (a *savedQueryAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := CloudAssetSavedQuerySpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if a.id.Project != "" {
		parentStr = fmt.Sprintf("projects/%s", a.id.Project)
	} else if a.id.Folder != "" {
		parentStr = fmt.Sprintf("folders/%s", a.id.Folder)
	} else if a.id.Organization != "" {
		parentStr = fmt.Sprintf("organizations/%s", a.id.Organization)
	}
...
func (a *savedQueryAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log.V(2).Info("updating cloudasset saved query fields", "paths", updateMaskPaths)
...
func (a *savedQueryAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	id := &krm.SavedQueryIdentity{}
	if err := id.FromExternal(a.actual.Name); err != nil {
		return nil, fmt.Errorf("parsing name %q: %w", a.actual.Name, err)
	}
	if id.Project != "" {
		obj.Spec.Parent.ProjectRef = &refs.ProjectRef{External: "projects/" + id.Project}
	} else if id.Folder != "" {
		obj.Spec.Parent.FolderRef = &refs.FolderRef{External: "folders/" + id.Folder}
	} else if id.Organization != "" {
		obj.Spec.Parent.OrganizationRef = &refs.OrganizationRef{External: "organizations/" + id.Organization}
	} else {
	...
	u.SetName(id.SavedQuery) // Use the parsed ID from the name field
