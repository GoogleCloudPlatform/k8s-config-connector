...
import (
	...
	batchpb "cloud.google.com/go/batch/v1/batchpb"
...
func (a *TaskAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &batchpb.GetTaskRequest{Name: a.id}
...
func (a *TaskAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BatchTaskSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &batchpb.CreateTaskRequest{
	...
	status.ObservedState = BatchTaskObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TaskAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BatchTaskSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BatchTaskStatus{}
		status.ObservedState = BatchTaskObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BatchTaskStatus{}
	status.ObservedState = BatchTaskObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TaskAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &batchpb.DeleteTaskRequest{Name: a.id.String()}
