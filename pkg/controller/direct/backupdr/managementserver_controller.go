...
type ManagementServerAdapter struct {
	...
	actual    *backupdrpb.ManagementServer
}
...
func (a *ManagementServerAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &backupdrpb.GetManagementServerRequest{Name: a.id}
...
func (a *ManagementServerAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BackupdrManagementServerSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &backupdrpb.UpdateManagementServerRequest{
		Name:             a.id.External,
		UpdateMask:       updateMask,
		ManagementServer: desiredPb,
	}
	op, err := a.gcpClient.UpdateManagementServer(ctx, req)
	if err != nil {
		return fmt.Errorf("updating ManagementServer %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ManagementServer %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ManagementServer", "name", a.id.External)

	status := &krm.BackupdrManagementServerStatus{}
	status.ObservedState = BackupdrManagementServerObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ManagementServerAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &backupdrpb.DeleteManagementServerRequest{Name: a.id.String()}
