...
func (a *BackupVaultAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &backupdrpb.GetBackupVaultRequest{Name: a.id}
...
func (a *BackupVaultAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BackupdrBackupVaultSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BackupdrBackupVaultStatus{}
		status.ObservedState = BackupdrBackupVaultObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *BackupVaultAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &backupdrpb.DeleteBackupVaultRequest{Name: a.id.String()}
