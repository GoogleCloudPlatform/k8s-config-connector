...
func init() {
	registry.RegisterModel(krm.BackupdrBackupGVK, NewBackupModel)
}
...
type BackupAdapter struct {
	...
	desired   *krm.BackupdrBackup
	actual    *backupdrpb.Backup
}
...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &backupdrpb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BackupdrBackupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &backupdrpb.CreateBackupRequest{
	...
	status := &krm.BackupdrBackupStatus{}
	status.ObservedState = BackupdrBackupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BackupdrBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BackupdrBackupStatus{}
		status.ObservedState = BackupdrBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BackupdrBackupStatus{}
	status.ObservedState = BackupdrBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BackupdrBackup{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BackupdrBackupSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BackupdrBackupGVK)
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &backupdrpb.DeleteBackupRequest{Name: a.id.String()}
