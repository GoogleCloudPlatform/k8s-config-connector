...
import (
	...
	backupdrpb "cloud.google.com/go/backupdr/v1/backupdrpb"
...
func (m *modelBackupPlanAssociation) client(ctx context.Context) (*gcp.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building BackupPlanAssociation client: %w", err)
	}
...
type BackupPlanAssociationAdapter struct {
	...
	actual    *backupdrpb.BackupPlanAssociation
}
...
func (a *BackupPlanAssociationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &backupdrpb.GetBackupPlanAssociationRequest{Name: a.id}
...
func (a *BackupPlanAssociationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BackupdrBackupPlanAssociationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &backupdrpb.UpdateBackupPlanAssociationRequest{
		...
		BackupPlanAssociation: desiredPb,
	}
	op, err := a.gcpClient.UpdateBackupPlanAssociation(ctx, req)
	if err != nil {
		return fmt.Errorf("updating BackupPlanAssociation %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("BackupPlanAssociation %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated BackupPlanAssociation", "name", a.id.External)

	status := &krm.BackupdrBackupPlanAssociationStatus{}
	status.ObservedState = BackupdrBackupPlanAssociationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupPlanAssociationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &backupdrpb.DeleteBackupPlanAssociationRequest{Name: a.id.String()}
