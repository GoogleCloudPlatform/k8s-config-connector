...
func resolveForwardingRuleRefs(ctx context.Context, reader client.Reader, obj *krm.ComputeForwardingRule) error {
	...
	if obj.Spec.Target != nil {
		...
		if obj.Spec.Target.MemorystoreInstanceServiceAttachmentRef != nil {
			memorystoreInstanceServiceAttachmentRef := new(refs.MemorystoreInstanceServiceAttachmentRef)
			*memorystoreInstanceServiceAttachmentRef = *obj.Spec.Target.MemorystoreInstanceServiceAttachmentRef
			err := refs.ResolveMemorystoreInstanceServiceAttachment(ctx, reader, obj.GetNamespace(), memorystoreInstanceServiceAttachmentRef)
			if err != nil {
				return err
			}
			obj.Spec.Target.MemorystoreInstanceServiceAttachmentRef.ServiceAttachmentExternal = memorystoreInstanceServiceAttachmentRef.ServiceAttachmentExternal
		}
