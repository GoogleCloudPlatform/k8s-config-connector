...
func (a *FutureReservationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	// We need to manually preserve these fields because the Compute API protos
	// lack the 'OUTPUT_ONLY' annotations required for common.CompareProtoMessage
	// to automatically skip them. Without this, these fields would be included
	// in the UpdateMask, leading to API errors when attempting to update
	// immutable fields.
