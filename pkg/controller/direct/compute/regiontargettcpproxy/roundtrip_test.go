...
func FuzzComputeRegionTargetTCPProxySpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		k := ComputeRegionTargetTCPProxySpec_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := ComputeRegionTargetTCPProxySpec_ToProto(ctx, k)
		if ctx.Err() != nil {
