...
func computeRouterNATFuzzer() fuzztesting.KRMFuzzer {
	...
	// - rules                               -> .rules
	//   - action                            -> .rules[].action
	//     - sourceNatActiveIpsRefs          -> .rules[].action.source_nat_active_ips
	//     - sourceNatDrainIpsRefs           -> .rules[].action.source_nat_drain_ips
	//   - description                       -> .rules[].description
	//   - match                             -> .rules[].match
	//   - ruleNumber                        -> .rules[].rule_number
	// - sourceSubnetworkIpRangesToNat       -> .source_subnetwork_ip_ranges_to_nat
	// - subnetwork                          -> .subnetworks
	//   - secondaryIpRangeNames             -> .subnetworks[].secondary_ip_range_names
	//   - sourceIpRangesToNat               -> .subnetworks[].source_ip_ranges_to_nat
	//   - subnetworkRef                     -> .subnetworks[].name
	...
	// - region                              -> (not mapped directly to pb.RouterNat, parent URL field)
	// - resourceID                          -> .name (Identity)
	// - routerRef                           -> (not mapped directly to pb.RouterNat, parent URL field)
	...
	f.FilterSpec = func(in *pb.RouterNat) {
		in.Name = nil
		in.AutoNetworkTier = nil
		in.EndpointTypes = nil
		in.Nat64Subnetworks = nil
		in.SourceSubnetworkIpRangesToNat64 = nil
		in.Type = nil
		for _, rule := range in.Rules {
			if rule.Action != nil {
				rule.Action.SourceNatActiveRanges = nil
				rule.Action.SourceNatDrainRanges = nil
			}
		}
		if in.LogConfig != nil {
