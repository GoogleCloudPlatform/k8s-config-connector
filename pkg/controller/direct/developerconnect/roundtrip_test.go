...
func FuzzDevConnectInsightsConfigSpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		randStream := rand.New(rand.NewSource(seed))

		p1 := &pb.InsightsConfig{}
		fuzz.FillWithRandom(t, randStream, p1)

		// Status fields & identity name and unmapped spec fields
		clearPaths := sets.New(
			".name",
			".create_time",
			".update_time",
			...
			".projects",
			".errors[].details",
			".runtime_configs[].app_hub_service",
			".runtime_configs[].google_cloud_run",
		)
		...
		k := DevConnectInsightsConfigSpec_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := DevConnectInsightsConfigSpec_ToProto(ctx, k)
		if ctx.Err() != nil {
...
func FuzzDevConnectAccountConnectorSpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		randStream := rand.New(rand.NewSource(seed))

		p1 := &pb.AccountConnector{}
		fuzz.FillWithRandom(t, randStream, p1)
		...
		ctx := &direct.MapContext{}
		k := DevConnectAccountConnectorSpec_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := DevConnectAccountConnectorSpec_ToProto(ctx, k)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from krm to proto: %v", ctx.Err())
		}
