...
func init() {
	registry.RegisterModel(krm.DatalabelingDatasetGVK, NewDatasetModel)
}
...
type DatasetAdapter struct {
	...
	desired   *krm.DatalabelingDataset
	actual    *datalabelingpb.Dataset
}
...
func (a *DatasetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datalabelingpb.GetDatasetRequest{Name: a.id}
...
func (a *DatasetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DatalabelingDatasetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.DatalabelingDatasetStatus{}
	status.ObservedState = DatalabelingDatasetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatasetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatalabelingDatasetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatalabelingDatasetStatus{}
		status.ObservedState = DatalabelingDatasetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &datalabelingpb.UpdateDatasetRequest{
		Name:       a.id.External,
	...
	status := &krm.DatalabelingDatasetStatus{}
	status.ObservedState = DatalabelingDatasetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatasetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DatalabelingDataset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DatalabelingDatasetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DatalabelingDatasetGVK)
...
func (a *DatasetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datalabelingpb.DeleteDatasetRequest{Name: a.id.String()}
