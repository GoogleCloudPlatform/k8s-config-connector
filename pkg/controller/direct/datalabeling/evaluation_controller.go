...
func init() {
	registry.RegisterModel(krm.DatalabelingEvaluationGVK, NewEvaluationModel)
}
...
func (m *modelEvaluation) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.DatalabelingEvaluation{}
...
type EvaluationAdapter struct {
	...
	desired   *krm.DatalabelingEvaluation
	actual    *datalabelingpb.Evaluation
}
...
func (a *EvaluationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datalabelingpb.GetEvaluationRequest{Name: a.id}
...
func (a *EvaluationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DatalabelingEvaluationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &datalabelingpb.CreateEvaluationRequest{
	...
	status := &krm.DatalabelingEvaluationStatus{}
	status.ObservedState = DatalabelingEvaluationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EvaluationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatalabelingEvaluationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatalabelingEvaluationStatus{}
		status.ObservedState = DatalabelingEvaluationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &datalabelingpb.UpdateEvaluationRequest{
		Name:       a.id.External,
	...
	status := &krm.DatalabelingEvaluationStatus{}
	status.ObservedState = DatalabelingEvaluationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EvaluationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DatalabelingEvaluation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DatalabelingEvaluationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DatalabelingEvaluationGVK)
...
func (a *EvaluationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datalabelingpb.DeleteEvaluationRequest{Name: a.id.String()}
