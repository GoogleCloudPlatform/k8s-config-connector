...
func init() {
	registry.RegisterModel(krm.SecurityCertificateAuthorityGVK, NewCertificateAuthorityModel)
}
...
func (m *modelCertificateAuthority) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.SecurityCertificateAuthority{}
...
type CertificateAuthorityAdapter struct {
	...
	desired   *krm.SecurityCertificateAuthority
	actual    *securitypb.CertificateAuthority
}
...
func (a *CertificateAuthorityAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitypb.GetCertificateAuthorityRequest{Name: a.id}
...
func (a *CertificateAuthorityAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecurityCertificateAuthoritySpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &securitypb.CreateCertificateAuthorityRequest{
	...
	status := &krm.SecurityCertificateAuthorityStatus{}
	status.ObservedState = SecurityCertificateAuthorityObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CertificateAuthorityAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecurityCertificateAuthoritySpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecurityCertificateAuthorityStatus{}
		status.ObservedState = SecurityCertificateAuthorityObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &securitypb.UpdateCertificateAuthorityRequest{
		Name:                 a.id.External,
	...
	status := &krm.SecurityCertificateAuthorityStatus{}
	status.ObservedState = SecurityCertificateAuthorityObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CertificateAuthorityAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecurityCertificateAuthority{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecurityCertificateAuthoritySpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecurityCertificateAuthorityGVK)
...
func (a *CertificateAuthorityAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitypb.DeleteCertificateAuthorityRequest{Name: a.id.String()}
