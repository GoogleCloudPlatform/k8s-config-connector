...
func init() {
	registry.RegisterModel(krm.SecurityCertificateGVK, NewCertificateModel)
}
...
type CertificateAdapter struct {
	...
	desired   *krm.SecurityCertificate
	actual    *securitypb.Certificate
}
...
func (a *CertificateAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitypb.GetCertificateRequest{Name: a.id}
...
func (a *CertificateAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecurityCertificateSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &securitypb.CreateCertificateRequest{
	...
	status := &krm.SecurityCertificateStatus{}
	status.ObservedState = SecurityCertificateObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CertificateAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecurityCertificateSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecurityCertificateStatus{}
		status.ObservedState = SecurityCertificateObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &securitypb.UpdateCertificateRequest{
		Name:        a.id.External,
	...
	status := &krm.SecurityCertificateStatus{}
	status.ObservedState = SecurityCertificateObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CertificateAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecurityCertificate{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecurityCertificateSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecurityCertificateGVK)
...
func (a *CertificateAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitypb.DeleteCertificateRequest{Name: a.id.String()}
