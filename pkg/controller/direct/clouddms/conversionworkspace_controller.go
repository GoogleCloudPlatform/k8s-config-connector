...
func (m *modelConversionWorkspace) client(ctx context.Context) (*gcp.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building ConversionWorkspace client: %w", err)
	}
...
type ConversionWorkspaceAdapter struct {
	...
	actual    *clouddmspb.ConversionWorkspace
}
...
func (a *ConversionWorkspaceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &clouddmspb.GetConversionWorkspaceRequest{Name: a.id}
...
func (a *ConversionWorkspaceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ClouddmsConversionWorkspaceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ClouddmsConversionWorkspaceStatus{}
		status.ObservedState = ClouddmsConversionWorkspaceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &clouddmspb.UpdateConversionWorkspaceRequest{
		...
		ConversionWorkspace: desiredPb,
	}
	op, err := a.gcpClient.UpdateConversionWorkspace(ctx, req)
	if err != nil {
		return fmt.Errorf("updating ConversionWorkspace %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ConversionWorkspace %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ConversionWorkspace", "name", a.id.External)
...
func (a *ConversionWorkspaceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &clouddmspb.DeleteConversionWorkspaceRequest{Name: a.id.String()}
