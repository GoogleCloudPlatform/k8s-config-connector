...
type PrivateConnectionAdapter struct {
	...
	actual    *clouddmspb.PrivateConnection
}
...
func (a *PrivateConnectionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &clouddmspb.GetPrivateConnectionRequest{Name: a.id}
...
func (a *PrivateConnectionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ClouddmsPrivateConnectionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &clouddmspb.UpdatePrivateConnectionRequest{
		Name:              a.id.External,
	...
	status := &krm.ClouddmsPrivateConnectionStatus{}
	status.ObservedState = ClouddmsPrivateConnectionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *PrivateConnectionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &clouddmspb.DeletePrivateConnectionRequest{Name: a.id.String()}
