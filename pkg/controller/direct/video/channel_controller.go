...
func init() {
	registry.RegisterModel(krm.VideoChannelGVK, NewChannelModel)
}
...
type ChannelAdapter struct {
	...
	desired   *krm.VideoChannel
	actual    *videopb.Channel
}
...
func (a *ChannelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &videopb.GetChannelRequest{Name: a.id}
...
func (a *ChannelAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VideoChannelSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &videopb.CreateChannelRequest{
	...
	status := &krm.VideoChannelStatus{}
	status.ObservedState = VideoChannelObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ChannelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VideoChannelSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VideoChannelStatus{}
		status.ObservedState = VideoChannelObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &videopb.UpdateChannelRequest{
		Name:       a.id.External,
	...
	status := &krm.VideoChannelStatus{}
	status.ObservedState = VideoChannelObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ChannelAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VideoChannel{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VideoChannelSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VideoChannelGVK)
...
func (a *ChannelAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &videopb.DeleteChannelRequest{Name: a.id.String()}
