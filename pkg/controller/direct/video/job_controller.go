...
func init() {
	registry.RegisterModel(krm.VideoJobGVK, NewJobModel)
}
...
func (a *JobAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &videopb.GetJobRequest{Name: a.id}
...
func (a *JobAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VideoJobSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &videopb.CreateJobRequest{
	...
	status := &krm.VideoJobStatus{}
	status.ObservedState = VideoJobObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *JobAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VideoJobSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VideoJobStatus{}
		status.ObservedState = VideoJobObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.VideoJobStatus{}
	status.ObservedState = VideoJobObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *JobAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VideoJob{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VideoJobSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VideoJobGVK)
...
func (a *JobAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &videopb.DeleteJobRequest{Name: a.id.String()}
