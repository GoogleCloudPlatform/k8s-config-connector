...
func init() {
	registry.RegisterModel(krm.VideoAssetGVK, NewAssetModel)
}
...
type AssetAdapter struct {
	...
	desired   *krm.VideoAsset
	actual    *videopb.Asset
}
...
func (a *AssetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &videopb.GetAssetRequest{Name: a.id}
...
func (a *AssetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VideoAssetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &videopb.CreateAssetRequest{
	...
	status := &krm.VideoAssetStatus{}
	status.ObservedState = VideoAssetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *AssetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VideoAssetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VideoAssetStatus{}
		status.ObservedState = VideoAssetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &videopb.UpdateAssetRequest{
		Name:       a.id.External,
	...
	status := &krm.VideoAssetStatus{}
	status.ObservedState = VideoAssetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *AssetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VideoAsset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VideoAssetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VideoAssetGVK)
...
func (a *AssetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &videopb.DeleteAssetRequest{Name: a.id.String()}
