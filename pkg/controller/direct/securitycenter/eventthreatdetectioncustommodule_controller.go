...
func (m *modelEventThreatDetectionCustomModule) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.SecuritycenterEventThreatDetectionCustomModule{}
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &obj); err != nil {
		return nil, fmt.Errorf("error converting to %T: %w", obj, err)
	}

	id, err := krm.NewEventThreatDetectionCustomModuleIdentity(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
...
type EventThreatDetectionCustomModuleAdapter struct {
	...
	actual    *securitycenterpb.EventThreatDetectionCustomModule
}
...
func (a *EventThreatDetectionCustomModuleAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitycenterpb.GetEventThreatDetectionCustomModuleRequest{Name: a.id}
...
func (a *EventThreatDetectionCustomModuleAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecuritycenterEventThreatDetectionCustomModuleSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &securitycenterpb.CreateEventThreatDetectionCustomModuleRequest{
		Parent:                           a.id.Parent().String(),
		EventThreatDetectionCustomModule: resource,
	}
	op, err := a.gcpClient.CreateEventThreatDetectionCustomModule(ctx, req)
	if err != nil {
		return fmt.Errorf("creating EventThreatDetectionCustomModule %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("EventThreatDetectionCustomModule %s waiting creation: %w", a.id, err)
	}
...
func (a *EventThreatDetectionCustomModuleAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecuritycenterEventThreatDetectionCustomModuleSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	op, err := a.gcpClient.UpdateEventThreatDetectionCustomModule(ctx, req)
	if err != nil {
		return fmt.Errorf("updating EventThreatDetectionCustomModule %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("EventThreatDetectionCustomModule %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated EventThreatDetectionCustomModule", "name", a.id.External)
...
func (a *EventThreatDetectionCustomModuleAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecuritycenterEventThreatDetectionCustomModule{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecuritycenterEventThreatDetectionCustomModuleSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecuritycenterEventThreatDetectionCustomModuleGVK)
...
func (a *EventThreatDetectionCustomModuleAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitycenterpb.DeleteEventThreatDetectionCustomModuleRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteEventThreatDetectionCustomModule(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete EventThreatDetectionCustomModule %s: %w", a.id, err)
	}
	return true, nil
}
