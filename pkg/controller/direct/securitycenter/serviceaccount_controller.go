...
func init() {
	registry.RegisterModel(krm.SecuritycenterServiceAccountGVK, NewServiceAccountModel)
}
...
func (m *modelServiceAccount) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.SecuritycenterServiceAccount{}
...
type ServiceAccountAdapter struct {
	...
	desired   *krm.SecuritycenterServiceAccount
	actual    *securitycenterpb.ServiceAccount
}
...
func (a *ServiceAccountAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitycenterpb.GetServiceAccountRequest{Name: a.id}
...
func (a *ServiceAccountAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecuritycenterServiceAccountSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &securitycenterpb.CreateServiceAccountRequest{
	...
	status := &krm.SecuritycenterServiceAccountStatus{}
	status.ObservedState = SecuritycenterServiceAccountObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAccountAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecuritycenterServiceAccountSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecuritycenterServiceAccountStatus{}
		status.ObservedState = SecuritycenterServiceAccountObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &securitycenterpb.UpdateServiceAccountRequest{
		Name:           a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating ServiceAccount %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ServiceAccount %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ServiceAccount", "name", a.id.External)

	status := &krm.SecuritycenterServiceAccountStatus{}
	status.ObservedState = SecuritycenterServiceAccountObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ServiceAccountAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecuritycenterServiceAccount{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecuritycenterServiceAccountSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecuritycenterServiceAccountGVK)
...
func (a *ServiceAccountAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitycenterpb.DeleteServiceAccountRequest{Name: a.id.String()}
