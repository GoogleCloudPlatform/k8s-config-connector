...
func init() {
	registry.RegisterModel(krm.SecuritycenterSourceGVK, NewSourceModel)
}
...
type SourceAdapter struct {
	...
	desired   *krm.SecuritycenterSource
	actual    *securitycenterpb.Source
}
...
func (a *SourceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitycenterpb.GetSourceRequest{Name: a.id}
...
func (a *SourceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecuritycenterSourceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.SecuritycenterSourceStatus{}
	status.ObservedState = SecuritycenterSourceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SourceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecuritycenterSourceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecuritycenterSourceStatus{}
		status.ObservedState = SecuritycenterSourceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.SecuritycenterSourceStatus{}
	status.ObservedState = SecuritycenterSourceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SourceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecuritycenterSource{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecuritycenterSourceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecuritycenterSourceGVK)
...
func (a *SourceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitycenterpb.DeleteSourceRequest{Name: a.id.String()}
