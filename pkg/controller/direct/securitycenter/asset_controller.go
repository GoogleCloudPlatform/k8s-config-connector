...
func init() {
	registry.RegisterModel(krm.SecuritycenterAssetGVK, NewAssetModel)
}
...
type AssetAdapter struct {
	...
	desired   *krm.SecuritycenterAsset
	actual    *securitycenterpb.Asset
}
...
func (a *AssetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &securitycenterpb.GetAssetRequest{Name: a.id}
...
func (a *AssetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecuritycenterAssetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &securitycenterpb.CreateAssetRequest{
	...
	status := &krm.SecuritycenterAssetStatus{}
	status.ObservedState = SecuritycenterAssetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *AssetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecuritycenterAssetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecuritycenterAssetStatus{}
		status.ObservedState = SecuritycenterAssetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &securitycenterpb.UpdateAssetRequest{
		Name:       a.id.External,
	...
	status := &krm.SecuritycenterAssetStatus{}
	status.ObservedState = SecuritycenterAssetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *AssetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecuritycenterAsset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecuritycenterAssetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecuritycenterAssetGVK)
...
func (a *AssetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &securitycenterpb.DeleteAssetRequest{Name: a.id.String()}
