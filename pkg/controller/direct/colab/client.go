...
func (m *gcpClient) newNotebookClient(ctx context.Context) (*api.NotebookClient, error) {
	opts, err := m.config.RESTClientOptions()
	if err != nil {
		return nil, err
	}
	client, err := api.NewNotebookRESTClient(ctx, opts...)
	if err != nil {
		return nil, fmt.Errorf("building notebook client: %w", err)
	}
	return client, err
}
