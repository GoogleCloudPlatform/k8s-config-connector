...
import (
	...
	gcp "cloud.google.com/go/shell/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	shellpb "cloud.google.com/go/shell/v1/shellpb"
...
func init() {
	registry.RegisterModel(krm.ShellEnvironmentGVK, NewEnvironmentModel)
}
...
type EnvironmentAdapter struct {
	...
	desired   *krm.ShellEnvironment
	actual    *shellpb.Environment
}
...
func (a *EnvironmentAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &shellpb.GetEnvironmentRequest{Name: a.id}
...
func (a *EnvironmentAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ShellEnvironmentSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &shellpb.CreateEnvironmentRequest{
	...
	status := &krm.ShellEnvironmentStatus{}
	status.ObservedState = ShellEnvironmentObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EnvironmentAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ShellEnvironmentSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ShellEnvironmentStatus{}
		status.ObservedState = ShellEnvironmentObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &shellpb.UpdateEnvironmentRequest{
		Name:        a.id.External,
	...
	status := &krm.ShellEnvironmentStatus{}
	status.ObservedState = ShellEnvironmentObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EnvironmentAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ShellEnvironment{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ShellEnvironmentSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ShellEnvironmentGVK)
...
func (a *EnvironmentAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &shellpb.DeleteEnvironmentRequest{Name: a.id.String()}
