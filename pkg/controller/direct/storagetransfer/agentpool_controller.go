...
func (a *AgentPoolAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &storagetransferpb.GetAgentPoolRequest{Name: a.id}
...
func (a *AgentPoolAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating AgentPool %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("AgentPool %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated AgentPool", "name", a.id.External)
