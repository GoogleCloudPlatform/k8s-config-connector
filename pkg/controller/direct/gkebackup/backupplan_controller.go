...
func (m *backupPlanModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
...
func (m *backupPlanModel) AdapterForURL(ctx context.Context, url string) (directbase.Adapter, error) {
	// Format is //gkebackup.googleapis.com/projects/PROJECT_ID/locations/LOCATION/backupPlans/BACKUP_PLAN_ID
	if !strings.HasPrefix(url, "//gkebackup.googleapis.com/") {
		return nil, nil
	}

	external := strings.TrimPrefix(url, "//gkebackup.googleapis.com/")
	parent, id, err := krm.ParseBackupPlanExternal(external)
	if err != nil {
		return nil, nil
	}
	...
	return &backupPlanAdapter{
		gcpClient: client,
		id:        krm.NewBackupPlanIdentityFromParts(parent, id),
	}, nil
}
...
func (a *backupPlanAdapter) GetIAMPolicy(ctx context.Context) (*iampb.Policy, error) {
	if a.id == nil {
...
func (a *backupPlanAdapter) SetIAMPolicy(ctx context.Context, policy *iampb.Policy) (*iampb.Policy, error) {
	if a.id == nil {
