...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &gkebackuppb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := GkebackupBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.GkebackupBackupStatus{}
		status.ObservedState = GkebackupBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &gkebackuppb.DeleteBackupRequest{Name: a.id.String()}
