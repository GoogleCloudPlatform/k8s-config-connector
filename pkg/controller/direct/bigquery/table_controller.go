...
func init() {
	registry.RegisterModel(krm.BigqueryTableGVK, NewTableModel)
}
...
func (a *TableAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigquerypb.GetTableRequest{Name: a.id}
...
func (a *TableAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BigqueryTableSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &bigquerypb.CreateTableRequest{
	...
	status := &krm.BigqueryTableStatus{}
	status.ObservedState = BigqueryTableObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TableAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigqueryTableSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigqueryTableStatus{}
		status.ObservedState = BigqueryTableObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigquerypb.UpdateTableRequest{
		Name:       a.id.External,
	...
	status := &krm.BigqueryTableStatus{}
	status.ObservedState = BigqueryTableObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TableAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigquerypb.DeleteTableRequest{Name: a.id.String()}
