...
func init() {
	registry.RegisterModel(krm.BigqueryReservationGVK, NewReservationModel)
}
...
type ReservationAdapter struct {
	...
	desired   *krm.BigqueryReservation
	actual    *bigquerypb.Reservation
}
...
func (a *ReservationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigquerypb.GetReservationRequest{Name: a.id}
...
func (a *ReservationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BigqueryReservationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &bigquerypb.CreateReservationRequest{
	...
	status := &krm.BigqueryReservationStatus{}
	status.ObservedState = BigqueryReservationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ReservationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigqueryReservationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigqueryReservationStatus{}
		status.ObservedState = BigqueryReservationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigquerypb.UpdateReservationRequest{
		Name:        a.id.External,
	...
	status := &krm.BigqueryReservationStatus{}
	status.ObservedState = BigqueryReservationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ReservationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BigqueryReservation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BigqueryReservationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BigqueryReservationGVK)
...
func (a *ReservationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigquerypb.DeleteReservationRequest{Name: a.id.String()}
