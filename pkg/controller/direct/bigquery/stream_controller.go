...
import (
	...
	bigquerypb "cloud.google.com/go/bigquery/v1beta1/bigquerypb"
...
func init() {
	registry.RegisterModel(krm.BigqueryStreamGVK, NewStreamModel)
}
...
type StreamAdapter struct {
	...
	desired   *krm.BigqueryStream
	actual    *bigquerypb.Stream
}
...
func (a *StreamAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &bigquerypb.GetStreamRequest{Name: a.id}
...
func (a *StreamAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BigqueryStreamSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &bigquerypb.CreateStreamRequest{
	...
	status := &krm.BigqueryStreamStatus{}
	status.ObservedState = BigqueryStreamObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *StreamAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BigqueryStreamSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BigqueryStreamStatus{}
		status.ObservedState = BigqueryStreamObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &bigquerypb.UpdateStreamRequest{
		Name:       a.id.External,
	...
	status := &krm.BigqueryStreamStatus{}
	status.ObservedState = BigqueryStreamObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *StreamAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BigqueryStream{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BigqueryStreamSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BigqueryStreamGVK)
...
func (a *StreamAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &bigquerypb.DeleteStreamRequest{Name: a.id.String()}
