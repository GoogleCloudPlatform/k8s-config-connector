...
type NamespaceAdapter struct {
	...
	desired   *krm.ServicedirectoryNamespace
	actual    *servicedirectorypb.Namespace
}
...
func (a *NamespaceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &servicedirectorypb.GetNamespaceRequest{Name: a.id}
...
func (a *NamespaceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ServicedirectoryNamespaceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ServicedirectoryNamespaceStatus{}
	status.ObservedState = ServicedirectoryNamespaceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *NamespaceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ServicedirectoryNamespaceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ServicedirectoryNamespaceStatus{}
		status.ObservedState = ServicedirectoryNamespaceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.ServicedirectoryNamespaceStatus{}
	status.ObservedState = ServicedirectoryNamespaceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *NamespaceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &servicedirectorypb.DeleteNamespaceRequest{Name: a.id.String()}
