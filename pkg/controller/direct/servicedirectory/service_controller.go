...
import (
	...
	gcp "cloud.google.com/go/servicedirectory/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	servicedirectorypb "cloud.google.com/go/servicedirectory/v1/servicedirectorypb"
...
func init() {
	registry.RegisterModel(krm.ServicedirectoryServiceGVK, NewServiceModel)
}
...
func (m *modelService) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	...
	// Get servicedirectory GCP client
...
type ServiceAdapter struct {
	...
	desired   *krm.ServicedirectoryService
	actual    *servicedirectorypb.Service
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &servicedirectorypb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ServicedirectoryServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ServicedirectoryServiceStatus{}
	status.ObservedState = ServicedirectoryServiceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ServicedirectoryServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ServicedirectoryServiceStatus{}
		status.ObservedState = ServicedirectoryServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	if err != nil {
		return fmt.Errorf("updating Service %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Service %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Service", "name", a.id.External)

	status := &krm.ServicedirectoryServiceStatus{}
	status.ObservedState = ServicedirectoryServiceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &servicedirectorypb.DeleteServiceRequest{Name: a.id.String()}
