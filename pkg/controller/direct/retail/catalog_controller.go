...
func init() {
	registry.RegisterModel(krm.RetailCatalogGVK, NewCatalogModel)
}
...
type CatalogAdapter struct {
	...
	desired   *krm.RetailCatalog
	actual    *retailpb.Catalog
}
...
func (a *CatalogAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &retailpb.GetCatalogRequest{Name: a.id}
...
func (a *CatalogAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RetailCatalogSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &retailpb.CreateCatalogRequest{
	...
	status := &krm.RetailCatalogStatus{}
	status.ObservedState = RetailCatalogObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CatalogAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RetailCatalogSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RetailCatalogStatus{}
		status.ObservedState = RetailCatalogObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.RetailCatalogStatus{}
	status.ObservedState = RetailCatalogObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CatalogAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.RetailCatalog{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(RetailCatalogSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.RetailCatalogGVK)
...
func (a *CatalogAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &retailpb.DeleteCatalogRequest{Name: a.id.String()}
