...
func init() {
	registry.RegisterModel(krm.RetailModelGVK, NewModelModel)
}
...
type ModelAdapter struct {
	...
	desired   *krm.RetailModel
	actual    *retailpb.Model
}
...
func (a *ModelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &retailpb.GetModelRequest{Name: a.id}
...
func (a *ModelAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RetailModelSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.RetailModelStatus{}
	status.ObservedState = RetailModelObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ModelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RetailModelSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RetailModelStatus{}
		status.ObservedState = RetailModelObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.RetailModelStatus{}
	status.ObservedState = RetailModelObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ModelAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.RetailModel{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(RetailModelSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.RetailModelGVK)
...
func (a *ModelAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &retailpb.DeleteModelRequest{Name: a.id.String()}
