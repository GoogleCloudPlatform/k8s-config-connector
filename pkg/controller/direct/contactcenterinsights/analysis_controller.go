...
func init() {
	registry.RegisterModel(krm.ContactcenterinsightsAnalysisGVK, NewAnalysisModel)
}
...
func (m *modelAnalysis) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.ContactcenterinsightsAnalysis{}
...
type AnalysisAdapter struct {
	...
	desired   *krm.ContactcenterinsightsAnalysis
	actual    *contactcenterinsightspb.Analysis
}
...
func (a *AnalysisAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &contactcenterinsightspb.GetAnalysisRequest{Name: a.id}
...
func (a *AnalysisAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ContactcenterinsightsAnalysisSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ContactcenterinsightsAnalysisStatus{}
	status.ObservedState = ContactcenterinsightsAnalysisObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *AnalysisAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ContactcenterinsightsAnalysisSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ContactcenterinsightsAnalysisStatus{}
		status.ObservedState = ContactcenterinsightsAnalysisObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &contactcenterinsightspb.UpdateAnalysisRequest{
		Name:       a.id.External,
	...
	status := &krm.ContactcenterinsightsAnalysisStatus{}
	status.ObservedState = ContactcenterinsightsAnalysisObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *AnalysisAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ContactcenterinsightsAnalysis{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ContactcenterinsightsAnalysisSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ContactcenterinsightsAnalysisGVK)
...
func (a *AnalysisAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &contactcenterinsightspb.DeleteAnalysisRequest{Name: a.id.String()}
