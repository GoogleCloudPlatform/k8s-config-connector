...
func init() {
	registry.RegisterModel(krm.ContactcenterinsightsConversationGVK, NewConversationModel)
}
...
func (m *modelConversation) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.ContactcenterinsightsConversation{}
...
type ConversationAdapter struct {
	...
	desired   *krm.ContactcenterinsightsConversation
...
func (a *ConversationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &contactcenterinsightspb.GetConversationRequest{Name: a.id}
...
func (a *ConversationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ContactcenterinsightsConversationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ContactcenterinsightsConversationStatus{}
	status.ObservedState = ContactcenterinsightsConversationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ConversationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ContactcenterinsightsConversationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ContactcenterinsightsConversationStatus{}
		status.ObservedState = ContactcenterinsightsConversationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.ContactcenterinsightsConversationStatus{}
	status.ObservedState = ContactcenterinsightsConversationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ConversationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ContactcenterinsightsConversation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ContactcenterinsightsConversationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ContactcenterinsightsConversationGVK)
...
func (a *ConversationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &contactcenterinsightspb.DeleteConversationRequest{Name: a.id.String()}
