...
func (a *DiscoveredServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apphubpb.GetDiscoveredServiceRequest{Name: a.id}
...
func (a *DiscoveredServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("creating DiscoveredService", "name", a.id)
	mapCtx := &direct.MapContext{}

	desired := a.desired.DeepCopy()
	resource := ApphubDiscoveredServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &apphubpb.CreateDiscoveredServiceRequest{
		Parent:            a.id.Parent().String(),
		DiscoveredService: resource,
	}
	op, err := a.gcpClient.CreateDiscoveredService(ctx, req)
	if err != nil {
		return fmt.Errorf("creating DiscoveredService %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("DiscoveredService %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created DiscoveredService", "name", a.id)

	status := &krm.ApphubDiscoveredServiceStatus{}
	status.ObservedState = ApphubDiscoveredServiceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DiscoveredServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating DiscoveredService", "name", a.id)
	mapCtx := &direct.MapContext{}

	desiredPb := ApphubDiscoveredServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &apphubpb.UpdateDiscoveredServiceRequest{
		Name:              a.id.External,
		UpdateMask:        updateMask,
		DiscoveredService: desiredPb,
	}
	op, err := a.gcpClient.UpdateDiscoveredService(ctx, req)
	if err != nil {
		return fmt.Errorf("updating DiscoveredService %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("DiscoveredService %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated DiscoveredService", "name", a.id.External)

	status := &krm.ApphubDiscoveredServiceStatus{}
	status.ObservedState = ApphubDiscoveredServiceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DiscoveredServiceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting DiscoveredService", "name", a.id)

	req := &apphubpb.DeleteDiscoveredServiceRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteDiscoveredService(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting DiscoveredService %s: %w", a.id, err)
	}
	log.V(2).Info("successfully deleted DiscoveredService", "name", a.id)

	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete DiscoveredService %s: %w", a.id, err)
	}
	return true, nil
}
