...
func (a *ApplicationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apphubpb.GetApplicationRequest{Name: a.id}
...
func (a *ApplicationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating Application %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Application %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Application", "name", a.id.External)
