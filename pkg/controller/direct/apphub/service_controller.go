...
func init() {
	registry.RegisterModel(krm.ApphubServiceGVK, NewServiceModel)
}
...
func (a *ServiceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apphubpb.GetServiceRequest{Name: a.id}
...
func (a *ServiceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ApphubServiceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status.ObservedState = ApphubServiceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ServiceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApphubServiceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApphubServiceStatus{}
		status.ObservedState = ApphubServiceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.ApphubServiceStatus{}
	status.ObservedState = ApphubServiceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ServiceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ApphubService{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ApphubServiceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
