...
func (a *ServiceProjectAttachmentAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apphubpb.GetServiceProjectAttachmentRequest{Name: a.id}
...
func (a *ServiceProjectAttachmentAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating ServiceProjectAttachment %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ServiceProjectAttachment %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ServiceProjectAttachment", "name", a.id.External)
