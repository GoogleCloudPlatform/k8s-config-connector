...
func init() {
	registry.RegisterModel(krm.AdvisorynotificationsSettingsGVK, NewSettingsModel)
}
...
func (m *modelSettings) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.AdvisorynotificationsSettings{}
...
type SettingsAdapter struct {
	...
	desired   *krm.AdvisorynotificationsSettings
	actual    *advisorynotificationspb.Settings
}
...
func (a *SettingsAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &advisorynotificationspb.GetSettingsRequest{Name: a.id.String()}
...
func (a *SettingsAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AdvisorynotificationsSettingsSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.AdvisorynotificationsSettingsStatus{}
	status.ObservedState = AdvisorynotificationsSettingsObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SettingsAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AdvisorynotificationsSettingsSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AdvisorynotificationsSettingsStatus{}
		status.ObservedState = AdvisorynotificationsSettingsObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AdvisorynotificationsSettingsStatus{}
	status.ObservedState = AdvisorynotificationsSettingsObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SettingsAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AdvisorynotificationsSettings{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AdvisorynotificationsSettingsSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
