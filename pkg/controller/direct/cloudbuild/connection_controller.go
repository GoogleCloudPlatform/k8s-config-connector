...
func (a *ConnectionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &cloudbuildpb.GetConnectionRequest{Name: a.id}
...
func (a *ConnectionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := CloudbuildConnectionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ConnectionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := CloudbuildConnectionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.CloudbuildConnectionStatus{}
		status.ObservedState = CloudbuildConnectionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
