...
import (
	...
	memorystorepb "cloud.google.com/go/memorystore/v1/memorystorepb"
...
func init() {
	registry.RegisterModel(krm.MemorystoreCertificateAuthorityGVK, NewCertificateAuthorityModel)
}
...
func (m *modelCertificateAuthority) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.MemorystoreCertificateAuthority{}
...
type CertificateAuthorityAdapter struct {
	...
	desired   *krm.MemorystoreCertificateAuthority
	actual    *memorystorepb.CertificateAuthority
}
...
func (a *CertificateAuthorityAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &memorystorepb.GetCertificateAuthorityRequest{Name: a.id}
...
func (a *CertificateAuthorityAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := MemorystoreCertificateAuthoritySpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &memorystorepb.CreateCertificateAuthorityRequest{
	...
	status := &krm.MemorystoreCertificateAuthorityStatus{}
	status.ObservedState = MemorystoreCertificateAuthorityObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CertificateAuthorityAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := MemorystoreCertificateAuthoritySpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.MemorystoreCertificateAuthorityStatus{}
		status.ObservedState = MemorystoreCertificateAuthorityObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &memorystorepb.UpdateCertificateAuthorityRequest{
		Name:                 a.id.External,
	...
	status := &krm.MemorystoreCertificateAuthorityStatus{}
	status.ObservedState = MemorystoreCertificateAuthorityObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CertificateAuthorityAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.MemorystoreCertificateAuthority{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(MemorystoreCertificateAuthoritySpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.MemorystoreCertificateAuthorityGVK)
...
func (a *CertificateAuthorityAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &memorystorepb.DeleteCertificateAuthorityRequest{Name: a.id.String()}
