...
func init() {
	registry.RegisterModel(krm.BinaryauthorizationPolicyGVK, NewPolicyModel)
}
...
func (m *modelPolicy) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.BinaryauthorizationPolicy{}
...
type PolicyAdapter struct {
	...
	desired   *krm.BinaryauthorizationPolicy
	actual    *binaryauthorizationpb.Policy
}
...
func (a *PolicyAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &binaryauthorizationpb.GetPolicyRequest{Name: a.id}
...
func (a *PolicyAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := BinaryauthorizationPolicySpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &binaryauthorizationpb.CreatePolicyRequest{
	...
	status := &krm.BinaryauthorizationPolicyStatus{}
	status.ObservedState = BinaryauthorizationPolicyObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *PolicyAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := BinaryauthorizationPolicySpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.BinaryauthorizationPolicyStatus{}
		status.ObservedState = BinaryauthorizationPolicyObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.BinaryauthorizationPolicyStatus{}
	status.ObservedState = BinaryauthorizationPolicyObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *PolicyAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.BinaryauthorizationPolicy{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(BinaryauthorizationPolicySpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.BinaryauthorizationPolicyGVK)
...
func (a *PolicyAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &binaryauthorizationpb.DeletePolicyRequest{Name: a.id.String()}
