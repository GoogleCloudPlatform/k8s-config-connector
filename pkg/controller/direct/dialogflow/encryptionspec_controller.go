...
func init() {
	registry.RegisterModel(krm.DialogflowEncryptionSpecGVK, NewEncryptionSpecModel)
}
...
func (m *modelEncryptionSpec) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.DialogflowEncryptionSpec{}
...
type EncryptionSpecAdapter struct {
	...
	desired   *krm.DialogflowEncryptionSpec
	actual    *dialogflowpb.EncryptionSpec
}
...
func (a *EncryptionSpecAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dialogflowpb.GetEncryptionSpecRequest{Name: a.id}
...
func (a *EncryptionSpecAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DialogflowEncryptionSpecSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &dialogflowpb.CreateEncryptionSpecRequest{
	...
	status := &krm.DialogflowEncryptionSpecStatus{}
	status.ObservedState = DialogflowEncryptionSpecObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EncryptionSpecAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DialogflowEncryptionSpecSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DialogflowEncryptionSpecStatus{}
		status.ObservedState = DialogflowEncryptionSpecObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &dialogflowpb.UpdateEncryptionSpecRequest{
		Name:           a.id.External,
	...
	status := &krm.DialogflowEncryptionSpecStatus{}
	status.ObservedState = DialogflowEncryptionSpecObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EncryptionSpecAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DialogflowEncryptionSpec{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DialogflowEncryptionSpecSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DialogflowEncryptionSpecGVK)
...
func (a *EncryptionSpecAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &dialogflowpb.DeleteEncryptionSpecRequest{Name: a.id.String()}
