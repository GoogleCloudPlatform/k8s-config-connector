...
func init() {
	registry.RegisterModel(krm.DialogflowExampleGVK, NewExampleModel)
}
...
type ExampleAdapter struct {
	...
	desired   *krm.DialogflowExample
	actual    *dialogflowpb.Example
}
...
func (a *ExampleAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dialogflowpb.GetExampleRequest{Name: a.id}
...
func (a *ExampleAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DialogflowExampleSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &dialogflowpb.CreateExampleRequest{
	...
	status := &krm.DialogflowExampleStatus{}
	status.ObservedState = DialogflowExampleObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ExampleAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DialogflowExampleSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DialogflowExampleStatus{}
		status.ObservedState = DialogflowExampleObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &dialogflowpb.UpdateExampleRequest{
		Name:       a.id.External,
	...
	status := &krm.DialogflowExampleStatus{}
	status.ObservedState = DialogflowExampleObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ExampleAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DialogflowExample{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DialogflowExampleSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DialogflowExampleGVK)
...
func (a *ExampleAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &dialogflowpb.DeleteExampleRequest{Name: a.id.String()}
