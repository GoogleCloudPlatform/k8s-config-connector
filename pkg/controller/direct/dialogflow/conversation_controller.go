...
func init() {
	registry.RegisterModel(krm.DialogflowConversationGVK, NewConversationModel)
}
...
func (m *modelConversation) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.DialogflowConversation{}
...
type ConversationAdapter struct {
	...
	desired   *krm.DialogflowConversation
	actual    *dialogflowpb.Conversation
}
...
func (a *ConversationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dialogflowpb.GetConversationRequest{Name: a.id}
...
func (a *ConversationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DialogflowConversationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.DialogflowConversationStatus{}
	status.ObservedState = DialogflowConversationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ConversationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DialogflowConversationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DialogflowConversationStatus{}
		status.ObservedState = DialogflowConversationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.DialogflowConversationStatus{}
	status.ObservedState = DialogflowConversationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ConversationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DialogflowConversation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DialogflowConversationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DialogflowConversationGVK)
...
func (a *ConversationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &dialogflowpb.DeleteConversationRequest{Name: a.id.String()}
