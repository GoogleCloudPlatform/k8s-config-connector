...
func (m *entitlementModel) AdapterForObject(ctx context.Context, op *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	id := i.(*krm.PrivilegedAccessManagerEntitlementIdentity)
...
func (a *Adapter) Find(ctx context.Context) (bool, error) {
	...
	log.V(2).Info("getting PrivilegedAccessManagerEntitlement", "name", a.id.String())

	req := &privilegedaccessmanagerpb.GetEntitlementRequest{Name: a.id.String()}
	entitlementpb, err := a.gcpClient.GetEntitlement(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("error getting PrivilegedAccessManagerEntitlement %q: %w", a.id.String(), err)
	}
...
func (a *Adapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	log := klog.FromContext(ctx)
	log.V(2).Info("creating PrivilegedAccessManagerEntitlement", "name", a.id.String())
	mapCtx := &direct.MapContext{}

	desired := a.desired.DeepCopy()
	resourceType, resource, err := getResourceTypeAndResourceFromContainer(a.id.Parent())
	if err != nil {
	...
	req := &privilegedaccessmanagerpb.CreateEntitlementRequest{
		Parent:        a.id.FullParent(),
		EntitlementId: a.id.Entitlement,
		Entitlement:   entitlement,
	}
	op, err := a.gcpClient.CreateEntitlement(ctx, req)
	if err != nil {
		return fmt.Errorf("error creating PrivilegedAccessManagerEntitlement %s: %w", a.id.String(), err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("error waiting for PrivilegedAccessManagerEntitlement %s to be created: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully created PrivilegedAccessManagerEntitlement", "name", a.id.String())
	...
	status.ExternalRef = direct.PtrTo("//" + a.id.Host() + "/" + a.id.String())
...
func (a *Adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log := klog.FromContext(ctx)
	log.V(2).Info("updating PrivilegedAccessManagerEntitlement", "name", a.id.String())
	mapCtx := &direct.MapContext{}
	...
	if len(updateMask.Paths) == 0 {
		log.V(2).Info("underlying PrivilegedAccessManagerEntitlement already up to date", "name", a.id.String())
	...
	if err != nil {
		return fmt.Errorf("error updating PrivilegedAccessManagerEntitlement %s: %w", a.id.String(), err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("error waiting for PrivilegedAccessManagerEntitlement %s to be updated: %w", a.id.String(), err)
	}
	log.V(2).Info("successfully updated PrivilegedAccessManagerEntitlement", "name", a.id.String())
...
func (a *Adapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	if strings.HasPrefix(a.id.Parent(), "projects") {
		obj.Spec.ProjectRef = &refs.ProjectRef{External: a.id.Parent()}
	} else if strings.HasPrefix(a.id.Parent(), "folders") {
		obj.Spec.FolderRef = &refs.FolderRef{External: a.id.Parent()}
	} else {
		obj.Spec.OrganizationRef = &refs.OrganizationRef{External: a.id.Parent()}
	}
...
func (a *Adapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	log := klog.FromContext(ctx)
	log.V(2).Info("deleting PrivilegedAccessManagerEntitlement", "name", a.id.String())

	req := &privilegedaccessmanagerpb.DeleteEntitlementRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteEntitlement(ctx, req)
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("error deleting PrivilegedAccessManagerEntitlement %s: %w", a.id.String(), err)
	}

	_, err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("error waiting for PrivilegedAccessManagerEntitlement %s to be deleted: %w", a.id.String(), err)
	}

	log.V(2).Info("successfully deleted PrivilegedAccessManagerEntitlement", "name", a.id.String())
	return true, nil
}
