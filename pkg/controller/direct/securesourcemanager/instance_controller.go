...
func init() {
	registry.RegisterModel(krm.SecureSourceManagerInstanceGVK, newInstanceModel)
}
...
func (a *instanceAdapter) Delete(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("deleting instance %q: %w", a.fullyQualifiedName(), err)
	}
...
func (a *instanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	if err := mc.Err(); err != nil {
		return nil, fmt.Errorf("error building SecureSourceManagerInstance from API %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error converting SecureSourceManagerInstance to unstructured: %w", err)
	}
