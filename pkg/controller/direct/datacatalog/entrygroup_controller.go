...
type EntryGroupAdapter struct {
	...
	actual    *datacatalogpb.EntryGroup
}
...
func (a *EntryGroupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datacatalogpb.GetEntryGroupRequest{Name: a.id}
...
func (a *EntryGroupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DatacatalogEntryGroupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EntryGroupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatacatalogEntryGroupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatacatalogEntryGroupStatus{}
		status.ObservedState = DatacatalogEntryGroupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
...
func (a *EntryGroupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datacatalogpb.DeleteEntryGroupRequest{Name: a.id.String()}
