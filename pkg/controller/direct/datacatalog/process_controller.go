...
func init() {
	registry.RegisterModel(krm.DatacatalogProcessGVK, NewProcessModel)
}
...
type ProcessAdapter struct {
	...
	desired   *krm.DatacatalogProcess
	actual    *datacatalogpb.Process
}
...
func (a *ProcessAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datacatalogpb.GetProcessRequest{Name: a.id}
...
func (a *ProcessAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DatacatalogProcessSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &datacatalogpb.CreateProcessRequest{
	...
	status := &krm.DatacatalogProcessStatus{}
	status.ObservedState = DatacatalogProcessObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ProcessAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatacatalogProcessSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatacatalogProcessStatus{}
		status.ObservedState = DatacatalogProcessObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &datacatalogpb.UpdateProcessRequest{
		Name:       a.id.External,
	...
	status := &krm.DatacatalogProcessStatus{}
	status.ObservedState = DatacatalogProcessObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ProcessAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DatacatalogProcess{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DatacatalogProcessSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DatacatalogProcessGVK)
...
func (a *ProcessAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datacatalogpb.DeleteProcessRequest{Name: a.id.String()}
