...
func (m *modelFirewallPolicy) client(ctx context.Context) (*gcp.Client, error) {
...
type FirewallPolicyAdapter struct {
	...
	actual    *recaptchaenterprisepb.FirewallPolicy
}
...
func (a *FirewallPolicyAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &recaptchaenterprisepb.GetFirewallPolicyRequest{Name: a.id}
...
func (a *FirewallPolicyAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	op, err := a.gcpClient.CreateFirewallPolicy(ctx, req)
	if err != nil {
		return fmt.Errorf("creating FirewallPolicy %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("FirewallPolicy %s waiting creation: %w", a.id, err)
	}
...
func (a *FirewallPolicyAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RecaptchaenterpriseFirewallPolicySpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RecaptchaenterpriseFirewallPolicyStatus{}
		status.ObservedState = RecaptchaenterpriseFirewallPolicyObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	op, err := a.gcpClient.UpdateFirewallPolicy(ctx, req)
	if err != nil {
		return fmt.Errorf("updating FirewallPolicy %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("FirewallPolicy %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated FirewallPolicy", "name", a.id.External)
...
func (a *FirewallPolicyAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &recaptchaenterprisepb.DeleteFirewallPolicyRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteFirewallPolicy(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete FirewallPolicy %s: %w", a.id, err)
	}
	return true, nil
}
