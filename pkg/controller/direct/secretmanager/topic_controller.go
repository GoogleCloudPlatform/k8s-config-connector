...
import (
	...
	secretmanagerpb "cloud.google.com/go/secretmanager/v1/secretmanagerpb"
...
func init() {
	registry.RegisterModel(krm.SecretmanagerTopicGVK, NewTopicModel)
}
...
type TopicAdapter struct {
	...
	desired   *krm.SecretmanagerTopic
	actual    *secretmanagerpb.Topic
}
...
func (a *TopicAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &secretmanagerpb.GetTopicRequest{Name: a.id}
...
func (a *TopicAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := SecretmanagerTopicSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &secretmanagerpb.CreateTopicRequest{
		Parent: a.id.Parent().String(),
		Topic:  resource,
	}
	op, err := a.gcpClient.CreateTopic(ctx, req)
	if err != nil {
		return fmt.Errorf("creating Topic %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Topic %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created Topic", "name", a.id)

	status := &krm.SecretmanagerTopicStatus{}
	status.ObservedState = SecretmanagerTopicObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TopicAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := SecretmanagerTopicSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.SecretmanagerTopicStatus{}
		status.ObservedState = SecretmanagerTopicObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &secretmanagerpb.UpdateTopicRequest{
		Name:       a.id.External,
		UpdateMask: updateMask,
		Topic:      desiredPb,
	}
	op, err := a.gcpClient.UpdateTopic(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Topic %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Topic %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Topic", "name", a.id.External)

	status := &krm.SecretmanagerTopicStatus{}
	status.ObservedState = SecretmanagerTopicObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TopicAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.SecretmanagerTopic{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(SecretmanagerTopicSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.SecretmanagerTopicGVK)
...
func (a *TopicAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &secretmanagerpb.DeleteTopicRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteTopic(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete Topic %s: %w", a.id, err)
	}
	return true, nil
}
