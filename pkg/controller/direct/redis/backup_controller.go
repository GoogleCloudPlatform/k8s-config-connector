...
func init() {
	registry.RegisterModel(krm.RedisBackupGVK, NewBackupModel)
}
...
type BackupAdapter struct {
	...
	desired   *krm.RedisBackup
	actual    *redispb.Backup
}
...
func (a *BackupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &redispb.GetBackupRequest{Name: a.id}
...
func (a *BackupAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := RedisBackupSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &redispb.CreateBackupRequest{
	...
	status := &krm.RedisBackupStatus{}
	status.ObservedState = RedisBackupObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *BackupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := RedisBackupSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.RedisBackupStatus{}
		status.ObservedState = RedisBackupObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &redispb.UpdateBackupRequest{
		Name:       a.id.External,
	...
	status := &krm.RedisBackupStatus{}
	status.ObservedState = RedisBackupObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *BackupAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.RedisBackup{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(RedisBackupSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.RedisBackupGVK)
...
func (a *BackupAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &redispb.DeleteBackupRequest{Name: a.id.String()}
