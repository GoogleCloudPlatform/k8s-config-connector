...
func FuzzBigQueryAnalyticsHubDataExchangeSpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		randStream := rand.New(rand.NewSource(seed))

		p1 := &bigqueryanalyticshubpb.DataExchange{}
		...
		k := BigQueryAnalyticsHubDataExchangeSpec_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := BigQueryAnalyticsHubDataExchangeSpec_ToProto(ctx, k)
		if ctx.Err() != nil {
