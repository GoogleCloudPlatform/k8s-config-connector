...
func init() {
	registry.RegisterModel(krm.PubsubliteTopicGVK, NewTopicModel)
}
...
type TopicAdapter struct {
	...
	desired   *krm.PubsubliteTopic
	actual    *pubsublitepb.Topic
}
...
func (a *TopicAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &pubsublitepb.GetTopicRequest{Name: a.id}
...
func (a *TopicAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PubsubliteTopicSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.PubsubliteTopicStatus{}
	status.ObservedState = PubsubliteTopicObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TopicAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PubsubliteTopicSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PubsubliteTopicStatus{}
		status.ObservedState = PubsubliteTopicObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.PubsubliteTopicStatus{}
	status.ObservedState = PubsubliteTopicObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TopicAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PubsubliteTopic{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PubsubliteTopicSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PubsubliteTopicGVK)
...
func (a *TopicAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &pubsublitepb.DeleteTopicRequest{Name: a.id.String()}
