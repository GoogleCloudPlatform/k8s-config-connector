...
import (
	...
	gcp "cloud.google.com/go/pubsublite/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	pubsublitepb "cloud.google.com/go/pubsublite/v1/pubsublitepb"
...
func init() {
	registry.RegisterModel(krm.PubsubliteReservationGVK, NewReservationModel)
}
...
type ReservationAdapter struct {
	...
	desired   *krm.PubsubliteReservation
	actual    *pubsublitepb.Reservation
}
...
func (a *ReservationAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &pubsublitepb.GetReservationRequest{Name: a.id}
...
func (a *ReservationAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PubsubliteReservationSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	op, err := a.gcpClient.CreateReservation(ctx, req)
	if err != nil {
		return fmt.Errorf("creating Reservation %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Reservation %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created Reservation", "name", a.id)

	status := &krm.PubsubliteReservationStatus{}
	status.ObservedState = PubsubliteReservationObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ReservationAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PubsubliteReservationSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PubsubliteReservationStatus{}
		status.ObservedState = PubsubliteReservationObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	op, err := a.gcpClient.UpdateReservation(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Reservation %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Reservation %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Reservation", "name", a.id.External)

	status := &krm.PubsubliteReservationStatus{}
	status.ObservedState = PubsubliteReservationObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ReservationAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PubsubliteReservation{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PubsubliteReservationSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PubsubliteReservationGVK)
...
func (a *ReservationAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &pubsublitepb.DeleteReservationRequest{Name: a.id.String()}
	op, err := a.gcpClient.DeleteReservation(ctx, req)
	if err != nil {
		return false, fmt.Errorf("deleting Reservation %s: %w", a.id, err)
	}
	log.V(2).Info("successfully deleted Reservation", "name", a.id)

	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete Reservation %s: %w", a.id, err)
	}
	return true, nil
}
