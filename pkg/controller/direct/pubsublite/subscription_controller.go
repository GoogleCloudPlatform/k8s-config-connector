...
func init() {
	registry.RegisterModel(krm.PubsubliteSubscriptionGVK, NewSubscriptionModel)
}
...
func (m *modelSubscription) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.PubsubliteSubscription{}
...
type SubscriptionAdapter struct {
	...
	desired   *krm.PubsubliteSubscription
	actual    *pubsublitepb.Subscription
}
...
func (a *SubscriptionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &pubsublitepb.GetSubscriptionRequest{Name: a.id}
...
func (a *SubscriptionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PubsubliteSubscriptionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.PubsubliteSubscriptionStatus{}
	status.ObservedState = PubsubliteSubscriptionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SubscriptionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PubsubliteSubscriptionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PubsubliteSubscriptionStatus{}
		status.ObservedState = PubsubliteSubscriptionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.PubsubliteSubscriptionStatus{}
	status.ObservedState = PubsubliteSubscriptionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SubscriptionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PubsubliteSubscription{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PubsubliteSubscriptionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.PubsubliteSubscriptionGVK)
...
func (a *SubscriptionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &pubsublitepb.DeleteSubscriptionRequest{Name: a.id.String()}
