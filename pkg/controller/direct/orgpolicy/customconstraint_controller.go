...
func (a *CustomConstraintAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &orgpolicypb.GetCustomConstraintRequest{Name: a.id}
...
func (a *CustomConstraintAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	op, err := a.gcpClient.CreateCustomConstraint(ctx, req)
	if err != nil {
		return fmt.Errorf("creating CustomConstraint %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("CustomConstraint %s waiting creation: %w", a.id, err)
	}
...
func (a *CustomConstraintAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if len(paths) == 0 {
		...
		status := &krm.OrgpolicyCustomConstraintStatus{}
		status.ObservedState = OrgpolicyCustomConstraintObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	op, err := a.gcpClient.UpdateCustomConstraint(ctx, req)
	if err != nil {
		return fmt.Errorf("updating CustomConstraint %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("CustomConstraint %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated CustomConstraint", "name", a.id.External)
...
func (a *CustomConstraintAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	op, err := a.gcpClient.DeleteCustomConstraint(ctx, req)
	if err != nil {
	...
	err = op.Wait(ctx)
	if err != nil {
		return false, fmt.Errorf("waiting delete CustomConstraint %s: %w", a.id, err)
	}
	return true, nil
}
