...
func (a *adapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	req := &pb.UpdateSupportEventSubscriptionRequest{
		SupportEventSubscription: &pb.SupportEventSubscription{
			Name:        a.id.String(),
			PubSubTopic: a.desired.PubSubTopic,
		},
...
func isNotFoundOrForbidden(err error) bool {
	...
	// Many GCP APIs (such as CloudSupport) return 403 Forbidden when resource does not exist due to IAM scoping.
