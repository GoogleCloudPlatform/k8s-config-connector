...
func normalizeExternal(ctx context.Context, reader client.Reader, src client.Object, store *krm.VertexAIMetadataStore) error {
	...
	if store.Spec.EncryptionSpec != nil && store.Spec.EncryptionSpec.KMSKeyNameRef != nil {
		kmsKey, err := refs.ResolveKMSCryptoKeyRef(ctx, reader, src, store.Spec.EncryptionSpec.KMSKeyNameRef)
		if err != nil {
			return err
		}
		store.Spec.EncryptionSpec.KMSKeyNameRef = kmsKey
	}
	return nil
}
