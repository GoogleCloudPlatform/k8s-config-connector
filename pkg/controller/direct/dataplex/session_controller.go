...
func init() {
	registry.RegisterModel(krm.DataplexSessionGVK, NewSessionModel)
}
...
type SessionAdapter struct {
	...
	desired   *krm.DataplexSession
	actual    *dataplexpb.Session
}
...
func (a *SessionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dataplexpb.GetSessionRequest{Name: a.id}
...
func (a *SessionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DataplexSessionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &dataplexpb.CreateSessionRequest{
	...
	status := &krm.DataplexSessionStatus{}
	status.ObservedState = DataplexSessionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *SessionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DataplexSessionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DataplexSessionStatus{}
		status.ObservedState = DataplexSessionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &dataplexpb.UpdateSessionRequest{
		Name:       a.id.External,
	...
	status := &krm.DataplexSessionStatus{}
	status.ObservedState = DataplexSessionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *SessionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DataplexSession{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DataplexSessionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DataplexSessionGVK)
...
func (a *SessionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &dataplexpb.DeleteSessionRequest{Name: a.id.String()}
