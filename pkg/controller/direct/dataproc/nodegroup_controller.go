...
func (a *NodeGroupAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dataprocpb.GetNodeGroupRequest{Name: a.id}
...
func (a *NodeGroupAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating NodeGroup %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("NodeGroup %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated NodeGroup", "name", a.id.External)
