...
func (a *SessionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &dataprocpb.GetSessionRequest{Name: a.id}
...
func (a *SessionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DataprocSessionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	req := &dataprocpb.UpdateSessionRequest{
		Name:       a.id.External,
	...
	if err != nil {
		return fmt.Errorf("updating Session %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Session %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Session", "name", a.id.External)

	status := &krm.DataprocSessionStatus{}
	status.ObservedState = DataprocSessionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
