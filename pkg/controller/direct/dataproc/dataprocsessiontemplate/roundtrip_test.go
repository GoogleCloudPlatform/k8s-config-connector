...
func FuzzDataprocSessionTemplateSpec(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64) {
		randStream := rand.New(rand.NewSource(seed))

		p1 := &pb.SessionTemplate{}
		...
		unimplementedFields := sets.New(
			...
			".environment_config.execution_config.resource_manager_tags",
		)
		...
		k := dataprocgcp.DataprocSessionTemplateSpec_v1alpha1_FromProto(ctx, p1)
		if ctx.Err() != nil {
			t.Fatalf("error mapping from proto to krm: %v", ctx.Err())
		}

		p2 := dataprocgcp.DataprocSessionTemplateSpec_v1alpha1_ToProto(ctx, k)
		if ctx.Err() != nil {
