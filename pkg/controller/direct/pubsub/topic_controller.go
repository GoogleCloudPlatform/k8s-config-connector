...
import (
	...
	gcp "cloud.google.com/go/pubsub/apiv1"

	// TODO(contributor): Update the import with the google cloud client api protobuf
	pubsubpb "cloud.google.com/go/pubsub/v1/pubsubpb"
...
type TopicAdapter struct {
	...
	desired   *krm.PubsubTopic
	actual    *pubsubpb.Topic
}
...
func (a *TopicAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &pubsubpb.GetTopicRequest{Name: a.id}
...
func (a *TopicAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := PubsubTopicSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status.ObservedState = PubsubTopicObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *TopicAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := PubsubTopicSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.PubsubTopicStatus{}
		status.ObservedState = PubsubTopicObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.PubsubTopicStatus{}
	status.ObservedState = PubsubTopicObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *TopicAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.PubsubTopic{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(PubsubTopicSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
...
func (a *TopicAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &pubsubpb.DeleteTopicRequest{Name: a.id.String()}
