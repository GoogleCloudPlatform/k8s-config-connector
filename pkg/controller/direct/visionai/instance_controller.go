...
func init() {
	registry.RegisterModel(krm.VisionaiInstanceGVK, NewInstanceModel)
}
...
type InstanceAdapter struct {
	...
	desired   *krm.VisionaiInstance
	actual    *visionaipb.Instance
}
...
func (a *InstanceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &visionaipb.GetInstanceRequest{Name: a.id}
...
func (a *InstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VisionaiInstanceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &visionaipb.CreateInstanceRequest{
	...
	status := &krm.VisionaiInstanceStatus{}
	status.ObservedState = VisionaiInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VisionaiInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VisionaiInstanceStatus{}
		status.ObservedState = VisionaiInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &visionaipb.UpdateInstanceRequest{
		Name:       a.id.External,
	...
	status := &krm.VisionaiInstanceStatus{}
	status.ObservedState = VisionaiInstanceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VisionaiInstance{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VisionaiInstanceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VisionaiInstanceGVK)
...
func (a *InstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &visionaipb.DeleteInstanceRequest{Name: a.id.String()}
