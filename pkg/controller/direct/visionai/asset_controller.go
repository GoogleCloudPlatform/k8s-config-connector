...
func init() {
	registry.RegisterModel(krm.VisionaiAssetGVK, NewAssetModel)
}
...
type AssetAdapter struct {
	...
	desired   *krm.VisionaiAsset
	actual    *visionaipb.Asset
}
...
func (a *AssetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &visionaipb.GetAssetRequest{Name: a.id}
...
func (a *AssetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VisionaiAssetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VisionaiAssetStatus{}
	status.ObservedState = VisionaiAssetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *AssetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VisionaiAssetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VisionaiAssetStatus{}
		status.ObservedState = VisionaiAssetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.VisionaiAssetStatus{}
	status.ObservedState = VisionaiAssetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *AssetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VisionaiAsset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VisionaiAssetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VisionaiAssetGVK)
...
func (a *AssetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &visionaipb.DeleteAssetRequest{Name: a.id.String()}
