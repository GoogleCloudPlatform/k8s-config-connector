...
func init() {
	registry.RegisterModel(krm.VisionaiChannelGVK, NewChannelModel)
}
...
type ChannelAdapter struct {
	...
	desired   *krm.VisionaiChannel
	actual    *visionaipb.Channel
}
...
func (a *ChannelAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &visionaipb.GetChannelRequest{Name: a.id}
...
func (a *ChannelAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VisionaiChannelSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}

	// TODO(contributor): Complete the gcp "CREATE" or "INSERT" request.
	req := &visionaipb.CreateChannelRequest{
	...
	status := &krm.VisionaiChannelStatus{}
	status.ObservedState = VisionaiChannelObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ChannelAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VisionaiChannelSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VisionaiChannelStatus{}
		status.ObservedState = VisionaiChannelObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &visionaipb.UpdateChannelRequest{
		Name:       a.id.External,
	...
	status := &krm.VisionaiChannelStatus{}
	status.ObservedState = VisionaiChannelObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ChannelAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VisionaiChannel{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VisionaiChannelSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VisionaiChannelGVK)
...
func (a *ChannelAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &visionaipb.DeleteChannelRequest{Name: a.id.String()}
