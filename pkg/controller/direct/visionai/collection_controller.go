...
func init() {
	registry.RegisterModel(krm.VisionaiCollectionGVK, NewCollectionModel)
}
...
func (m *modelCollection) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.VisionaiCollection{}
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &obj); err != nil {
		return nil, fmt.Errorf("error converting to %T: %w", obj, err)
	}

	id, err := krm.NewCollectionIdentity(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
...
type CollectionAdapter struct {
	id        *krm.CollectionIdentity
	gcpClient *gcp.Client
	desired   *krm.VisionaiCollection
	actual    *visionaipb.Collection
}
...
func (a *CollectionAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &visionaipb.GetCollectionRequest{Name: a.id}
...
func (a *CollectionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VisionaiCollectionSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VisionaiCollectionStatus{}
	status.ObservedState = VisionaiCollectionObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CollectionAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VisionaiCollectionSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VisionaiCollectionStatus{}
		status.ObservedState = VisionaiCollectionObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &visionaipb.UpdateCollectionRequest{
		...
		Collection: desiredPb,
	}
	op, err := a.gcpClient.UpdateCollection(ctx, req)
	if err != nil {
		return fmt.Errorf("updating Collection %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Collection %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated Collection", "name", a.id.External)

	status := &krm.VisionaiCollectionStatus{}
	status.ObservedState = VisionaiCollectionObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CollectionAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VisionaiCollection{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VisionaiCollectionSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VisionaiCollectionGVK)
...
func (a *CollectionAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &visionaipb.DeleteCollectionRequest{Name: a.id.String()}
