...
func init() {
	registry.RegisterModel(krm.VisionaiClusterGVK, NewClusterModel)
}
...
type ClusterAdapter struct {
	...
	desired   *krm.VisionaiCluster
	actual    *visionaipb.Cluster
}
...
func (a *ClusterAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &visionaipb.GetClusterRequest{Name: a.id}
...
func (a *ClusterAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := VisionaiClusterSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.VisionaiClusterStatus{}
	status.ObservedState = VisionaiClusterObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ClusterAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := VisionaiClusterSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.VisionaiClusterStatus{}
		status.ObservedState = VisionaiClusterObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.VisionaiClusterStatus{}
	status.ObservedState = VisionaiClusterObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ClusterAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.VisionaiCluster{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(VisionaiClusterSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.VisionaiClusterGVK)
...
func (a *ClusterAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &visionaipb.DeleteClusterRequest{Name: a.id.String()}
