...
func (a *gkeHubScopeRBACRoleBindingAdapter) updateStatus(ctx context.Context, op directbase.Operation) error {
	status := &krm.GKEHubScopeRBACRoleBindingStatus{}
	mapCtx := &direct.MapContext{}
	status.ObservedState = a.apiToKrmObservedState(mapCtx)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *gkeHubScopeRBACRoleBindingAdapter) apiToKrmObservedState(mapCtx *direct.MapContext) *krm.GKEHubScopeRBACRoleBindingObservedState {
	if a.actual == nil {
		return nil
	}
	out := &krm.GKEHubScopeRBACRoleBindingObservedState{}
	out.Uid = direct.LazyPtr(a.actual.Uid)
	out.CreateTime = direct.LazyPtr(a.actual.CreateTime)
	out.UpdateTime = direct.LazyPtr(a.actual.UpdateTime)
	out.DeleteTime = direct.LazyPtr(a.actual.DeleteTime)
	out.Name = direct.LazyPtr(a.actual.Name)
	if a.actual.State != nil {
		out.State = &krm.ScopeRBACRoleBindingStateStatus{
			Code: direct.LazyPtr(a.actual.State.Code),
		}
	}
	return out
}
