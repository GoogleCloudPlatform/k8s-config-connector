...
func (a *gkeHubAdapter) Find(ctx context.Context) (bool, error) {
	...
	if membershipRef.External != "" {
		...
		if location := a.desired.Spec.MembershipLocation; location != nil {
			a.membershipID = fmt.Sprintf("projects/%s/locations/%s/memberships/%s", a.projectID, common.ValueOf(location), membershipRef.Name)
		}
