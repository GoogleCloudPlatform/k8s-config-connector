...
import (
	...
	datafusionpb "cloud.google.com/go/datafusion/v1/datafusionpb"
...
func init() {
	registry.RegisterModel(krm.DatafusionInstanceGVK, NewInstanceModel)
}
...
type InstanceAdapter struct {
	...
	desired   *krm.DatafusionInstance
	actual    *datafusionpb.Instance
}
...
func (a *InstanceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &datafusionpb.GetInstanceRequest{Name: a.id}
...
func (a *InstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := DatafusionInstanceSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.DatafusionInstanceStatus{}
	status.ObservedState = DatafusionInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := DatafusionInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.DatafusionInstanceStatus{}
		status.ObservedState = DatafusionInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.DatafusionInstanceStatus{}
	status.ObservedState = DatafusionInstanceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.DatafusionInstance{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(DatafusionInstanceSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.DatafusionInstanceGVK)
...
func (a *InstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &datafusionpb.DeleteInstanceRequest{Name: a.id.String()}
