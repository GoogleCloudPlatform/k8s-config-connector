...
func init() {
	registry.RegisterModel(krm.ApigeeregistryArtifactGVK, NewArtifactModel)
}
...
func (m *modelArtifact) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.ApigeeregistryArtifact{}
...
type ArtifactAdapter struct {
	...
	desired   *krm.ApigeeregistryArtifact
	actual    *apigeeregistrypb.Artifact
}
...
func (a *ArtifactAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigeeregistrypb.GetArtifactRequest{Name: a.id}
...
func (a *ArtifactAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ApigeeregistryArtifactSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ApigeeregistryArtifactStatus{}
	status.ObservedState = ApigeeregistryArtifactObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ArtifactAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApigeeregistryArtifactSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApigeeregistryArtifactStatus{}
		status.ObservedState = ApigeeregistryArtifactObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &apigeeregistrypb.UpdateArtifactRequest{
		Name:       a.id.External,
	...
	status := &krm.ApigeeregistryArtifactStatus{}
	status.ObservedState = ApigeeregistryArtifactObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ArtifactAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ApigeeregistryArtifact{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ApigeeregistryArtifactSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ApigeeregistryArtifactGVK)
...
func (a *ArtifactAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &apigeeregistrypb.DeleteArtifactRequest{Name: a.id.String()}
