...
func init() {
	registry.RegisterModel(krm.ApigeeregistryApiGVK, NewApiModel)
}
...
type ApiAdapter struct {
	...
	desired   *krm.ApigeeregistryApi
	actual    *apigeeregistrypb.Api
}
...
func (a *ApiAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigeeregistrypb.GetApiRequest{Name: a.id}
...
func (a *ApiAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := ApigeeregistryApiSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.ApigeeregistryApiStatus{}
	status.ObservedState = ApigeeregistryApiObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *ApiAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApigeeregistryApiSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApigeeregistryApiStatus{}
		status.ObservedState = ApigeeregistryApiObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.ApigeeregistryApiStatus{}
	status.ObservedState = ApigeeregistryApiObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *ApiAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.ApigeeregistryApi{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(ApigeeregistryApiSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.ApigeeregistryApiGVK)
...
func (a *ApiAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &apigeeregistrypb.DeleteApiRequest{Name: a.id.String()}
