...
func init() {
	registry.RegisterModel(krm.ApigeeregistryInstanceGVK, NewInstanceModel)
}
...
type InstanceAdapter struct {
	...
	actual    *apigeeregistrypb.Instance
}
...
func (a *InstanceAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigeeregistrypb.GetInstanceRequest{Name: a.id}
...
func (a *InstanceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	status.ObservedState = ApigeeregistryInstanceObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *InstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := ApigeeregistryInstanceSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.ApigeeregistryInstanceStatus{}
		status.ObservedState = ApigeeregistryInstanceObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &apigeeregistrypb.UpdateInstanceRequest{
		Name:       a.id.External,
	...
	status := &krm.ApigeeregistryInstanceStatus{}
	status.ObservedState = ApigeeregistryInstanceObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *InstanceAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &apigeeregistrypb.DeleteInstanceRequest{Name: a.id.String()}
