...
func (a *ApiDeploymentAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &apigeeregistrypb.GetApiDeploymentRequest{Name: a.id}
...
func (a *ApiDeploymentAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("updating ApiDeployment %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("ApiDeployment %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated ApiDeployment", "name", a.id.External)
