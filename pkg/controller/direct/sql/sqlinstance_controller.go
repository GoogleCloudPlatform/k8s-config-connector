...
func (a *sqlInstanceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if currentSpecHash == direct.ValueOf(a.desired.Status.LastAppliedSpecHash) {
		...
		if currentGCPHash == direct.ValueOf(a.desired.Status.LastAppliedGCPHash) {
