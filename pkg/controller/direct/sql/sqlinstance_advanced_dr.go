...
// IsDesiredPrimaryWithDR returns true if the DatabaseInstance is configured as a Primary instance for Advanced DR in KRM.
//
// In Cloud SQL:
...
func IsDesiredPrimaryWithDR(inst *api.DatabaseInstance) bool {
	if inst == nil {
		return false
	}
...
// IsDesiredReplicaWithDR returns true if the DatabaseInstance is configured as a Replica for Advanced DR in KRM.
//
// In Cloud SQL:
...
// 3. A replica instance does NOT designate another failover DR replica (replicationCluster.failoverDrReplicaName must be empty).
func IsDesiredReplicaWithDR(inst *api.DatabaseInstance) bool {
	if inst == nil {
		return false
	}
	...
	if inst.ReplicationCluster != nil && inst.ReplicationCluster.FailoverDrReplicaName != "" {
		return false
	}
	return true
}

// IsActualPrimaryWithDR returns true if the DatabaseInstance is currently acting as a Primary instance for Advanced DR in GCP.
// It verifies that it meets the desired primary configuration, and that the output-only DrReplica field is false.
func IsActualPrimaryWithDR(inst *api.DatabaseInstance) bool {
	if !IsDesiredPrimaryWithDR(inst) {
		return false
	}
	return !inst.ReplicationCluster.DrReplica
}

// IsActualReplicaWithDR returns true if the DatabaseInstance is currently acting as a DR Replica in GCP.
// It verifies that it meets the desired replica configuration, and that the output-only DrReplica field is true.
func IsActualReplicaWithDR(inst *api.DatabaseInstance) bool {
	if !IsDesiredReplicaWithDR(inst) {
		return false
	}
	return inst.ReplicationCluster != nil && inst.ReplicationCluster.DrReplica
}
...
func ShouldSkipUpdateForAdvancedDR(desiredKRM *krmv1beta1.SQLInstance, desiredProto *api.DatabaseInstance, actualProto *api.DatabaseInstance) bool {
	...
	// Case 1: KRM spec defines Primary with DR, but GCP state is now a DR Replica (demoted via switchover).
	if IsDesiredPrimaryWithDR(desiredProto) && IsActualReplicaWithDR(actualProto) {
		return true
	}

	// Case 2: KRM spec defines DR Replica, but GCP state is now Primary (promoted via switchover/failover).
	if IsDesiredReplicaWithDR(desiredProto) && IsActualPrimaryWithDR(actualProto) {
		return true
	}
