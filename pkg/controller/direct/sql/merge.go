...
// for every API field.
func MergeDesiredSQLInstanceWithActual(desired *krm.SQLInstance, actual *api.DatabaseInstance) (*api.DatabaseInstance, bool, error) {
	...
	if desired.Spec.EncryptionKMSCryptoKeyRef != nil {
		if actual.DiskEncryptionConfiguration == nil {
			// Add key
			updateRequired = true
		} else if desired.Spec.EncryptionKMSCryptoKeyRef.Name != actual.DiskEncryptionConfiguration.KmsKeyName {
			// Change keys
			updateRequired = true
		}
		merged.DiskEncryptionConfiguration = &api.DiskEncryptionConfiguration{
			Kind:       "sql#diskEncryptionConfiguration",
			KmsKeyName: desired.Spec.EncryptionKMSCryptoKeyRef.Name,
		}
	...
	if desired.Spec.MasterInstanceRef != nil {
		if desired.Spec.MasterInstanceRef.Name != actual.MasterInstanceName {
			// Change master
			updateRequired = true
		}
		merged.MasterInstanceName = desired.Spec.MasterInstanceRef.Name
	...
	if desired.Spec.ReplicaConfiguration != nil {
		...
		if desired.Spec.ReplicaConfiguration.Password != nil {
			if existingMysqlReplicaConfig {
				if direct.ValueOf(desired.Spec.ReplicaConfiguration.Password.Value) != actual.ReplicaConfiguration.MysqlReplicaConfiguration.Password {
					// Change password
					updateRequired = true
				}
			} else {
			...
			merged.ReplicaConfiguration.MysqlReplicaConfiguration.Password = direct.ValueOf(desired.Spec.ReplicaConfiguration.Password.Value)
	...
	if desired.Spec.RootPassword != nil && desired.Spec.RootPassword.Value != nil {
		if direct.ValueOf(desired.Spec.RootPassword.Value) != actual.RootPassword {
			// Change root password
			updateRequired = true
		}
		merged.RootPassword = direct.ValueOf(desired.Spec.RootPassword.Value)
	} else {
		// Keep root password
	...
	if desired.Spec.Settings.IpConfiguration != nil {
		if actual.Settings.IpConfiguration == nil {
			...
			(desired.Spec.Settings.IpConfiguration.PrivateNetworkRef.Name != actual.Settings.IpConfiguration.PrivateNetwork) ||
		...
		merged.Settings.IpConfiguration = &api.IpConfiguration{
			...
			PrivateNetwork:                          desired.Spec.Settings.IpConfiguration.PrivateNetworkRef.Name,
	...
	if desired.Spec.Settings.SqlServerAuditConfig != nil {
		if actual.Settings.SqlServerAuditConfig == nil {
			...
			updateRequired = true
		} else if (desired.Spec.Settings.SqlServerAuditConfig.BucketRef.Name != actual.Settings.SqlServerAuditConfig.Bucket) ||
		...
		merged.Settings.SqlServerAuditConfig = &api.SqlServerAuditConfig{
			Bucket:            desired.Spec.Settings.SqlServerAuditConfig.BucketRef.Name,
