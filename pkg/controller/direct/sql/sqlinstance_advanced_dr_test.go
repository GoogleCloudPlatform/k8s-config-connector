...
func TestIsPrimaryWithDR(t *testing.T) {
	tests := []struct {
		name string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := IsPrimaryWithDR(tc.inst)
			if got != tc.want {
				t.Errorf("IsPrimaryWithDR() = %v, want %v", got, tc.want)
			}
		})
	}
}

func TestIsReplicaWithDR(t *testing.T) {
	tests := []struct {
		name string
		inst *api.DatabaseInstance
		want bool
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := IsReplicaWithDR(tc.inst)
			if got != tc.want {
				t.Errorf("IsReplicaWithDR() = %v, want %v", got, tc.want)
			}
		})
	}
}
