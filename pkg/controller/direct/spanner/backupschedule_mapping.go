...
func CreateBackupEncryptionConfig_FromProto(mapCtx *direct.MapContext, in *pb.CreateBackupEncryptionConfig) *krm.CreateBackupEncryptionConfig {
	...
	if in.GetKmsKeyName() != "" {
		out.KMSKeyRef = &kmsv1beta1.KMSKeyRef_OneOf{External: in.GetKmsKeyName()}
	}
	var kmsKeyNameRefs []*kmsv1beta1.KMSKeyRef_OneOf
	if in.GetKmsKeyNames() != nil {
		for _, kmsKeyName := range in.GetKmsKeyNames() {
			kmsKeyNameRefs = append(kmsKeyNameRefs, &kmsv1beta1.KMSKeyRef_OneOf{External: kmsKeyName})
		}
	}
