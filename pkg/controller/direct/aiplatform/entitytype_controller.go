...
func init() {
	registry.RegisterModel(krm.AiplatformEntityTypeGVK, NewEntityTypeModel)
}
...
type EntityTypeAdapter struct {
	...
	desired   *krm.AiplatformEntityType
	actual    *aiplatformpb.EntityType
}
...
func (a *EntityTypeAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &aiplatformpb.GetEntityTypeRequest{Name: a.id}
...
func (a *EntityTypeAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AiplatformEntityTypeSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.AiplatformEntityTypeStatus{}
	status.ObservedState = AiplatformEntityTypeObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *EntityTypeAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AiplatformEntityTypeSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AiplatformEntityTypeStatus{}
		status.ObservedState = AiplatformEntityTypeObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AiplatformEntityTypeStatus{}
	status.ObservedState = AiplatformEntityTypeObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *EntityTypeAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AiplatformEntityType{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AiplatformEntityTypeSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AiplatformEntityTypeGVK)
...
func (a *EntityTypeAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &aiplatformpb.DeleteEntityTypeRequest{Name: a.id.String()}
