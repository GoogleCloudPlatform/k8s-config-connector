...
func (a *ExtensionAdapter) Find(ctx context.Context) (bool, error) {
	...
	mapCtx := &direct.MapContext{}
	observedState := VertexAIExtensionObservedState_FromProto(mapCtx, extensionpb)
	if mapCtx.Err() != nil {
		return false, fmt.Errorf("mapping from proto to observed state: %w", mapCtx.Err())
	}
	a.desired.Status.ObservedState = observedState
	a.desired.Status.ExternalRef = direct.LazyPtr(extensionpb.Name)
	return true, nil
}
...
func (a *ExtensionAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	if err != nil {
		return fmt.Errorf("importing VertexAIExtension %q: %w", a.id.String(), err)
	}

	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("waiting for creation of VertexAIExtension %q: %w", a.id.String(), err)
	}
