...
func init() {
	registry.RegisterModel(krm.AiplatformDatasetGVK, NewDatasetModel)
}
...
type DatasetAdapter struct {
	...
	desired   *krm.AiplatformDataset
	actual    *aiplatformpb.Dataset
}
...
func (a *DatasetAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &aiplatformpb.GetDatasetRequest{Name: a.id}
...
func (a *DatasetAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AiplatformDatasetSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	status := &krm.AiplatformDatasetStatus{}
	status.ObservedState = AiplatformDatasetObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *DatasetAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	desiredPb := AiplatformDatasetSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AiplatformDatasetStatus{}
		status.ObservedState = AiplatformDatasetObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	status := &krm.AiplatformDatasetStatus{}
	status.ObservedState = AiplatformDatasetObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *DatasetAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AiplatformDataset{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AiplatformDatasetSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AiplatformDatasetGVK)
...
func (a *DatasetAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &aiplatformpb.DeleteDatasetRequest{Name: a.id.String()}
