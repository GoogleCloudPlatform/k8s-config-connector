...
func init() {
	registry.RegisterModel(krm.AiplatformCustomJobGVK, NewCustomJobModel)
}
...
func (m *modelCustomJob) client(ctx context.Context) (*gcp.Client, error) {
...
func (m *modelCustomJob) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.AiplatformCustomJob{}
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &obj); err != nil {
		return nil, fmt.Errorf("error converting to %T: %w", obj, err)
	}

	id, err := krm.NewCustomJobIdentity(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
...
type CustomJobAdapter struct {
	id        *krm.CustomJobIdentity
	gcpClient *gcp.Client
	desired   *krm.AiplatformCustomJob
	actual    *aiplatformpb.CustomJob
}
...
func (a *CustomJobAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &aiplatformpb.GetCustomJobRequest{Name: a.id}
...
func (a *CustomJobAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AiplatformCustomJobSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	op, err := a.gcpClient.CreateCustomJob(ctx, req)
	if err != nil {
		return fmt.Errorf("creating CustomJob %s: %w", a.id, err)
	}
	created, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("CustomJob %s waiting creation: %w", a.id, err)
	}
	log.V(2).Info("successfully created CustomJob", "name", a.id)

	status := &krm.AiplatformCustomJobStatus{}
	status.ObservedState = AiplatformCustomJobObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *CustomJobAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating CustomJob", "name", a.id)
	mapCtx := &direct.MapContext{}

	desiredPb := AiplatformCustomJobSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AiplatformCustomJobStatus{}
		status.ObservedState = AiplatformCustomJobObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &aiplatformpb.UpdateCustomJobRequest{
		Name:       a.id.External,
		UpdateMask: updateMask,
		CustomJob:  desiredPb,
	}
	op, err := a.gcpClient.UpdateCustomJob(ctx, req)
	if err != nil {
		return fmt.Errorf("updating CustomJob %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("CustomJob %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated CustomJob", "name", a.id.External)

	status := &krm.AiplatformCustomJobStatus{}
	status.ObservedState = AiplatformCustomJobObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *CustomJobAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AiplatformCustomJob{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AiplatformCustomJobSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AiplatformCustomJobGVK)
...
func (a *CustomJobAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &aiplatformpb.DeleteCustomJobRequest{Name: a.id.String()}
