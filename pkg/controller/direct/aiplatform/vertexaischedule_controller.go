...
func (m *scheduleModel) client(ctx context.Context, location string) (*gcp.ScheduleClient, error) {
...
func (m *scheduleModel) AdapterForObject(ctx context.Context, reader *directbase.AdapterForObjectOperation) (directbase.Adapter, error) {
	...
	if desiredpb.GetCreateNotebookExecutionJobRequest() != nil {
		desiredpb.GetCreateNotebookExecutionJobRequest().Parent = parent
	}
...
func (a *ScheduleAdapter) Find(ctx context.Context) (bool, error) {
	...
	if a.desired != nil {
		a.desired.Status.ExternalRef = direct.LazyPtr(schedulepb.GetName())
	}
...
func compareSchedule(ctx context.Context, actual, desired *pb.Schedule) (*structuredreporting.Diff, *fieldmaskpb.FieldMask, error) {
	...
	populateDefaults := func(obj *pb.Schedule) {
		if obj.MaxConcurrentRunCount == 0 {
			obj.MaxConcurrentRunCount = 1
		}
	}
	populateDefaults(maskedActual)
	populateDefaults(clonedDesired)

	// Immutable fields should be ignored for diffing/updating
	maskedActual.Request = nil
	clonedDesired.Request = nil
	maskedActual.MaxRunCount = 0
	clonedDesired.MaxRunCount = 0

	// Normalize optional StartTime and EndTime if not specified in spec
	if clonedDesired.StartTime == nil {
		maskedActual.StartTime = nil
	}
	if clonedDesired.EndTime == nil {
		maskedActual.EndTime = nil
	}
