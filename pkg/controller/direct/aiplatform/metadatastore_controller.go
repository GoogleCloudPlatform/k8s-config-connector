...
func init() {
	registry.RegisterModel(krm.AiplatformMetadataStoreGVK, NewMetadataStoreModel)
}
...
func (m *modelMetadataStore) client(ctx context.Context) (*gcp.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building MetadataStore client: %w", err)
	}
...
func (m *modelMetadataStore) AdapterForObject(ctx context.Context, reader client.Reader, u *unstructured.Unstructured) (directbase.Adapter, error) {
	obj := &krm.AiplatformMetadataStore{}
...
type MetadataStoreAdapter struct {
	...
	desired   *krm.AiplatformMetadataStore
	actual    *aiplatformpb.MetadataStore
}
...
func (a *MetadataStoreAdapter) Find(ctx context.Context) (bool, error) {
	...
	req := &aiplatformpb.GetMetadataStoreRequest{Name: a.id}
...
func (a *MetadataStoreAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	resource := AiplatformMetadataStoreSpec_ToProto(mapCtx, &desired.Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if err != nil {
		return fmt.Errorf("creating MetadataStore %s: %w", a.id, err)
	}
	...
	status := &krm.AiplatformMetadataStoreStatus{}
	status.ObservedState = AiplatformMetadataStoreObservedState_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *MetadataStoreAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating MetadataStore", "name", a.id)
	mapCtx := &direct.MapContext{}

	desiredPb := AiplatformMetadataStoreSpec_ToProto(mapCtx, &a.desired.DeepCopy().Spec)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	...
	if len(paths) == 0 {
		...
		status := &krm.AiplatformMetadataStoreStatus{}
		status.ObservedState = AiplatformMetadataStoreObservedState_FromProto(mapCtx, a.actual)
		if mapCtx.Err() != nil {
			return mapCtx.Err()
		}
	...
	req := &aiplatformpb.UpdateMetadataStoreRequest{
		Name:          a.id.External,
		UpdateMask:    updateMask,
		MetadataStore: desiredPb,
	}
	op, err := a.gcpClient.UpdateMetadataStore(ctx, req)
	if err != nil {
		return fmt.Errorf("updating MetadataStore %s: %w", a.id.External, err)
	}
	updated, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("MetadataStore %s waiting update: %w", a.id.External, err)
	}
	log.V(2).Info("successfully updated MetadataStore", "name", a.id.External)

	status := &krm.AiplatformMetadataStoreStatus{}
	status.ObservedState = AiplatformMetadataStoreObservedState_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	return updateOp.UpdateStatus(ctx, status, nil)
...
func (a *MetadataStoreAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	obj := &krm.AiplatformMetadataStore{}
	mapCtx := &direct.MapContext{}
	obj.Spec = direct.ValueOf(AiplatformMetadataStoreSpec_FromProto(mapCtx, a.actual))
	if mapCtx.Err() != nil {
	...
	u.SetGroupVersionKind(krm.AiplatformMetadataStoreGVK)
...
func (a *MetadataStoreAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	...
	req := &aiplatformpb.DeleteMetadataStoreRequest{Name: a.id.String()}
