...
# Generate the KCC type structs from the Google Cloud proto definitions
...
# Generate the mapper functions that convert between the KCC structs and the Google Cloud proto structs
...
*   `types.generated.go`: Contains the Go structs generated directly from the Google Cloud protos. **DO NOT EDIT THIS FILE.**
...
To ensure the mapping between the KCC API object and the Google Cloud proto is correct, you must add a fuzzer.
...
3.  The fuzzer needs to know which fields to ignore during the round-trip comparison. Use `f.SpecFields.Insert()` and `f.StatusFields.Insert()` to specify fields that exist in the KCC object but not the Google Cloud proto (like annotations). Use `f.UnimplementedFields.Insert()` for fields that are not yet mapped.
