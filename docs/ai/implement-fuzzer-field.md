...
1. Next, find a test and add test coverage for the missing field to one of the test cases.  Ideally the test case will be `pkg/test/resourcefixture/testdata/basic/<service>/<version>/<kind>/full<kind>`, but we are not consistent.

1. In the fuzzer, replace the call that marks the method as unimplemented with either a call to `f.Spec("<fieldName>")` or `f.Status("<fieldName>")`.  The fuzzer should be `pkg/controller/direct/<service>/<kind>_fuzzer.go`
