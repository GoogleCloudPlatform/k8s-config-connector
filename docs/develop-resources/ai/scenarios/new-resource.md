...
### 1. Generate Initial API, CRD, and Mapper

This is the first step to create the basic building blocks from the GCP API definition.

*   **Gemini Prompt:**
    ```bash
    "Add generate.sh for apis/<service>/v1alpha1, similar to the example in apis/bigquerybiglake/v1alpha1/generate.sh, then run the new generate.sh."
    ```
*   **Human Verification:**
...
*   **Manual Developer Tasks:**
    ...
    2. Remove the `name` field if it was included in the `Spec` or `Status`; Ensure `resourceID` field in `Spec` and `externalRef` field in `Status`.
    3. Add the fields that represent the resource's parent (e.g., `ProjectRef`, `FolderRef`). Our framework uses interfaces to enforce parent structure, but this is not yet automatically verified.
    4. Replace raw `string` fields that represent other GCP resources with a proper KCC reference struct (e.g., `parent.ProjectRef`), even if the corresponding KCC controller isn't implemented yet.
