...
func main() {
	...
	flag.StringVar(&syncerIntegration, "syncer-integration", "disabled", "Enable integration with the KRMSyncer for suspending sync operations. One of: disabled, pull. Must be used with multi-cluster leader election.")
	...
	var enableSyncer bool
	switch syncerIntegration {
	case "disabled":
		enableSyncer = false
	case "pull":
		enableSyncer = true
	default:
		logging.Fatal(fmt.Errorf("invalid syncer-integration: %v", syncerIntegration), "error parsing flags")
	}

	if enableSyncer && !multiClusterElection {
	...
	mgr, err := newManager(ctx, restCfg, scopedNamespace, userProjectOverride, billingProject, multiClusterElection, enableSyncer)
	if err != nil {
