...
func TestUnmanagedDetector(t *testing.T) {
	...
	unmanagedDetector.ManagerOptions = manager.Options{}

	unmanagedDetectorErr := make(chan error, 1)
	go func() {
		err := unmanagedDetector.Run(ctx)
		unmanagedDetectorErr <- err
	}()
	...
	// Wait for the unmanaged detector to log an event
	for {
		time.Sleep(1 * time.Second)
		latest, err := dynamic.Resource(gvr).Namespace(u.GetNamespace()).Get(ctx, u.GetName(), metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get test bucket after creation: %v", err)
		}

		b, err := latest.MarshalJSON()
		if err != nil {
			t.Fatalf("Failed to marshal latest StorageBucket resource: %v", err)
		}
		s := string(b)
		t.Logf("Latest StorageBucket resource: %s", s)
		if strings.Contains(s, "No controller is managing this resource") {
			t.Logf("Detected unmanaged resource event")
			break
		}
	}
	...
	if err := <-unmanagedDetectorErr; err != nil {
		if !errors.Is(err, context.Canceled) {
			t.Errorf("UnmanagedDetector failed: %v", err)
		}
	}
