...
func ClusterToUnstructured(r *dclService.Cluster) *unstructured.Resource {
	...
	if r.ControlPlane != nil && r.ControlPlane != dclService.EmptyClusterControlPlane {
		...
		if r.ControlPlane.InstancePlacement != nil && r.ControlPlane.InstancePlacement != dclService.EmptyClusterControlPlaneInstancePlacement {
			rControlPlaneInstancePlacement := make(map[string]interface{})
			if r.ControlPlane.InstancePlacement.Tenancy != nil {
				rControlPlaneInstancePlacement["tenancy"] = string(*r.ControlPlane.InstancePlacement.Tenancy)
			}
			rControlPlane["instancePlacement"] = rControlPlaneInstancePlacement
		}
...
func UnstructuredToCluster(u *unstructured.Resource) (*dclService.Cluster, error) {
	...
	if _, ok := u.Object["controlPlane"]; ok {
		if rControlPlane, ok := u.Object["controlPlane"].(map[string]interface{}); ok {
			...
			if _, ok := rControlPlane["instancePlacement"]; ok {
				if rControlPlaneInstancePlacement, ok := rControlPlane["instancePlacement"].(map[string]interface{}); ok {
					r.ControlPlane.InstancePlacement = &dclService.ClusterControlPlaneInstancePlacement{}
					if _, ok := rControlPlaneInstancePlacement["tenancy"]; ok {
						if s, ok := rControlPlaneInstancePlacement["tenancy"].(string); ok {
							r.ControlPlane.InstancePlacement.Tenancy = dclService.ClusterControlPlaneInstancePlacementTenancyEnumRef(s)
						} else {
							return nil, fmt.Errorf("r.ControlPlane.InstancePlacement.Tenancy: expected string")
						}
					}
				} else {
					return nil, fmt.Errorf("r.ControlPlane.InstancePlacement: expected map[string]interface{}")
				}
			}
