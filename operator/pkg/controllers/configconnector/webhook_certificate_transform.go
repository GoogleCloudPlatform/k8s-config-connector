...
func (r *Reconciler) injectWebhookCertificates(kubeClient client.Client) declarative.ObjectTransform {
	return func(ctx context.Context, o declarative.DeclarativeObject, m *manifest.Objects) error {
		...
		serviceIDs := sets.New[types.NamespacedName]()
		for _, obj := range m.Items {
			...
			if len(tokens) != 2 {
				log.Info("ignoring invalid CA annotation", "annotation", InjectCAAnnotation, "value", s)
				continue
			}
			serviceIDs.Insert(types.NamespacedName{Namespace: tokens[1], Name: tokens[0]})
		}
		...
		for serviceID := range serviceIDs {
			...
			for _, obj := range m.Items {
				...
				for k := range annotations {
					if _, found := oldAnnotations[k]; found {
