...
func TestConfigConnectorE2E(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		...
		if err := c.Get(ctx, nn, newCC); err != nil {
			t.Logf("error getting ConfigConnector: %v", err)
		} else {
			s := newCC.GetCommonStatus()
			healthy = s.Healthy
			errors = s.Errors
			ok = true
			if s.ObservedGeneration == newCC.Generation && s.Healthy {
				break
			}
		}
	...
	if !ok {
		t.Fatalf("failed to get ConfigConnector status")
	}
