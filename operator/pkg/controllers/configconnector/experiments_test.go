				...
				if IsControllerManagerStatefulSet(item) {
					...
					err := item.MutateContainers(func(container map[string]interface{}) error {
						...
						for _, arg := range args {
							...
							if arg == "--syncer-integration=pull" {
								foundSyncerArg = true
							}
						}
						return nil
					...
					if tc.expectEnabled {
						...
						if !foundSyncerArg {
							t.Errorf("expected --syncer-integration=pull arg, but not found")
						}
					}
					if !tc.expectEnabled {
						if foundLeaderElectionArg {
						...
						if foundSyncerArg {
							t.Errorf("did not expect --syncer-integration=pull arg, but found")
						}
					}
