//go:build !ignore_autogenerated

// Code generated by controller-gen. DO NOT EDIT.

package v1beta1

import (
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/k8s"
	runtime "k8s.io/apimachinery/pkg/runtime"
)

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *CCExperiments) DeepCopyInto(out *CCExperiments) {
	*out = *in
	if in.MultiClusterLease != nil {
		in, out := &in.MultiClusterLease, &out.MultiClusterLease
		*out = new(MultiClusterLeaseSpec)
		**out = **in
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new CCExperiments.
func (in *CCExperiments) DeepCopy() *CCExperiments {
	if in == nil {
		return nil
	}
	out := new(CCExperiments)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnector) DeepCopyInto(out *ConfigConnector) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ObjectMeta.DeepCopyInto(&out.ObjectMeta)
	in.Spec.DeepCopyInto(&out.Spec)
	in.Status.DeepCopyInto(&out.Status)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnector.
func (in *ConfigConnector) DeepCopy() *ConfigConnector {
	if in == nil {
		return nil
	}
	out := new(ConfigConnector)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *ConfigConnector) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorContext) DeepCopyInto(out *ConfigConnectorContext) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ObjectMeta.DeepCopyInto(&out.ObjectMeta)
	in.Spec.DeepCopyInto(&out.Spec)
	in.Status.DeepCopyInto(&out.Status)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorContext.
func (in *ConfigConnectorContext) DeepCopy() *ConfigConnectorContext {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorContext)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *ConfigConnectorContext) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorContextList) DeepCopyInto(out *ConfigConnectorContextList) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ListMeta.DeepCopyInto(&out.ListMeta)
	if in.Items != nil {
		in, out := &in.Items, &out.Items
		*out = make([]ConfigConnectorContext, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorContextList.
func (in *ConfigConnectorContextList) DeepCopy() *ConfigConnectorContextList {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorContextList)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *ConfigConnectorContextList) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorContextSpec) DeepCopyInto(out *ConfigConnectorContextSpec) {
	*out = *in
	if in.StateIntoSpec != nil {
		in, out := &in.StateIntoSpec, &out.StateIntoSpec
		*out = new(StateIntoSpecValue)
		**out = **in
	}
	if in.Experiments != nil {
		in, out := &in.Experiments, &out.Experiments
		*out = new(Experiments)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorContextSpec.
func (in *ConfigConnectorContextSpec) DeepCopy() *ConfigConnectorContextSpec {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorContextSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorContextStatus) DeepCopyInto(out *ConfigConnectorContextStatus) {
	*out = *in
	in.CommonStatus.DeepCopyInto(&out.CommonStatus)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorContextStatus.
func (in *ConfigConnectorContextStatus) DeepCopy() *ConfigConnectorContextStatus {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorContextStatus)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorList) DeepCopyInto(out *ConfigConnectorList) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ListMeta.DeepCopyInto(&out.ListMeta)
	if in.Items != nil {
		in, out := &in.Items, &out.Items
		*out = make([]ConfigConnector, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorList.
func (in *ConfigConnectorList) DeepCopy() *ConfigConnectorList {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorList)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *ConfigConnectorList) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorSpec) DeepCopyInto(out *ConfigConnectorSpec) {
	*out = *in
	out.CommonSpec = in.CommonSpec
	if in.StateIntoSpec != nil {
		in, out := &in.StateIntoSpec, &out.StateIntoSpec
		*out = new(StateIntoSpecValue)
		**out = **in
	}
	if in.Experiments != nil {
		in, out := &in.Experiments, &out.Experiments
		*out = new(CCExperiments)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorSpec.
func (in *ConfigConnectorSpec) DeepCopy() *ConfigConnectorSpec {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConfigConnectorStatus) DeepCopyInto(out *ConfigConnectorStatus) {
	*out = *in
	in.CommonStatus.DeepCopyInto(&out.CommonStatus)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConfigConnectorStatus.
func (in *ConfigConnectorStatus) DeepCopy() *ConfigConnectorStatus {
	if in == nil {
		return nil
	}
	out := new(ConfigConnectorStatus)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *Experiments) DeepCopyInto(out *Experiments) {
	*out = *in
	if in.ControllerOverrides != nil {
		in, out := &in.ControllerOverrides, &out.ControllerOverrides
		*out = make(map[string]k8s.ReconcilerType, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.ReconcileControls != nil {
		in, out := &in.ReconcileControls, &out.ReconcileControls
		*out = make(map[string]ReconcileControl, len(*in))
		for key, val := range *in {
			(*out)[key] = *val.DeepCopy()
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new Experiments.
func (in *Experiments) DeepCopy() *Experiments {
	if in == nil {
		return nil
	}
	out := new(Experiments)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *MultiClusterLeaseSpec) DeepCopyInto(out *MultiClusterLeaseSpec) {
	*out = *in
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new MultiClusterLeaseSpec.
func (in *MultiClusterLeaseSpec) DeepCopy() *MultiClusterLeaseSpec {
	if in == nil {
		return nil
	}
	out := new(MultiClusterLeaseSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ReconcileControl) DeepCopyInto(out *ReconcileControl) {
	*out = *in
	if in.MaxConcurrentReconciles != nil {
		in, out := &in.MaxConcurrentReconciles, &out.MaxConcurrentReconciles
		*out = new(int)
		**out = **in
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ReconcileControl.
func (in *ReconcileControl) DeepCopy() *ReconcileControl {
	if in == nil {
		return nil
	}
	out := new(ReconcileControl)
	in.DeepCopyInto(out)
	return out
}
