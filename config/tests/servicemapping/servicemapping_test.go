...
func TestImmutableOptionalFieldsWithDefault(t *testing.T) {
	t.Parallel()
	serviceMappings := testservicemappingloader.New(t).GetServiceMappings()
	provider := tfprovider.NewOrLogFatal(tfprovider.UnitTestConfig())
	for _, sm := range serviceMappings {
		sm := sm
		if sm.Spec.Version == "v1alpha1" {
			continue
		}
		t.Run(sm.Name, func(t *testing.T) {
			t.Parallel()
			for _, rc := range sm.Spec.Resources {
				...
				tfResource := provider.ResourcesMap[rc.Name]
				t.Logf("===%v================", rc.Name)
				goThroughTFSchema(t, "", tfResource.Schema, rc.IDTemplate)
			}
		})
	}
}

func goThroughTFSchema(t *testing.T, parentPath string, schemas map[string]*schema.Schema, idTemplate string) {
	for f, s := range schemas {
		path := f
		if parentPath != "" {
			path = fmt.Sprintf("%v.%v", parentPath, f)
		}
		if s.ForceNew && s.Optional && s.Default != nil {
			if !strings.Contains(idTemplate, fmt.Sprintf("{{%v}}", path)) {
				t.Logf("------immutable and optional field %q has default value %v", path, s.Default)
			}
		}
		switch s.Type {
		case schema.TypeList, schema.TypeSet:
			...
			case *schema.Resource:
				goThroughTFSchema(t, path, elem.Schema, idTemplate)
			}
		}
	}
	return
