...
class Attachment(object):
    ...
    def apply(self):
        stub = attachment_pb2_grpc.ApigeeAlphaAttachmentServiceStub(channel.Channel())
        request = attachment_pb2.ApplyApigeeAlphaAttachmentRequest()
        ...
        response = stub.ApplyApigeeAlphaAttachment(request)
    ...
    def delete(self):
        stub = attachment_pb2_grpc.ApigeeAlphaAttachmentServiceStub(channel.Channel())
        request = attachment_pb2.DeleteApigeeAlphaAttachmentRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteApigeeAlphaAttachment(request)

    @classmethod
    def list(self, envgroup, service_account_file=""):
        stub = attachment_pb2_grpc.ApigeeAlphaAttachmentServiceStub(channel.Channel())
        request = attachment_pb2.ListApigeeAlphaAttachmentRequest()
        request.service_account_file = service_account_file
        request.Envgroup = envgroup

        return stub.ListApigeeAlphaAttachment(request).items

    def to_proto(self):
        resource = attachment_pb2.ApigeeAlphaAttachment()
        if Primitive.to_proto(self.name):
