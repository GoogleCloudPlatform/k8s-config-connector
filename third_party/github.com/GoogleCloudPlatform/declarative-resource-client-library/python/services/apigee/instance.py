...
class Instance(object):
    ...
    def apply(self):
        stub = instance_pb2_grpc.ApigeeInstanceServiceStub(channel.Channel())
        request = instance_pb2.ApplyApigeeInstanceRequest()
        ...
        response = stub.ApplyApigeeInstance(request)
    ...
    def delete(self):
        stub = instance_pb2_grpc.ApigeeInstanceServiceStub(channel.Channel())
        request = instance_pb2.DeleteApigeeInstanceRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteApigeeInstance(request)

    @classmethod
    def list(self, apigeeOrganization, service_account_file=""):
        stub = instance_pb2_grpc.ApigeeInstanceServiceStub(channel.Channel())
        request = instance_pb2.ListApigeeInstanceRequest()
        request.service_account_file = service_account_file
        request.ApigeeOrganization = apigeeOrganization

        return stub.ListApigeeInstance(request).items

    def to_proto(self):
        resource = instance_pb2.ApigeeInstance()
        if Primitive.to_proto(self.name):
...
class InstancePeeringCidrRangeEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return instance_pb2.ApigeeInstancePeeringCidrRangeEnum.Value(
            "ApigeeInstancePeeringCidrRangeEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return instance_pb2.ApigeeInstancePeeringCidrRangeEnum.Name(resource)[
            len("ApigeeInstancePeeringCidrRangeEnum") :
        ]
...
class InstanceStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return instance_pb2.ApigeeInstanceStateEnum.Value(
            "ApigeeInstanceStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return instance_pb2.ApigeeInstanceStateEnum.Name(resource)[
            len("ApigeeInstanceStateEnum") :
        ]
