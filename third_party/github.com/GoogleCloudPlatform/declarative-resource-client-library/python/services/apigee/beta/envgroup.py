...
class Envgroup(object):
    ...
    def apply(self):
        stub = envgroup_pb2_grpc.ApigeeBetaEnvgroupServiceStub(channel.Channel())
        request = envgroup_pb2.ApplyApigeeBetaEnvgroupRequest()
        ...
        response = stub.ApplyApigeeBetaEnvgroup(request)
    ...
    def delete(self):
        stub = envgroup_pb2_grpc.ApigeeBetaEnvgroupServiceStub(channel.Channel())
        request = envgroup_pb2.DeleteApigeeBetaEnvgroupRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteApigeeBetaEnvgroup(request)

    @classmethod
    def list(self, apigeeOrganization, service_account_file=""):
        stub = envgroup_pb2_grpc.ApigeeBetaEnvgroupServiceStub(channel.Channel())
        request = envgroup_pb2.ListApigeeBetaEnvgroupRequest()
        request.service_account_file = service_account_file
        request.ApigeeOrganization = apigeeOrganization

        return stub.ListApigeeBetaEnvgroup(request).items

    def to_proto(self):
        resource = envgroup_pb2.ApigeeBetaEnvgroup()
        if Primitive.to_proto(self.name):
...
class EnvgroupStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return envgroup_pb2.ApigeeBetaEnvgroupStateEnum.Value(
            "ApigeeBetaEnvgroupStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return envgroup_pb2.ApigeeBetaEnvgroupStateEnum.Name(resource)[
            len("ApigeeBetaEnvgroupStateEnum") :
        ]
