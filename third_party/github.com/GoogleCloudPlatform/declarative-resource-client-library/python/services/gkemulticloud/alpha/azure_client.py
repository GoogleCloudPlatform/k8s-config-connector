...
class AzureClient(object):
    ...
    def apply(self):
        stub = azure_client_pb2_grpc.GkemulticloudAlphaAzureClientServiceStub(
            channel.Channel()
        )
        request = azure_client_pb2.ApplyGkemulticloudAlphaAzureClientRequest()
        ...
        response = stub.ApplyGkemulticloudAlphaAzureClient(request)
    ...
    def delete(self):
        ...
        request = azure_client_pb2.DeleteGkemulticloudAlphaAzureClientRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteGkemulticloudAlphaAzureClient(request)

    def list(self):
        stub = azure_client_pb2_grpc.GkemulticloudAlphaAzureClientServiceStub(
            channel.Channel()
        )
        request = azure_client_pb2.ListGkemulticloudAlphaAzureClientRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        return stub.ListGkemulticloudAlphaAzureClient(request).items

    def to_proto(self):
        resource = azure_client_pb2.GkemulticloudAlphaAzureClient()
        if Primitive.to_proto(self.name):
