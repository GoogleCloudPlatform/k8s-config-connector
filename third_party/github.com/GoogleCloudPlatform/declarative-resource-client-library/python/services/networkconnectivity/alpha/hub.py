...
class Hub(object):
    ...
    def apply(self):
        ...
        request = hub_pb2.ApplyNetworkconnectivityAlphaHubRequest()
        ...
        response = stub.ApplyNetworkconnectivityAlphaHub(request)
    ...
    def delete(self):
        stub = hub_pb2_grpc.NetworkconnectivityAlphaHubServiceStub(channel.Channel())
        request = hub_pb2.DeleteNetworkconnectivityAlphaHubRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
    ...
    def to_proto(self):
        resource = hub_pb2.NetworkconnectivityAlphaHub()
        if Primitive.to_proto(self.name):
...
class HubStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return hub_pb2.NetworkconnectivityAlphaHubStateEnum.Value(
            "NetworkconnectivityAlphaHubStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return hub_pb2.NetworkconnectivityAlphaHubStateEnum.Name(resource)[
            len("NetworkconnectivityAlphaHubStateEnum") :
        ]
