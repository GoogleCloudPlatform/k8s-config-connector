...
class WebApp(object):
    ...
    def apply(self):
        stub = web_app_pb2_grpc.FirebaseAlphaWebAppServiceStub(channel.Channel())
        request = web_app_pb2.ApplyFirebaseAlphaWebAppRequest()
        ...
        response = stub.ApplyFirebaseAlphaWebApp(request)
    ...
    def delete(self):
        stub = web_app_pb2_grpc.FirebaseAlphaWebAppServiceStub(channel.Channel())
        request = web_app_pb2.DeleteFirebaseAlphaWebAppRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteFirebaseAlphaWebApp(request)

    @classmethod
    def list(self, project, service_account_file=""):
        stub = web_app_pb2_grpc.FirebaseAlphaWebAppServiceStub(channel.Channel())
        request = web_app_pb2.ListFirebaseAlphaWebAppRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListFirebaseAlphaWebApp(request).items

    def to_proto(self):
        resource = web_app_pb2.FirebaseAlphaWebApp()
        if Primitive.to_proto(self.name):
