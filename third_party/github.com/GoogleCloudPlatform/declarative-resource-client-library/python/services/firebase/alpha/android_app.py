...
class AndroidApp(object):
    ...
    def apply(self):
        stub = android_app_pb2_grpc.FirebaseAlphaAndroidAppServiceStub(
            channel.Channel()
        )
        request = android_app_pb2.ApplyFirebaseAlphaAndroidAppRequest()
        ...
        response = stub.ApplyFirebaseAlphaAndroidApp(request)
    ...
    def delete(self):
        ...
        request = android_app_pb2.DeleteFirebaseAlphaAndroidAppRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteFirebaseAlphaAndroidApp(request)
    ...
    def list(self, project, service_account_file=""):
        ...
        request = android_app_pb2.ListFirebaseAlphaAndroidAppRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListFirebaseAlphaAndroidApp(request).items

    def to_proto(self):
        resource = android_app_pb2.FirebaseAlphaAndroidApp()
        if Primitive.to_proto(self.name):
