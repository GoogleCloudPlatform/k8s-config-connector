...
class AppleApp(object):
    ...
    def apply(self):
        stub = apple_app_pb2_grpc.FirebaseAlphaAppleAppServiceStub(channel.Channel())
        request = apple_app_pb2.ApplyFirebaseAlphaAppleAppRequest()
        ...
        response = stub.ApplyFirebaseAlphaAppleApp(request)
    ...
    def delete(self):
        stub = apple_app_pb2_grpc.FirebaseAlphaAppleAppServiceStub(channel.Channel())
        request = apple_app_pb2.DeleteFirebaseAlphaAppleAppRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteFirebaseAlphaAppleApp(request)

    @classmethod
    def list(self, project, service_account_file=""):
        stub = apple_app_pb2_grpc.FirebaseAlphaAppleAppServiceStub(channel.Channel())
        request = apple_app_pb2.ListFirebaseAlphaAppleAppRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListFirebaseAlphaAppleApp(request).items

    def to_proto(self):
        resource = apple_app_pb2.FirebaseAlphaAppleApp()
        if Primitive.to_proto(self.name):
