...
class MetricsScope(object):
    ...
    def apply(self):
        ...
        request = metrics_scope_pb2.ApplyMonitoringMetricsScopeRequest()
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)

        request.service_account_file = self.service_account_file

        response = stub.ApplyMonitoringMetricsScope(request)
    ...
    def delete(self):
        ...
        request = metrics_scope_pb2.DeleteMonitoringMetricsScopeRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
    ...
    def to_proto(self):
        resource = metrics_scope_pb2.MonitoringMetricsScope()
        if Primitive.to_proto(self.name):
...
class MetricsScopeMonitoredProjects(object):
    ...
    def to_proto(self, resource):
        ...
        res = metrics_scope_pb2.MonitoringMetricsScopeMonitoredProjects()
