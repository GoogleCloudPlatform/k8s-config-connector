...
class MetricsScope(object):
    ...
    def delete(self):
        ...
        request = metrics_scope_pb2.DeleteMonitoringBetaMetricsScopeRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)

        response = stub.DeleteMonitoringBetaMetricsScope(request)
    ...
    def list(self, service_account_file=""):
        ...
        request = metrics_scope_pb2.ListMonitoringBetaMetricsScopeRequest()
        request.service_account_file = service_account_file
        return stub.ListMonitoringBetaMetricsScope(request).items

    def to_proto(self):
        resource = metrics_scope_pb2.MonitoringBetaMetricsScope()
        if Primitive.to_proto(self.name):
...
class MetricsScopeMonitoredProjects(object):
    ...
    def to_proto(self, resource):
        ...
        res = metrics_scope_pb2.MonitoringBetaMetricsScopeMonitoredProjects()
