...
class TagValue(object):
    ...
    def apply(self):
        stub = tag_value_pb2_grpc.CloudresourcemanagerTagValueServiceStub(
            channel.Channel()
        )
        request = tag_value_pb2.ApplyCloudresourcemanagerTagValueRequest()
        ...
        response = stub.ApplyCloudresourcemanagerTagValue(request)
    ...
    def delete(self):
        ...
        request = tag_value_pb2.DeleteCloudresourcemanagerTagValueRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteCloudresourcemanagerTagValue(request)
    ...
    def list(self, service_account_file=""):
        ...
        request = tag_value_pb2.ListCloudresourcemanagerTagValueRequest()
        request.service_account_file = service_account_file
        return stub.ListCloudresourcemanagerTagValue(request).items

    def to_proto(self):
        resource = tag_value_pb2.CloudresourcemanagerTagValue()
        if Primitive.to_proto(self.name):
