...
class LogBucket(object):
    ...
    def apply(self):
        ...
        request = log_bucket_pb2.ApplyLoggingAlphaLogBucketRequest()
    ...
    def delete(self):
        stub = log_bucket_pb2_grpc.LoggingAlphaLogBucketServiceStub(channel.Channel())
        request = log_bucket_pb2.DeleteLoggingAlphaLogBucketRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
    ...
    def list(self, location, parent, service_account_file=""):
        stub = log_bucket_pb2_grpc.LoggingAlphaLogBucketServiceStub(channel.Channel())
        request = log_bucket_pb2.ListLoggingAlphaLogBucketRequest()
        request.service_account_file = service_account_file
        request.Location = location

        request.Parent = parent

        return stub.ListLoggingAlphaLogBucket(request).items

    def to_proto(self):
        resource = log_bucket_pb2.LoggingAlphaLogBucket()
        if Primitive.to_proto(self.name):
...
class LogBucketLifecycleStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return log_bucket_pb2.LoggingAlphaLogBucketLifecycleStateEnum.Value(
            "LoggingAlphaLogBucketLifecycleStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return log_bucket_pb2.LoggingAlphaLogBucketLifecycleStateEnum.Name(resource)[
            len("LoggingAlphaLogBucketLifecycleStateEnum") :
        ]
