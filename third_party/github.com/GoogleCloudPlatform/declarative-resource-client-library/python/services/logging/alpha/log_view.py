...
class LogView(object):
    ...
    def apply(self):
        stub = log_view_pb2_grpc.LoggingAlphaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.ApplyLoggingAlphaLogViewRequest()
        ...
        response = stub.ApplyLoggingAlphaLogView(request)
    ...
    def delete(self):
        stub = log_view_pb2_grpc.LoggingAlphaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.DeleteLoggingAlphaLogViewRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteLoggingAlphaLogView(request)

    @classmethod
    def list(self, location, bucket, parent, service_account_file=""):
        stub = log_view_pb2_grpc.LoggingAlphaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.ListLoggingAlphaLogViewRequest()
        ...
        return stub.ListLoggingAlphaLogView(request).items

    def to_proto(self):
        resource = log_view_pb2.LoggingAlphaLogView()
        if Primitive.to_proto(self.name):
