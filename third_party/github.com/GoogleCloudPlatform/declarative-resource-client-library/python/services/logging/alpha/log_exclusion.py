...
class LogExclusion(object):
    ...
    def apply(self):
        stub = log_exclusion_pb2_grpc.LoggingAlphaLogExclusionServiceStub(
            channel.Channel()
        )
        request = log_exclusion_pb2.ApplyLoggingAlphaLogExclusionRequest()
        ...
        response = stub.ApplyLoggingAlphaLogExclusion(request)
    ...
    def delete(self):
        ...
        request = log_exclusion_pb2.DeleteLoggingAlphaLogExclusionRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteLoggingAlphaLogExclusion(request)
    ...
    def list(self, parent, service_account_file=""):
        ...
        request = log_exclusion_pb2.ListLoggingAlphaLogExclusionRequest()
        request.service_account_file = service_account_file
        request.Parent = parent

        return stub.ListLoggingAlphaLogExclusion(request).items

    def to_proto(self):
        resource = log_exclusion_pb2.LoggingAlphaLogExclusion()
        if Primitive.to_proto(self.name):
