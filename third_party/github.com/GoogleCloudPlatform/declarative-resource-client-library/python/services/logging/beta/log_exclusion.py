...
class LogExclusion(object):
    ...
    def apply(self):
        stub = log_exclusion_pb2_grpc.LoggingBetaLogExclusionServiceStub(
            channel.Channel()
        )
        request = log_exclusion_pb2.ApplyLoggingBetaLogExclusionRequest()
        ...
        response = stub.ApplyLoggingBetaLogExclusion(request)
    ...
    def delete(self):
        ...
        request = log_exclusion_pb2.DeleteLoggingBetaLogExclusionRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteLoggingBetaLogExclusion(request)
    ...
    def list(self, parent, service_account_file=""):
        ...
        request = log_exclusion_pb2.ListLoggingBetaLogExclusionRequest()
        request.service_account_file = service_account_file
        request.Parent = parent

        return stub.ListLoggingBetaLogExclusion(request).items

    def to_proto(self):
        resource = log_exclusion_pb2.LoggingBetaLogExclusion()
        if Primitive.to_proto(self.name):
