...
class LogView(object):
    ...
    def apply(self):
        stub = log_view_pb2_grpc.LoggingBetaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.ApplyLoggingBetaLogViewRequest()
        ...
        response = stub.ApplyLoggingBetaLogView(request)
    ...
    def delete(self):
        stub = log_view_pb2_grpc.LoggingBetaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.DeleteLoggingBetaLogViewRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteLoggingBetaLogView(request)

    @classmethod
    def list(self, location, bucket, parent, service_account_file=""):
        stub = log_view_pb2_grpc.LoggingBetaLogViewServiceStub(channel.Channel())
        request = log_view_pb2.ListLoggingBetaLogViewRequest()
        ...
        return stub.ListLoggingBetaLogView(request).items

    def to_proto(self):
        resource = log_view_pb2.LoggingBetaLogView()
        if Primitive.to_proto(self.name):
