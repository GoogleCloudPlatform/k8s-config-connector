...
class Backup(object):
    ...
    def apply(self):
        stub = backup_pb2_grpc.FilestoreBetaBackupServiceStub(channel.Channel())
        request = backup_pb2.ApplyFilestoreBetaBackupRequest()
        ...
        response = stub.ApplyFilestoreBetaBackup(request)
    ...
    def delete(self):
        stub = backup_pb2_grpc.FilestoreBetaBackupServiceStub(channel.Channel())
        request = backup_pb2.DeleteFilestoreBetaBackupRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteFilestoreBetaBackup(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = backup_pb2_grpc.FilestoreBetaBackupServiceStub(channel.Channel())
        request = backup_pb2.ListFilestoreBetaBackupRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListFilestoreBetaBackup(request).items

    def to_proto(self):
        resource = backup_pb2.FilestoreBetaBackup()
        if Primitive.to_proto(self.name):
...
class BackupStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return backup_pb2.FilestoreBetaBackupStateEnum.Value(
            "FilestoreBetaBackupStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return backup_pb2.FilestoreBetaBackupStateEnum.Name(resource)[
            len("FilestoreBetaBackupStateEnum") :
        ]
...
class BackupSourceInstanceTierEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return backup_pb2.FilestoreBetaBackupSourceInstanceTierEnum.Value(
            "FilestoreBetaBackupSourceInstanceTierEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return backup_pb2.FilestoreBetaBackupSourceInstanceTierEnum.Name(resource)[
            len("FilestoreBetaBackupSourceInstanceTierEnum") :
        ]
