...
class Mesh(object):
    ...
    def apply(self):
        stub = mesh_pb2_grpc.NetworkservicesBetaMeshServiceStub(channel.Channel())
        request = mesh_pb2.ApplyNetworkservicesBetaMeshRequest()
        ...
        response = stub.ApplyNetworkservicesBetaMesh(request)
    ...
    def delete(self):
        stub = mesh_pb2_grpc.NetworkservicesBetaMeshServiceStub(channel.Channel())
        request = mesh_pb2.DeleteNetworkservicesBetaMeshRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteNetworkservicesBetaMesh(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = mesh_pb2_grpc.NetworkservicesBetaMeshServiceStub(channel.Channel())
        request = mesh_pb2.ListNetworkservicesBetaMeshRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListNetworkservicesBetaMesh(request).items

    def to_proto(self):
        resource = mesh_pb2.NetworkservicesBetaMesh()
        if Primitive.to_proto(self.name):
