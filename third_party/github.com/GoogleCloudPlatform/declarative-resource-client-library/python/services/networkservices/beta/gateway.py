...
class Gateway(object):
    ...
    def apply(self):
        stub = gateway_pb2_grpc.NetworkservicesBetaGatewayServiceStub(channel.Channel())
        request = gateway_pb2.ApplyNetworkservicesBetaGatewayRequest()
        ...
        response = stub.ApplyNetworkservicesBetaGateway(request)
    ...
    def delete(self):
        stub = gateway_pb2_grpc.NetworkservicesBetaGatewayServiceStub(channel.Channel())
        request = gateway_pb2.DeleteNetworkservicesBetaGatewayRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteNetworkservicesBetaGateway(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = gateway_pb2_grpc.NetworkservicesBetaGatewayServiceStub(channel.Channel())
        request = gateway_pb2.ListNetworkservicesBetaGatewayRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListNetworkservicesBetaGateway(request).items

    def to_proto(self):
        resource = gateway_pb2.NetworkservicesBetaGateway()
        if Primitive.to_proto(self.name):
...
class GatewayTypeEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return gateway_pb2.NetworkservicesBetaGatewayTypeEnum.Value(
            "NetworkservicesBetaGatewayTypeEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return gateway_pb2.NetworkservicesBetaGatewayTypeEnum.Name(resource)[
            len("NetworkservicesBetaGatewayTypeEnum") :
        ]
