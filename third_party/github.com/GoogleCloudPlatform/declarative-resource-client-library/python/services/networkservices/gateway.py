...
class Gateway(object):
    ...
    def apply(self):
        stub = gateway_pb2_grpc.NetworkservicesGatewayServiceStub(channel.Channel())
        request = gateway_pb2.ApplyNetworkservicesGatewayRequest()
        ...
        response = stub.ApplyNetworkservicesGateway(request)
    ...
    def delete(self):
        stub = gateway_pb2_grpc.NetworkservicesGatewayServiceStub(channel.Channel())
        request = gateway_pb2.DeleteNetworkservicesGatewayRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteNetworkservicesGateway(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = gateway_pb2_grpc.NetworkservicesGatewayServiceStub(channel.Channel())
        request = gateway_pb2.ListNetworkservicesGatewayRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListNetworkservicesGateway(request).items

    def to_proto(self):
        resource = gateway_pb2.NetworkservicesGateway()
        if Primitive.to_proto(self.name):
...
class GatewayTypeEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return gateway_pb2.NetworkservicesGatewayTypeEnum.Value(
            "NetworkservicesGatewayTypeEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return gateway_pb2.NetworkservicesGatewayTypeEnum.Name(resource)[
            len("NetworkservicesGatewayTypeEnum") :
        ]
