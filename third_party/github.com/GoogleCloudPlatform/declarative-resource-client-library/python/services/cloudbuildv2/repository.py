...
class Repository(object):
    ...
    def apply(self):
        stub = repository_pb2_grpc.Cloudbuildv2RepositoryServiceStub(channel.Channel())
        request = repository_pb2.ApplyCloudbuildv2RepositoryRequest()
        ...
        response = stub.ApplyCloudbuildv2Repository(request)
    ...
    def delete(self):
        stub = repository_pb2_grpc.Cloudbuildv2RepositoryServiceStub(channel.Channel())
        request = repository_pb2.DeleteCloudbuildv2RepositoryRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteCloudbuildv2Repository(request)

    @classmethod
    def list(self, project, location, connection, service_account_file=""):
        stub = repository_pb2_grpc.Cloudbuildv2RepositoryServiceStub(channel.Channel())
        request = repository_pb2.ListCloudbuildv2RepositoryRequest()
        ...
        return stub.ListCloudbuildv2Repository(request).items

    def to_proto(self):
        resource = repository_pb2.Cloudbuildv2Repository()
        if Primitive.to_proto(self.name):
