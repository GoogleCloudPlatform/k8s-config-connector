...
class Client(object):
    ...
    def apply(self):
        stub = client_pb2_grpc.ContainerazureClientServiceStub(channel.Channel())
        request = client_pb2.ApplyContainerazureClientRequest()
        ...
        response = stub.ApplyContainerazureClient(request)
    ...
    def delete(self):
        stub = client_pb2_grpc.ContainerazureClientServiceStub(channel.Channel())
        request = client_pb2.DeleteContainerazureClientRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteContainerazureClient(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = client_pb2_grpc.ContainerazureClientServiceStub(channel.Channel())
        request = client_pb2.ListContainerazureClientRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListContainerazureClient(request).items

    def to_proto(self):
        resource = client_pb2.ContainerazureClient()
        if Primitive.to_proto(self.name):
