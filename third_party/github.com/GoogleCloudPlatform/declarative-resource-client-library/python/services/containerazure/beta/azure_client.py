...
class Client(object):
    ...
    def apply(self):
        stub = client_pb2_grpc.ContainerazureBetaClientServiceStub(channel.Channel())
        request = client_pb2.ApplyContainerazureBetaClientRequest()
        ...
        response = stub.ApplyContainerazureBetaClient(request)
    ...
    def delete(self):
        stub = client_pb2_grpc.ContainerazureBetaClientServiceStub(channel.Channel())
        request = client_pb2.DeleteContainerazureBetaClientRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteContainerazureBetaClient(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = client_pb2_grpc.ContainerazureBetaClientServiceStub(channel.Channel())
        request = client_pb2.ListContainerazureBetaClientRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListContainerazureBetaClient(request).items

    def to_proto(self):
        resource = client_pb2.ContainerazureBetaClient()
        if Primitive.to_proto(self.name):
