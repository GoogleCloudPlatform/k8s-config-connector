...
func canonicalizeClientTlsPolicyClientCertificate(des, initial *ClientTlsPolicyClientCertificate, opts ...dcl.ApplyOption) *ClientTlsPolicyClientCertificate {
	...
	cDes.LocalFilepath = canonicalizeClientTlsPolicyClientCertificateLocalFilepath(des.LocalFilepath, initial.LocalFilepath, opts...)
...
func canonicalizeClientTlsPolicyClientCertificateLocalFilepath(des, initial *ClientTlsPolicyClientCertificateLocalFilepath, opts ...dcl.ApplyOption) *ClientTlsPolicyClientCertificateLocalFilepath {
	...
	cDes := &ClientTlsPolicyClientCertificateLocalFilepath{}

	if dcl.StringCanonicalize(des.CertificatePath, initial.CertificatePath) || dcl.IsZeroValue(des.CertificatePath) {
		cDes.CertificatePath = initial.CertificatePath
	} else {
		cDes.CertificatePath = des.CertificatePath
	}
	if dcl.StringCanonicalize(des.PrivateKeyPath, initial.PrivateKeyPath) || dcl.IsZeroValue(des.PrivateKeyPath) {
		cDes.PrivateKeyPath = initial.PrivateKeyPath
	} else {
		cDes.PrivateKeyPath = des.PrivateKeyPath
	}

	return cDes
}

func canonicalizeClientTlsPolicyClientCertificateLocalFilepathSlice(des, initial []ClientTlsPolicyClientCertificateLocalFilepath, opts ...dcl.ApplyOption) []ClientTlsPolicyClientCertificateLocalFilepath {
	if dcl.IsEmptyValueIndirect(des) {
		return initial
	}
	...
	if len(des) != len(initial) {
		...
		items := make([]ClientTlsPolicyClientCertificateLocalFilepath, 0, len(des))
		for _, d := range des {
			cd := canonicalizeClientTlsPolicyClientCertificateLocalFilepath(&d, nil, opts...)
			if cd != nil {
				items = append(items, *cd)
			}
		}
	...
	items := make([]ClientTlsPolicyClientCertificateLocalFilepath, 0, len(des))
	for i, d := range des {
		cd := canonicalizeClientTlsPolicyClientCertificateLocalFilepath(&d, &initial[i], opts...)
		if cd != nil {
			items = append(items, *cd)
		}
	}
...
func canonicalizeNewClientTlsPolicyClientCertificateLocalFilepathSet(c *Client, des, nw []ClientTlsPolicyClientCertificateLocalFilepath) []ClientTlsPolicyClientCertificateLocalFilepath {
	...
	for _, d := range des {
		matchedIndex := -1
		for i, n := range nw {
			if diffs, _ := compareClientTlsPolicyClientCertificateLocalFilepathNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedIndex = i
				break
			}
		}
		if matchedIndex != -1 {
			items = append(items, *canonicalizeNewClientTlsPolicyClientCertificateLocalFilepath(c, &d, &nw[matchedIndex]))
			nw = append(nw[:matchedIndex], nw[matchedIndex+1:]...)
		}
	}
	// Also include elements in nw that are not matched in des.
...
func canonicalizeClientTlsPolicyServerValidationCa(des, initial *ClientTlsPolicyServerValidationCa, opts ...dcl.ApplyOption) *ClientTlsPolicyServerValidationCa {
	...
	if dcl.StringCanonicalize(des.CaCertPath, initial.CaCertPath) || dcl.IsZeroValue(des.CaCertPath) {
		cDes.CaCertPath = initial.CaCertPath
	} else {
		cDes.CaCertPath = des.CaCertPath
	}
...
func expandClientTlsPolicyClientCertificate(c *Client, f *ClientTlsPolicyClientCertificate, res *ClientTlsPolicy) (map[string]interface{}, error) {
	...
	m := make(map[string]interface{})
	if v, err := expandClientTlsPolicyClientCertificateLocalFilepath(c, f.LocalFilepath, res); err != nil {
...
func flattenClientTlsPolicyClientCertificate(c *Client, i interface{}, res *ClientTlsPolicy) *ClientTlsPolicyClientCertificate {
	...
	r.LocalFilepath = flattenClientTlsPolicyClientCertificateLocalFilepath(c, m["localFilepath"], res)
...
// request object.
func expandClientTlsPolicyClientCertificateLocalFilepathMap(c *Client, f map[string]ClientTlsPolicyClientCertificateLocalFilepath, res *ClientTlsPolicy) (map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
	...
	items := make(map[string]interface{})
	for k, item := range f {
		i, err := expandClientTlsPolicyClientCertificateLocalFilepath(c, &item, res)
		if err != nil {
			return nil, err
		}
		if i != nil {
...
// request object.
func expandClientTlsPolicyClientCertificateLocalFilepathSlice(c *Client, f []ClientTlsPolicyClientCertificateLocalFilepath, res *ClientTlsPolicy) ([]map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
...
// response object.
func flattenClientTlsPolicyClientCertificateLocalFilepathMap(c *Client, i interface{}, res *ClientTlsPolicy) map[string]ClientTlsPolicyClientCertificateLocalFilepath {
	a, ok := i.(map[string]interface{})
	if !ok {
	...
	for k, item := range a {
		items[k] = *flattenClientTlsPolicyClientCertificateLocalFilepath(c, item.(map[string]interface{}), res)
	}
...
// response object.
func flattenClientTlsPolicyClientCertificateLocalFilepathSlice(c *Client, i interface{}, res *ClientTlsPolicy) []ClientTlsPolicyClientCertificateLocalFilepath {
	a, ok := i.([]interface{})
	if !ok {
	...
	for _, item := range a {
		items = append(items, *flattenClientTlsPolicyClientCertificateLocalFilepath(c, item.(map[string]interface{}), res))
	}
...
// request object.
func expandClientTlsPolicyClientCertificateLocalFilepath(c *Client, f *ClientTlsPolicyClientCertificateLocalFilepath, res *ClientTlsPolicy) (map[string]interface{}, error) {
	if dcl.IsEmptyValueIndirect(f) {
		return nil, nil
	}
...
// response object.
func flattenClientTlsPolicyClientCertificateLocalFilepath(c *Client, i interface{}, res *ClientTlsPolicy) *ClientTlsPolicyClientCertificateLocalFilepath {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}
...
func extractClientTlsPolicyClientCertificateFields(r *ClientTlsPolicy, o *ClientTlsPolicyClientCertificate) error {
	vLocalFilepath := o.LocalFilepath
	if vLocalFilepath == nil {
		// note: explicitly not the empty object.
		vLocalFilepath = &ClientTlsPolicyClientCertificateLocalFilepath{}
	}
...
func extractClientTlsPolicyClientCertificateLocalFilepathFields(r *ClientTlsPolicy, o *ClientTlsPolicyClientCertificateLocalFilepath) error {
	return nil
}
...
func postReadExtractClientTlsPolicyClientCertificateLocalFilepathFields(r *ClientTlsPolicy, o *ClientTlsPolicyClientCertificateLocalFilepath) error {
	return nil
}
