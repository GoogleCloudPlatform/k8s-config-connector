...
func (r *ServerTlsPolicy) ID() (string, error) {
	...
	params := map[string]interface{}{
		...
		"allow_open":         dcl.ValueOrEmptyString(nr.AllowOpen),
		"server_certificate": dcl.ValueOrEmptyString(nr.ServerCertificate),
		"mtls_policy":        dcl.ValueOrEmptyString(nr.MtlsPolicy),
...
func (c *Client) GetServerTlsPolicy(ctx context.Context, r *ServerTlsPolicy) (*ServerTlsPolicy, error) {
	...
	result, err := unmarshalServerTlsPolicy(b, c, r)
	if err != nil {
		return nil, err
	}
...
func applyServerTlsPolicyDiff(c *Client, ctx context.Context, desired *ServerTlsPolicy, rawDesired *ServerTlsPolicy, ops []serverTlsPolicyApiOperation, opts ...dcl.ApplyOption) (*ServerTlsPolicy, error) {
	...
	rawNew, err := c.GetServerTlsPolicy(ctx, desired)
	if err != nil {
		return nil, err
	}
	...
	if len(ops) > 0 {
		...
		if o, ok := lastOp.(*createServerTlsPolicyOperation); ok {
			if r, hasR := o.FirstResponse(); hasR {
				...
				fullResp, err := unmarshalMapServerTlsPolicy(r, c, rawDesired)
				if err != nil {
					return nil, err
				}
