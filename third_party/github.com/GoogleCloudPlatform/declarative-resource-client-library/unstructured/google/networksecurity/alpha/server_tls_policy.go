...
func ServerTlsPolicyToUnstructured(r *dclService.ServerTlsPolicy) *unstructured.Resource {
	...
	if r.MtlsPolicy != nil && r.MtlsPolicy != dclService.EmptyServerTlsPolicyMtlsPolicy {
		...
		for _, rMtlsPolicyClientValidationCaVal := range r.MtlsPolicy.ClientValidationCa {
			...
			if rMtlsPolicyClientValidationCaVal.CaCertPath != nil {
				rMtlsPolicyClientValidationCaObject["caCertPath"] = *rMtlsPolicyClientValidationCaVal.CaCertPath
			}
	...
	if r.ServerCertificate != nil && r.ServerCertificate != dclService.EmptyServerTlsPolicyServerCertificate {
		...
		if r.ServerCertificate.LocalFilepath != nil && r.ServerCertificate.LocalFilepath != dclService.EmptyServerTlsPolicyServerCertificateLocalFilepath {
			rServerCertificateLocalFilepath := make(map[string]interface{})
			if r.ServerCertificate.LocalFilepath.CertificatePath != nil {
				rServerCertificateLocalFilepath["certificatePath"] = *r.ServerCertificate.LocalFilepath.CertificatePath
			}
			if r.ServerCertificate.LocalFilepath.PrivateKeyPath != nil {
				rServerCertificateLocalFilepath["privateKeyPath"] = *r.ServerCertificate.LocalFilepath.PrivateKeyPath
			}
			rServerCertificate["localFilepath"] = rServerCertificateLocalFilepath
		}
...
func UnstructuredToServerTlsPolicy(u *unstructured.Resource) (*dclService.ServerTlsPolicy, error) {
						...
						if objval, ok := o.(map[string]interface{}); ok {
							...
							if _, ok := objval["caCertPath"]; ok {
								if s, ok := objval["caCertPath"].(string); ok {
									rMtlsPolicyClientValidationCa.CaCertPath = dcl.String(s)
								} else {
									return nil, fmt.Errorf("rMtlsPolicyClientValidationCa.CaCertPath: expected string")
								}
							}
	...
	if _, ok := u.Object["serverCertificate"]; ok {
		if rServerCertificate, ok := u.Object["serverCertificate"].(map[string]interface{}); ok {
			...
			if _, ok := rServerCertificate["localFilepath"]; ok {
				if rServerCertificateLocalFilepath, ok := rServerCertificate["localFilepath"].(map[string]interface{}); ok {
					r.ServerCertificate.LocalFilepath = &dclService.ServerTlsPolicyServerCertificateLocalFilepath{}
					if _, ok := rServerCertificateLocalFilepath["certificatePath"]; ok {
						if s, ok := rServerCertificateLocalFilepath["certificatePath"].(string); ok {
							r.ServerCertificate.LocalFilepath.CertificatePath = dcl.String(s)
						} else {
							return nil, fmt.Errorf("r.ServerCertificate.LocalFilepath.CertificatePath: expected string")
						}
					}
					if _, ok := rServerCertificateLocalFilepath["privateKeyPath"]; ok {
						if s, ok := rServerCertificateLocalFilepath["privateKeyPath"].(string); ok {
							r.ServerCertificate.LocalFilepath.PrivateKeyPath = dcl.String(s)
						} else {
							return nil, fmt.Errorf("r.ServerCertificate.LocalFilepath.PrivateKeyPath: expected string")
						}
					}
				} else {
					return nil, fmt.Errorf("r.ServerCertificate.LocalFilepath: expected map[string]interface{}")
				}
			}
		} else {
