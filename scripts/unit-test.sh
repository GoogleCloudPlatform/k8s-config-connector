...
if [ -z ${GITHUB_ACTION+x} ]; then
    go run gotest.tools/gotestsum@v1.13.0 --format testname -- ${UNIT_TEST_PACKAGES} -coverprofile cover.out -count=1
else
    go run gotest.tools/gotestsum@v1.13.0 --jsonfile unittest_result.json --format pkgname --format-hide-empty-pkg --format-hivis -- ${UNIT_TEST_PACKAGES} -coverprofile cover.out -count=1
fi
