...
# HACK: Temporarily ignore memorystoreInstanceServiceAttachment and metastoreservices
added_reference_lines=$(echo "${added_lines}" | grep "${MISSING_REFERENCE_STRING}" | grep -v memorystoreInstanceServiceAttachment | grep -v metastoreservices.metastore || true)
