...
func helperFunctions() template.FuncMap {
	return template.FuncMap{
		// See https://github.com/golang/go/issues/54380
		"AutoGeneratedWarning": func() template.HTML {
			return template.HTML("<!-- AUTOGENERATED. DO NOT EDIT. -->\n")
		},
	}
}
...
func generateListOfResources(resources []schema.GroupVersionKind, outputFileName string) error {
	...
	template, err := template.New(filepath.Base(templateFilePath)).Funcs(helperFunctions()).ParseFiles(templateFilePath)
	if err != nil {
		return fmt.Errorf("error parsing template file %q: %v", templateFilePath, err)
	}
	...
	if err := template.Execute(outputFile, templateData); err != nil {
		return fmt.Errorf("error while executing template %q: %v", templateFilePath, err)
	}
	return nil
}
