...
func run(file, outputDir string) error {
	...
	if err != nil {
		return fmt.Errorf("error splitting bytes into YAMLs: %v", err)
	}
	crds := make([]*apiextensions.CustomResourceDefinition, 0)
	for _, y := range yamls {
		var crd apiextensions.CustomResourceDefinition
		if err := yaml.Unmarshal(y, &crd); err != nil {
			return fmt.Errorf("error unmarshalling bytes into CRD: %v", err)
		}
	...
	for _, crd := range crds {
		...
		if err != nil {
			return fmt.Errorf("error marshalling CRD into bytes: %v", err)
		}
