...
class Realm(object):
    ...
    def apply(self):
        ...
        request = realm_pb2.ApplyGameservicesBetaRealmRequest()
        ...
        response = stub.ApplyGameservicesBetaRealm(request)
    ...
    def delete(self):
        stub = realm_pb2_grpc.GameservicesBetaRealmServiceStub(channel.Channel())
        request = realm_pb2.DeleteGameservicesBetaRealmRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
    ...
    def to_proto(self):
        resource = realm_pb2.GameservicesBetaRealm()
        if Primitive.to_proto(self.name):
