...
class AddressGroup(object):
    ...
    def apply(self):
        stub = address_group_pb2_grpc.NetworksecurityAlphaAddressGroupServiceStub(
            channel.Channel()
        )
        request = address_group_pb2.ApplyNetworksecurityAlphaAddressGroupRequest()
        ...
        response = stub.ApplyNetworksecurityAlphaAddressGroup(request)
    ...
    def delete(self):
        ...
        request = address_group_pb2.DeleteNetworksecurityAlphaAddressGroupRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteNetworksecurityAlphaAddressGroup(request)
    ...
    def list(self, location, parent, service_account_file=""):
        ...
        request = address_group_pb2.ListNetworksecurityAlphaAddressGroupRequest()
        request.service_account_file = service_account_file
        request.Location = location

        request.Parent = parent

        return stub.ListNetworksecurityAlphaAddressGroup(request).items

    def to_proto(self):
        resource = address_group_pb2.NetworksecurityAlphaAddressGroup()
        if Primitive.to_proto(self.name):
...
class AddressGroupTypeEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return address_group_pb2.NetworksecurityAlphaAddressGroupTypeEnum.Value(
            "NetworksecurityAlphaAddressGroupTypeEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return address_group_pb2.NetworksecurityAlphaAddressGroupTypeEnum.Name(
            resource
        )[len("NetworksecurityAlphaAddressGroupTypeEnum") :]
