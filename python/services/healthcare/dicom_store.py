...
class DicomStore(object):
    ...
    def apply(self):
        stub = dicom_store_pb2_grpc.HealthcareDicomStoreServiceStub(channel.Channel())
        request = dicom_store_pb2.ApplyHealthcareDicomStoreRequest()
        ...
        response = stub.ApplyHealthcareDicomStore(request)
    ...
    def delete(self):
        stub = dicom_store_pb2_grpc.HealthcareDicomStoreServiceStub(channel.Channel())
        request = dicom_store_pb2.DeleteHealthcareDicomStoreRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteHealthcareDicomStore(request)

    @classmethod
    def list(self, project, location, dataset, service_account_file=""):
        stub = dicom_store_pb2_grpc.HealthcareDicomStoreServiceStub(channel.Channel())
        request = dicom_store_pb2.ListHealthcareDicomStoreRequest()
        ...
        return stub.ListHealthcareDicomStore(request).items

    def to_proto(self):
        resource = dicom_store_pb2.HealthcareDicomStore()
        if Primitive.to_proto(self.name):
...
class DicomStoreNotificationConfig(object):
    ...
    def to_proto(self, resource):
        ...
        res = dicom_store_pb2.HealthcareDicomStoreNotificationConfig()
