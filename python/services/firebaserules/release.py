...
class Release(object):
    ...
    def apply(self):
        stub = release_pb2_grpc.FirebaserulesReleaseServiceStub(channel.Channel())
        request = release_pb2.ApplyFirebaserulesReleaseRequest()
        ...
        response = stub.ApplyFirebaserulesRelease(request)
    ...
    def list(self, project, service_account_file=""):
        stub = release_pb2_grpc.FirebaserulesReleaseServiceStub(channel.Channel())
        request = release_pb2.ListFirebaserulesReleaseRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListFirebaserulesRelease(request).items

    def to_proto(self):
        resource = release_pb2.FirebaserulesRelease()
        if Primitive.to_proto(self.name):
