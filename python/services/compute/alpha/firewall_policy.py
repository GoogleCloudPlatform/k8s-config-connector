...
class FirewallPolicy(object):
    ...
    def apply(self):
        stub = firewall_policy_pb2_grpc.ComputeAlphaFirewallPolicyServiceStub(
            channel.Channel()
        )
        request = firewall_policy_pb2.ApplyComputeAlphaFirewallPolicyRequest()
        ...
        response = stub.ApplyComputeAlphaFirewallPolicy(request)
    ...
    def delete(self):
        ...
        request = firewall_policy_pb2.DeleteComputeAlphaFirewallPolicyRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteComputeAlphaFirewallPolicy(request)
    ...
    def list(self, parent, service_account_file=""):
        ...
        request = firewall_policy_pb2.ListComputeAlphaFirewallPolicyRequest()
        request.service_account_file = service_account_file
        request.Parent = parent

        return stub.ListComputeAlphaFirewallPolicy(request).items

    def to_proto(self):
        resource = firewall_policy_pb2.ComputeAlphaFirewallPolicy()
        if Primitive.to_proto(self.name):
