...
class Route(object):
    ...
    def apply(self):
        stub = route_pb2_grpc.ComputeAlphaRouteServiceStub(channel.Channel())
        request = route_pb2.ApplyComputeAlphaRouteRequest()
        ...
        response = stub.ApplyComputeAlphaRoute(request)
    ...
    def delete(self):
        stub = route_pb2_grpc.ComputeAlphaRouteServiceStub(channel.Channel())
        request = route_pb2.DeleteComputeAlphaRouteRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteComputeAlphaRoute(request)

    @classmethod
    def list(self, project, service_account_file=""):
        stub = route_pb2_grpc.ComputeAlphaRouteServiceStub(channel.Channel())
        request = route_pb2.ListComputeAlphaRouteRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListComputeAlphaRoute(request).items

    def to_proto(self):
        resource = route_pb2.ComputeAlphaRoute()
        if Primitive.to_proto(self.name):
...
class RouteWarning(object):
    ...
    def to_proto(self, resource):
        ...
        res = route_pb2.ComputeAlphaRouteWarning()
...
class RouteWarningCodeEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return route_pb2.ComputeAlphaRouteWarningCodeEnum.Value(
            "ComputeAlphaRouteWarningCodeEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return route_pb2.ComputeAlphaRouteWarningCodeEnum.Name(resource)[
            len("ComputeAlphaRouteWarningCodeEnum") :
        ]
