...
class Topic(object):
    ...
    def apply(self):
        stub = topic_pb2_grpc.PubsubAlphaTopicServiceStub(channel.Channel())
        request = topic_pb2.ApplyPubsubAlphaTopicRequest()
        ...
        response = stub.ApplyPubsubAlphaTopic(request)
    ...
    def delete(self):
        stub = topic_pb2_grpc.PubsubAlphaTopicServiceStub(channel.Channel())
        request = topic_pb2.DeletePubsubAlphaTopicRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeletePubsubAlphaTopic(request)

    @classmethod
    def list(self, project, service_account_file=""):
        stub = topic_pb2_grpc.PubsubAlphaTopicServiceStub(channel.Channel())
        request = topic_pb2.ListPubsubAlphaTopicRequest()
        request.service_account_file = service_account_file
        request.Project = project

        return stub.ListPubsubAlphaTopic(request).items

    def to_proto(self):
        resource = topic_pb2.PubsubAlphaTopic()
        if Primitive.to_proto(self.name):
...
class TopicMessageStoragePolicy(object):
    ...
    def to_proto(self, resource):
        ...
        res = topic_pb2.PubsubAlphaTopicMessageStoragePolicy()
