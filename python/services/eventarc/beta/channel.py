...
class Channel(object):
    ...
    def apply(self):
        stub = channel_pb2_grpc.EventarcBetaChannelServiceStub(channel.Channel())
        request = channel_pb2.ApplyEventarcBetaChannelRequest()
        ...
        response = stub.ApplyEventarcBetaChannel(request)
    ...
    def delete(self):
        stub = channel_pb2_grpc.EventarcBetaChannelServiceStub(channel.Channel())
        request = channel_pb2.DeleteEventarcBetaChannelRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteEventarcBetaChannel(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = channel_pb2_grpc.EventarcBetaChannelServiceStub(channel.Channel())
        request = channel_pb2.ListEventarcBetaChannelRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListEventarcBetaChannel(request).items

    def to_proto(self):
        resource = channel_pb2.EventarcBetaChannel()
        if Primitive.to_proto(self.name):
...
class ChannelStateEnum(object):
    ...
    def to_proto(self, resource):
        if not resource:
            return resource
        return channel_pb2.EventarcBetaChannelStateEnum.Value(
            "EventarcBetaChannelStateEnum%s" % resource
        )
    ...
    def from_proto(self, resource):
        if not resource:
            return resource
        return channel_pb2.EventarcBetaChannelStateEnum.Name(resource)[
            len("EventarcBetaChannelStateEnum") :
        ]
