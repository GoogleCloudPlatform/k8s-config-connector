...
class Mesh(object):
    ...
    def apply(self):
        ...
        request = mesh_pb2.ApplyNetworkservicesAlphaMeshRequest()
        ...
        response = stub.ApplyNetworkservicesAlphaMesh(request)
    ...
    def delete(self):
        stub = mesh_pb2_grpc.NetworkservicesAlphaMeshServiceStub(channel.Channel())
        request = mesh_pb2.DeleteNetworkservicesAlphaMeshRequest()
        request.service_account_file = self.service_account_file
        if Primitive.to_proto(self.name):
            request.resource.name = Primitive.to_proto(self.name)
        ...
        response = stub.DeleteNetworkservicesAlphaMesh(request)

    @classmethod
    def list(self, project, location, service_account_file=""):
        stub = mesh_pb2_grpc.NetworkservicesAlphaMeshServiceStub(channel.Channel())
        request = mesh_pb2.ListNetworkservicesAlphaMeshRequest()
        request.service_account_file = service_account_file
        request.Project = project

        request.Location = location

        return stub.ListNetworkservicesAlphaMesh(request).items

    def to_proto(self):
        resource = mesh_pb2.NetworkservicesAlphaMesh()
        if Primitive.to_proto(self.name):
