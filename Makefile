...
.PHONY: deploy-kcc-operator-autopilot
deploy-kcc-operator-autopilot: config-connector-manifests-autopilot push-operator-manifest
	kubectl apply -f config/installbundle/release-manifests/autopilot/manifests.yaml ${CONTEXT_FLAG}

.PHONY: install-crds
install-crds: manifests install

.PHONY: build-push-all-images
build-push-all-images: docker-build docker-push

.PHONY: deploy-kcc-autopilot
deploy-kcc-autopilot: install-crds build-push-all-images deploy-kcc-operator-autopilot
	kustomize build config/installbundle/releases/scopes/cluster/withworkloadidentity | sed -e 's/$${PROJECT_ID?}/${PROJECT_ID}/g'| kubectl apply -f - ${CONTEXT_FLAG}

.PHONY: deploy-kcc-standard
deploy-kcc-autopilot: install-crds build-push-all-images deploy-kcc-operator-standard
	kustomize build config/installbundle/releases/scopes/cluster/withworkloadidentity | sed -e 's/$${PROJECT_ID?}/${PROJECT_ID}/g'| kubectl apply -f - ${CONTEXT_FLAG}
