...
DOCKER_BUILD = docker buildx build --platform="$(PLATFORM)" --output=$(OUTPUT_TYPE)
DOCKER_PUSH = docker buildx build --platform="$(PLATFORM)" --push
...
docker-build-builder:
	docker buildx build . -f build/builder/Dockerfile -t ${BUILDER_IMG} --platform="$(BUILDER_PLATFORM)" --output=$(OUTPUT_TYPE)
