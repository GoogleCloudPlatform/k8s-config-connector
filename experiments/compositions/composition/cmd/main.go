...
func main() {
	...
	if err = (&controller.FacadeBindingReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		Recorder:      mgr.GetEventRecorderFor("facadeBinding"),
		ImageRegistry: imageRegistry,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "FacadeBinding")
		os.Exit(1)
	}
