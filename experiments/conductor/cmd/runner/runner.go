...
func RunRunner(ctx context.Context, opts *RunnerOptions) error {
	...
	case cmdPushBranch: // 9
		processors := []BranchProcessor{
			{Fn: runAPIChecks, CommitMsg: "Normalize api checks"},
			{Fn: makeReadyPR, CommitMsg: "make ready-pr", AttemptsOnChanges: 5},
			{Fn: pushBranch},
		}
	...
	case cmdRunMockTests: // 16
		processBranches(ctx, opts, branches.Branches, "Mock Tests", []BranchProcessor{{Fn: fixMockgcpFailures, CommitMsg: "Verify and Fix mock tests", VerifyFn: runMockgcpTests, VerifyAttempts: 10, AttemptsOnNoChange: 2}})
	...
	case cmdAdjustTypes: // 21
		processors := []BranchProcessor{
			...
			{Fn: removeNameField, CommitMsg: "Remove Name Field"},
			{Fn: moveEtagField, CommitMsg: "Move Etag Field", AttemptsOnNoChange: 1},
			// add a kubebuilder required field label to the fields that are marked required in proto
			{Fn: addRequiredFieldTags, CommitMsg: "Add Required Field Tags"},
	...
	case cmdRunAndFixFuzzTests: // 25
		processBranches(ctx, opts, branches.Branches, "Fuzzer Tests", []BranchProcessor{{Fn: fixFuzzerFailures, CommitMsg: "Verify and Fix fuzzer tests", VerifyFn: runFuzzerTests, VerifyAttempts: 5, AttemptsOnNoChange: 2}})
