# Config Service Journal

### 2026-07-07 ConfigDeploymentGroup Types & Identity Implementation
- **Context**: Implementing KRM types and IdentityV2 for `ConfigDeploymentGroup` under `apis/config/v1alpha1/` (Issue #9020).
...
- **Impact**: Any future `config` service implementations (such as `ConfigDeployment` or `ConfigRevision`) must use a googleapis commit at or after `1a91fbee8fd1c7f9bdc3d585ff8032b1619a2051` to ensure the proto definitions are present in the build cache.
