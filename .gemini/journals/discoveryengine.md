...
### [2026-08-04] DiscoveryEngineEngine, DiscoveryEngineRecommendationEngine, and DiscoveryEngineSearchEngine/ChatEngine sharing Engine proto
- **Context**: Greenfield KRM types implementation for `DiscoveryEngineRecommendationEngine` (Issue #12016).
- **Problem**: RecommendationEngine, ChatEngine and SearchEngine all map to the same `Engine` proto under `google.cloud.discoveryengine.v1`. However, each resource only contains one of the `EngineConfig` "one of" fields (e.g. `mediaRecommendationEngineConfig` for `DiscoveryEngineRecommendationEngine`). Mapping `DiscoveryEngineRecommendationEngine` to `Engine` in `generate.sh` is redundant and causes duplicate generator mappings because `DiscoveryEngineEngine` is already mapped to the `Engine` proto.
- **Solution**: We removed `DiscoveryEngineRecommendationEngine:Engine` from `generate.sh`. The shared nested sub-types (such as `Engine_CommonConfig`, `Engine_MediaRecommendationEngineConfig`) remain generated in `v1_types.generated.go` because `DiscoveryEngineEngine:Engine` is still registered. `DiscoveryEngineRecommendationEngineSpec` and `DiscoveryEngineRecommendationEngineObservedState` fields were kept as handcrafted types in `discoveryenginerecommendationengine_types.go`, referencing the generated sub-types.
- **Impact**: Avoids redundant type mapping in `generate.sh` and duplicate generated mappers when multiple direct resources map to a single GCP proto with distinct "one of" fields.
