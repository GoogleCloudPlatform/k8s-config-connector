...
### [2026-07-02] DatabaseMigrationConnectionProfile Greenfield Types Implementation
- **Context**: Implementing Greenfield types scaffolding, CRD generation, and IdentityV2 for `DatabaseMigrationConnectionProfile`.
- **Problem**: When generating types, nested/shared types inside a oneof field (like `MySqlConnectionProfile`, `PostgreSqlConnectionProfile`, `OracleConnectionProfile`, `CloudSqlConnectionProfile`, `AlloyDbConnectionProfile`) are originally treated as unreachable and pruned/commented out from `types.generated.go`. If we manually reference these types in the primary spec or observed state, the pruner comments them out or fails due to child types (e.g. `AlloyDbSettings_UserPassword`, `AlloyDbSettings_PrimaryInstanceSettings`) being recursively pruned.
- **Solution**: We hand-declared the custom inner structs, such as `PrivateConnectivity`, `CloudSQLSettings`, `AlloyDbSettings`, `AlloyDbSettings_UserPassword`, `AlloyDbSettings_PrimaryInstanceSettings`, `AlloyDbSettings_PrimaryInstanceSettings_MachineConfig`, `AlloyDbSettings_EncryptionConfig`, and their `ObservedState` equivalents, directly inside `databasemigrationconnectionprofile_types.go`. This tells the generator they are existing non-generated types and stops them/their sub-components from being pruned/commented out during codegen.
- **Impact**: Ensures that all child configuration options and sub-profiles for DMS connection profiles generate perfectly and cleanly, with correct `_reference.go` and `_identity.go` parsing formats.
