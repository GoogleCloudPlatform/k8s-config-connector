...
### [2026-07-08] types.generated.go Overwritten by Multiple generate-types Runs in Same Package
- **Context**: Implementing types and Identity for `NetworkSecurityMirroringEndpointGroupAssociation` (Issue #8733).
- **Problem**: `apis/networksecurity/v1alpha1/generate.sh` runs `go run . generate-types` twice for different services in the same package (first for `google.cloud.networksecurity.v1`, then for `google.cloud.networksecurity.v1alpha1`). The second run completely overwrote `types.generated.go`, causing nested/observed state types of the first run (such as `MirroringEndpointGroupAssociation_LocationDetailsObservedState`) to be commented out as "unreachable" and discarded.
- **Solution**: Defined `MirroringEndpointGroupAssociation_LocationDetailsObservedState` manually inside `networksecuritymirroringendpointgroupassociation_types.go`, and reused `MirroringLocationObservedState` which was already defined in a sibling types file (`networksecuritymirroringendpointgroup_types.go`).
- **Impact**: When a package has multiple service generation runs, any nested/unreachable types of the first service run that are deleted or commented out of `types.generated.go` can be cleanly hand-written in `<kind>_types.go` or imported/reused from other sibling types in the same package.
