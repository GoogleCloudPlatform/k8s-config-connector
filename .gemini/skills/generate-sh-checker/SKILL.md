...
3.  **Create generate.sh**: Create a `generate.sh` file in the directory. Ensure the year in the copyright header is current (2026).
    ...
    **Notes on Existing Resources:**
    - If the API types already exist and were previously handwritten or scaffolded, add `--skip-scaffold-files` to the `generate-types` command to prevent overwriting them with conflicting newly scaffolded files.
    - If the direct controller for the service already has manually handwritten mappers (e.g. `mapper.go`) that use a different go SDK like `google.golang.org/api`, **do not** include the `generate-mapper` block. Otherwise, it will try to import non-existent proto packages or overwrite existing code.
