...
3.  **Create generate.sh**: Create a `generate.sh` file in the directory. Ensure the year in the copyright header is current (2026).
    ...
    *   **Multiple Services**: If the API uses multiple proto services (e.g. `google.spanner.admin.instance.v1` and `google.spanner.admin.database.v1`), you must invoke `generate-types` multiple times and rename the output `types.generated.go` to avoid overwriting, for example:
        ```bash
        go run . generate-types \
