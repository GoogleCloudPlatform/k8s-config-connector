...
4.  **Special Handling (Multi-version & Promotion/Consolidation)**:
    ...
    - **Non-regeneratable types**: If `generate-types` fails for some resources (e.g. proto message missing from the specified service) but those types already exist in `types.generated.go`, move them to a separate file (e.g. `types.go` or append to the resource's main `*_types.go` file) before running `generate-types`. This prevents them from being deleted when `generate-types` overwrites the file.
