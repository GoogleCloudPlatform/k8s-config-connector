...
3.  **Create generate.sh**: Create a `generate.sh` file in the directory. Ensure the year in the copyright header is current (2026).
    ...
    *Note on troubleshooting syntax errors:*
    - If `generate-types --include-skipped-output` causes a syntax error (like `expected declaration`), it may be due to `*/` strings present inside protobuf comments terminating the Go block comment `/* ... */` prematurely. If so, simply remove `--include-skipped-output` from the script.
    - If `generate-mapper` generates invalid Go code (for example when mapping complex reference structures like `[]*TopicRef`), or if you find that `pkg/controller/direct/<SERVICE>/mapper.generated.go` was already heavily customized by hand and `generate-mapper` destroys it, you should omit `go run . generate-mapper ...` from the script entirely.
