...
*   **1:1 Kind to Proto Mapping:** Enforce a strict 1:1 relationship between resource Kinds and Proto definitions. Verify that no single Proto is shared by multiple Kinds, and no single Kind points to multiple Protos.
...
## 6. Exception Files
*   Greenfield PRs must **not** add exceptions to any exception files (e.g., tracking omitted fields, unmapped types) with the sole exception of `missingrefs.txt`.
...
* **Exception Files**: [Pass/Fail] - (List any modified exception files other than missingrefs.txt)
