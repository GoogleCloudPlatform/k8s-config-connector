...
3.  **Controller Structure & Patterns**:
    ...
    - **Delete Idempotency Check**: In the `Delete` method, check if the resource has already been deleted using `direct.IsNotFound(err)` to ensure idempotency, returning `true, nil` to gracefully exit:
      ...
      ```
    - **Server-Generated IDs in `Find`**: For resources with service-generated IDs where the user does not specify an ID at creation time, the KRM `spec.resourceID` (or `GetIdentity()` output) can be empty.
      * At the beginning of `Find(ctx)`, check if the resolved identifier is empty. If so, return `false, nil` immediately to trigger creation, saving an unnecessary GCP GET call.
      * If the identifier is non-empty, perform the GCP GET call. Do NOT handle `direct.IsNotFound(err)` specially inside `Find()` for server-generated ID resources; if the previously-acquired resource is not found, return the error.
    - **ProjectMapper Normalization**: When GCP APIs return project numbers instead of project IDs, use the `ProjectMapper`'s `ReplaceProjectNumberWithIDInLink` method to normalize references, parent links, or any resource paths (excluding the `name` of the target resource inside `Find()`). Normalize target resource `name` fields to Project IDs *only* during `updateStatus` before setting `status.externalRef`. This ensures that `status.externalRef` contains the Project ID format, making custom project/equality checks unnecessary in `GetIdentity()`.
...
5.  **Fuzzer**:
    ...
    - **Fuzzer Implementation Guidelines**:
      * **Central Fuzzer Registration**: You must register the direct resource package in `pkg/controller/direct/register/register.go` using a blank import (e.g. `_ "github.com/GoogleCloudPlatform/k8s-config-connector/pkg/controller/direct/myservice"`). Without this registration, central fuzz testing under `pkg/fuzztesting/fuzztests` will not trigger the fuzzer's `init()` function or run the registered fuzzer.
      * **Fuzzer Path Conventions & Redundancy**: Registering a parent message struct (e.g., `f.SpecField(".manifest")`) automatically covers all primitive child fields in `fuzztesting`. Enumerating every nested subfield (such as `.manifest.api_spec.open_api_yaml`) is redundant and discouraged. Subfield paths only need to be specified for slice/list element properties or specific `Unimplemented_NotYetTriaged` leaf fields.
