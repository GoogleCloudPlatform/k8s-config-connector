...
2. **Legacy Controller Target Mapping Bug:**
   - There remains a minor, correct difference in the target reference field for some test cases (such as TCP forwarding rules). The legacy controller erroneously exported target TCP proxy references under `targetHTTPProxyRef` instead of `targetTCPProxyRef`. The direct controller correctly maps these to `targetTCPProxyRef` based on the resource URI type, resulting in a cleaner and more accurate KRM representation.
