...
- Therefore, we defined `ComputeBackendBucketRef` in `apis/refs/v1beta1/computerefs.go` that omits the `kind` field, which is shared centrally.
...
### 3. OneOf KeyValue Validation & secret.Legacy
- The `KeyValue` field uses a `value/valueFrom` pattern. We used `secret.Legacy` from `apis/refs/v1beta1/secret` which perfectly matches the required structure. We also registered `ComputeBackendBucketSignedURLKey` under the `value,valueFrom` signature check in `scripts/add-validation-to-crds/parse-crds.go` to automatically inject the OpenAPI `oneOf` constraint blocks into the generated CRD schema.
