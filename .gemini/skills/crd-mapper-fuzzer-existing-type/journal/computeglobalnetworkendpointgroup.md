...
The `NetworkEndpointGroup` GCP protobuf type defines `default_port` as an `int32` pointer (`*int32`) and `network_endpoint_type` as a `string` pointer (`*string`), whereas the existing KRM/CRD baseline schema defines `defaultPort` as `*int64` and `networkEndpointType` as a plain `string`. Additionally, the resource name is represented as `Name` in GCP Proto and `ResourceID` in KRM.
...
- These functions properly handle port conversion, string mapping, project reference omission, and mapping KRM `ResourceID` to/from proto `Name`.
