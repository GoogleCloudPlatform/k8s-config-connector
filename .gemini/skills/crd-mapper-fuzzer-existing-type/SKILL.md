...
- **Hand-code custom reference types**: If a resource reference structure in the baseline CRD (like `FolderRef`, `OrganizationRef`, or `BillingAccountRef`) lacks a `kind` field or retains specific fields (like `name`/`namespace` in `OrganizationRef`), you **must hand-code custom reference types** locally in `<kind>_types.go`. For a project reference without a `kind` field, you MUST import and use `ProjectRef` from `github.com/GoogleCloudPlatform/k8s-config-connector/apis/refs` (defined in `apis/refs/project_reference.go`) instead of defining a local structure or using `v1beta1.ProjectRef` (which exposes `kind`). For sensitive key/value secret reference structures matching standard legacy patterns, import and use the shared `secret.Legacy` from `github.com/GoogleCloudPlatform/k8s-config-connector/apis/refs/v1beta1/secret` (defined in `apis/refs/v1beta1/secret/legacy.go`).
