...
9. **CI/CD & Golden File Traps (Gotchas)**:
   ...
   - **Out-of-Date Resource Documentation**: If you modify any CRD API types (e.g., changing a field's validation tags from required to optional, or adding new fields), you **MUST** run `make resource-docs` to regenerate the documentation under `scripts/generate-google3-docs/resource-reference/generated/resource-docs/`. Forgetting to commit these regenerated `.md` documentation files will cause the `validate-generated-files` presubmit check to fail.
   - **Out-of-Date Go Clients**: If you modify any CRD API types (e.g., changing field validations, field types, or adding fields), the Go client's generated deepcopy implementation (`zz_generated.deepcopy.go`) must be updated. You **MUST** run `make ready-pr` (or `make generate-go-client ensure fmt`) and commit the resulting changes under `pkg/clients/generated/` to prevent the `validations` CI/CD job from failing.
   - **Mandatory `projectRef` for Direct Resources in E2E Manifests**: Newer "direct" resources (reconciled via controller-runtime and direct SDK calls) require `projectRef` to be specified explicitly in their specs according to their schemas. While the active Kubernetes control plane's mutating webhooks dynamically inject a fallback namespace project ID at runtime (allowing E2E tests to pass), the offline schema verification test `TestYAMLValidatesAgainstCRDSchema` does not run webhooks. If you omit `projectRef` for any direct resources in E2E fixtures/dependencies, the `unit-tests` presubmit check will fail with schema violations. Always specify `projectRef: { external: "${projectId}" }` for direct resources in E2E manifests.
   - **Do Not Skip Group E2E Presubmits**: Adding new optional or immutable fields to an existing CRD can silently break *other* pre-existing test fixtures of the same kind. You **MUST** run the group presubmit E2E tests locally (e.g. `dev/ci/presubmits/tests-e2e-fixtures-<service_name>`) even if they are slow, to ensure there are no regressions or drift failures in existing tests.
   - **TF Provider `Read` Method Missing New Fields (Gotcha)**: When exposing existing TF schema fields in a KCC CRD, double check the TF provider's `Read` method (`resource<Resource>Read`). TF provider developers sometimes add fields to the schema but forget to call `d.Set()` to populate them from the GCP response in the `Read` method. This causes the field to be permanently empty in the TF state. If the field is immutable and has a schema default, this will cause update calls to fail with an immutable field mutation error because TF plans to set it to the default but the state is empty. You must patch the TF provider's `Read` method to call `d.Set()` for these fields.
