...
- Implement `getIdentityFrom<Kind>Spec(...)` to extract fields from the spec/obj (often using `refs.ResolveProjectID`, `refs.GetLocation`, etc.).
  ...
  - **Note:** For resources with server-generated IDs (where the ID is only available in `status.externalRef` or a status field like `status.triggerId`), `getIdentityFrom<Kind>Spec` should return `(nil, nil)` if the ID is not yet available in the status.
