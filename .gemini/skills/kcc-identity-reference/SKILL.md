...
- Implement `getIdentityFrom<Kind>Spec(...)` to extract fields from the spec/obj (often using `refs.ResolveProjectID`, `refs.GetLocation`, etc.).
  - **Important:** This function MUST handle both typed and `*unstructured.Unstructured` objects, as it is used by `Normalize`. To keep code clean and avoid error-prone manual parsing of `*unstructured.Unstructured` objects, you should convert `obj` into a typed pointer of the resource (e.g. `*<Kind>`) using `common.ToStructuredType[*<Kind>](obj)`. This allows you to work purely with typed fields in both cases.
