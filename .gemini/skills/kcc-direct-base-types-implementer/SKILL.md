...
- **Empty ObservedState**: If a resource does not have any output-only fields, do not include an empty `observedState` field in its `Status` struct. This prevents generating CRDs with an empty `status.observedState` object schema, which fails API validation and pollutes the schema. In such cases, comment out `ObservedState` under `Status` and comment out the empty `ObservedState` type definition. Do NOT allow `knownInvalidCRDs` to grow because of empty `status.observedState` fields.
