...
if [[ -z "${VERSION:-}" ]]; then
  echo "VERSION not set, verifying against config/crds/resources"
  CRDS_FILE=$(mktemp)
  first=true
...
for filepath in ${TEMP_DIR}/*.yaml; do
    ...
    # Replace the specific version with the placeholder ${CNRM_VERSION}
    sed "s/cnrm.cloud.google.com\/version: .*/cnrm.cloud.google.com\/version: \${CNRM_VERSION}/g" ${filepath} >> ${TMP_FILE}
...
if ! diff -r -N ${TARGET_CRD_DIR} ${TEMP_DIR}; then
    ...
    echo "Please run 'dev/tasks/sync-crds-folder.sh' (with VERSION set if needed) and commit the changes."
    exit 1
