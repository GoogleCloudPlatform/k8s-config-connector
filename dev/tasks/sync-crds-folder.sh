...
if [[ -z "${VERSION:-}" ]]; then
  echo "VERSION not set, syncing from config/crds/resources"
  CRDS_FILE=$(mktemp)
  first=true
...
for filepath in ${TEMP_DIR}/*.yaml; do
    ...
    sed "s/cnrm.cloud.google.com\/version: .*/cnrm.cloud.google.com\/version: \${CNRM_VERSION}/g" ${filepath} >> ${TARGET_CRD_DIR}/${filename}
done
