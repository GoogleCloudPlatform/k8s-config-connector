...
func LoadPackage(goPackage string, path string) (*Package, error) {
	...
	visitFilePredicate := func(info fs.FileInfo) bool {
		p := filepath.Join(path, info.Name())
		// Skip test files (for speed)
		if strings.HasSuffix(info.Name(), "_test.go") {
			return false
		}
		// Skip autogenerated files (in case they are broken)
		b, err := os.ReadFile(p)
		if err != nil {
			// Unexpected - panic
			klog.Fatalf("failed to read file %q: %v", p, err)
		}
		if strings.Contains(string(b), "//go:build !ignore_autogenerated") {
			return false
		}
		return true
	}

	// Note: parser.ParseDir is deprecated, but a naive switch to the replacement was much slower.
	packages, err := parser.ParseDir(fileSet, path, visitFilePredicate, mode)
	if err != nil {
