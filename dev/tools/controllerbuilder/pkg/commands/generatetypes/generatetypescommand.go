...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.PrepopulateSpec, "prepopulate-spec", false, "populate <Kind>Spec and <Kind>ObservedState with fields mapped from the proto definition")
}
			...
			if scaffolder.TypeFileExists(resource) {
				...
				if o.PrepopulateSpec {
					...
					if details, ok := typeGenerator.OutputFieldsFor(string(msg.FullName())); ok {
						prepopulated.ObservedStateFields = scaffold.PrepopulateObservedState(details, typeGenerator.ObservedStateMessages(), writeOptions)
					}
					prepopulated.ExtraImports = scaffold.ExtraImportsFor(prepopulated.SpecFields, prepopulated.ObservedStateFields)
				}
				err := scaffolder.AddTypeFile(resource, prepopulated)
				if err != nil {
