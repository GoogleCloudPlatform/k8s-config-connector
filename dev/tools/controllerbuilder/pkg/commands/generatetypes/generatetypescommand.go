...
type GenerateCRDOptions struct {
	*options.GenerateOptions
	...
	// ProtoOutputs is a list of strings where we want to force a particular proto field to be output-only
	ProtoOutputs []string

	// ProtoIgnores is a list of strings where we want to force a particular proto field to be ignored
	ProtoIgnores []string

	// PruneUnusedTypes determines whether to prune unreachable types
...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringSliceVar(&o.ProtoOutputs, "proto-output", o.ProtoOutputs, "force a particular proto field to be output-only")
	cmd.Flags().StringSliceVar(&o.ProtoIgnores, "proto-ignore", o.ProtoIgnores, "force a particular proto field to be ignored")
...
func RunGenerateCRD(ctx context.Context, o *GenerateCRDOptions) error {
	...
	protoOverrides := make(map[string]*codegen.ProtoOverride)
	for _, protoOutput := range o.ProtoOutputs {
		o := &codegen.ProtoOverride{}
		o.OutputOnly = new(true)
		protoOverrides[protoOutput] = o
	}
	for _, protoIgnore := range o.ProtoIgnores {
		o := &codegen.ProtoOverride{}
		o.Ignore = true
		protoOverrides[protoIgnore] = o
	}
	typeGenerator.WithProtoOverrides(protoOverrides)
