...
type GenerateCRDOptions struct {
	*options.GenerateOptions
	...
	// ProtoOverrides is a list of strings where we want to force a particular proto behaviour e.g. output or input
	ProtoOverrides []string
}
...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringSliceVar(&o.ProtoOverrides, "proto-override", o.ProtoOverrides, "override proto handling, for example whether fields are considered output only")
}
...
func RunGenerateCRD(ctx context.Context, o *GenerateCRDOptions) error {
	...
	for _, protoOverride := range o.ProtoOverrides {
		tokens := strings.Split(protoOverride, "=")
		if len(tokens) != 2 {
			return fmt.Errorf("unhandled proto-override %q", protoOverride)
		}
		o := &codegen.ProtoOverride{}
		if tokens[1] == "input-only" {
			o.OutputOnly = new(false)
		} else if tokens[1] == "output-only" {
			o.OutputOnly = new(true)
		} else {
			return fmt.Errorf("unhandled proto-override %q", protoOverride)
		}
		protoOverrides[tokens[0]] = o
	}
