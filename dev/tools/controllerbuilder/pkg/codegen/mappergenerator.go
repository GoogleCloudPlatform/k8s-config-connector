...
func (v *MapperGenerator) GenerateMappers(goImports map[string]string) error {
	...
	for _, pair := range v.typePairs {
		...
		if out.fileAnnotation == nil {
			out.fileAnnotation = v.generatedFileAnnotation
			out.addBuildTag("go:build !ignore_autogenerated")
			out.addBuildTag("+build !ignore_autogenerated")
...
func GoPackageForProto(parentFile protoreflect.FileDescriptor) string {
	...
	case "cloud.google.com/go/networksecurity/apiv1/networksecuritypb":
		return "github.com/GoogleCloudPlatform/k8s-config-connector/mockgcp/generated/mockgcp/cloud/networksecurity/v1"
