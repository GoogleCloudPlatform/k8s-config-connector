...
func deduplicateAndSortOutputMessages(messages []*OutputMessageDetails) []*OutputMessageDetails {
	...
	for _, msg := range messages {
		...
		existing, ok := m[key]
		if !ok {
			m[key] = &OutputMessageDetails{
				Message:      msg.Message,
				OutputFields: append([]protoreflect.FieldDescriptor(nil), msg.OutputFields...),
			}
			continue
		}
		// Prioritize v1 over v1beta1 (do not let v1beta1 overwrite v1)
		...
		// Merge OutputFields so we don't lose output-only fields discovered across different reachability paths
		seenFields := make(map[string]bool)
		for _, f := range existing.OutputFields {
			seenFields[string(f.Name())] = true
		}
		for _, f := range msg.OutputFields {
			if !seenFields[string(f.Name())] {
				seenFields[string(f.Name())] = true
				existing.OutputFields = append(existing.OutputFields, f)
			}
		}
	}
	var keys []string
