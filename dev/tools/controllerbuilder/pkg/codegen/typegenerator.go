...
func deduplicateAndSort(messages []protoreflect.MessageDescriptor) []protoreflect.MessageDescriptor {
	// Sort messages by FullName to ensure deterministic map insertion order and stable winner of deduplication
	sort.Slice(messages, func(i, j int) bool {
		return string(messages[i].FullName()) < string(messages[j].FullName())
	})
...
func deduplicateAndSortOutputMessages(messages []*OutputMessageDetails) []*OutputMessageDetails {
	// Sort messages by FullName to ensure deterministic map insertion order and stable winner of deduplication
	sort.Slice(messages, func(i, j int) bool {
		return string(messages[i].Message.FullName()) < string(messages[j].Message.FullName())
	})
