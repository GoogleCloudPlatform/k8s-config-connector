...
func NewTypeGenerator(filter FilterFunc) *TypeGenerator {
	g := &TypeGenerator{
		filter: filter,
	}
...
type FilterFunc func(msg protoreflect.MessageDescriptor) bool
...
func (g *TypeGenerator) visitFile(f protoreflect.FileDescriptor) {
	...
	krmVersion := "v1beta1"
	{
		...
		protoPackagePath = strings.TrimPrefix(protoPackagePath, ".v1beta1")
		goPackage := "apis/" + strings.Join(strings.Split(protoPackagePath, "."), "/") + "/" + krmVersion
