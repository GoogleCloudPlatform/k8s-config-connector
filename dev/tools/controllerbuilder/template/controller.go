...
func (a *Adapter) Create(ctx context.Context, u *unstructured.Unstructured) error {
	...
	// TODO(user): Please add the Status_FromProto mappers under the same package
	status := {{.Kind}}Status_FromProto(mapCtx, created)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
	// TODO(user): (Optional) Please set status.ObservedState
...
func (a *Adapter) Update(ctx context.Context, u *unstructured.Unstructured) error {
	...
	status := &krm.{{.Kind}}Status{}
	status := {{.Kind}}Status_FromProto(mapCtx, updated)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
