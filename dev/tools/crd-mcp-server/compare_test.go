          ...
          spec:
            ...
            properties:
              region:
                ...
                - rule: 'self == "us-west1"'
`
	old, _ := parseCRD([]byte(oldYaml))
	new, _ := parseCRD([]byte(newYaml))
                ...
                items:
                  ...
                  properties:
                    ...
                    status:
                      type: string
`
	old, _ := parseCRD([]byte(baseCRD))
	new, _ := parseCRD([]byte(modified))

	result := compareBackwardCompatibility(old, new)
	if len(result.Diffs) != 0 {
		t.Errorf("expected no diffs for validation addition in backward compatibility check, got: %v", result.Diffs)
	}
	if len(result.Notes) == 0 {
		t.Error("expected notes for validation addition in backward compatibility check")
	}
}
