	...
	t.Run("fixtures", func(t *testing.T) {
		...
		for _, fixture := range fixtures {
			...
			t.Run(fixture.Name, func(t *testing.T) {
				...
				forceDirect := false
				if resourceconfig.IsControllerSupported(fixture.GVK, k8scontrollertype.ReconcilerTypeDirect) {
					if resourceconfig.IsControllerSupported(fixture.GVK, k8scontrollertype.ReconcilerTypeTerraform) || resourceconfig.IsControllerSupported(fixture.GVK, k8scontrollertype.ReconcilerTypeDCL) {
						switch fixture.GVK.Group {
						case "bigquery.cnrm.cloud.google.com",
							"spanner.cnrm.cloud.google.com":
							// Gradually moving these over
							t.Logf("not yet using direct controller for group %q", fixture.GVK.Group)
							forceDirect = false
						default:
							forceDirect = true
						}
...
func createDiffs(t *testing.T, ctx context.Context, fixture resourcefixture.ResourceFixture) {
	...
	computeDiff := func(oldP, newP string) string {
		...
		out.WriteString(fmt.Sprintf("# autogenerated diff %s %s\n", filepath.Base(oldP), filepath.Base(newP)))
	...
	// _final_object.yaml
	{
		oldPath := filepath.Join(dir, "_exported_object_old_controller.golden.yaml")
		// newPath := filepath.Join(dir, "_exported_object.yaml")
		newPath := filepath.Join(dir, "_generated_export_"+fixture.Name+".golden")

		if fileExists(oldPath) && fileExists(newPath) {
			diff := computeDiff(oldPath, newPath)
			test.CompareGoldenFile(t, filepath.Join(dir, "_exported_object.diff"), diff)
		}
	}
