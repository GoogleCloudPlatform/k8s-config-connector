...
func TestPauseInSeries(t *testing.T) {
	...
	t.Run("fixtures", func(t *testing.T) {
		fixtures := resourcefixture.Load(t)
		for _, fixture := range fixtures {
			runTests(ctx, t, fixture, true)
		}
	})
}

// TestFixturesInParallel An option to run fixtures test in parallel by services
func TestFixturesInParallel(t *testing.T) {
	if os.Getenv("RUN_E2E") == "" {
		t.Skip("RUN_E2E not set; skipping")
	}
...
func runTests(ctx context.Context, t *testing.T, fixture resourcefixture.ResourceFixture, testPause bool) {
	t.Helper()
