...
func TestAllInSeries(t *testing.T) {
	...
	t.Run("samples", func(t *testing.T) {
		allSampleKeys := create.ListAllSamples(t)

		samplesKeysByGroup := make(map[string][]create.SampleKey)
		for _, sampleKey := range allSampleKeys {
			group := strings.TrimSuffix(sampleKey.PrimaryObjectGVK.Group, ".cnrm.cloud.google.com")
			samplesKeysByGroup[group] = append(samplesKeysByGroup[group], sampleKey)
		}

		for group, sampleKeys := range samplesKeysByGroup {
...
func testFixturesInSeries(ctx context.Context, t *testing.T, testPause bool, cancel context.CancelFunc) {
	...
	t.Run("fixtures", func(t *testing.T) {
		...
		fixturesByGroup := make(map[string][]resourcefixture.ResourceFixture)
		for _, fixture := range allFixtures {
			group := strings.TrimSuffix(fixture.GVK.Group, ".cnrm.cloud.google.com")
			fixturesByGroup[group] = append(fixturesByGroup[group], fixture)
		}

		for group, fixtures := range fixturesByGroup {
			fixtures := fixtures
