...
func LegacyNormalize(t *testing.T, h *create.Harness, project testgcp.GCPProject, uniqueID string, events test.LogEntries) (string, []func(string) string) {
	...
	jsonMutators = append(jsonMutators, func(requestURL string, obj map[string]any) {
		...
		if !strings.Contains(name, "pipelinejob") && !strings.Contains(name, "trainingpipeline") && !strings.Contains(name, "featureonlinestore") {
			return
		}
		...
		if !strings.Contains(strings.ToLower(t.Name()), "vertexaipipelinejob") {
			return
		}
		if !strings.Contains(requestURL, "PipelineService") && !strings.Contains(requestURL, "Operations") {
			return
		}
