...
## Golden Log Rules (STRICT)

* **CRITICAL**: NEVER copy `_http_mock.log` to `_http.log` (`cp _http_mock.log _http.log`). `_http.log` represents canonical traffic recorded against REAL GCP. Copying mock logs to `_http.log` corrupts real GCP test ground truth. `_http.log` MUST only be updated via `WRITE_GOLDEN_OUTPUT=1 E2E_GCP_TARGET=real` (or `hack/record-gcp`).
