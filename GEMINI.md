...
**CRITICAL MANDATORY PRE-PUSH VERIFICATION RULE**:
Before pushing any commit or PR update to GitHub, you MUST run the corresponding presubmit script in `dev/ci/presubmits/` (for example, `dev/ci/presubmits/tests-e2e-fixtures-<service>`) locally with `WRITE_GOLDEN_OUTPUT=0` (standard verification mode) and confirm a **100% clean PASS**. Never push code or assume a refactoring/cleanup is safe without running the actual E2E presubmit script first.
...
Running these scripts will typically fix out-of-date golden files if `WRITE_GOLDEN_OUTPUT=1` is set (which is often the default in these scripts). Once golden files are regenerated, always re-run with `WRITE_GOLDEN_OUTPUT=0` to verify clean verification before pushing.
