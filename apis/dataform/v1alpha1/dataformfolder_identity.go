...
var FolderIdentityFormat = gcpurls.Template[FolderIdentity]("dataform.googleapis.com", "projects/{project}/locations/{location}/folders/{folder}")
...
func (i *FolderIdentity) FromExternal(ref string) error {
	...
	if err != nil {
		return fmt.Errorf("format of DataformFolder external=%q was not known (use %s): %w", ref, FolderIdentityFormat.CanonicalForm(), err)
	}
	if !match {
		return fmt.Errorf("format of DataformFolder external=%q was not known (use %s)", ref, FolderIdentityFormat.CanonicalForm())
	}
...
func getIdentityFromDataformFolderSpec(ctx context.Context, reader client.Reader, obj client.Object) (*FolderIdentity, error) {
