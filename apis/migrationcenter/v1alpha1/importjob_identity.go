...
// ImportJobIdentity defines the resource reference to MigrationcenterImportJob, which "External" field
...
func NewImportJobIdentity(ctx context.Context, reader client.Reader, obj *MigrationcenterImportJob) (*ImportJobIdentity, error) {
...
func ParseImportJobExternal(external string) (parent *ImportJobParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "importjobs" {
		return nil, "", fmt.Errorf("format of MigrationcenterImportJob external=%q was not known (use projects/{{projectID}}/locations/{{location}}/importjobs/{{importjobID}})", external)
	}
	parent = &ImportJobParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
