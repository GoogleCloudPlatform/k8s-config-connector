...
func getIdentityFromMigrationCenterGroupSpec(ctx context.Context, reader client.Reader, obj client.Object) (*MigrationCenterGroupIdentity, error) {
	groupObj, ok := obj.(*MigrationCenterGroup)
	if !ok {
		return nil, fmt.Errorf("object is not a MigrationCenterGroup")
	}
	...
	location := groupObj.Spec.Location
	if location == "" {
