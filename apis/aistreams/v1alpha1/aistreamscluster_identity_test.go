...
func TestAIStreamsClusterIdentity_FromExternal(t *testing.T) {
	tests := []struct {
		name         string
		ref          string
		wantProject  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			i := &AIStreamsClusterIdentity{}
			if err := i.FromExternal(tt.ref); (err != nil) != tt.wantErr {
				t.Errorf("AIStreamsClusterIdentity.FromExternal() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr {
				if i.Project != tt.wantProject {
					t.Errorf("AIStreamsClusterIdentity.FromExternal() Project = %v, want %v", i.Project, tt.wantProject)
				}
				if i.Location != tt.wantLocation {
					t.Errorf("AIStreamsClusterIdentity.FromExternal() Location = %v, want %v", i.Location, tt.wantLocation)
				}
