...
# We need a newer googleapis to get workloadmanager
...
if [ -d "${REPO_ROOT}/pkg/controller/direct/workloadmanager" ]; then
  go run -mod=readonly golang.org/x/tools/cmd/goimports@${GOLANG_X_TOOLS_VERSION} -w pkg/controller/direct/workloadmanager/
fi
