...
// InstanceConfigIdentity defines the resource reference to BaremetalsolutionInstanceConfig, which "External" field
...
func NewInstanceConfigIdentity(ctx context.Context, reader client.Reader, obj *BaremetalsolutionInstanceConfig) (*InstanceConfigIdentity, error) {
...
func ParseInstanceConfigExternal(external string) (parent *InstanceConfigParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "instanceconfigs" {
		return nil, "", fmt.Errorf("format of BaremetalsolutionInstanceConfig external=%q was not known (use projects/{{projectID}}/locations/{{location}}/instanceconfigs/{{instanceconfigID}})", external)
	}
	parent = &InstanceConfigParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
