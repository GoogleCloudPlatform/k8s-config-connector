...
// NetworkIdentity defines the resource reference to BaremetalsolutionNetwork, which "External" field
...
func NewNetworkIdentity(ctx context.Context, reader client.Reader, obj *BaremetalsolutionNetwork) (*NetworkIdentity, error) {
...
func ParseNetworkExternal(external string) (parent *NetworkParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "networks" {
		return nil, "", fmt.Errorf("format of BaremetalsolutionNetwork external=%q was not known (use projects/{{projectID}}/locations/{{location}}/networks/{{networkID}})", external)
	}
	parent = &NetworkParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
