...
func ParseDatabaseRoleExternal(external string) (parent *DatabaseRoleParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "databaseroles" {
		return nil, "", fmt.Errorf("format of SpannerDatabaseRole external=%q was not known (use projects/{{projectID}}/locations/{{location}}/databaseroles/{{databaseroleID}})", external)
	}
	parent = &DatabaseRoleParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
