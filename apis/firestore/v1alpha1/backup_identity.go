...
// BackupIdentity defines the resource reference to FirestoreBackup, which "External" field
...
func NewBackupIdentity(ctx context.Context, reader client.Reader, obj *FirestoreBackup) (*BackupIdentity, error) {
...
func ParseBackupExternal(external string) (parent *BackupParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "backups" {
		return nil, "", fmt.Errorf("format of FirestoreBackup external=%q was not known (use projects/{{projectID}}/locations/{{location}}/backups/{{backupID}})", external)
	}
	parent = &BackupParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
