...
// BackupScheduleIdentity defines the resource reference to FirestoreBackupSchedule, which "External" field
...
func NewBackupScheduleIdentity(ctx context.Context, reader client.Reader, obj *FirestoreBackupSchedule) (*BackupScheduleIdentity, error) {
...
func ParseBackupScheduleExternal(external string) (parent *BackupScheduleParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "backupschedules" {
		return nil, "", fmt.Errorf("format of FirestoreBackupSchedule external=%q was not known (use projects/{{projectID}}/locations/{{location}}/backupschedules/{{backupscheduleID}})", external)
	}
	parent = &BackupScheduleParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
