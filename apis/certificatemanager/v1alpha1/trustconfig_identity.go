...
// TrustConfigIdentity defines the resource reference to CertificateManagerTrustConfig, which "External" field
...
func NewTrustConfigIdentity(ctx context.Context, reader client.Reader, obj *CertificateManagerTrustConfig) (*TrustConfigIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseTrustConfigExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
func ParseTrustConfigExternal(external string) (parent *TrustConfigParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "trustconfigs" {
		return nil, "", fmt.Errorf("format of CertificateManagerTrustConfig external=%q was not known (use projects/{{projectID}}/locations/{{location}}/trustconfigs/{{trustconfigID}})", external)
	}
	parent = &TrustConfigParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
