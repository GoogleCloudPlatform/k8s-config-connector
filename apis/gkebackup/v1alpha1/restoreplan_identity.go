...
func (i *RestorePlanIdentity) Parent() *RestorePlanParent {
	return i.parent
}

type RestorePlanParent struct {
	ProjectID string
	Location  string
}

func (p *RestorePlanParent) String() string {
	return "projects/" + p.ProjectID + "/locations/" + p.Location
}

// New builds a RestorePlanIdentity from the Config Connector RestorePlan object.
func NewRestorePlanIdentity(ctx context.Context, reader client.Reader, obj *GKEBackupRestorePlan) (*RestorePlanIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseRestorePlanExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
// var _ identity.Identity = &RestorePlanIdentity{} // Tracking in issue #6073
