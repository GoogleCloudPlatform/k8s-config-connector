...
// JobIdentity defines the resource reference to TalentJob, which "External" field
...
func NewJobIdentity(ctx context.Context, reader client.Reader, obj *TalentJob) (*JobIdentity, error) {
...
func ParseJobExternal(external string) (parent *JobParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "jobs" {
		return nil, "", fmt.Errorf("format of TalentJob external=%q was not known (use projects/{{projectID}}/locations/{{location}}/jobs/{{jobID}})", external)
	}
	parent = &JobParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
