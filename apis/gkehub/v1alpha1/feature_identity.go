...
func NewFeatureIdentity(ctx context.Context, reader client.Reader, obj *GkehubFeature) (*FeatureIdentity, error) {
...
func ParseFeatureExternal(external string) (parent *FeatureParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "features" {
		return nil, "", fmt.Errorf("format of GkehubFeature external=%q was not known (use projects/{{projectID}}/locations/{{location}}/features/{{featureID}})", external)
	}
	parent = &FeatureParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
