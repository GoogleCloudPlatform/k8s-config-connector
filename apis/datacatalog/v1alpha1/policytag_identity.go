...
func ParsePolicyTagExternal(external string) (parent *PolicyTagParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "policytags" {
		return nil, "", fmt.Errorf("format of DatacatalogPolicyTag external=%q was not known (use projects/{{projectID}}/locations/{{location}}/policytags/{{policytagID}})", external)
	}
	parent = &PolicyTagParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
