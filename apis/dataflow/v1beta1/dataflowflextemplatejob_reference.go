...
// DataflowFlexTemplateJobRef defines the resource reference to DataflowFlexTemplateJob, which "External" field
// holds the GCP identifier for the KRM object.
type DataflowFlexTemplateJobRef struct {
	// A reference to an externally managed DataflowFlexTemplateJob resource. Should be in the format "projects/{{projectID}}/locations/{{location}}/jobs/{{jobID}}"
...
func init() {
	refs.Register(&DataflowFlexTemplateJobRef{})
}
...
func (r *DataflowFlexTemplateJobRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		id, err := getIdentityFromDataflowFlexTemplateJobSpec(ctx, reader, u)
		if err != nil || id == nil {
			return ""
		}
		return id.String()
