...
// GatewayIdentity defines the resource reference to ApigatewayGateway, which "External" field
...
func NewGatewayIdentity(ctx context.Context, reader client.Reader, obj *ApigatewayGateway) (*GatewayIdentity, error) {
...
func ParseGatewayExternal(external string) (parent *GatewayParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "gateways" {
		return nil, "", fmt.Errorf("format of ApigatewayGateway external=%q was not known (use projects/{{projectID}}/locations/{{location}}/gateways/{{gatewayID}})", external)
	}
	parent = &GatewayParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
