...
func (r *ReservationGroupRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on ReservationGroup reference")
	}
	// From given External
	if r.External != "" {
	...
	gvk := schema.GroupVersionKind{
		...
		Version: "v1alpha1",
		Kind:    "BigQueryReservationReservationGroup",
	}
...
func ParseReservationGroupExternal(external string) (parent *ReservationParent, resourceID string, err error) {
