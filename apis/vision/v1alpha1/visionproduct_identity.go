...
func getIdentityFromVisionProductSpec(ctx context.Context, reader client.Reader, obj *VisionProduct) (*VisionProductIdentity, error) {
...
func (obj *VisionProduct) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	...
	if externalRef != "" {
		...
		if !strings.Contains(externalRef, "/") {
			if externalRef != specIdentity.Product {
				return nil, fmt.Errorf("cannot change VisionProduct identity (old=%q, new=%q)", externalRef, specIdentity.Product)
			}
		} else {
