...
// ProviderIdentity defines the resource reference to ConnectorsProvider, which "External" field
...
func NewProviderIdentity(ctx context.Context, reader client.Reader, obj *ConnectorsProvider) (*ProviderIdentity, error) {
...
func ParseProviderExternal(external string) (parent *ProviderParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "providers" {
		return nil, "", fmt.Errorf("format of ConnectorsProvider external=%q was not known (use projects/{{projectID}}/locations/{{location}}/providers/{{providerID}})", external)
	}
	parent = &ProviderParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
