...
var EdgeContainerClusterGVK = schema.GroupVersionKind{
...
// ClusterRef is a reference to an EdgeContainerCluster.
type ClusterRef struct {
	...
	// The name of an EdgeContainerCluster resource.
	Name string `json:"name,omitempty"`

	// The namespace of an EdgeContainerCluster resource.
...
func ParseClusterExternal(external string) (parent *MachineParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "clusters" {
		return nil, "", fmt.Errorf("format of EdgeContainerCluster external=%q was not known (use projects/{{projectID}}/locations/{{location}}/clusters/{{clusterID}})", external)
	}
