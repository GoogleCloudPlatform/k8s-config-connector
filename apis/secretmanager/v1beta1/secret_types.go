...
type SecretManagerSecretSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="self == null || (size(self) <= 64 && self.keys().all(k, k.matches('^[a-zA-Z][a-zA-Z0-9_-]{0,62}$')) && self.values().all(v, v.matches('^[a-zA-Z0-9_-]{0,63}$')))"
