...
// TopicIdentity defines the resource reference to SecretmanagerTopic, which "External" field
...
func NewTopicIdentity(ctx context.Context, reader client.Reader, obj *SecretmanagerTopic) (*TopicIdentity, error) {
...
func ParseTopicExternal(external string) (parent *TopicParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "topics" {
		return nil, "", fmt.Errorf("format of SecretmanagerTopic external=%q was not known (use projects/{{projectID}}/locations/{{location}}/topics/{{topicID}})", external)
	}
	parent = &TopicParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
