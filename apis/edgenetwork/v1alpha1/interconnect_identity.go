...
// InterconnectIdentity defines the resource reference to EdgenetworkInterconnect, which "External" field
...
func NewInterconnectIdentity(ctx context.Context, reader client.Reader, obj *EdgenetworkInterconnect) (*InterconnectIdentity, error) {
...
func ParseInterconnectExternal(external string) (parent *InterconnectParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "interconnects" {
		return nil, "", fmt.Errorf("format of EdgenetworkInterconnect external=%q was not known (use projects/{{projectID}}/locations/{{location}}/interconnects/{{interconnectID}})", external)
	}
	parent = &InterconnectParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
