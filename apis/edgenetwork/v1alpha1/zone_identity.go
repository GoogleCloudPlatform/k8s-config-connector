...
// ZoneIdentity defines the resource reference to EdgenetworkZone, which "External" field
...
func NewZoneIdentity(ctx context.Context, reader client.Reader, obj *EdgenetworkZone) (*ZoneIdentity, error) {
...
func ParseZoneExternal(external string) (parent *ZoneParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "zones" {
		return nil, "", fmt.Errorf("format of EdgenetworkZone external=%q was not known (use projects/{{projectID}}/locations/{{location}}/zones/{{zoneID}})", external)
	}
	parent = &ZoneParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
