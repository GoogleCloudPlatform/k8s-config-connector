...
var _ reference.ExternalNormalizer = &MembershipRef{}
...
// If the "External" is given in the other resource's spec.CloudIdentityMembershipRef, the given value will be used.
// Otherwise, the "Name" and "Namespace" will be used to query the actual CloudIdentityMembership object from the cluster.
func (r *MembershipRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", CloudIdentityMembershipGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseMembershipExternal(r.External); err != nil {
			return "", err
		}
	...
	u.SetGroupVersionKind(CloudIdentityMembershipGVK)
	if err := reader.Get(ctx, key, u); err != nil {
		if apierrors.IsNotFound(err) {
			return "", k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
		}
		return "", fmt.Errorf("reading referenced %s %s: %w", CloudIdentityMembershipGVK, key, err)
	}
