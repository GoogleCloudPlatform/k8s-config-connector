...
func getIdentityFromMarketingPlatformAdminAnalyticsAccountLinkSpec(ctx context.Context, reader client.Reader, obj *MarketingPlatformAdminAnalyticsAccountLink) (*MarketingPlatformAdminAnalyticsAccountLinkIdentity, error) {
	if obj.Spec.AnalyticsAccountRef == nil {
		return nil, fmt.Errorf("spec.analyticsAccountRef is required")
	}

	analyticsAccountExternal, err := obj.Spec.AnalyticsAccountRef.NormalizedExternal(ctx, reader, obj.GetNamespace())
	if err != nil {
		return nil, fmt.Errorf("resolving spec.analyticsAccountRef: %w", err)
	}

	analyticsAccountID, err := analyticsv1alpha1.ParseAccountExternal(analyticsAccountExternal)
	if err != nil {
		return nil, fmt.Errorf("parsing analyticsAccountRef external: %w", err)
	}
