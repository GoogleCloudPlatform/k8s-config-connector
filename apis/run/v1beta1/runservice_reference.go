...
var _ refsv1beta1.ExternalNormalizer = &RunServiceRef{}
...
// NormalizedExternal provision the "External" value for other resource that depends on RunService.
// If the "External" is given in the other resource's spec.RunServiceRef, the given value will be used.
// Otherwise, the "Name" and "Namespace" will be used to query the actual RunService object from the cluster.
func (r *RunServiceRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", RunServiceGVK.Kind)
	}
	// From given External
	if r.External != "" {
	...
	project, err := refsv1beta1.ResolveProject(ctx, reader, r.Namespace, projectRef)
	if err != nil {
