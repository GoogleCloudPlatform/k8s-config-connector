...
// CloudTalentSolutionCompanyIdentity represents the GCP Identity of a CloudTalentSolutionCompany resource.
...
func (i *CloudTalentSolutionCompanyIdentity) ID() string {
	return i.Company
}
...
func getIdentityFromCloudTalentSolutionCompanySpec(ctx context.Context, reader client.Reader, obj *CloudTalentSolutionCompany) (*CloudTalentSolutionCompanyIdentity, error) {
	if obj.Spec.TenantRef == nil {
		return nil, fmt.Errorf("spec.tenantRef is required")
	}

	tenantRef := obj.Spec.TenantRef.DeepCopy()
	if err := tenantRef.Normalize(ctx, reader, obj.GetNamespace()); err != nil {
		return nil, err
	}

	tenantIdentity, err := tenantRef.ParseExternalToIdentity()
	if err != nil {
		return nil, err
	}
	if tenantIdentity == nil {
		return nil, fmt.Errorf("spec.tenantRef external value is empty or invalid")
	}
	tenantId := tenantIdentity.(*CloudTalentSolutionTenantIdentity)
	...
	return &CloudTalentSolutionCompanyIdentity{
...
func getIdentityFromCloudTalentSolutionCompanyUnstructured(ctx context.Context, reader client.Reader, obj *unstructured.Unstructured) (*CloudTalentSolutionCompanyIdentity, error) {
	structuredObj, err := common.ToStructuredType[*CloudTalentSolutionCompany](obj)
	if err != nil {
		return nil, fmt.Errorf("converting unstructured to CloudTalentSolutionCompany: %w", err)
	}
	return getIdentityFromCloudTalentSolutionCompanySpec(ctx, reader, structuredObj)
}
