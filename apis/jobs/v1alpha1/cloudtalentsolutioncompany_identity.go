...
func getIdentityFromCloudTalentSolutionCompanySpec(ctx context.Context, reader client.Reader, obj client.Object) (*CloudTalentSolutionCompanyIdentity, error) {
	company := obj.(*CloudTalentSolutionCompany)
	...
	if company.Spec.TenantRef == nil {
		return nil, fmt.Errorf("spec.tenantRef is required")
	}

	tenantRef := company.Spec.TenantRef
	if err := tenantRef.Normalize(ctx, reader, company.Namespace); err != nil {
		return nil, fmt.Errorf("normalizing spec.tenantRef: %w", err)
	}

	tenantIdentity, err := tenantRef.ParseExternalToIdentity()
	if err != nil {
		return nil, fmt.Errorf("parsing spec.tenantRef identity: %w", err)
	}

	if tenantIdentity == nil {
		return nil, fmt.Errorf("spec.tenantRef external reference is required (e.g. projects/{{project}}/tenants/{{tenant}})")
	}

	ti := tenantIdentity.(*TenantIdentity)
