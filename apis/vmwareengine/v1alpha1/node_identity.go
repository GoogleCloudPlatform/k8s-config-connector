...
// NodeIdentity defines the resource reference to VmwareengineNode, which "External" field
...
func NewNodeIdentity(ctx context.Context, reader client.Reader, obj *VmwareengineNode) (*NodeIdentity, error) {
...
func ParseNodeExternal(external string) (parent *NodeParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "nodes" {
		return nil, "", fmt.Errorf("format of VmwareengineNode external=%q was not known (use projects/{{projectID}}/locations/{{location}}/nodes/{{nodeID}})", external)
	}
	parent = &NodeParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
