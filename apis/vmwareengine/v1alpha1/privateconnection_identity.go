...
// PrivateConnectionIdentity defines the resource reference to VmwareenginePrivateConnection, which "External" field
...
func NewPrivateConnectionIdentity(ctx context.Context, reader client.Reader, obj *VmwareenginePrivateConnection) (*PrivateConnectionIdentity, error) {
...
func ParsePrivateConnectionExternal(external string) (parent *PrivateConnectionParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "privateconnections" {
		return nil, "", fmt.Errorf("format of VmwareenginePrivateConnection external=%q was not known (use projects/{{projectID}}/locations/{{location}}/privateconnections/{{privateconnectionID}})", external)
	}
	parent = &PrivateConnectionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
