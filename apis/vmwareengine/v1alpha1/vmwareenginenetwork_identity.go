...
// VmwareEngineNetworkIdentity defines the resource reference to VmwareengineVmwareEngineNetwork, which "External" field
...
func NewVmwareEngineNetworkIdentity(ctx context.Context, reader client.Reader, obj *VmwareengineVmwareEngineNetwork) (*VmwareEngineNetworkIdentity, error) {
...
func ParseVmwareEngineNetworkExternal(external string) (parent *VmwareEngineNetworkParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "vmwareenginenetworks" {
		return nil, "", fmt.Errorf("format of VmwareengineVmwareEngineNetwork external=%q was not known (use projects/{{projectID}}/locations/{{location}}/vmwareenginenetworks/{{vmwareenginenetworkID}})", external)
	}
	parent = &VmwareEngineNetworkParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
