...
// ClusterIdentity defines the resource reference to VmwareengineCluster, which "External" field
...
func NewClusterIdentity(ctx context.Context, reader client.Reader, obj *VmwareengineCluster) (*ClusterIdentity, error) {
...
func ParseClusterExternal(external string) (parent *ClusterParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "clusters" {
		return nil, "", fmt.Errorf("format of VmwareengineCluster external=%q was not known (use projects/{{projectID}}/locations/{{location}}/clusters/{{clusterID}})", external)
	}
	parent = &ClusterParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
