...
// NormalizedExternal provision the "External" value for other resource that depends on BigtableInstance.
// If the "External" is given in the other resource's spec.BigtableInstanceRef, the given value will be used.
// Otherwise, the "Name" and "Namespace" will be used to query the actual BigtableInstance object from the cluster.
func (r *InstanceRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", BigtableInstanceGVK.Kind)
	}
	// From given External
	...
	u.SetGroupVersionKind(BigtableInstanceGVK)
