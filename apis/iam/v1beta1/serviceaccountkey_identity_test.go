...
func TestServiceAccountKeyParse(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: &ServiceAccountKeyIdentity{
				ServiceAccountKeyParent: ServiceAccountKeyParent{
					Project: "myProject",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got.ServiceAccountKeyParent.Project != tc.want.ServiceAccountKeyParent.Project {
				t.Errorf("FromExternal(%q).ServiceAccountKeyParent.Project = %q, want %q",
					tc.input, got.ServiceAccountKeyParent.Project, tc.want.ServiceAccountKeyParent.Project)
			}
			if got.ServiceAccountKeyParent.Account != tc.want.ServiceAccountKeyParent.Account {
				t.Errorf("FromExternal(%q).ServiceAccountKeyParent.Account = %q, want %q",
					tc.input, got.ServiceAccountKeyParent.Account, tc.want.ServiceAccountKeyParent.Account)
			}
