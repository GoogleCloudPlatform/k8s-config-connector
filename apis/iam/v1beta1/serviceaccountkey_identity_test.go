...
func TestServiceAccountKeyParse(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: &ServiceAccountKeyIdentity{
				parent: &ServiceAccountKeyParent{
					ProjectID:        "myProject",
					ServiceAccountID: "my-sa@myProject.iam.gserviceaccount.com",
				},
				id: "mykey123",
			},
			wantError: false,
		},
		{
			...
			want: &ServiceAccountKeyIdentity{
				parent: &ServiceAccountKeyParent{
					ProjectID:        "p1",
					ServiceAccountID: "sa@p1.iam.gserviceaccount.com",
				},
				id: "k1",
			},
			wantError: false,
		},
		{
			...
			want: &ServiceAccountKeyIdentity{
				parent: &ServiceAccountKeyParent{
					ProjectID:        "first",
					ServiceAccountID: "second@first.iam.gserviceaccount.com",
				},
				id: "third",
			},
			wantError: false,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantError {
				if err == nil {
					t.Errorf("ParseServiceAccountKeyExternal(%q) expected error but got none", tc.input)
				}
				return
			}
			if err != nil {
				t.Errorf("ParseServiceAccountKeyExternal(%q) unexpected error: %v", tc.input, err)
				return
			}
			if got == nil {
				t.Errorf("ParseServiceAccountKeyExternal(%q) returned nil", tc.input)
				return
			}
			if got.parent.ProjectID != tc.want.parent.ProjectID {
				t.Errorf("ParseServiceAccountKeyExternal(%q).parent.ProjectID = %q, want %q", tc.input, got.parent.ProjectID, tc.want.parent.ProjectID)
			}
			if got.parent.ServiceAccountID != tc.want.parent.ServiceAccountID {
				t.Errorf("ParseServiceAccountKeyExternal(%q).parent.ServiceAccountID = %q, want %q", tc.input, got.parent.ServiceAccountID, tc.want.parent.ServiceAccountID)
			}
			if got.id != tc.want.id {
				t.Errorf("ParseServiceAccountKeyExternal(%q).id = %q, want %q", tc.input, got.id, tc.want.id)
			}
		})
	}
}
