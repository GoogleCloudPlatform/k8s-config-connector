...
var WorkloadIdentityPoolGVK = GroupVersion.WithKind("WorkloadIdentityPool")

type WorkloadIdentityPoolRef struct {
	// The GKE cluster. Valid formats:
	//  `projects/{projectID}/locations/{location}/workloadIdentityPools/{workloadIdentityPool}`
	External string `json:"external,omitempty"`

	// Name of the project resource.
	Name string `json:"name,omitempty"`

	// Namespace of the project resource.
...
func (r *WorkloadIdentityPoolRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", WorkloadIdentityPoolGVK.Kind)
	}
	...
	u.SetGroupVersionKind(WorkloadIdentityPoolGVK)
	if err := reader.Get(ctx, key, u); err != nil {
		if apierrors.IsNotFound(err) {
			return "", k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
		}
		return "", fmt.Errorf("reading referenced %s %s: %w", WorkloadIdentityPoolGVK, key, err)
	}
