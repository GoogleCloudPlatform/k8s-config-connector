...
func ParsePolicyBindingExternal(external string) (parent *PolicyBindingParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "policybindings" {
		return nil, "", fmt.Errorf("format of IAMPolicyBinding external=%q was not known (use projects/{{projectID}}/locations/{{location}}/policybindings/{{policybindingID}})", external)
	}
	parent = &PolicyBindingParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
