...
// EnvironmentIdentity defines the resource reference to ShellEnvironment, which "External" field
...
func NewEnvironmentIdentity(ctx context.Context, reader client.Reader, obj *ShellEnvironment) (*EnvironmentIdentity, error) {
...
func ParseEnvironmentExternal(external string) (parent *EnvironmentParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "environments" {
		return nil, "", fmt.Errorf("format of ShellEnvironment external=%q was not known (use projects/{{projectID}}/locations/{{location}}/environments/{{environmentID}})", external)
	}
	parent = &EnvironmentParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
