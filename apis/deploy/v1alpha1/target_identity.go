...
func NewTargetIdentity(ctx context.Context, reader client.Reader, obj *DeployTarget) (*TargetIdentity, error) {
...
func ParseTargetExternal(external string) (parent *TargetParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "targets" {
		return nil, "", fmt.Errorf("format of DeployTarget external=%q was not known (use projects/{{projectID}}/locations/{{location}}/targets/{{targetID}})", external)
	}
	parent = &TargetParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
