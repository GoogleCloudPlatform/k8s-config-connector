...
# We need a newer googleapis to get appoptimize
...
if [ -d "${REPO_ROOT}/pkg/controller/direct/appoptimize" ]; then
  go run -mod=readonly golang.org/x/tools/cmd/goimports@${GOLANG_X_TOOLS_VERSION} -w pkg/controller/direct/appoptimize/
fi
