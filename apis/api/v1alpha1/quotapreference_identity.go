...
func NewQuotaPreferenceIdentity(ctx context.Context, reader client.Reader, obj *ApiQuotaPreference) (*QuotaPreferenceIdentity, error) {
	...
	// Get Parent
	projectRefStruct := &refsv1beta1.ProjectRef{External: obj.Spec.ProjectRef.External}
	projectRef, err := refsv1beta1.ResolveProject(ctx, reader, obj.GetNamespace(), projectRefStruct)
	if err != nil {
		return nil, err
	}
...
func ParseQuotaPreferenceExternal(external string) (parent *QuotaPreferenceParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "quotapreferences" {
		return nil, "", fmt.Errorf("format of ApiQuotaPreference external=%q was not known (use projects/{{projectID}}/locations/{{location}}/quotapreferences/{{quotapreferenceID}})", external)
	}
	parent = &QuotaPreferenceParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
