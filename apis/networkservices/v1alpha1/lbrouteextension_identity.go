...
func (i *LbRouteExtensionIdentity) Parent() *LbRouteExtensionParent {
	return  i.parent
}
...
// New builds a LbRouteExtensionIdentity from the Config Connector LbRouteExtension object.
func NewLbRouteExtensionIdentity(ctx context.Context, reader client.Reader, obj *NetworkservicesLbRouteExtension) (*LbRouteExtensionIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseLbRouteExtensionExternal(externalRef)
		if err != nil {
			return nil, err
		}
	...
	return &LbRouteExtensionIdentity{
		parent: &LbRouteExtensionParent{
			ProjectID: projectID,
			Location:  location,
		},
...
func ParseLbRouteExtensionExternal(external string) (parent *LbRouteExtensionParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "lbrouteextensions" {
		return nil, "", fmt.Errorf("format of NetworkservicesLbRouteExtension external=%q was not known (use projects/{{projectID}}/locations/{{location}}/lbrouteextensions/{{lbrouteextensionID}})", external)
	}
	parent = &LbRouteExtensionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
