...
// NewEdgeCacheServiceIdentity builds an EdgeCacheServiceIdentity from the Config Connector EdgeCacheService object.
...
func ParseEdgeCacheServiceExternal(external string) (parent *EdgeCacheServiceParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "edgeCacheServices" {
		return nil, "", fmt.Errorf("format of NetworkServicesEdgeCacheService external=%q was not known (use projects/{{projectID}}/locations/{{location}}/edgeCacheServices/{{edgeCacheServiceID}})", external)
	}
	parent = &EdgeCacheServiceParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
