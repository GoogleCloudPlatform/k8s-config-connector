...
type VirtualMachineRef struct {
	...
	// The name of a TPUVirtualMachine resource.
	Name string `json:"name,omitempty"`

	// The namespace of a TPUVirtualMachine resource.
...
func (r *VirtualMachineRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	u.SetGroupVersionKind(TPUVirtualMachineGVK)
	if err := reader.Get(ctx, key, u); err != nil {
		if apierrors.IsNotFound(err) {
			return "", k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
		}
		return "", fmt.Errorf("reading referenced %s %s: %w", TPUVirtualMachineGVK, key, err)
	}
