...
func TestNetworkConnectivityRemoteTransportProfileIdentity_FromExternal(t *testing.T) {
	tests := []struct {
		name         string
		ref          string
		wantIdentity NetworkConnectivityRemoteTransportProfileIdentity
		wantErr      bool
	}{
		{
			name: "valid external ref",
			...
			wantIdentity: NetworkConnectivityRemoteTransportProfileIdentity{
				Project:                "my-project",
				Location:               "us-central1",
				RemoteTransportProfile: "my-profile",
			},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			i := &NetworkConnectivityRemoteTransportProfileIdentity{}
			if err := i.FromExternal(tt.ref); (err != nil) != tt.wantErr {
				t.Fatalf("NetworkConnectivityRemoteTransportProfileIdentity.FromExternal() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr {
				if diff := cmp.Diff(tt.wantIdentity, *i); diff != "" {
					t.Errorf("NetworkConnectivityRemoteTransportProfileIdentity.FromExternal() mismatch (-want +got):\n%s", diff)
				}
				if got := i.String(); got != tt.ref {
					t.Errorf("NetworkConnectivityRemoteTransportProfileIdentity.String() = %v, want %v", got, tt.ref)
				}
				wantParent := "projects/my-project/locations/us-central1"
				if gotParent := i.ParentString(); gotParent != wantParent {
					t.Errorf("NetworkConnectivityRemoteTransportProfileIdentity.ParentString() = %v, want %v", gotParent, wantParent)
				}
			}
		})
	}
