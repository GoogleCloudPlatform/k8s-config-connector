...
type NetworkConnectivityMulticloudDataTransferConfigSpec struct {
	...
	// Optional. Maps services to their current or planned states. Service names are keys, and the associated values describe the state of the service.
	// +kubebuilder:validation:Optional
	Services map[string]StateTimeline `json:"services,omitempty"`
}

// +kcc:spec:proto=mockgcp.cloud.networkconnectivity.v1.StateTimeline
...
type NetworkConnectivityMulticloudDataTransferConfigObservedState struct {
	...
	Services map[string]StateTimelineObservedState `json:"services,omitempty"`
}

// +kcc:observedstate:proto=mockgcp.cloud.networkconnectivity.v1.StateTimeline
type StateTimelineObservedState struct {
	// Output only. The state and activation time details of the resource state.
	States []StateMetadataObservedState `json:"states,omitempty"`
}

// +kcc:observedstate:proto=mockgcp.cloud.networkconnectivity.v1.StateMetadata
type StateMetadataObservedState struct {
