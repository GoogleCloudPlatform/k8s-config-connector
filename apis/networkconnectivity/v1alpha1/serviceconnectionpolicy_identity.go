...
var (
	...
	_ identity.Resource   = &NetworkConnectivityServiceConnectionPolicy{}
)
...
func getIdentityFromNetworkConnectivityServiceConnectionPolicySpec(ctx context.Context, reader client.Reader, obj client.Object) (*NetworkConnectivityServiceConnectionPolicyIdentity, error) {
	...
	identity := &NetworkConnectivityServiceConnectionPolicyIdentity{
		Project:                 projectID,
		Location:                location,
		ServiceConnectionPolicy: resourceID,
	}
	return identity, nil
}

func (obj *NetworkConnectivityServiceConnectionPolicy) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	specIdentity, err := getIdentityFromNetworkConnectivityServiceConnectionPolicySpec(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
	...
	if externalRef != "" {
		...
		statusIdentity := &NetworkConnectivityServiceConnectionPolicyIdentity{}
		if err := statusIdentity.FromExternal(externalRef); err != nil {
			return nil, err
		}

		if statusIdentity.String() != specIdentity.String() {
			return nil, fmt.Errorf("cannot change NetworkConnectivityServiceConnectionPolicy identity (old=%q, new=%q)", statusIdentity.String(), specIdentity.String())
		}
	}
