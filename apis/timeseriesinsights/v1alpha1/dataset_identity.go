...
// DataSetIdentity defines the resource reference to TimeseriesinsightsDataSet, which "External" field
...
func NewDataSetIdentity(ctx context.Context, reader client.Reader, obj *TimeseriesinsightsDataSet) (*DataSetIdentity, error) {
...
func ParseDataSetExternal(external string) (parent *DataSetParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "datasets" {
		return nil, "", fmt.Errorf("format of TimeseriesinsightsDataSet external=%q was not known (use projects/{{projectID}}/locations/{{location}}/datasets/{{datasetID}})", external)
	}
	parent = &DataSetParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
