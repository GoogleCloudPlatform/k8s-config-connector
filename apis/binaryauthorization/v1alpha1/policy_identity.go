...
// PolicyIdentity defines the resource reference to BinaryauthorizationPolicy, which "External" field
...
func NewPolicyIdentity(ctx context.Context, reader client.Reader, obj *BinaryauthorizationPolicy) (*PolicyIdentity, error) {
...
func ParsePolicyExternal(external string) (parent *PolicyParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "policys" {
		return nil, "", fmt.Errorf("format of BinaryauthorizationPolicy external=%q was not known (use projects/{{projectID}}/locations/{{location}}/policys/{{policyID}})", external)
	}
	parent = &PolicyParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
