...
var VertexAINotebookExecutionJobIdentityFormat = gcpurls.Template[VertexAINotebookExecutionJobIdentity]("aiplatform.googleapis.com", "projects/{project}/locations/{location}/notebookExecutionJobs/{notebook_execution_job}")
...
func getIdentityFromVertexAINotebookExecutionJobSpec(ctx context.Context, reader client.Reader, obj client.Object) (*VertexAINotebookExecutionJobIdentity, error) {
	...
	// Read location directly from the Spec. Since notebookexecutionjob_types.go uses location string directly:
	kccObj, ok := obj.(*VertexAINotebookExecutionJob)
	if !ok {
		return nil, fmt.Errorf("object is not a VertexAINotebookExecutionJob")
	}
	location := kccObj.Spec.Location
	if location == "" {
	...
	identity := &VertexAINotebookExecutionJobIdentity{
		Project:                projectID,
		Location:               location,
		Notebook_Execution_Job: resourceID,
	}
	return identity, nil
}
