...
// EffectiveSecurityHealthAnalyticsCustomModuleIdentity defines the resource reference to SecuritycentermanagementEffectiveSecurityHealthAnalyticsCustomModule, which "External" field
...
func NewEffectiveSecurityHealthAnalyticsCustomModuleIdentity(ctx context.Context, reader client.Reader, obj *SecuritycentermanagementEffectiveSecurityHealthAnalyticsCustomModule) (*EffectiveSecurityHealthAnalyticsCustomModuleIdentity, error) {
...
func ParseEffectiveSecurityHealthAnalyticsCustomModuleExternal(external string) (parent *EffectiveSecurityHealthAnalyticsCustomModuleParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "effectivesecurityhealthanalyticscustommodules" {
		return nil, "", fmt.Errorf("format of SecuritycentermanagementEffectiveSecurityHealthAnalyticsCustomModule external=%q was not known (use projects/{{projectID}}/locations/{{location}}/effectivesecurityhealthanalyticscustommodules/{{effectivesecurityhealthanalyticscustommoduleID}})", external)
	}
	parent = &EffectiveSecurityHealthAnalyticsCustomModuleParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
