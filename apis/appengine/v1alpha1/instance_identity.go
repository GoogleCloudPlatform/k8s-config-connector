...
// InstanceIdentity defines the resource reference to AppengineInstance, which "External" field
...
func NewInstanceIdentity(ctx context.Context, reader client.Reader, obj *AppengineInstance) (*InstanceIdentity, error) {
...
func ParseInstanceExternal(external string) (parent *InstanceParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "instances" {
		return nil, "", fmt.Errorf("format of AppengineInstance external=%q was not known (use projects/{{projectID}}/locations/{{location}}/instances/{{instanceID}})", external)
	}
	parent = &InstanceParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
