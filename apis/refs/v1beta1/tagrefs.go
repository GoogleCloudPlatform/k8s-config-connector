...
type TagValueRef struct {
	...
	// `[CONTAINER_ID]/[tag_key_shortname]` for the tag key holding the value, where `CONTAINER_ID` is `[project_id]` or `[org_id]`.
...
type TagKeyRef struct {
	//  If provided must be in the format `[CONTAINER_ID]/[tag_key_id]`, where `CONTAINER_ID` is `[project_id]` or `[org_id]`.
	External string `json:"external,omitempty"`
...
func ResolveTagKeyRef(ctx context.Context, reader client.Reader, src client.Object, ref *TagKeyRef) (*TagKey, error) {
	...
	// External should be in the `[CONTAINER_ID]/[tag_key_id]` format
	if ref.External != "" {
		tokens := strings.Split(ref.External, "/")
		if len(tokens) == 2 {
			ref = &TagKeyRef{
		...
		return nil, fmt.Errorf("format of TagKeyRef external=%q was not known (use [CONTAINER_ID]/[tag_key_id])", ref.External)
	}
