...
type BigQueryDatasetTableRef struct {
...
func ResolveBigQueryTable(ctx context.Context, reader client.Reader, src client.Object, ref *BigQueryDatasetTableRef) (*BigQueryTable, error) {
	if ref == nil {
		return nil, nil
	}

	if ref.Name == "" && ref.External == "" {
		return nil, fmt.Errorf("must specify either name or external on BigQueryDatasetTableRef")
	}
	if ref.Name != "" && ref.External != "" {
		return nil, fmt.Errorf("cannot specify both name and external on BigQueryDatasetTableRef")
	}
	...
	if ref.External != "" {
		...
		return nil, fmt.Errorf("format of BigQueryDatasetTableRef external=%q was not known (use projects/{projectId}/datasets/{datasetId}/tables/{tableId})", ref.External)
