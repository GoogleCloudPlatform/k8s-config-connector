...
func (r *refNormalizer) VisitField(path string, v any) error {
	...
	if kmsCryptoKeyRef, ok := v.(*refs.KMSCryptoKeyRef); ok {
		resolved, err := refs.ResolveKMSCryptoKeyRef(r.ctx, r.kube, r.src, kmsCryptoKeyRef)
		if err != nil {
			return err
		}
		*kmsCryptoKeyRef = *resolved
	}
...
func ResolveReferences(ctx context.Context, kube client.Reader, obj client.Object, projectRef *refs.Project) error {
