...
func (r *StorageBucketRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	// From given External
	if r.External != "" {
		if _, err := ParseStorageBucketExternal(r.External); err != nil {
			return "", err
		}
...
func ParseStorageBucketExternal(external string) (*StorageBucketIdentity, error) {
	...
	if len(tokens) != 4 || tokens[0] != "projects" || tokens[2] != "buckets" {
		return nil, fmt.Errorf("format of SpannerInstance external=%q was not known (use projects/{{projectId}}/buckets/{{bucketID}})", external)
	}
