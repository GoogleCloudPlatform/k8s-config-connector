...
// AccessContextManagerAccessLevelIdentity represents the identity of an accessLevel.
...
func getIdentityFromAccessContextManagerAccessLevelSpec(ctx context.Context, reader client.Reader, obj client.Object) (*AccessContextManagerAccessLevelIdentity, error) {
	...
	if typedObj, ok := obj.(*AccessContextManagerAccessLevel); ok {
		accessPolicyRef = typedObj.Spec.AccessPolicyRef
	} else if unstrObj, ok := obj.(*unstructured.Unstructured); ok {
		refValue, ok, _ := unstructured.NestedMap(unstrObj.Object, "spec", "accessPolicyRef")
		if ok {
			accessPolicyRef = &AccessPolicyRef{}
			if external, ok := refValue["external"].(string); ok {
				accessPolicyRef.External = external
			}
			if name, ok := refValue["name"].(string); ok {
				accessPolicyRef.Name = name
			}
			if namespace, ok := refValue["namespace"].(string); ok {
				accessPolicyRef.Namespace = namespace
			}
		}
	}
	...
	normalizedAccessPolicy, err := accessPolicyRef.NormalizedExternal(ctx, reader, obj.GetNamespace())
	if err != nil {
		return nil, err
	}

	accessPolicyID := strings.TrimPrefix(normalizedAccessPolicy, "accessPolicies/")
