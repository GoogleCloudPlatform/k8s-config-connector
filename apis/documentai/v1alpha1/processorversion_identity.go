...
// ProcessorVersionIdentity defines the resource reference to DocumentaiProcessorVersion, which "External" field
...
func ParseProcessorVersionExternal(external string) (parent *ProcessorVersionParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "processorversions" {
		return nil, "", fmt.Errorf("format of DocumentaiProcessorVersion external=%q was not known (use projects/{{projectID}}/locations/{{location}}/processorversions/{{processorversionID}})", external)
	}
	parent = &ProcessorVersionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
