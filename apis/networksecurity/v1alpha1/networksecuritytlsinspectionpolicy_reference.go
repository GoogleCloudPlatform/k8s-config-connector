...
// NetworkSecurityTLSInspectionPolicyRef is a reference to a NetworkSecurityTLSInspectionPolicy.
type NetworkSecurityTLSInspectionPolicyRef struct {
	...
	// Should be in the format "projects/{{projectID}}/locations/{{location}}/tlsInspectionPolicies/{{tlsInspectionPolicyID}}".
...
func init() {
	refs.Register(&NetworkSecurityTLSInspectionPolicyRef{}, &NetworkSecurityTLSInspectionPolicy{})
}
...
func (r *NetworkSecurityTLSInspectionPolicyRef) SetExternal(ref string) {
	r.External = ref
	r.Name = ""
	r.Namespace = ""
}

func (r *NetworkSecurityTLSInspectionPolicyRef) ValidateExternal(ref string) error {
...
func (r *NetworkSecurityTLSInspectionPolicyRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		structured, err := common.ToStructuredType[*NetworkSecurityTLSInspectionPolicy](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromNetworkSecurityTLSInspectionPolicySpec(ctx, reader, structured)
		if err != nil {
			return ""
		}
