...
func NewChannelIdentity(ctx context.Context, reader client.Reader, obj *EventarcChannel) (*ChannelIdentity, error) {
...
func ParseChannelExternal(external string) (parent *ChannelParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "channels" {
		return nil, "", fmt.Errorf("format of EventarcChannel external=%q was not known (use projects/{{projectID}}/locations/{{location}}/channels/{{channelID}})", external)
	}
	parent = &ChannelParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
...
var _ identity.Identity = &ChannelIdentity{}
