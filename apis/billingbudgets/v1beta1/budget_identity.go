...
// BudgetIdentity defines the resource reference to BillingBudgetsBudget, which "External" field
...
func NewBudgetIdentity(ctx context.Context, reader client.Reader, obj *BillingBudgetsBudget) (*BudgetIdentity, error) {
...
func ParseBudgetExternal(external string) (parent *BudgetParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "budgets" {
		return nil, "", fmt.Errorf("format of BillingBudgetsBudget external=%q was not known (use projects/{{projectID}}/locations/{{location}}/budgets/{{budgetID}})", external)
	}
	parent = &BudgetParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
