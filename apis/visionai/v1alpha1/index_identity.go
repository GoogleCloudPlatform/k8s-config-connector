...
// IndexIdentity defines the resource reference to VisionaiIndex, which "External" field
...
func NewIndexIdentity(ctx context.Context, reader client.Reader, obj *VisionaiIndex) (*IndexIdentity, error) {
...
func ParseIndexExternal(external string) (parent *IndexParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "indexs" {
		return nil, "", fmt.Errorf("format of VisionaiIndex external=%q was not known (use projects/{{projectID}}/locations/{{location}}/indexs/{{indexID}})", external)
	}
	parent = &IndexParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
