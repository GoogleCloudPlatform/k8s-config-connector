...
// ProcessorIdentity defines the resource reference to VisionaiProcessor, which "External" field
...
func NewProcessorIdentity(ctx context.Context, reader client.Reader, obj *VisionaiProcessor) (*ProcessorIdentity, error) {
...
func ParseProcessorExternal(external string) (parent *ProcessorParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "processors" {
		return nil, "", fmt.Errorf("format of VisionaiProcessor external=%q was not known (use projects/{{projectID}}/locations/{{location}}/processors/{{processorID}})", external)
	}
	parent = &ProcessorParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
