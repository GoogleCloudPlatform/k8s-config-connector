...
// ClusterIdentity defines the resource reference to VisionaiCluster, which "External" field
...
func NewClusterIdentity(ctx context.Context, reader client.Reader, obj *VisionaiCluster) (*ClusterIdentity, error) {
...
func ParseClusterExternal(external string) (parent *ClusterParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "clusters" {
		return nil, "", fmt.Errorf("format of VisionaiCluster external=%q was not known (use projects/{{projectID}}/locations/{{location}}/clusters/{{clusterID}})", external)
	}
	parent = &ClusterParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
