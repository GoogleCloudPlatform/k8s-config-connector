...
func ParseApplicationExternal(external string) (parent *ApplicationParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "applications" {
		return nil, "", fmt.Errorf("format of AppHubApplication external=%q was not known (use projects/{{projectID}}/locations/{{location}}/applications/{{applicationID}})", external)
	}
	parent = &ApplicationParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
