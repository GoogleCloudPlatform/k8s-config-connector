...
go run . generate-mapper --service google.cloud.contentwarehouse.v1 --api-version contentwarehouse.cnrm.cloud.google.com/v1alpha1

cd "${REPO_ROOT}"
dev/tasks/generate-crds

if [ -d "${REPO_ROOT}/pkg/controller/direct/contentwarehouse" ]; then
  go run -mod=readonly golang.org/x/tools/cmd/goimports@${GOLANG_X_TOOLS_VERSION} -w pkg/controller/direct/contentwarehouse/
fi
