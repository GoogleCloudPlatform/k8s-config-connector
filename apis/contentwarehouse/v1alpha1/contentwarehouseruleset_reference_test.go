...
func TestContentWarehouseRuleSetRef_ValidateExternal(t *testing.T) {
	tests := []struct {
		name    string
		ref     string
		wantErr bool
		...
		{
			name:    "invalid prefix",
			ref:     "invalid/my-project/locations/us-central1/ruleSets/my-ruleset",
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := &v1alpha1.ContentWarehouseRuleSetRef{}
			if err := r.ValidateExternal(tt.ref); (err != nil) != tt.wantErr {
				t.Errorf("ContentWarehouseRuleSetRef.ValidateExternal() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
