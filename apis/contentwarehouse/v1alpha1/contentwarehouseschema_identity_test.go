...
func TestContentWarehouseSchemaIdentity_FromExternal(t *testing.T) {
	tests := []struct {
		name     string
		external string
		want     *ContentWarehouseSchemaIdentity
		wantErr  bool
	}{
		{
			name:     "canonical format",
			external: "projects/my-project/locations/us-central1/documentSchemas/my-schema",
			want: &ContentWarehouseSchemaIdentity{
				Project:        "my-project",
				Location:       "us-central1",
				DocumentSchema: "my-schema",
			},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			i := &ContentWarehouseSchemaIdentity{}
