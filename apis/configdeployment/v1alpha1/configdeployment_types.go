...
var ConfigDeploymentGVK = GroupVersion.WithKind("ConfigDeployment")
...
type ConfigDeploymentSpec struct {
	...
	// The ConfigDeployment name. If not given, the metadata.name will be used.
	...
	// +kcc:proto:field=google.cloud.config.v1.Deployment.provider_config
	ProviderConfig *ProviderConfig `json:"providerConfig,omitempty"`
}

// ConfigDeploymentStatus defines the config connector machine state of ConfigDeployment
type ConfigDeploymentStatus struct {
	...
	// A unique specifier for the ConfigDeployment resource in GCP.
	ExternalRef *string `json:"externalRef,omitempty"`

	// ObservedState is the state of the resource as most recently observed in GCP.
	ObservedState *ConfigDeploymentObservedState `json:"observedState,omitempty"`
}

// ConfigDeploymentObservedState is the state of the ConfigDeployment resource as most recently observed in GCP.
// +kcc:observedstate:proto=google.cloud.config.v1.Deployment
type ConfigDeploymentObservedState struct {
	...
	TfErrors []TerraformErrorObservedState `json:"tfErrors,omitempty"`
...
// +kubebuilder:resource:categories=gcp,shortName=gcpconfigdeployment;gcpconfigdeployments
...
// ConfigDeployment is the Schema for the ConfigDeployment API
