...
// EntryIdentity defines the resource reference to DataplexEntry, which "External" field
...
func NewEntryIdentity(ctx context.Context, reader client.Reader, obj *DataplexEntry) (*EntryIdentity, error) {
...
func ParseEntryExternal(external string) (parent *EntryParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "entrys" {
		return nil, "", fmt.Errorf("format of DataplexEntry external=%q was not known (use projects/{{projectID}}/locations/{{location}}/entrys/{{entryID}})", external)
	}
	parent = &EntryParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
