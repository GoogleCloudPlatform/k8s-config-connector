...
// ApiIdentity defines the resource reference to ApigeeregistryApi, which "External" field
...
func NewApiIdentity(ctx context.Context, reader client.Reader, obj *ApigeeregistryApi) (*ApiIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseApiExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
func ParseApiExternal(external string) (parent *ApiParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "apis" {
		return nil, "", fmt.Errorf("format of ApigeeregistryApi external=%q was not known (use projects/{{projectID}}/locations/{{location}}/apis/{{apiID}})", external)
	}
	parent = &ApiParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
