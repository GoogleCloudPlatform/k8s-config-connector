...
var _ refsv1beta1.ExternalNormalizer = &BigQueryDataPolicyDataPolicyRef{}

// DataPolicyRef defines the resource reference to BigQueryDataPolicy, which "External" field
// holds the GCP identifier for the KRM object.
type BigQueryDataPolicyDataPolicyRef struct {
...
func (r *BigQueryDataPolicyDataPolicyRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", BigQueryDataPolicyDataPolicyGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseDataPolicyExternal(r.External); err != nil {
			return "", err
		}
