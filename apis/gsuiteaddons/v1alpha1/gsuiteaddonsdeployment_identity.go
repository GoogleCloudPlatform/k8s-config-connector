...
func (obj *GSuiteAddonsDeployment) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	...
	if externalRef != "" {
		statusIdentity := &GSuiteAddonsDeploymentIdentity{}
		if err := statusIdentity.FromExternal(externalRef); err != nil {
			return nil, err
		}

		if statusIdentity.String() != specIdentity.String() {
			return nil, fmt.Errorf("cannot change GSuiteAddonsDeployment identity (old=%q, new=%q)", statusIdentity.String(), specIdentity.String())
		}
	}
...
func (obj *GSuiteAddonsDeployment) ExternalIdentifier() *string {
