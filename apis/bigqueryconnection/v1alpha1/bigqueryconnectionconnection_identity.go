...
func getIdentityFromBigQueryConnectionConnectionSpec(ctx context.Context, reader client.Reader, obj client.Object) (*BigQueryConnectionConnectionIdentity, error) {
	resourceID, err := refs.GetResourceID(obj)
	if err != nil {
		return nil, fmt.Errorf("cannot resolve resource ID")
	}

	// UUID validation for v1alpha1
	if resourceID != "" {
		if _, err := uuid.Parse(resourceID); err != nil {
			return nil, fmt.Errorf("spec.resourceID should be in a UUID format, got %s ", resourceID)
		}
	}
