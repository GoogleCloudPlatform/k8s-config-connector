...
// DatabaseIdentity defines the resource reference to BigqueryDatabase, which "External" field
...
func NewDatabaseIdentity(ctx context.Context, reader client.Reader, obj *BigqueryDatabase) (*DatabaseIdentity, error) {
...
func ParseDatabaseExternal(external string) (parent *DatabaseParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "databases" {
		return nil, "", fmt.Errorf("format of BigqueryDatabase external=%q was not known (use projects/{{projectID}}/locations/{{location}}/databases/{{databaseID}})", external)
	}
	parent = &DatabaseParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
