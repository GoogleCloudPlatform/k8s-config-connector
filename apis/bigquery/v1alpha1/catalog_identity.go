...
// CatalogIdentity defines the resource reference to BigqueryCatalog, which "External" field
...
func NewCatalogIdentity(ctx context.Context, reader client.Reader, obj *BigqueryCatalog) (*CatalogIdentity, error) {
...
func ParseCatalogExternal(external string) (parent *CatalogParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "catalogs" {
		return nil, "", fmt.Errorf("format of BigqueryCatalog external=%q was not known (use projects/{{projectID}}/locations/{{location}}/catalogs/{{catalogID}})", external)
	}
	parent = &CatalogParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
