...
// AssignmentIdentity defines the resource reference to BigqueryAssignment, which "External" field
...
func NewAssignmentIdentity(ctx context.Context, reader client.Reader, obj *BigqueryAssignment) (*AssignmentIdentity, error) {
...
func ParseAssignmentExternal(external string) (parent *AssignmentParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "assignments" {
		return nil, "", fmt.Errorf("format of BigqueryAssignment external=%q was not known (use projects/{{projectID}}/locations/{{location}}/assignments/{{assignmentID}})", external)
	}
	parent = &AssignmentParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
