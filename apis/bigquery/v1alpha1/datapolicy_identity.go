...
func (i *DataPolicyIdentity) String() string {
	return  i.parent.String() + "/datapolicys/" + i.id
}
...
func ParseDataPolicyExternal(external string) (parent *DataPolicyParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "datapolicys" {
		return nil, "", fmt.Errorf("format of BigqueryDataPolicy external=%q was not known (use projects/{{projectID}}/locations/{{location}}/datapolicys/{{datapolicyID}})", external)
	}
	parent = &DataPolicyParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
