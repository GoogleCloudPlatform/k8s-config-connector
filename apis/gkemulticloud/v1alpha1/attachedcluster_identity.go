...
// AttachedClusterIdentity defines the resource reference to GkeMultiCloudAttachedCluster, which "External" field
...
func NewAttachedClusterIdentity(ctx context.Context, reader client.Reader, obj *GkeMultiCloudAttachedCluster) (*AttachedClusterIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseAttachedClusterExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
func ParseAttachedClusterExternal(external string) (parent *AttachedClusterParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "attachedclusters" {
		return nil, "", fmt.Errorf("format of GkeMultiCloudAttachedCluster external=%q was not known (use projects/{{projectID}}/locations/{{location}}/attachedclusters/{{attachedclusterID}})", external)
	}
	parent = &AttachedClusterParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
