...
// CertificateAuthorityIdentity defines the resource reference to MemorystoreCertificateAuthority, which "External" field
...
func NewCertificateAuthorityIdentity(ctx context.Context, reader client.Reader, obj *MemorystoreCertificateAuthority) (*CertificateAuthorityIdentity, error) {
...
func ParseCertificateAuthorityExternal(external string) (parent *CertificateAuthorityParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "certificateauthoritys" {
		return nil, "", fmt.Errorf("format of MemorystoreCertificateAuthority external=%q was not known (use projects/{{projectID}}/locations/{{location}}/certificateauthoritys/{{certificateauthorityID}})", external)
	}
	parent = &CertificateAuthorityParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
