...
// FeaturestoreIdentity defines the resource reference to AiplatformFeaturestore, which "External" field
...
func NewFeaturestoreIdentity(ctx context.Context, reader client.Reader, obj *AiplatformFeaturestore) (*FeaturestoreIdentity, error) {
...
func ParseFeaturestoreExternal(external string) (parent *FeaturestoreParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "featurestores" {
		return nil, "", fmt.Errorf("format of AiplatformFeaturestore external=%q was not known (use projects/{{projectID}}/locations/{{location}}/featurestores/{{featurestoreID}})", external)
	}
	parent = &FeaturestoreParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
