...
func TestAIPlatformModelIdentity_FromExternal(t *testing.T) {
	tests := []struct {
		...
		{
			name: "full url",
			ref:  "https://aiplatform.googleapis.com/projects/my-project/locations/us-central1/models/my-model",
			want: &AIPlatformModelIdentity{
				Project:  "my-project",
				Location: "us-central1",
				Model:    "my-model",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				if i.Model != tt.want.Model {
					t.Errorf("Model = %v, want %v", i.Model, tt.want.Model)
				}
			}
		})
	}
