...
type VertexAIRAGCorpusRef struct {
	// A reference to an externally managed VertexAIRAGCorpus resource. Should be in the format "projects/{{projectID}}/locations/{{location}}/ragCorpora/{rag_corpus}"
...
func init() {
	refs.Register(&VertexAIRAGCorpusRef{})
}
...
func (r *VertexAIRAGCorpusRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		structured, err := common.ToStructuredType[*VertexAIRAGCorpus](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromVertexAIRAGCorpusSpec(ctx, reader, structured)
		if err != nil {
			return ""
		}
