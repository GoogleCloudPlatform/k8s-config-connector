...
// DatasetIdentity defines the resource reference to AiplatformDataset, which "External" field
...
func NewDatasetIdentity(ctx context.Context, reader client.Reader, obj *AiplatformDataset) (*DatasetIdentity, error) {
...
func ParseDatasetExternal(external string) (parent *DatasetParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "datasets" {
		return nil, "", fmt.Errorf("format of AiplatformDataset external=%q was not known (use projects/{{projectID}}/locations/{{location}}/datasets/{{datasetID}})", external)
	}
	parent = &DatasetParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
