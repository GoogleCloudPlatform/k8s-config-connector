...
// NotebookRuntimeTemplateIdentity defines the resource reference to AiplatformNotebookRuntimeTemplate, which "External" field
...
func NewNotebookRuntimeTemplateIdentity(ctx context.Context, reader client.Reader, obj *AiplatformNotebookRuntimeTemplate) (*NotebookRuntimeTemplateIdentity, error) {
...
func ParseNotebookRuntimeTemplateExternal(external string) (parent *NotebookRuntimeTemplateParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "notebookruntimetemplates" {
		return nil, "", fmt.Errorf("format of AiplatformNotebookRuntimeTemplate external=%q was not known (use projects/{{projectID}}/locations/{{location}}/notebookruntimetemplates/{{notebookruntimetemplateID}})", external)
	}
	parent = &NotebookRuntimeTemplateParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
