...
// ExecutionIdentity defines the resource reference to AiplatformExecution, which "External" field
...
func NewExecutionIdentity(ctx context.Context, reader client.Reader, obj *AiplatformExecution) (*ExecutionIdentity, error) {
...
func ParseExecutionExternal(external string) (parent *ExecutionParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "executions" {
		return nil, "", fmt.Errorf("format of AiplatformExecution external=%q was not known (use projects/{{projectID}}/locations/{{location}}/executions/{{executionID}})", external)
	}
	parent = &ExecutionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
