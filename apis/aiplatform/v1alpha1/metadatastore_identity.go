...
// MetadataStoreIdentity defines the resource reference to AiplatformMetadataStore, which "External" field
...
func NewMetadataStoreIdentity(ctx context.Context, reader client.Reader, obj *AiplatformMetadataStore) (*MetadataStoreIdentity, error) {
...
func ParseMetadataStoreExternal(external string) (parent *MetadataStoreParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "metadatastores" {
		return nil, "", fmt.Errorf("format of AiplatformMetadataStore external=%q was not known (use projects/{{projectID}}/locations/{{location}}/metadatastores/{{metadatastoreID}})", external)
	}
	parent = &MetadataStoreParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
