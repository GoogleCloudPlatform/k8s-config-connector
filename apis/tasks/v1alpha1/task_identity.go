...
// TaskIdentity defines the resource reference to TasksTask, which "External" field
...
func NewTaskIdentity(ctx context.Context, reader client.Reader, obj *TasksTask) (*TaskIdentity, error) {
...
func ParseTaskExternal(external string) (parent *TaskParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "tasks" {
		return nil, "", fmt.Errorf("format of TasksTask external=%q was not known (use projects/{{projectID}}/locations/{{location}}/tasks/{{taskID}})", external)
	}
	parent = &TaskParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
