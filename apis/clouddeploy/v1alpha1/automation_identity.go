...
// AutomationIdentity defines the resource reference to DeployAutomation, which "External" field
...
func ParseAutomationExternal(external string) (parent *AutomationParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "automations" {
		return nil, "", fmt.Errorf("format of DeployAutomation external=%q was not known (use projects/{{projectID}}/locations/{{location}}/automations/{{automationID}})", external)
	}
	parent = &AutomationParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
