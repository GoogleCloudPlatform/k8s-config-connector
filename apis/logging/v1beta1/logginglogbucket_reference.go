...
func (r *LoggingLogBucketRef) ParseExternalToIdentity() (identity.Identity, error) {
	id := &LogBucketIdentity{}
...
func (r *LoggingLogBucketRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		typed, err := common.ToStructuredType[*LoggingLogBucket](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromLoggingLogBucketSpec(ctx, reader, typed)
		if err != nil {
			return ""
		}
