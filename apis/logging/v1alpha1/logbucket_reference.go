...
var _ refsv1beta1.ExternalNormalizer = &LogBucketRef{}

// LogBucketRef defines the resource reference to LoggingLogBucket, which "External" field
// holds the GCP identifier for the KRM object.
type LogBucketRef struct {
	// A reference to an externally managed LoggingLogBucket resource.
	// Should be in the format "projects/{{projectID}}/locations/{{location}}/logbuckets/{{logbucketID}}".
...
func (r *LogBucketRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", LoggingLogBucketGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseLogBucketExternal(r.External); err != nil {
			return "", err
		}
