...
// EventIdentity defines the resource reference to VideoEvent, which "External" field
...
func NewEventIdentity(ctx context.Context, reader client.Reader, obj *VideoEvent) (*EventIdentity, error) {
...
func ParseEventExternal(external string) (parent *EventParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "events" {
		return nil, "", fmt.Errorf("format of VideoEvent external=%q was not known (use projects/{{projectID}}/locations/{{location}}/events/{{eventID}})", external)
	}
	parent = &EventParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
