...
// PolicyIdentity defines the resource reference to ResourceManagerPolicy, which "External" field
...
// NewPolicyIdentity builds a PolicyIdentity from the Config Connector Policy object.
func NewPolicyIdentity(ctx context.Context, reader client.Reader, obj *ResourceManagerPolicy) (*PolicyIdentity, error) {
	...
	if obj.Spec.ProjectRef != nil {
		...
		return nil, fmt.Errorf("one of spec.projectRef, spec.folderRef, or spec.organizationRef must be set")
	}

	// For ResourceManagerPolicy, the ID is the constraint name.
	resourceID := obj.Spec.Constraint
	if resourceID == "" {
		return nil, fmt.Errorf("spec.constraint must be set")
	}
	resourceID = strings.TrimPrefix(resourceID, "constraints/")
	...
	if externalRef != "" {
		...
		if actualResourceID != resourceID {
			return nil, fmt.Errorf("cannot reset `spec.constraint` to %s, since it has already assigned to %s",
...
func ParsePolicyExternal(external string) (parent *PolicyParent, resourceID string, err error) {
	...
	if len(tokens) != 4 || tokens[2] != "policies" {
		return nil, "", fmt.Errorf("format of ResourceManagerPolicy external=%q was not known (use projects|folders|organizations/{{ID}}/policies/{{policyID}})", external)
	}
	parentType := tokens[0]
	parentVal := tokens[1]
	parent = &PolicyParent{}
