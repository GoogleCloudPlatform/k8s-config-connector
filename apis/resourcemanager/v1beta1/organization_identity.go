...
func ParseOrganizationExternal(external string) (*OrganizationIdentity, error) {
	...
	if len(tokens) == 2 && tokens[0] == "organizations" {
		...
		return &OrganizationIdentity{ResourceID: tokens[0]}, nil
	}
	return nil, fmt.Errorf("format of 'organizationRef.external'=%q was not known (use organizations/{{organizationID}} or {{organizationID}})", external)
}
