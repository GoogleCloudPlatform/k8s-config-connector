...
// FolderIdentity defines the resource reference to ResourcemanagerFolder, which "External" field
...
func NewFolderIdentity(ctx context.Context, reader client.Reader, obj *ResourcemanagerFolder) (*FolderIdentity, error) {
...
func ParseFolderExternal(external string) (parent *FolderParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "folders" {
		return nil, "", fmt.Errorf("format of ResourcemanagerFolder external=%q was not known (use projects/{{projectID}}/locations/{{location}}/folders/{{folderID}})", external)
	}
	parent = &FolderParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
