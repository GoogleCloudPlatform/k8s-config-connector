...
// TagKeyIdentity defines the resource reference to ResourcemanagerTagKey, which "External" field
...
func NewTagKeyIdentity(ctx context.Context, reader client.Reader, obj *ResourcemanagerTagKey) (*TagKeyIdentity, error) {
...
func ParseTagKeyExternal(external string) (parent *TagKeyParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "tagkeys" {
		return nil, "", fmt.Errorf("format of ResourcemanagerTagKey external=%q was not known (use projects/{{projectID}}/locations/{{location}}/tagkeys/{{tagkeyID}})", external)
	}
	parent = &TagKeyParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
