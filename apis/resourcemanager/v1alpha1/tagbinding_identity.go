...
// TagBindingIdentity defines the resource reference to ResourcemanagerTagBinding, which "External" field
...
func (i *TagBindingIdentity) String() string {
	return  i.parent.String() + "/tagbindings/" + i.id
}
...
func NewTagBindingIdentity(ctx context.Context, reader client.Reader, obj *ResourcemanagerTagBinding) (*TagBindingIdentity, error) {
...
func ParseTagBindingExternal(external string) (parent *TagBindingParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "tagbindings" {
		return nil, "", fmt.Errorf("format of ResourcemanagerTagBinding external=%q was not known (use projects/{{projectID}}/locations/{{location}}/tagbindings/{{tagbindingID}})", external)
	}
	parent = &TagBindingParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
