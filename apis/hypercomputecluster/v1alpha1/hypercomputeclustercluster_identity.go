...
func (obj *HypercomputeClusterCluster) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	...
	if obj.Spec.ProjectAndLocationRef == nil {
		return nil, fmt.Errorf("spec.projectAndLocationRef cannot be nil")
	}

	// Resolve user-configured Parent
	if err := obj.Spec.ProjectAndLocationRef.Build(ctx, reader, obj.GetNamespace(), cluster.parent); err != nil {
		return nil, err
	}
