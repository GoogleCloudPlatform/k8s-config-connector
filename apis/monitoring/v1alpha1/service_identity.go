...
// ServiceIdentity defines the resource reference to MonitoringService, which "External" field
...
func NewServiceIdentity(ctx context.Context, reader client.Reader, obj *MonitoringService) (*ServiceIdentity, error) {
...
func ParseServiceExternal(external string) (parent *ServiceParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "services" {
		return nil, "", fmt.Errorf("format of MonitoringService external=%q was not known (use projects/{{projectID}}/locations/{{location}}/services/{{serviceID}})", external)
	}
	parent = &ServiceParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
