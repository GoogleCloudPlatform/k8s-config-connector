...
var _ refsv1beta1.ExternalNormalizer = &MonitoredProjectRef{}

// MonitoredProjectRef defines the resource reference to MonitoringMonitoredProject, which "External" field
// holds the GCP identifier for the KRM object.
type MonitoredProjectRef struct {
...
func (r *MonitoredProjectRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", MonitoringMonitoredProjectGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseMonitoredProjectExternal(r.External); err != nil {
			return "", err
		}
