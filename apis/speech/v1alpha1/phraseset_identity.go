...
func (i *PhraseSetIdentity) Parent() *PhraseSetParent {
	return i.parent
}

type PhraseSetParent struct {
	ProjectID string
	Location  string
}

func (p *PhraseSetParent) String() string {
	return "projects/" + p.ProjectID + "/locations/" + p.Location
}

// New builds a PhraseSetIdentity from the Config Connector PhraseSet object.
func NewPhraseSetIdentity(ctx context.Context, reader client.Reader, obj *SpeechPhraseSet) (*PhraseSetIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParsePhraseSetExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
var _ identity.Identity = &PhraseSetIdentity{}
