...
var _ refsv1beta1.ExternalNormalizer = &WorkflowRef{}

// WorkflowRef defines the resource reference to WorkflowsWorkflow, which "External" field
...
func (r *WorkflowRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", WorkflowsWorkflowGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseWorkflowExternal(r.External); err != nil {
			return "", err
		}
