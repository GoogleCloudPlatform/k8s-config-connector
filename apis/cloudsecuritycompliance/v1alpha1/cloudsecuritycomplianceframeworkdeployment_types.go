...
type CloudSecurityComplianceFrameworkDeploymentSpec struct {
	...
	TargetResourceConfig *TargetResourceConfig `json:"targetResourceConfig"`
	...
	CloudControlMetadata []CloudControlMetadata `json:"cloudControlMetadata"`
}

// +kcc:proto=google.cloud.cloudsecuritycompliance.v1.FrameworkReference
type FrameworkReference struct {
	// Required. Reference to a CloudSecurityComplianceFramework resource.
...
type CloudSecurityComplianceFrameworkDeploymentObservedState struct {
	...
	// Output only. To prevent concurrent updates from overwriting each other, always
