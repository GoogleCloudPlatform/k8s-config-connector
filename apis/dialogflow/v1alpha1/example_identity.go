...
// ExampleIdentity defines the resource reference to DialogflowExample, which "External" field
...
func NewExampleIdentity(ctx context.Context, reader client.Reader, obj *DialogflowExample) (*ExampleIdentity, error) {
...
func ParseExampleExternal(external string) (parent *ExampleParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "examples" {
		return nil, "", fmt.Errorf("format of DialogflowExample external=%q was not known (use projects/{{projectID}}/locations/{{location}}/examples/{{exampleID}})", external)
	}
	parent = &ExampleParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
