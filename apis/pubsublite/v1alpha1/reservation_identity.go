...
// ReservationIdentity defines the resource reference to PubsubliteReservation, which "External" field
...
func NewReservationIdentity(ctx context.Context, reader client.Reader, obj *PubsubliteReservation) (*ReservationIdentity, error) {
...
func ParseReservationExternal(external string) (parent *ReservationParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "reservations" {
		return nil, "", fmt.Errorf("format of PubsubliteReservation external=%q was not known (use projects/{{projectID}}/locations/{{location}}/reservations/{{reservationID}})", external)
	}
	parent = &ReservationParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
