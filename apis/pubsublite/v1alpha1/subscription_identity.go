...
// SubscriptionIdentity defines the resource reference to PubsubliteSubscription, which "External" field
...
func NewSubscriptionIdentity(ctx context.Context, reader client.Reader, obj *PubsubliteSubscription) (*SubscriptionIdentity, error) {
...
func ParseSubscriptionExternal(external string) (parent *SubscriptionParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "subscriptions" {
		return nil, "", fmt.Errorf("format of PubsubliteSubscription external=%q was not known (use projects/{{projectID}}/locations/{{location}}/subscriptions/{{subscriptionID}})", external)
	}
	parent = &SubscriptionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
