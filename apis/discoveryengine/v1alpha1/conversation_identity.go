...
// ConversationIdentity defines the resource reference to DiscoveryengineConversation, which "External" field
...
func NewConversationIdentity(ctx context.Context, reader client.Reader, obj *DiscoveryengineConversation) (*ConversationIdentity, error) {
...
func ParseConversationExternal(external string) (parent *ConversationParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "conversations" {
		return nil, "", fmt.Errorf("format of DiscoveryengineConversation external=%q was not known (use projects/{{projectID}}/locations/{{location}}/conversations/{{conversationID}})", external)
	}
	parent = &ConversationParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
