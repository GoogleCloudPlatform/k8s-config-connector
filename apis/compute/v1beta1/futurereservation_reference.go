...
type FutureReservationRef struct {
	...
	// Should be in the format "projects/{{projectID}}/zones/{{zone}}/futureReservations/{{futurereservationID}}".
...
func (r *FutureReservationRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	// From given External
	if r.External != "" {
		if _, _, err := ParseFutureReservationExternal(r.External); err != nil {
			return "", err
		}
