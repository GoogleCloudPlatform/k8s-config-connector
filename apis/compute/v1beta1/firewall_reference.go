...
type ComputeFirewallRef struct {
	...
	// Should be in the format "projects/{{projectID}}/global/firewalls/{{firewall}}".
...
func init() {
	refs.Register(&ComputeFirewallRef{})
}
...
func (r *ComputeFirewallRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	...
	fallback := func(u *unstructured.Unstructured) string {
		...
		obj, err := common.ToStructuredType[*ComputeFirewall](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromComputeFirewallSpec(ctx, reader, obj)
		if err != nil {
			return ""
		}
