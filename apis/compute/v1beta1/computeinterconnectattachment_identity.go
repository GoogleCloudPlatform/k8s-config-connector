// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package v1beta1

import (
	"context"
	"fmt"

	"github.com/GoogleCloudPlatform/k8s-config-connector/apis/common"
	"github.com/GoogleCloudPlatform/k8s-config-connector/apis/common/identity"
	apirefs "github.com/GoogleCloudPlatform/k8s-config-connector/apis/refs"
	refs "github.com/GoogleCloudPlatform/k8s-config-connector/apis/refs/v1beta1"
	"github.com/GoogleCloudPlatform/k8s-config-connector/pkg/gcpurls"
	"sigs.k8s.io/controller-runtime/pkg/client"
)

var (
	_ identity.IdentityV2 = &ComputeInterconnectAttachmentIdentity{}
	_ identity.Resource   = &ComputeInterconnectAttachment{}
)

var ComputeInterconnectAttachmentIdentityFormat = gcpurls.Template[ComputeInterconnectAttachmentIdentity]("compute.googleapis.com", "projects/{project}/regions/{region}/interconnectAttachments/{interconnectAttachment}")

// ComputeInterconnectAttachmentIdentity is the identity of a GCP ComputeInterconnectAttachment resource.
// +k8s:deepcopy-gen=false
type ComputeInterconnectAttachmentIdentity struct {
	Project                string
	Region                 string
	InterconnectAttachment string
}

func (i *ComputeInterconnectAttachmentIdentity) String() string {
	return ComputeInterconnectAttachmentIdentityFormat.ToString(*i)
}

func (i *ComputeInterconnectAttachmentIdentity) FromExternal(ref string) error {
	trimmedRef := apirefs.TrimComputeURIPrefix(ref)
	parsed, match, err := ComputeInterconnectAttachmentIdentityFormat.Parse(trimmedRef)
	if err != nil {
		return fmt.Errorf("format of ComputeInterconnectAttachment external=%q was not known (use %s): %w", ref, ComputeInterconnectAttachmentIdentityFormat.CanonicalForm(), err)
	}
	if !match {
		return fmt.Errorf("format of ComputeInterconnectAttachment external=%q was not known (use %s)", ref, ComputeInterconnectAttachmentIdentityFormat.CanonicalForm())
	}

	*i = *parsed
	return nil
}

func (i *ComputeInterconnectAttachmentIdentity) Host() string {
	return ComputeInterconnectAttachmentIdentityFormat.Host()
}

func (i *ComputeInterconnectAttachmentIdentity) ParentString() string {
	return fmt.Sprintf("projects/%s/regions/%s", i.Project, i.Region)
}

func ParseComputeInterconnectAttachmentExternal(external string) (*ComputeInterconnectAttachmentIdentity, error) {
	if external == "" {
		return nil, fmt.Errorf("empty ComputeInterconnectAttachment external value")
	}
	id := &ComputeInterconnectAttachmentIdentity{}
	if err := id.FromExternal(external); err != nil {
		return nil, err
	}
	return id, nil
}

func getIdentityFromComputeInterconnectAttachmentSpec(ctx context.Context, reader client.Reader, obj *ComputeInterconnectAttachment) (*ComputeInterconnectAttachmentIdentity, error) {
	resourceID, err := refs.GetResourceID(obj)
	if err != nil {
		return nil, fmt.Errorf("cannot resolve resource ID")
	}

	region := obj.Spec.Region
	if region == "" {
		return nil, fmt.Errorf("cannot resolve region: spec.region is empty")
	}

	projectID, err := refs.ResolveProjectID(ctx, reader, obj)
	if err != nil {
		return nil, fmt.Errorf("cannot resolve project")
	}

	identity := &ComputeInterconnectAttachmentIdentity{
		Project:                projectID,
		Region:                 region,
		InterconnectAttachment: resourceID,
	}
	return identity, nil
}

func (obj *ComputeInterconnectAttachment) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	specIdentity, err := getIdentityFromComputeInterconnectAttachmentSpec(ctx, reader, obj)
	if err != nil {
		return nil, err
	}

	// Cross-check the identity against status.selfLink, if present.
	selfLink := common.ValueOf(obj.Status.SelfLink)
	if selfLink != "" {
		statusIdentity := &ComputeInterconnectAttachmentIdentity{}
		if err := statusIdentity.FromExternal(selfLink); err != nil {
			return nil, err
		}

		if statusIdentity.String() != specIdentity.String() {
			return nil, fmt.Errorf("cannot change ComputeInterconnectAttachment identity (old=%q, new=%q)", statusIdentity.String(), specIdentity.String())
		}
	}

	return specIdentity, nil
}
