...
type ComputeHTTPHealthCheckRef struct {
	...
	// Should be in the format "projects/{{projectID}}/global/httpHealthChecks/{{httphealthcheckID}}".
...
func init() {
	refs.Register(&ComputeHTTPHealthCheckRef{}, &ComputeHTTPHealthCheck{})
}
...
func (r *ComputeHTTPHealthCheckRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	...
	fallback := func(u *unstructured.Unstructured) string {
		...
		obj, err := common.ToStructuredType[*ComputeHTTPHealthCheck](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromComputeHTTPHealthCheckSpec(ctx, reader, obj)
		if err != nil {
			return ""
		}
