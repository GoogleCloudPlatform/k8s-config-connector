...
type ComputeBackendBucketRef struct {
	...
	// Should be in the format "projects/{{projectID}}/global/backendBuckets/{{backendBucket}}".
...
func init() {
	refs.Register(&ComputeBackendBucketRef{})
}
...
func (r *ComputeBackendBucketRef) ParseExternalToIdentity() (identity.Identity, error) {
	id := &ComputeBackendBucketIdentity{}
...
func (r *ComputeBackendBucketRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		identity, err := getIdentityFromComputeBackendBucketSpec(ctx, reader, u)
		if err != nil {
			return ""
		}
