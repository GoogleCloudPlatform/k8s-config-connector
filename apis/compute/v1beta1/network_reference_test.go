...
func TestNormalizedExternal(t *testing.T) {
	testCases := []struct {
		name           string
		...
		{
			name: "external and name both specified",
			ref: &ComputeNetworkRef{
				External: "projects/test-project/global/networks/test-network",
				Name:     "test-network",
			},
			wantErr: "cannot specify both name and external on ComputeNetwork reference",
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotExternal, err := tc.ref.NormalizedExternal(context.TODO(), fakeClient, tc.otherNamespace)
			if tc.wantErr != "" {
				if err == nil {
