...
func TestComputeNodeGroupIdentity_FromExternal(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			id := &v1beta1.ComputeNodeGroupIdentity{}
			if err := id.FromExternal(tt.ref); (err != nil) != tt.wantErr {
				t.Errorf("ComputeNodeGroupIdentity.FromExternal() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
