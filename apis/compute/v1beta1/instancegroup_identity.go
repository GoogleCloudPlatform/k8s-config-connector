...
func (i *ComputeInstanceGroupIdentity) FromExternal(ref string) error {
	...
	parsed, match, err := ComputeInstanceGroupIdentityFormat.Parse(focused)
	if err != nil {
...
func getIdentityFromComputeInstanceGroupSpec(ctx context.Context, reader client.Reader, obj client.Object) (*ComputeInstanceGroupIdentity, error) {
	// If the object status has selfLink, use that!
	...
	} else if cig, ok := obj.(*ComputeInstanceGroup); ok {
		selfLink := common.ValueOf(cig.Status.SelfLink)
		if selfLink != "" {
			id := &ComputeInstanceGroupIdentity{}
			if err := id.FromExternal(selfLink); err == nil {
				return id, nil
