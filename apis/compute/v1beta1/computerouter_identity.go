...
var (
	...
	_ identity.Resource   = &ComputeRouter{}
)
...
func getIdentityFromComputeRouterSpec(ctx context.Context, reader client.Reader, obj *ComputeRouter) (*ComputeRouterIdentity, error) {
...
func (obj *ComputeRouter) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	specIdentity, err := getIdentityFromComputeRouterSpec(ctx, reader, obj)
	if err != nil {
		return nil, err
	}
	...
	if selfLink != "" {
		statusIdentity := &ComputeRouterIdentity{}
		if err := statusIdentity.FromExternal(selfLink); err != nil {
			return nil, err
		}

		if statusIdentity.String() != specIdentity.String() {
			return nil, fmt.Errorf("cannot change ComputeRouter identity (old=%q, new=%q)", statusIdentity.String(), specIdentity.String())
		}
	}
