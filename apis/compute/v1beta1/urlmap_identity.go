...
var ComputeURLMapRegionalIdentityFormat = gcpurls.Template[ComputeURLMapIdentity]("compute.googleapis.com", "projects/{project}/regions/{region}/urlMaps/{name}")
...
func (i *ComputeURLMapIdentity) FromExternal(ref string) error {
	...
	if parsed, match, err := ComputeURLMapRegionalIdentityFormat.Parse(ref); err == nil && match {
		*i = *parsed
		return nil
	}
...
func getIdentityFromComputeURLMapSpec(ctx context.Context, reader client.Reader, obj client.Object) (*ComputeURLMapIdentity, error) {
	...
	if err != nil {
		// Default to global if not found in spec (for compute URL maps)
		location = "global"
	}
