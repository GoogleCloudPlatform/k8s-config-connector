...
type ComputeVPNTunnelRef struct {
	// The value of an externally managed ComputeVPNTunnel resource.
	// Should be in the format "https://www.googleapis.com/compute/{{version}}/projects/{{projectId}}/regions/{{region}}/vpnTunnels/{{name}}" or "projects/{{projectId}}/regions/{{region}}/vpnTunnels/{{name}}"
...
func (r *ComputeVPNTunnelRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	if r.External != "" {
		_, err := ParseComputeVPNTunnelExternal(r.External)
		if err != nil {
			return err
		}
	...
	u.SetGroupVersionKind(ComputeVPNTunnelGVK)
	if err := reader.Get(ctx, key, u); err != nil {
		if apierrors.IsNotFound(err) {
			return k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
		}
		return fmt.Errorf("reading referenced %s %s: %w", ComputeVPNTunnelGVK, key, err)
	}
