...
var ComputeRouterInterfaceIdentityFormat = gcpurls.Template[ComputeRouterInterfaceIdentity](
	"compute.googleapis.com",
	"projects/{project}/regions/{region}/routers/{router}/interfaces/{name}",
)
...
func (i *ComputeRouterInterfaceIdentity) FromExternal(ref string) error {
	parsed, match, err := ComputeRouterInterfaceIdentityFormat.Parse(ref)
	if err != nil {
...
func getIdentityFromComputeRouterInterfaceSpec(ctx context.Context, reader client.Reader, obj *ComputeRouterInterface) (*ComputeRouterInterfaceIdentity, error) {
	if obj.Spec.RouterRef == nil {
		return nil, fmt.Errorf("spec.routerRef is required")
	}
	routerRef := obj.Spec.RouterRef.DeepCopy()
	if err := routerRef.Normalize(ctx, reader, obj.GetNamespace()); err != nil {
		return nil, err
	}
	parsedIdentity, err := routerRef.ParseExternalToIdentity()
	if err != nil {
		return nil, err
	}
	routerIdentity := parsedIdentity.(*ComputeRouterIdentity)
...
func (obj *ComputeRouterInterface) GetIdentity(ctx context.Context, reader client.Reader) (identity.Identity, error) {
	...
	if externalRef != "" {
		...
		statusIdentity := &ComputeRouterInterfaceIdentity{}
		if err := statusIdentity.FromExternal(externalRef); err != nil {
			return nil, err
		}

		if statusIdentity.String() != specIdentity.String() {
			return nil, fmt.Errorf("cannot change ComputeRouterInterface identity (old=%q, new=%q)", statusIdentity.String(), specIdentity.String())
		}
	}
