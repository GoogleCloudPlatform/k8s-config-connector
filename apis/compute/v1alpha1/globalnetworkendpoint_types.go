...
type ComputeGlobalNetworkEndpointSpec struct {
	...
	// Immutable. Port number of the external endpoint.
	// +required
	Port int32 `json:"port"`

	// Immutable. Fully qualified domain name of network endpoint.
	// This can only be specified when network_endpoint_type of the NEG is INTERNET_FQDN_PORT.
	// Exactly one of fqdn or ipAddress must be specified.
	// +optional
...
// +kubebuilder:validation:XValidation:rule="(has(self.spec.fqdn) && !has(self.spec.ipAddress)) || (!has(self.spec.fqdn) && has(self.spec.ipAddress))",message="Exactly one of spec.fqdn or spec.ipAddress must be specified, not both"
