...
func TestApiHubRuntimeProjectAttachmentIdentity_FromExternal(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Invalid format",
			input:    "projects/my-project/locations/us-central1/invalid/my-attachment",
			expected: nil,
			hasError: true,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if id.Project != tc.expected.Project || id.Location != tc.expected.Location || id.Runtime_project_attachment != tc.expected.Runtime_project_attachment {
