...
// AttributeIdentity defines the resource reference to ApihubAttribute, which "External" field
...
func NewAttributeIdentity(ctx context.Context, reader client.Reader, obj *ApihubAttribute) (*AttributeIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseAttributeExternal(externalRef)
		if err != nil {
			return nil, err
		}
...
func ParseAttributeExternal(external string) (parent *AttributeParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "attributes" {
		return nil, "", fmt.Errorf("format of ApihubAttribute external=%q was not known (use projects/{{projectID}}/locations/{{location}}/attributes/{{attributeID}})", external)
	}
	parent = &AttributeParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
