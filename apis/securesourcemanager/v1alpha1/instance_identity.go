...
func (i *InstanceIdentity) String() string {
	p := i.id
	return i.parent.String() + "/instances/" + p
}
...
func NewInstanceIdentity(ctx context.Context, reader client.Reader, obj *SecureSourceManagerInstance, u *unstructured.Unstructured) (*InstanceIdentity, error) {
	// Get projectID
	...
	if externalRef != "" {
		actualIdentity, err := parseSecureSourceManagerInstanceExternal(externalRef)
		if err != nil {
			return nil, err
		}
	...
	return &InstanceIdentity{
		parent: &InstanceParent{ProjectID: projectID, Location: location},
		id:     resourceID,
	}, nil
}
