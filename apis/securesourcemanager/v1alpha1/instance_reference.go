...
type SecureSourceManagerInstanceRef struct {
	...
	parent *SecureSourceManagerInstanceParent
}
...
func (r *SecureSourceManagerInstanceRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	// From given External
	if r.External != "" {
		if _, _, err := parseSecureSourceManagerInstanceExternal(r.External); err != nil {
			return "", err
		}
...
func NewSecureSourceManagerInstanceRef(ctx context.Context, reader client.Reader, obj *SecureSourceManagerInstance) (*SecureSourceManagerInstanceRef, error) {
	...
	id.parent = &SecureSourceManagerInstanceParent{ProjectID: projectID, Location: location}
	...
	if externalRef == "" {
		id.External = asSecureSourceManagerInstanceExternal(id.parent, resourceID)
		return id, nil
	}
...
func (r *SecureSourceManagerInstanceRef) Parent() (*SecureSourceManagerInstanceParent, error) {
	if r.parent != nil {
		return r.parent, nil
	}
	if r.External != "" {
	...
	return nil, fmt.Errorf("SecureSourceManagerInstanceRef not initialized from `NewSecureSourceManagerInstanceRef` or `NormalizedExternal`")
}

type SecureSourceManagerInstanceParent struct {
	ProjectID string
	Location  string
}

func (p *SecureSourceManagerInstanceParent) String() string {
	return "projects/" + p.ProjectID + "/locations/" + p.Location
}

func asSecureSourceManagerInstanceExternal(parent *SecureSourceManagerInstanceParent, resourceID string) (external string) {
	return parent.String() + "/instances/" + resourceID
}

func parseSecureSourceManagerInstanceExternal(external string) (parent *SecureSourceManagerInstanceParent, resourceID string, err error) {
