...
var _ refsv1beta1.ExternalNormalizer = &KeyRingRef{}

// KeyRingRef defines the resource reference to KmsKeyRing, which "External" field
...
// NormalizedExternal provision the "External" value for other resource that depends on KmsKeyRing.
// If the "External" is given in the other resource's spec.KmsKeyRingRef, the given value will be used.
// Otherwise, the "Name" and "Namespace" will be used to query the actual KmsKeyRing object from the cluster.
func (r *KeyRingRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", KmsKeyRingGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseKeyRingExternal(r.External); err != nil {
			return "", err
		}
