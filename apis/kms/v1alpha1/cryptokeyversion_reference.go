...
func ParseKMSCryptoKeyVersionExternal(external string) (parent *KMSCryptoKeyVersionIdentity, err error) {
	...
	// projects/{{kms_project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}}/cryptoKeyVersions/{{version}}
	if len(tokens) == 10 {
	...
	return nil, fmt.Errorf("format of KMSCryptoKeyVersion external=%q was not known (use projects/{{kms_project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}}/cryptoKeyVersions/{{version}})", external)
