...
type KMSKeyRingRef struct {
	...
	// Should be in the format `projects/{{project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}`.
...
func (r *KMSKeyRingRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	// From given External
	// External should be in the `projects/{{project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}` format
	if r.External != "" {
