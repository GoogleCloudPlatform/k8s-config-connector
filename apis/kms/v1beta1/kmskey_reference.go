...
type KMSKeyRef_OneOf struct {
	...
	*kmsCryptoKeyRef `json:",inline"`
	...
	// Should be in the format `projects/{{project_id}}/locations/{{location}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}}`.
	// For AutoKey, replace {{key_ring_id}} to `autokey`, i.e. `projects/{{project_id}}/locations/{{location}}/keyRings/autokey/cryptoKeys/{{key}}`.
...
func (r *KMSKeyRef_OneOf) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	...
	if r.External != "" {
		...
		return "", fmt.Errorf("format of KMSKeyRef external=%q was not known (use projects/{{project_id}}/locations/{{location}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}})", r.External)
	}
