...
func ParseKMSCryptoKeyExternal(external string) (*KMSCryptoKeyIdentity, error) {
	...
	// projects/{{project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}}
	...
	return nil, fmt.Errorf("format of KMSCryptoKey external=%q was not known (use projects/{{project_id}}/locations/{{region}}/keyRings/{{key_ring_id}}/cryptoKeys/{{key}})", external)
