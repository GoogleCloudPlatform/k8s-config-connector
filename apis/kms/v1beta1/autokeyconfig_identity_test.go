...
func TestNewAutokeyConfigIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "externalRef (folder) matches spec folderRef name (resolving skipped/ignored)",
			obj: &KMSAutokeyConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-autokeyconfig",
					Namespace: "my-namespace",
				...
				Spec: KMSAutokeyConfigSpec{
					FolderRef: &refsv1beta1.FolderRef{
						Name: "non-existent-folder-name", // normally fails resolution, but skipped because externalRef exists
					},
				},
				Status: KMSAutokeyConfigStatus{
					ExternalRef: func() *string { s := "folders/12345/autokeyConfig"; return &s }(),
		...
		},
		{
			...
			expectError: "parent changed, expect folders/12345, got project reference",
		},
		{
		...
		{
			name: "externalRef (project) matches spec projectRef name (resolving skipped)",
			obj: &KMSAutokeyConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-autokeyconfig",
					Namespace: "my-namespace",
		...
		},
		{
			...
			expectError: "parent changed, expect projects/my-project-id, got folder reference",
		},
	}
