...
// SnapshotIdentity defines the resource reference to FilestoreSnapshot, which "External" field
...
func NewSnapshotIdentity(ctx context.Context, reader client.Reader, obj *FilestoreSnapshot) (*SnapshotIdentity, error) {
...
func ParseSnapshotExternal(external string) (parent *SnapshotParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "snapshots" {
		return nil, "", fmt.Errorf("format of FilestoreSnapshot external=%q was not known (use projects/{{projectID}}/locations/{{location}}/snapshots/{{snapshotID}})", external)
	}
	parent = &SnapshotParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
