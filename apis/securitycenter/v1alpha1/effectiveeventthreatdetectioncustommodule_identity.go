...
// EffectiveEventThreatDetectionCustomModuleIdentity defines the resource reference to SecuritycenterEffectiveEventThreatDetectionCustomModule, which "External" field
...
func NewEffectiveEventThreatDetectionCustomModuleIdentity(ctx context.Context, reader client.Reader, obj *SecuritycenterEffectiveEventThreatDetectionCustomModule) (*EffectiveEventThreatDetectionCustomModuleIdentity, error) {
...
func ParseEffectiveEventThreatDetectionCustomModuleExternal(external string) (parent *EffectiveEventThreatDetectionCustomModuleParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "effectiveeventthreatdetectioncustommodules" {
		return nil, "", fmt.Errorf("format of SecuritycenterEffectiveEventThreatDetectionCustomModule external=%q was not known (use projects/{{projectID}}/locations/{{location}}/effectiveeventthreatdetectioncustommodules/{{effectiveeventthreatdetectioncustommoduleID}})", external)
	}
	parent = &EffectiveEventThreatDetectionCustomModuleParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
