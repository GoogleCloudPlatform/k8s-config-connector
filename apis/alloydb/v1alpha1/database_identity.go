...
// DatabaseIdentity defines the resource reference to AlloydbDatabase, which "External" field
...
func NewDatabaseIdentity(ctx context.Context, reader client.Reader, obj *AlloydbDatabase) (*DatabaseIdentity, error) {
...
func ParseDatabaseExternal(external string) (parent *DatabaseParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "databases" {
		return nil, "", fmt.Errorf("format of AlloydbDatabase external=%q was not known (use projects/{{projectID}}/locations/{{location}}/databases/{{databaseID}})", external)
	}
	parent = &DatabaseParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
