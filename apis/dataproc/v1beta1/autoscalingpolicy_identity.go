...
func NewAutoscalingPolicyIdentity(ctx context.Context, reader client.Reader, obj *DataprocAutoscalingPolicy) (*AutoscalingPolicyIdentity, error) {
	...
	if externalRef != "" {
		...
		actualParent, actualResourceID, err := ParseAutoscalingPolicyExternal(externalRef)
		if err != nil {
			return nil, err
		}
	...
	return &AutoscalingPolicyIdentity{
		parent: &AutoscalingPolicyParent{
			ProjectID: projectID,
			Location:  location,
		},
...
func ParseAutoscalingPolicyExternal(external string) (parent *AutoscalingPolicyParent, resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "autoscalingpolicys" {
		return nil, "", fmt.Errorf("format of DataprocAutoscalingPolicy external=%q was not known (use projects/{{projectID}}/locations/{{location}}/autoscalingpolicys/{{autoscalingpolicyID}})", external)
	}
	parent = &AutoscalingPolicyParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
