...
func ParseClusterExternal(external string) (resourceID string, err error) {
	tokens := strings.Split(external, "/")
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "regions" || tokens[4] != "clusters" {
		return "", fmt.Errorf("format of DataprocCluster external=%q was not known (use projects/{{projectID}}/regions/{{region}}/clusters/{{cluster}}", external)
	}
	return external, nil
}
