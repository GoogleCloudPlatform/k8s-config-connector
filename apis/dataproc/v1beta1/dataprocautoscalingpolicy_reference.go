...
// DataprocAutoscalingPolicyRef is a reference to a GCP DataprocAutoscalingPolicy.
type DataprocAutoscalingPolicyRef struct {
	// A reference to an externally managed DataprocAutoscalingPolicy resource. Should be in the format "projects/{{projectID}}/regions/{{region}}/autoscalingPolicies/{{policyID}}".
...
func (r *DataprocAutoscalingPolicyRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		obj, err := common.ToStructuredType[*DataprocAutoscalingPolicy](u)
		if err != nil {
			return ""
		}
		identity, err := getIdentityFromDataprocAutoscalingPolicySpec(ctx, reader, obj)
		if err != nil {
			return ""
		}
