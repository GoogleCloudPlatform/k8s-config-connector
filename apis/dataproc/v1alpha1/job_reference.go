...
// NormalizedExternal provision the "External" value for other resource that depends on DataprocJob.
// If the "External" is given in the other resource's spec.DataprocJobRef, the given value will be used.
// Otherwise, the "Name" and "Namespace" will be used to query the actual DataprocJob object from the cluster.
func (r *JobRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", DataprocJobGVK.Kind)
	}
	// From given External
	if r.External != "" {
	...
	u.SetGroupVersionKind(DataprocJobGVK)
	if err := reader.Get(ctx, key, u); err != nil {
		if apierrors.IsNotFound(err) {
			return "", k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
		}
		return "", fmt.Errorf("reading referenced %s %s: %w", DataprocJobGVK, key, err)
	}
