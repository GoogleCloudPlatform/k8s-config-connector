...
// SessionIdentity defines the resource reference to DataprocSession, which "External" field
...
func NewSessionIdentity(ctx context.Context, reader client.Reader, obj *DataprocSession) (*SessionIdentity, error) {
...
func ParseSessionExternal(external string) (parent *SessionParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "sessions" {
		return nil, "", fmt.Errorf("format of DataprocSession external=%q was not known (use projects/{{projectID}}/locations/{{location}}/sessions/{{sessionID}})", external)
	}
	parent = &SessionParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
