...
// PrivilegedAccessManagerEntitlementRef defines the resource reference to PrivilegedAccessManagerEntitlement, which "External" field
...
type PrivilegedAccessManagerEntitlementRef struct {
	...
	// Should be in the format "projects/{{projectID}}/locations/{{location}}/entitlements/{{entitlementID}}", "folders/{{folderID}}/locations/{{location}}/entitlements/{{entitlementID}}", or "organizations/{{organizationID}}/locations/{{location}}/entitlements/{{entitlementID}}".
...
func (r *PrivilegedAccessManagerEntitlementRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	fallback := func(u *unstructured.Unstructured) string {
		identity, err := getIdentityFromPrivilegedAccessManagerEntitlementSpec(ctx, reader, u)
		if err != nil {
			return ""
		}
