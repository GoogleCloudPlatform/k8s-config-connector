...
func (r *ClusterControlPlaneInstancePlacement) validate() error {
	return nil
}
...
func canonicalizeClusterControlPlane(des, initial *ClusterControlPlane, opts ...dcl.ApplyOption) *ClusterControlPlane {
	...
	cDes.InstancePlacement = canonicalizeClusterControlPlaneInstancePlacement(des.InstancePlacement, initial.InstancePlacement, opts...)
...
func canonicalizeNewClusterControlPlane(c *Client, des, nw *ClusterControlPlane) *ClusterControlPlane {
	...
	nw.InstancePlacement = canonicalizeNewClusterControlPlaneInstancePlacement(c, des.InstancePlacement, nw.InstancePlacement)
...
func canonicalizeClusterControlPlaneInstancePlacement(des, initial *ClusterControlPlaneInstancePlacement, opts ...dcl.ApplyOption) *ClusterControlPlaneInstancePlacement {
	if des == nil {
		return initial
	}
	if des.empty {
	...
	cDes := &ClusterControlPlaneInstancePlacement{}
...
func canonicalizeClusterControlPlaneInstancePlacementSlice(des, initial []ClusterControlPlaneInstancePlacement, opts ...dcl.ApplyOption) []ClusterControlPlaneInstancePlacement {
	if dcl.IsEmptyValueIndirect(des) {
		return initial
	}
	...
	if len(des) != len(initial) {
		...
		items := make([]ClusterControlPlaneInstancePlacement, 0, len(des))
		for _, d := range des {
			cd := canonicalizeClusterControlPlaneInstancePlacement(&d, nil, opts...)
			if cd != nil {
				items = append(items, *cd)
			}
		}
	...
	items := make([]ClusterControlPlaneInstancePlacement, 0, len(des))
	for i, d := range des {
		cd := canonicalizeClusterControlPlaneInstancePlacement(&d, &initial[i], opts...)
		if cd != nil {
			items = append(items, *cd)
		}
	}
...
func canonicalizeNewClusterControlPlaneInstancePlacement(c *Client, des, nw *ClusterControlPlaneInstancePlacement) *ClusterControlPlaneInstancePlacement {
	...
	if nw == nil {
		if dcl.IsNotReturnedByServer(des) {
			c.Config.Logger.Info("Found explicitly empty value for ClusterControlPlaneInstancePlacement while comparing non-nil desired to nil actual.  Returning desired object.")
			return des
		}
		return nil
	}
...
func canonicalizeNewClusterControlPlaneInstancePlacementSet(c *Client, des, nw []ClusterControlPlaneInstancePlacement) []ClusterControlPlaneInstancePlacement {
	if des == nil {
		return nw
	}
	var reorderedNew []ClusterControlPlaneInstancePlacement
	for _, d := range des {
		matchedNew := -1
		for idx, n := range nw {
			if diffs, _ := compareClusterControlPlaneInstancePlacementNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedNew = idx
				break
			}
		}
...
func canonicalizeNewClusterControlPlaneInstancePlacementSlice(c *Client, des, nw []ClusterControlPlaneInstancePlacement) []ClusterControlPlaneInstancePlacement {
	if des == nil {
		return nw
	}
	...
	var items []ClusterControlPlaneInstancePlacement
	for i, d := range des {
		n := nw[i]
		items = append(items, *canonicalizeNewClusterControlPlaneInstancePlacement(c, &d, &n))
	}
...
func diffCluster(c *Client, desired, actual *Cluster, opts ...dcl.ApplyOption) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.LoggingConfig, actual.LoggingConfig, dcl.Info{ObjectFunction: compareClusterLoggingConfigNewStyle, EmptyObject: EmptyClusterLoggingConfig, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("LoggingConfig")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
...
func compareClusterControlPlaneNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.InstanceType, actual.InstanceType, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("InstanceType")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.SecurityGroupIds, actual.SecurityGroupIds, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("SecurityGroupIds")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.InstancePlacement, actual.InstancePlacement, dcl.Info{ObjectFunction: compareClusterControlPlaneInstancePlacementNewStyle, EmptyObject: EmptyClusterControlPlaneInstancePlacement, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("InstancePlacement")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterControlPlaneConfigEncryptionNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.KmsKeyArn, actual.KmsKeyArn, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("KmsKeyArn")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterControlPlaneRootVolumeNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.SizeGib, actual.SizeGib, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("SizeGib")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.VolumeType, actual.VolumeType, dcl.Info{Type: "EnumType", OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("VolumeType")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.Iops, actual.Iops, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("Iops")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterControlPlaneAwsServicesAuthenticationNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.RoleArn, actual.RoleArn, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("RoleArn")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.RoleSessionName, actual.RoleSessionName, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("RoleSessionName")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterControlPlaneProxyConfigNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.SecretArn, actual.SecretArn, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("SecretArn")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.SecretVersion, actual.SecretVersion, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("SecretVersion")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterControlPlaneInstancePlacementNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	var diffs []*dcl.FieldDiff

	desired, ok := d.(*ClusterControlPlaneInstancePlacement)
	if !ok {
		desiredNotPointer, ok := d.(ClusterControlPlaneInstancePlacement)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a ClusterControlPlaneInstancePlacement or *ClusterControlPlaneInstancePlacement", d)
		}
		desired = &desiredNotPointer
	}
	actual, ok := a.(*ClusterControlPlaneInstancePlacement)
	if !ok {
		actualNotPointer, ok := a.(ClusterControlPlaneInstancePlacement)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a ClusterControlPlaneInstancePlacement", a)
		}
		actual = &actualNotPointer
	}
...
func compareClusterAuthorizationNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.AdminUsers, actual.AdminUsers, dcl.Info{ObjectFunction: compareClusterAuthorizationAdminUsersNewStyle, EmptyObject: EmptyClusterAuthorizationAdminUsers, OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("AdminUsers")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterAuthorizationAdminUsersNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Username, actual.Username, dcl.Info{OperationSelector: dcl.TriggersOperation("updateClusterUpdateAwsClusterOperation")}, fn.AddNest("Username")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterLoggingConfigNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.ComponentConfig, actual.ComponentConfig, dcl.Info{ObjectFunction: compareClusterLoggingConfigComponentConfigNewStyle, EmptyObject: EmptyClusterLoggingConfigComponentConfig, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("ComponentConfig")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareClusterLoggingConfigComponentConfigNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.EnableComponents, actual.EnableComponents, dcl.Info{Type: "EnumType", OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("EnableComponents")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func expandClusterControlPlane(c *Client, f *ClusterControlPlane, res *Cluster) (map[string]interface{}, error) {
	...
	if v, err := expandClusterControlPlaneInstancePlacement(c, f.InstancePlacement, res); err != nil {
...
func flattenClusterControlPlane(c *Client, i interface{}, res *Cluster) *ClusterControlPlane {
	...
	r.InstancePlacement = flattenClusterControlPlaneInstancePlacement(c, m["instancePlacement"], res)
...
// expandClusterControlPlaneInstancePlacementMap expands the contents of ClusterControlPlaneInstancePlacement into a JSON
// request object.
func expandClusterControlPlaneInstancePlacementMap(c *Client, f map[string]ClusterControlPlaneInstancePlacement, res *Cluster) (map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
	...
	items := make(map[string]interface{})
	for k, item := range f {
		i, err := expandClusterControlPlaneInstancePlacement(c, &item, res)
		if err != nil {
			return nil, err
		}
		if i != nil {
...
// expandClusterControlPlaneInstancePlacementSlice expands the contents of ClusterControlPlaneInstancePlacement into a JSON
// request object.
func expandClusterControlPlaneInstancePlacementSlice(c *Client, f []ClusterControlPlaneInstancePlacement, res *Cluster) ([]map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
...
// flattenClusterControlPlaneInstancePlacementMap flattens the contents of ClusterControlPlaneInstancePlacement from a JSON
// response object.
func flattenClusterControlPlaneInstancePlacementMap(c *Client, i interface{}, res *Cluster) map[string]ClusterControlPlaneInstancePlacement {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]ClusterControlPlaneInstancePlacement{}
	}
	...
	for k, item := range a {
		items[k] = *flattenClusterControlPlaneInstancePlacement(c, item.(map[string]interface{}), res)
	}

	return items
}

// flattenClusterControlPlaneInstancePlacementSlice flattens the contents of ClusterControlPlaneInstancePlacement from a JSON
// response object.
func flattenClusterControlPlaneInstancePlacementSlice(c *Client, i interface{}, res *Cluster) []ClusterControlPlaneInstancePlacement {
	a, ok := i.([]interface{})
	if !ok {
	...
	items := make([]ClusterControlPlaneInstancePlacement, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenClusterControlPlaneInstancePlacement(c, item.(map[string]interface{}), res))
	}

	return items
}

// expandClusterControlPlaneInstancePlacement expands an instance of ClusterControlPlaneInstancePlacement into a JSON
// request object.
func expandClusterControlPlaneInstancePlacement(c *Client, f *ClusterControlPlaneInstancePlacement, res *Cluster) (map[string]interface{}, error) {
	if dcl.IsEmptyValueIndirect(f) {
		return nil, nil
	}
...
// flattenClusterControlPlaneInstancePlacement flattens an instance of ClusterControlPlaneInstancePlacement from a JSON
// response object.
func flattenClusterControlPlaneInstancePlacement(c *Client, i interface{}, res *Cluster) *ClusterControlPlaneInstancePlacement {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}

	r := &ClusterControlPlaneInstancePlacement{}

	if dcl.IsEmptyValueIndirect(i) {
		return EmptyClusterControlPlaneInstancePlacement
	}
	r.Tenancy = flattenClusterControlPlaneInstancePlacementTenancyEnum(m["tenancy"])
...
// flattenClusterControlPlaneInstancePlacementTenancyEnumMap flattens the contents of ClusterControlPlaneInstancePlacementTenancyEnum from a JSON
// response object.
func flattenClusterControlPlaneInstancePlacementTenancyEnumMap(c *Client, i interface{}, res *Cluster) map[string]ClusterControlPlaneInstancePlacementTenancyEnum {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]ClusterControlPlaneInstancePlacementTenancyEnum{}
	}
	...
	for k, item := range a {
		items[k] = *flattenClusterControlPlaneInstancePlacementTenancyEnum(item.(interface{}))
	}

	return items
}

// flattenClusterControlPlaneInstancePlacementTenancyEnumSlice flattens the contents of ClusterControlPlaneInstancePlacementTenancyEnum from a JSON
// response object.
func flattenClusterControlPlaneInstancePlacementTenancyEnumSlice(c *Client, i interface{}, res *Cluster) []ClusterControlPlaneInstancePlacementTenancyEnum {
	a, ok := i.([]interface{})
	if !ok {
		return []ClusterControlPlaneInstancePlacementTenancyEnum{}
	}
	...
	items := make([]ClusterControlPlaneInstancePlacementTenancyEnum, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenClusterControlPlaneInstancePlacementTenancyEnum(item.(interface{})))
	}

	return items
}

// flattenClusterControlPlaneInstancePlacementTenancyEnum asserts that an interface is a string, and returns a
// pointer to a *ClusterControlPlaneInstancePlacementTenancyEnum with the same value as that string.
func flattenClusterControlPlaneInstancePlacementTenancyEnum(i interface{}) *ClusterControlPlaneInstancePlacementTenancyEnum {
	s, ok := i.(string)
	if !ok {
		return nil
	}

	return ClusterControlPlaneInstancePlacementTenancyEnumRef(s)
}
...
func extractClusterControlPlaneFields(r *Cluster, o *ClusterControlPlane) error {
	...
	if vInstancePlacement == nil {
		// note: explicitly not the empty object.
		vInstancePlacement = &ClusterControlPlaneInstancePlacement{}
	}
	if err := extractClusterControlPlaneInstancePlacementFields(r, vInstancePlacement); err != nil {
		return err
	}
...
func extractClusterControlPlaneInstancePlacementFields(r *Cluster, o *ClusterControlPlaneInstancePlacement) error {
	return nil
}
...
func postReadExtractClusterControlPlaneInstancePlacementFields(r *Cluster, o *ClusterControlPlaneInstancePlacement) error {
	return nil
}
