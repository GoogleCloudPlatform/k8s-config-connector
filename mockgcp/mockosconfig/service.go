...
type OSConfigZonalService struct {
	*MockService
	pb.UnimplementedOsConfigZonalServiceServer
}
...
func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterOsConfigZonalServiceServer(grpcServer, &OSConfigZonalService{MockService: s})
}

func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	mux, err := httpmux.NewServeMux(ctx, conn, pb.RegisterOsConfigZonalServiceHandler)
	if err != nil {
		return nil, err
	}
	return mux, nil
