...
func (s *MockService) ConfigureVisitor(url string, replacements mockgcpregistry.NormalizingVisitor) {
	...
	// Cluster
	{
		...
		replacements.TransformObject("", func(m map[string]any) {
			...
			if isGKENodePool {
				if queued, ok := m["queuedProvisioning"].(map[string]any); ok && queued["enabled"] == true {
					if upgradeSettings, ok := m["upgradeSettings"].(map[string]any); ok {
						upgradeSettings["strategy"] = "SHORT_LIVED"
						delete(upgradeSettings, "maxSurge")
						delete(upgradeSettings, "maxUnavailable")
					}
					if config, ok := m["config"].(map[string]any); ok {
						delete(config, "taints")
					}
				}
			}
		})
