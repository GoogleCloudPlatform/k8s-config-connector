...
func (s *MockService) Register(grpcServer *grpc.Server) {
	...
	pb.RegisterZoneOperationsServer(grpcServer, &ZoneOperationsV1{MockService: s})
...
func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	...
	skipRewriteResources := []string{"futureReservations"}

	rewritePaths := func(w http.ResponseWriter, r *http.Request) {
		u := r.URL
		...
		if strings.HasPrefix(u.Path, "/compute/v1/") {
			// Rewrite to beta if it IS futureReservations
			if strings.Contains(u.Path, "/futureReservations") {
				u2 := *u
				u2.Path = "/compute/beta/" + strings.TrimPrefix(u.Path, "/compute/v1/")
				r = httpmux.RewriteRequest(r, &u2)
			}
		}
	...
	return http.HandlerFunc(rewritePaths), nil
}
