...
import (
	...
	pb_http "github.com/GoogleCloudPlatform/k8s-config-connector/mockgcp/generated/google/identity/accesscontextmanager/v1"
)
...
type accessContextManager struct {
	*MockService
...
func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterAccessContextManagerServer(grpcServer, &accessContextManager{MockService: s})
}

func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	mux, err := httpmux.NewServeMux(ctx, conn, httpmux.Options{},
		pb_http.RegisterAccessContextManagerHandler,
