...
// MockService represents a mocked parameter manager service.
type MockService struct {
	*common.MockEnvironment
	storage storage.Storage

	v1 *ParameterManagerV1
}
...
func New(env *common.MockEnvironment, storage storage.Storage) mockgcpregistry.MockService {
	...
	s.v1 = &ParameterManagerV1{MockService: s}
	return s
}
...
func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterParameterManagerServer(grpcServer, s.v1)
}
...
func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	...
	grpcMux.AddService(pb.NewParameterManagerClient(conn))
