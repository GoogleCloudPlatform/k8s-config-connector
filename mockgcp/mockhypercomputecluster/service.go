...
// http.host: hypercomputecluster.googleapis.com
...
// MockService represents a mocked hypercomputecluster service.
...
type HypercomputeClusterServer struct {
	*MockService
...
func (s *MockService) ExpectedHosts() []string {
	return []string{"hypercomputecluster.googleapis.com"}
}

func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterHypercomputeClusterServer(grpcServer, &HypercomputeClusterServer{MockService: s})
}
...
func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	...
	mux.AddService(pb.NewHypercomputeClusterClient(conn))
