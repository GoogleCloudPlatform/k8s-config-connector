...
func (s *MockService) ConfigureVisitor(url string, replacements mockgcpregistry.NormalizingVisitor) {
	if !strings.Contains(url, "storageinsights") {
		return
	}
	...
	// we normalize the error response for storageinsights here!
	replacements.TransformObject(".error", func(m map[string]any) {
		if _, exists := m["status"]; !exists {
			if code, ok := m["code"].(float64); ok && code == 400 {
				m["status"] = "INVALID_ARGUMENT"
			}
		}
	})
}
