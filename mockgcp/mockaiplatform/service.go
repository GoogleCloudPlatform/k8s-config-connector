...
func (s *MockService) Register(grpcServer *grpc.Server) {
	...
	pb.RegisterExtensionRegistryServiceServer(grpcServer, &extensionRegistryService{MockService: s})
	...
	descExtensionRegistry := pb.ExtensionRegistryService_ServiceDesc
	descExtensionRegistry.ServiceName = "google.cloud.aiplatform.v1.ExtensionRegistryService"
	grpcServer.RegisterService(&descExtensionRegistry, &extensionRegistryService{MockService: s})
}
...
func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	...
	mux.AddService(pb.NewExtensionRegistryServiceClient(conn))
