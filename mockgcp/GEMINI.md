...
> [!CAUTION]
> **NEVER EVER copy `_http_mock.log` to `_http.log` (`cp _http_mock.log _http.log`)**. `_http.log` represents canonical traffic recorded against REAL GCP. Copying mock logs over `_http.log` corrupts real GCP test ground truth. `_http.log` MUST only be generated by running tests against real GCP (`E2E_GCP_TARGET=real` or `hack/record-gcp`).
