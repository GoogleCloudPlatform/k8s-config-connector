...
// MockService represents a mocked datastream service.
...
func New(env *common.MockEnvironment, storage storage.Storage) *MockService {
	...
	s.v1 = &DatastreamV1{MockService: s}
	return s
}

func (s *MockService) ExpectedHosts() []string {
	return []string{"datastream.googleapis.com"}
}

func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterDatastreamServer(grpcServer, s.v1)
}

func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	mux := runtime.NewServeMux()

	if err := pb.RegisterDatastreamHandler(ctx, mux, conn); err != nil {
		return nil, err
	}
