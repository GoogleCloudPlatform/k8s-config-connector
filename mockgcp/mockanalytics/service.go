...
type analyticsAdminServer struct {
	*MockService
	pb.UnimplementedAnalyticsAdminServiceServer
}
...
func (s *MockService) ExpectedHosts() []string {
	return []string{"analyticsadmin.googleapis.com"}
}

func (s *MockService) Register(grpcServer *grpc.Server) {
	pb.RegisterAnalyticsAdminServiceServer(grpcServer, &analyticsAdminServer{MockService: s})
}
...
func (s *MockService) NewHTTPMux(ctx context.Context, conn *grpc.ClientConn) (http.Handler, error) {
	...
	grpcMux.AddService(pb.NewAnalyticsAdminServiceClient(conn))
