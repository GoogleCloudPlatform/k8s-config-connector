...
func (s *MockService) ConfigureVisitor(url string, visitor mockgcpregistry.NormalizingVisitor) {
	if !strings.Contains(url, "dataplex.googleapis.com") {
		return
	}
	...
	visitor.ReplacePath(".assetStatus.updateTime", NormalizedTimestamp)
	visitor.ReplacePath(".metastoreStatus.updateTime", NormalizedTimestamp)
